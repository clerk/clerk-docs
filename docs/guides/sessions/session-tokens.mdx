...
<If is="llm">
  **For AI agents:** To fetch claims kept out of the token (metadata, organizations), run `npx clerk api users/<user_id>` to GET the Backend `User` object instead of hand-writing a `getUser()` request. Discover related endpoints with `clerk api ls users`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
