...
sdk: nextjs, react, astro, expressjs, react-router, tanstack-react-start, ios, android, expo, vue, nuxt, chrome-extension, fastify, js-frontend
---
...
<If sdk={["ios", "android"]}>
  The easiest way to add social connections to your Clerk app is by using [prebuilt views](/docs/reference/views/overview). If you need more control, you can [build a custom OAuth flow](/docs/guides/development/custom-flows/authentication/oauth-connections) with the Clerk SDK.
</If>
...
For **production** instances, you will need to configure the provider with custom OAuth credentials. See the social provider's [dedicated guide](/docs/guides/configure/auth-strategies/social-connections/overview#supported-social-providers) for more information.
...
<If sdk={["ios"]}>
  ...
  Call `prepareReauthorization(additionalScopes:)` on an existing `ExternalAccount` to prepare a new OAuth authorization, then `reauthorize()` to open the browser-based flow. The user will be prompted to approve the additional scopes.

  ```swift
  // `account` is an existing `ExternalAccount` on the signed-in user
  let prepared = try await account.prepareReauthorization(additionalScopes: ["foo", "bar"])
  try await prepared.reauthorize()
  ```
</If>
...
For each social provider, you can disable the option to sign up and sign in to your application using the provider. This is useful when you want users to connect their OAuth account _after_ authentication. For example, if your application needs to read a user's GitHub repository data but doesn't require GitHub for sign-in.
...
<If sdk={["ios", "android"]}>
  If you use [prebuilt views](/docs/reference/views/overview), the [`UserProfileView`](/docs/reference/views/user/user-profile-view) lets users manage their connections. For a custom UI, [build a custom OAuth flow](/docs/guides/development/custom-flows/authentication/oauth-connections) with the Clerk SDK.
</If>
...
<If sdk={["android"]}>
  ...
  Instead of the typical OAuth flow that performs redirects in a browser context, you can use Android's Credential Manager to authenticate with Google and provide the ID token to Clerk. Clerk ensures that the user will be verified in a secure and reliable way with the information that Google has provided about the user.
