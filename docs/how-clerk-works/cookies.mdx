...
Cookies are small pieces of information stored in the browser and sent automatically alongside requests coming from that browser.

By default, HTTP requests are independent and lack memory of previous interactions. Cookies solve this as they are stored long term and are sent alongside each request.
...
- If a cookie is set by `example.com`, it will be sent with requests _to_ `example.com`.
- If the cookie's domain is _explicitly_ set to `.example.com`, it will also be sent with requests to subdomains like `sub.example.com`.
- However, cookies set by subdomains (e.g., `sub.example.com`) won't be sent with requests to the parent domain (`example.com`) unless explicitly configured.
