...
1. The user **hasn't enabled multi-factor authentication (MFA)**.
1. The user is signing in from a **new device**.

When these conditions are met, Clerk sends a one-time verification code to the user's email address. The user must enter this code to complete the sign-in process.
...
| **Verification method** | Email code | TOTP, SMS code, backup codes |
...
If you've built a custom sign-in flow using the Clerk API that allows password-based sign-ins, you'll need to handle the `needs_second_factor` status that Client Trust can trigger.
...
1. Call `prepareSecondFactor()` to send the verification code to the user's email.
...
> [!TIP]
> If your application supports both Client Trust and user-enabled MFA, check for all possible second factor strategies when handling the `needs_second_factor` status.
...
- **Email required**: Client Trust requires the user to have a verified email address to receive the verification code.
