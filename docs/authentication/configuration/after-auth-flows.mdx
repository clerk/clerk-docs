...
When Personal Accounts are disabled (the default for applications using organizations), users are required to select or create an organization after signing up or signing in. If they haven't completed this step, their session will be in a `pending` state. Any pages that are protected using Clerk's protection utilities will treat the user's session as signed-out.
