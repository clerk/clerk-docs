...
This guide demonstrates how to implement organization-first flows where users must be part of an organization to access your application. You'll learn how to detect users without active organizations, redirect them to organization selection, and handle organization-centric access using middleware and custom flows. This is essential for B2B applications where Personal Accounts are disabled (the default) or when you want to enforce organization membership.
...
<Steps>
  ...
  > [!NOTE]
  > If Personal Accounts are disabled in your [Organizations Settings](https://dashboard.clerk.com/last-active?path=organizations-settings) (the default), users won't have access to personal accounts anyway. This section is relevant when Personal Accounts are enabled but you want to hide them from the UI components.

  To hide a user's personal account from organization components, you can use the `hidePersonal` prop. This removes the personal account option from the UI while still allowing users to have personal accounts in the background.

  If you have the [`<OrganizationList />`](/docs/components/organization/organization-list) and [`<OrganizationSwitcher />`](/docs/components/organization/organization-switcher) components in your application, these components will show a user's personal account by default when Personal Accounts are enabled. You can hide it by passing the `hidePersonal` prop.
