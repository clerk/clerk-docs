...
<Tabs items={[ "Plan", "Feature"]}>
  <Tab>
    ...
    <Tabs items={["Next.js", "Astro", "Express", "React Router", "Remix", "Tanstack React Start"]}>
      ...
      <Tab>
        ```tsx {{ filename: 'src/pages/protected.astro' }}
        // Use `local.auth()` helper to access the `has()` method
      ...
      <Tab>
        ```tsx {{ filename: 'src/routes/api/example.ts' }}
      ...
      <Tab>
        ...
        const premiumAccess = (req, res, next) => {
          ...
          if (!hasPremiumAccess) {
            return res
              .status(403)
              .send('Only subscribers to the Premium Access feature can access this content.')
          }
        ...
        app.get('/premium-only', requireAuth(), premiumAccess, (req, res) => {
          res.send('For Premium access only')
        })
        ```
      </Tab>
...
<Tabs items={["Plan", "Feature"]}>
  <Tab>
    <Include src="_partials/protect-component-plan" />
  </Tab>

  <Tab>
    <Include src="_partials/protect-component-feature" />
  </Tab>
</Tabs>
