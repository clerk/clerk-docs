...
<Tabs items={["Chrome", "Safari", "Firefox"]}>
  <Tab>
    ...
    1. Open **Settings**, then select **Preferences** > **Network**.
    1. Enable "Allow to generate HAR (with sensitive data)."
    1. Go to the Network tab.
    1. Enable "Preserve log".
    1. Ensure the record button is red. If not, select it to enable recording.
    1. Reproduce the issue.
    1. Select the export button, then select **Export HAR (with sensitive data)**.
  </Tab>
