...
<If notSdk={["ios", "android"]}>
  ...
  1. The [`User.createEmailAddress()`](/docs/reference/objects/user#createemailaddress) method is passed to the [`useReverification()`](/docs/reference/hooks/use-reverification) hook to require the user to reverify their credentials before being able to add an email address to their account.
