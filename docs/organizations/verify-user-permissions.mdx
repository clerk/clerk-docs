...
<Tabs items={["Next.js Server Actions", "Next.js Route Handlers", "Next.js Pages Router"]}>
  ...
  <Tab>
    <Tabs items={["has()", "auth.protect()"]}>
      <Tab>
        ...
        - uses the `userId` returned from [`auth()`](/docs/sdk/nextjs/auth) to check if the user is signed in. If the user is not **authenticated**, the Route Handler will return a `401` error.
  ...
  <Tab>
    Use the [`getAuth()`](/docs/sdk/nextjs/get-auth) helper to access the `has()` helper in a Next.js Pages Router application.
