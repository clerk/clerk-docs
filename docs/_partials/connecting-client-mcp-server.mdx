...
1. Select the **Needs login** option when it loads. You'll be redirected to a Clerk OAuth consent screen to authorize Cursor to access your user data. Select **Allow**.

1. Once authorized, the tool `get_clerk_user_data` should be listed as **enabled** and you should see a green active status.

You are now ready to open an **Agent chat** in Cursor and prompt it to fetch your Clerk user data (e.g. What is my Clerk user data). Select the **Run tool** option once it becomes available. The response will vary based on your prompt, but it should include key details from your Clerk profile:
