...
This guide shows you how to use the [`<SignIn />`](/docs/references/general/components/authentication/sign-in) component to build a custom page that **allows users to sign in or sign up within a single flow**.

To set up separate sign-in and sign-up pages, follow this guide, and then follow the [custom sign-up page guide](/docs/guides/development/custom-sign-up-page).

> [!NOTE]
> Just getting started with Clerk and Next.js? See the [quickstart tutorial](/docs/getting-started/quickstart)!

<Steps>
  ## Build a sign-in-or-up page

  The following example demonstrates how to render the [`<SignIn />`](/docs/references/general/components/authentication/sign-in) component on a dedicated page using the [Next.js optional catch-all route](https://nextjs.org/docs/pages/building-your-application/routing/dynamic-routes#catch-all-segments).
  ...
  - Create a check to see if the user's current route is a public route. If it is not a public route, use [`auth.protect()`](/docs/references/nextjs/app-router/auth#auth-protect) to protect the route.
...
<Cards>
  - [Custom sign-up page](/docs/guides/development/custom-sign-up-page)
  - Learn how to add a custom sign-up page to your Next.js app with Clerk's prebuilt components.

  ---

  - [Read user and session data](/docs/guides/users/reading)
