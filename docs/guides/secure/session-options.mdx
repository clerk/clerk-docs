...
<If sdk={["nextjs", "react", "expo", "react-router", "tanstack-react-start"]}>
  To redirect to a custom route, pass the [`afterMultiSessionSingleSignOutUrl`](/docs/reference/general/components/clerk-provider#properties) property to `<ClerkProvider>`.
</If>

## Customize session token

Session tokens are JWTs that contain a set of [default claims](/docs/guides/how-clerk-works/session-tokens) required by Clerk. You can customize these tokens by providing additional claims of your own.

To learn how to customize session tokens, see the [dedicated guide](/docs/guides/configure/session-token).
