...
To learn more about machine authentication with M2M tokens, see the [M2M tokens](/docs/guides/secure/machine-auth/m2m-tokens) guide.

## API keys

If you'd like for your application's users to be able to create API keys that can delegate access to your application's API on their behalf, this is the intended use case for Clerk's API keys feature. With this feature, you will be able to allow users to create API keys directly through [the `<UserProfile />` component](/docs/references/general/components/user/user-profile), or create your own UI for API keys via custom hooks. It will also be simple to verify API keys on your backend using Clerk's SDKs, and you or your users will be able to instantly revoke API keys at any time.
