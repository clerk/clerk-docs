...
> [!DEPRECATED]
> The [InstantDB team has joined OpenAI](https://www.instantdb.com/essays/instant_team_joins_openai) and Instant Cloud is shutting down. New signups are closed, and the hosted service terminates on August 31, 2027, so this tutorial can no longer be completed with a new InstantDB account. InstantDB is open source and existing users can [self-host](https://www.instantdb.com/docs/self-hosting) their apps. For alternative databases that integrate with Clerk, see the [integrations overview](/docs/guides/development/integrations/overview).
...
<TutorialHero
  beforeYouStart={[
    ...
    {
      title: "Have an existing InstantDB account",
      link: "https://www.instantdb.com/dash",
      icon: "cog-6-teeth",
    },
    {
...
<Steps>
  ...
  1. Add the Clerk [Publishable Key](!publishable-key) you copied in the previous step.
