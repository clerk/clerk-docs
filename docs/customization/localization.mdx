...
<Tabs items={["Next.js", "Remix", "Astro", "JavaScript"]}>
  <Tab>
  ...
  <Tab>
    In the following example, the fr-FR locale is imported as `frFR`. The imported localization is then passed to the `localization` prop in the [`ClerkApp`](/docs/sdk/remix/clerk-app) options.
  ...
  <Tab>
    In the following example, the fr-FR locale is imported as `frFR`. The imported localization is then passed to the `localization` prop on the [`clerk.load()`](/docs/references/javascript/clerk/clerk#load) method.
