...
Use the `user.reload()` method to immediately update the user object with the latest data. This is only needed if you are updating the user object without using the `user.update()` method or the Clerk hooks, eg. if you are making changes to the user object via an API endpoint.
