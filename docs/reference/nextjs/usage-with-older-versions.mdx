...
> [!WARNING]
> This page is only for apps still running Next.js 12 or older. It installs `@clerk/nextjs` v4, which is behind the current release and is no longer maintained. Instead, start from the [Next.js quickstart](/docs/nextjs/getting-started/quickstart), which installs the current version.
