...
<Steps>
  ...
  > [!IMPORTANT]
  > To use a satellite domain in production, add a **CNAME record** for your satellite subdomain in your Domain Name System (DNS) provider's settings. A CNAME record tells DNS to direct traffic for one domain (or subdomaiin) to another. 
  >
  > A typical CNAME record includes:
  > - **Domain**: The main domain the record is defined on (for e.g. `example.com`).
  > - **Name (Host)**: The subdomain you want to point. For example:
  >   - `www` for `www.example.com`
  >   - `satellite` for `satellite.example.com`
  >   - `@` for the root domain itself `example.com`
  >
  > - **Type**: `CNAME` for this type of record.
  > - **Value**: The deployed app's domain. This is the domain where your primary application is hosted, such as `your-app.com`.
  >
  > For example, if your satellite domain is `satellite.dev` and your app is hosted at `your-app.com`, your CNAME record would look like this:
  >
  > ```txt
  > Domain: satellite.dev
  > Name (Host): @ (the root, so "satellite.dev" itself)
  > Type: CNAME
  > Value: your-app.com
  > ```
  >
  > This means all requests to `satellite.dev` will be directed to `your-app.com`. This is not required for development domains like `localhost`.
