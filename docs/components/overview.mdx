...
> [!NOTE]
> By default, Clerk displays a **Secured by Clerk** badge on Clerk components. You can remove this badge by following these steps:
>
> - In the Clerk Dashboard, navigate to your application's [**Settings**](https://dashboard.clerk.com/last-active?path=settings).
>- Under **Branding**, toggle on the **Remove "Secured by Clerk" branding** option. Please note that this setting requires a [paid plan](/pricing){{ target: '_blank' }} for production use, but all features are free to use in development mode so that you can try out what works for you. See the [pricing](/pricing){{ target: '_blank' }} page for more information.
>
> You can also configure other branding elements like the logo and favicon in the same section.
