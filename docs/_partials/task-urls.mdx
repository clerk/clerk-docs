...
<If notSdk={["astro", "js-frontend", "vue", "nuxt", "fastify", "hono"]}>
...
<If sdk="hono">
  Configure the `taskUrls` option on the [`clerkMiddleware()`](/docs/reference/hono/clerk-middleware) integration.
...
<If sdk="js-frontend">
  Configure the `taskUrls` option on the [`clerk.load()`](/docs/reference/objects/clerk#load) method.
