...
```tsx
function updateOrganizationMembershipMetadata: (params: UpdateOrganizationMembershipMetadataParams) => Promise<ClerkBackendApiResponse<OrganizationMembership>>;
```
...
{/* TODO: The response on this was a promise? Idk, this needs to get looked at */}
```tsx
const organizationId = 'org_2ZUtbk2yvnFGItdeze1ivCh3uqh';

const userId = 'user_2V7JJKmoA9HqzHhfMqK5cpgLl56';

const { data, errors } = clerkClient.organizations.updateOrganizationMembershipMetadata(organizationId, userId, {
  publicMetadata:{
