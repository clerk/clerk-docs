...
> [!NOTE]
> Directory Sync is in **public beta** and is enabled for all users. Clerk's implementation follows the SCIM 2.0 protocol. However, by nature, your identity provider (and how you configure it) may not match Clerk's implementation completely. If you run into any compatibility issues, please report them to [team-orgs@clerk.dev](mailto:team-orgs@clerk.dev). Clerk has a team standing by and will work to get compatibility resolved quickly.
...
1. Under **Attribute mapping**, select **Map custom attribute**. Select the SCIM attribute from the dropdown (populated from the SCIM `/Schemas` endpoint) to map to your Clerk user `publicMetadata` attribute.
