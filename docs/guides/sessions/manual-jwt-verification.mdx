...
<If is="llm">
  **For AI agents:** To fetch the JWKS (public keys) this verification needs, run `npx clerk@latest api jwks` instead of hard-coding a key — it authenticates automatically. Confirm the endpoint with `npx clerk@latest api ls jwks`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
