...
<Tabs items={["Next.js", "JavaScript"]}>
  <Tab>
    You can use the [`useAuth()`](/docs/sdk/react/use-auth) hook to get access to the authentication context, which includes the `actor` object.
...
<Tabs items={["Next.js", "Node"]}>
  <Tab>
    The Next.js [`auth()`](/docs/sdk/nextjs/auth) helper provides the `actor` object in the authentication context.
