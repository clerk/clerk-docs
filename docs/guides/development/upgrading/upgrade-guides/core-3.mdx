...
>
  ...
  <AccordionPanel>
    In Core 2, satellite applications automatically synced authentication state with the primary domain on every first page load by redirecting users to the primary domain and back. This happened regardless of whether the user had an active session.

    In Core 3, this behavior is still the default but is now controlled by the `satelliteAutoSync` option, which defaults to `true`. If you want to disable the automatic sync to improve performance for apps where most visitors are anonymous, you can explicitly set `satelliteAutoSync` to `false`.

    | | Core 3 (default, `satelliteAutoSync: true`) | Core 3 (`satelliteAutoSync: false`) |
    | - | - | - |
    | First visit to satellite | Redirects to primary domain to sync state, then back | No redirect, page loads immediately |
    | Sign-in flow | Same as `false` | User selects "Sign in" → redirected to primary → signs in → redirected back |
    ...
    To opt out of automatic syncing, in your satellite domain's app, set `satelliteAutoSync` to `false` in your middleware and `<ClerkProvider>` component.
