...
<Properties>
  ...
  The email address of the user to sign in. This is an alternative to using the `signInParams` parameter. A [Secret Key](!secret-key) is required to be set in the `CLERK_SECRET_KEY` environment variable to use this parameter.

  ---

  - `signInParams`
  - [`ClerkSignInParams`](#clerksigninparams)

  The parameters to use for the sign-in operation. See [`ClerkSignInParams`](#clerksigninparams).
  ...
  Options to pass to `setupClerkTestingToken()`. See [`SetupClerkTestingTokenOptions`](#setupclerktestingtokenoptions).
</Properties>
...
<Properties>
  ...
  The [Frontend API URL](!frontend-api-url) for your Clerk dev instance, without the protocol. If provided, it overrides the Frontend API URL parsed from the [Publishable Key](!publishable-key).
</Properties>
...
<Properties>
  ...
  Options to pass to `clerk.signOut()`. See [`SignOutOptions`](#signoutoptions).
</Properties>
