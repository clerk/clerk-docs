...
- [`CurrentUser`](/docs/reference/astro/locals#locals-currentuser)

### `clerkMiddleware()`

The `clerkMiddleware()` helper integrates Clerk authentication into your Astro application through middleware. You can learn more about [the `clerkMiddleware()` helper](/docs/reference/astro/clerk-middleware).
...
The following example uses `clerkClient` to get information about the currently signed-in user. If the user is authenticated, their `userId` is passed to [`clerkClient.users.getUser()`](/docs/reference/backend/user/get-user) to get the current user's [`User`](/docs/reference/objects/user) object. If not authenticated, the user is redirected to the sign-in page.
