...
The Astro SDK provides [stores](https://github.com/nanostores/nanostores) that give you access to the [`Clerk`](/docs/reference/objects/clerk) object and helper methods for authentication flows.
...
[`clerkClient`](/docs/reference/backend/overview) is a wrapper around the [Backend API](/docs/reference/backend-api){{ target: '_blank' }} that makes it easier to interact with the API. For example, to retrieve a list of all users in your application, you can use the [`users.getUserList()`](/docs/reference/backend/user/get-user-list) method instead of manually making a fetch request to the `https://api.clerk.com/v1/users` endpoint.
...
The following example uses `clerkClient` to get information about the currently signed-in user. If the user is authenticated, their `userId` is passed to [`clerkClient.users.getUser()`](/docs/reference/backend/user/get-user){{ target: '_blank' }} to get the current user's [`User`](/docs/reference/objects/user){{ target: '_blank' }} object. If not authenticated, the user is redirected to the sign-in page.
