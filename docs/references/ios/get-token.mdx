...
The `getToken()` method retrieves the user's [session token](/docs/session-requests/session-tokens) or a [custom JWT template](/docs/session-requests/making/jwt-templates).
