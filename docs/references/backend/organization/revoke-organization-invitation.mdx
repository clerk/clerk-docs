...
```tsx
function revokeOrganizationInvitation: (params: RevokeOrganizationInvitationParams) => Promise<ClerkBackendApiResponse<OrganizationInvitation>>;
```
...
/* In this example, you can see that data is an OrganizationInvitation object. The 'status' is now 'revoked'.
{
  data: _OrganizationInvitation {
