...
<Steps>
  ...
  For this guide, you'll implement a single example tool that retrieves information about the authenticated Clerk user. For more documentation on how to build MCP tools, see the [MCP documentation](https://modelcontextprotocol.io/docs/concepts/tools).
  ...
  <If sdk="nextjs">
    ```ts {{ filename: 'app/[transport]/route.ts', mark: [[7, 21]] }}
    import { verifyClerkToken } from '@clerk/mcp-tools/next'
    import { createMcpHandler, withMcpAuth } from 'mcp-handler'
    import { auth, clerkClient } from '@clerk/nextjs/server'
  ...
  <If sdk="expressjs">
    ```ts {{ filename: 'index.ts', mark: [[14, 32]], collapsible: true }}
    import { type MachineAuthObject, clerkClient, clerkMiddleware } from '@clerk/express'
    import {
      mcpAuthClerk,
      protectedResourceHandlerClerk,
  ...
  Once this is complete, clients that support the latest MCP spec can now invoke your tool to securely fetch user data from your app, assuming the request is authorized with a Clerk OAuth token. To test this out, [learn how to connect your client LLM to the MCP server](/docs/guides/ai/mcp/connect-mcp-client).
