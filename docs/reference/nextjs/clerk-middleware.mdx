...
> [!WARNING]
> This pattern is no longer recommended. Protect access close to the resource instead. See [Protect pages and routes](/docs/guides/secure/protect-pages).
...
> [!WARNING]
> This pattern is no longer recommended. For API and machine routes, check the token type in the route handler itself with [`auth({ acceptsToken })`](/docs/reference/nextjs/app-router/auth) instead of gating in the middleware. See [Verifying OAuth access tokens](/docs/guides/development/verifying-oauth-access-tokens).
