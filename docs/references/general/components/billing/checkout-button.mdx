...
`<CheckoutButton />` must be wrapped inside a [`<SignedIn />`](/docs/references/general/components/control/signed-in) component to ensure the user is authenticated.
...
`<CheckoutButton />` will throw an error if the `for` prop is set to `'organization'` and no [active organization](/docs/guides/organizations/overview#active-organization) is set.
