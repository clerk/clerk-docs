  ...
  rows={[
    ...
    },
    {
      cells: [
        <code>apiRoutes?</code>,
        <code>string[]</code>,
        `A list of routes that should return 401. You can use glob patterns to match multiple routes or a function to match against the request object. For example: `['/foo', '/bar(.*)']` or `[/^\/foo\/.*$/]` `
      ],
    },
    {
      cells: [
