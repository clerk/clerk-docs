...
| `supportedFirstFactors` | `SignInFactor[]` | Array of the first factors that are supported in the current sign-in. Each factor contains information about the verification strategy that can be used.<br/>For example: <ul><li>`email_code` for email addresses</li><li>`phone_code` for phone numbers</li><li>`passkey` for passkeys</li></ul> As well as the identifier that the factor refers to. |
...
*   [`authenticateWithPasskey()`](/docs/references/javascript/sign-in/authenticate-with#authenticate-with-passkey)
