...
If you have [multi-factor authentication (MFA)](/docs/guides/configure/auth-strategies/multi-factor-authentication) enabled for your application, **the sign-in attempt will return a status of `needs_second_factor`**. Your custom sign-in flow needs to support handling whichever [second factor](!second-factor) strategy you've enabled in the Clerk Dashboard.
