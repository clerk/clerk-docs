...
> [!CAUTION]
> The localizations will only update the text in the [Clerk components](/docs/references/components/overview) used in your application. The hosted [Clerk Account Portal](/docs/guides/customizing-clerk/account-portal) will remain in English.
...
<Tabs items={["Next.js", "Astro", "JavaScript", "Remix", "Vue", "Nuxt"]}>
  <Tab>
    In the following example, the fr-FR locale is imported as `frFR`. The imported localization is then passed to the `localization` prop on the [`<ClerkProvider>`](/docs/references/components/clerk-provider).
...
<Tabs items={["Next.js", "Astro", "JavaScript", "Remix", "Vue", "Nuxt"]}>
  <Tab>
    In Next.js, pass the `localization` prop to the [`<ClerkProvider>`](/docs/references/components/clerk-provider) component.
