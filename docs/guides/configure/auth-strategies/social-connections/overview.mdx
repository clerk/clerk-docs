...
> [!NOTE]
> Clerk does not automatically keep OAuth access tokens fresh behind the scenes. When you request an access token using [the relevant backend API endpoint](/docs/reference/backend-api/tag/oauth-access-tokens/post/oauth_applications/access_tokens/verify){{ target: '_blank' }}, Clerk will attempt to obtain a fresh access as well as a new refresh token. However, this process occurs only when the you initiate the request; Clerk does not proactively refresh tokens for you.
