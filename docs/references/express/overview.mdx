...
The `clerkMiddleware()` middleware is required, and must be set before any other middleware. It checks the request's cookies and headers for a session JWT and if found, attaches the [`Auth`](/docs/references/backend/types/auth-object) object to the `request` object under the `auth` key. See the [reference doc](/docs/references/express/clerk-middleware) for more information.

## `requireAuth()`

The `requireAuth()` middleware protects your routes by redirecting unauthenticated users to the homepage. See the [reference doc](/docs/references/express/require-auth) for more information.

## `getAuth()`

The `getAuth()` helper retrieves authentication state from the `request` object. It returns the [`Auth` object](/docs/references/backend/types/auth-object){{ target: '_blank' }}, which includes helpful authentication information like the user's ID, session ID, and organization ID. It's also useful for protecting routes. See the [reference doc](/docs/references/express/get-auth) for more information.
...
app.get('/user', async (req, res) => {
  // Use `req.auth` to access `isAuthenticated` and the user's ID
