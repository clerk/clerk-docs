...
For [`ClerkAPIResponseError`](/docs/reference/types/clerk-api-response-error) objects, use `isClerkAPIResponseError()` and inspect `error.errors` for the specific API error code, while the reactive sign-in or sign-up `status` only describes the current step of the flow.
...
In the raw API response, `lockout_expires_in_seconds` represents the time remaining until the user is able to attempt authentication again.
In the above example, 1800 seconds (or 30 minutes) are left until they are able to retry, as of the current moment.

The admin might have [configured](/docs/guides/secure/user-lockout#customize-max-sign-in-attempts-and-lockout-duration) a custom lockout duration (e.g., 45 minutes).
Thus, 15 minutes after one has been locked, 30 minutes will still remain until the lockout lapses.

The Frontend SDK does not currently expose `lockout_expires_in_seconds` in the error's `meta` property. Instead, render the error's `longMessage`, which includes how long the user must wait before trying again.

```tsx {{ filename: 'app/sign-in/[[...sign-in]]/page.tsx' }}
import { isClerkAPIResponseError } from '@clerk/nextjs/errors'

if (isClerkAPIResponseError(error)) {
  const userLockedError = error.errors.find(({ code }) => code === 'user_locked')

  if (userLockedError) {
    // Display the message in your UI
    console.log(userLockedError.longMessage)
  }
}
```
...
export default function SignInForm() {
  ...
  const handleSignInWithPassword = async (e: React.FormEvent) => {
    ...
    if (signIn.status === 'complete') {
      ...
      error.errors.some(({ code }) => code === 'form_password_compromised')
    ) {
      ...
      <form onSubmit={(e) => handleSignInWithPassword(e)}>
        ...
        <div>
          ...
          {errors.fields.password && (
            <p>{errors.fields.password.longMessage || errors.fields.password.message}</p>
          )}
        </div>
      ...
      {errors.global && (
        <ul>
          {errors.global
            .flatMap((error) =>
              isClerkAPIResponseError(error)
                ? error.errors.map((error) => error.longMessage || error.message)
