...
```tsx
function getInvitationList: (params: GetInvitationListParams) => Promise<Invitation[]>;
```
...
### `getInvitationList()` with no params

```tsx
const response = await clerkClient.invitations.getInvitationList();

console.log(response);
/* In this example, you can see that the array of Invitation objects is populated with only one Invitation.
[
  _Invitation {
