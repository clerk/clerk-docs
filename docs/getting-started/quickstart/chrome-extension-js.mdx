...
<Steps>
  ...
  * {
  ...
  Create a `src/` directory and add the `popup.ts` file. This is the main script for the extension popup that handles authentication with Clerk.
  ...
  1. Navigate to where your project is located and select the `build/` folder. Then select **Select**. Your extension will now be loaded and shown in the list of extensions.
