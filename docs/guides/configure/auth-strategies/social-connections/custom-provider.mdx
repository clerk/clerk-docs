...
If you're configuring a provider for a native or mobile app, a single-page application (SPA), or another public client that can't securely store a client secret, plan to enable PKCE after creating the connection. PKCE is also a recommended best practice for modern OAuth clients more broadly.
...
If your provider should use PKCE, enable the **Use PKCE** toggle in the **Connection** tab before turning the provider on.

Enable the provider from the top of the details page when ready.

## PKCE support

PKCE (Proof Key for Code Exchange) is a security extension to the OAuth 2.0 Authorization Code flow. Instead of relying only on a static client secret, PKCE generates a per-request secret that protects the authorization code exchange.

Enable PKCE when configuring a custom provider for:

- Native and mobile apps
- Single-page applications (SPAs)
- Any public client where the source code is accessible to end users

Once enabled, Clerk automatically uses the Authorization Code with PKCE flow for that provider.
