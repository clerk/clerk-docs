...
1. **Sign in with \[Your App]** – Let users sign in to _other_ apps using their Clerk credentials from your app. In this case, _your app_ becomes the identity provider (IdP).
...
- **Support AI agents and automation** – Allow [MCP servers or AI tools](/docs/guides/ai/overview) to authenticate users through your app.
...
<If is="llm">
  **For AI agents:** Instead of creating the OAuth application through the Dashboard modal below, use the CLI: run `npx clerk@latest api ls oauth` to discover the OAuth applications endpoints, then create one with `npx clerk@latest api oauth_applications -d '<json>'` — the create response includes the Client ID and Client Secret. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
1. Select **Add application** → **Create OAuth application**. A modal will open.
1. Complete the following fields:
   - `Name` - Helps you identify your application.
   - `Scopes` - The scopes that you would like to leverage. Learn more about [OAuth scopes currently supported by Clerk](/docs/guides/configure/auth-strategies/oauth/how-clerk-implements-oauth#scopes).
...
After a user has successfully completed an OAuth 2.0 flow, you can retrieve additional user information from Clerk's [`/oauth/userinfo`](/docs/reference/frontend-api/tag/oauth2-identity-provider/POST/oauth/userinfo){{ target: '_blank' }} endpoint. When making the request to this endpoint, you must include the OAuth access token in the `Authorization` header.
...
For validating [access tokens](!oauth-access-token) or refresh tokens and retrieving additional token metadata, Clerk provides a standard OAuth 2.0 [Token Introspection Endpoint](https://datatracker.ietf.org/doc/html/rfc7662) at [`/oauth/token_info`](/docs/reference/frontend-api/tag/oauth2-identity-provider/POST/oauth/token_info){{ target: '_blank' }}.
...
You must verify the signature on the ID token before using any of the user information it contains. This will require your instance's public key. You can find your instance's public key by loading your [Frontend API URL](!frontend-api-url) with `/.well-known/jwks.json` appended to it. For example:
