...
Clerk also adds an `act` claim on the [Clerk session token](/docs/backend-requests/session-requests/session-tokens), which contains information like the impersonated session ID and the actor/impersonator's user ID.
...
<Tabs items={["Next.js", "Express"]}>
  ...
  <Tab>
    The Express SDK provides a [middleware](/docs/backend-requests/session-requests/handling/nodejs#clerk-middleware) that attaches the [`Auth`](/docs/references/backend/types/auth-object#auth-object) object to the `request` object.
