...
If you're currently protecting routes in `clerkMiddleware()`, move the auth check into the resource itself and remove it from `clerkMiddleware()`. **You still need to keep `clerkMiddleware()` as it's required for Clerk configuration and can still be used for things like redirects, headers, and other request handling.** Cross-cutting redirects (incomplete [session tasks](/docs/guides/configure/session-tasks), [onboarding](/docs/guides/development/add-onboarding-flow)) can stay in middleware as a redirect convenience, as long as the real enforcement also runs at the resource. For [checking authorization](!authorization-check) (Roles, Permissions, Features, or Plans), see the [Authorization checks](/docs/guides/secure/authorization-checks) guide.
