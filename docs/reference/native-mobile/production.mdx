...
By default, the [Frontend API](/docs/reference/frontend-api){{ target: '_blank' }} (FAPI) accepts cross-origin requests from any subdomain of your configured root domain. If you want to restrict which subdomains are allowed to make these requests, you can configure a [subdomain allowlist](/docs/guides/development/deployment/subdomain-allowlist).
