...
Clerk's JavaScript Backend SDK exposes [Clerk's Backend API](/docs/reference/backend-api) resources and low-level authentication utilities **for JavaScript environments**.

For example, if you wanted to get a list of all users in your application, instead of creating a fetch to Clerk's [`https://api.clerk.com/v1/users`](/docs/reference/backend-api/tag/Users#operation/GetUserList) endpoint, you can use the [`users.getUserList()`](/docs/references/backend/user/get-user-list) method provided by the JavaScript Backend SDK.
