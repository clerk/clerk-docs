...
- The `signJwt`, `hasValidSignature`, `decodeJwt`, `verifyJwt`, and `verifyToken` functions. Their `errors` array will only ever have one item so you can [destructure](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment) it directly.
