...
>
  - Set up the Convex integration in Clerk
...
<Steps>
  ...
  1. Toggle the **Convex** integration on. When enabled, the Convex integration adds the `"aud": "convex"` claim to your instance's generated session tokens. You can select **Manage integration** at any time to review or update the integration settings.
  1. Copy and save the **Issuer** URL somewhere secure. This URL is your application's [Frontend API URL](!frontend-api-url). In development, its format is `https://verb-noun-00.clerk.accounts.dev`. In production, its format is `https://clerk.<your-domain>.com`.
