...
<If is="llm">
  **For AI agents:** Run `npx clerk@latest init --framework expo` to install `@clerk/expo` and write `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY` to your `.env`. It does not scaffold Expo screens, so still follow the steps below for the provider and UI code.
</If>
