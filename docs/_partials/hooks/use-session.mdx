The following example uses the [`useSession()`](/docs/reference/hooks/use-session) hook to access the [`Session`](/docs/reference/objects/session) object. The `Session` object is used to access the `lastActiveAt` property, which is a `Date` object used to show the time the session was last active.
...
<If sdk="expo">
  ```tsx {{ filename: 'src/app/(session)/page.tsx' }}
