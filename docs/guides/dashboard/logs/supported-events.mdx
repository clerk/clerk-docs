...
description: A complete reference of all event types available in Application Logs, organized by category.
---

This page lists all event types that can appear in [Application Logs](/docs/guides/dashboard/logs/application-logs). Use these event type identifiers when filtering logs in the Dashboard.

Event types support trailing wildcards when filtering (e.g., `sign_in.*` matches all sign-in events).
