...
Options are encrypted in request headers for secure server-to-server propagation. When providing a `secretKey` in the options for `clerkMiddleware`, the `CLERK_ENCRYPTION_KEY` becomes mandatory. If no `secretKey` is provided, the `CLERK_ENCRYPTION_KEY` defaults to `CLERK_SECRET_KEY`.

These options are not available for client-side components. To use them, provide them via [ClerkProvider](/docs/components/clerk-provider) or [environment variables](/docs/deployments/clerk-environment-variables).
