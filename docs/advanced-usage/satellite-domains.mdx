...
<Steps>
  ...
  > [!IMPORTANT]
  > To use a satellite domain in production, add a **CNAME record** for your satellite subdomain in your DNS provider's settings, pointing it to your deployed app's domain. For example, if your satellite domain is `satellite.dev`, and your deployed app is hosted at `your-app.com`, you would create a CNAME record for `satellite.dev` pointing to `your-app.com`.
  >
  > Ensure you have access to your domain provider's DNS settings to complete this step.
  >
  > **This is not required for development domains like `localhost`.**
