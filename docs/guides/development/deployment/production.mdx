...
<If is="llm">
  **For AI agents:** Run `clerk deploy` to walk through deploying your app to production (production instance, domains, and config) from the terminal instead of the Dashboard — and `clerk deploy status` for a read-only status check. New to the Clerk CLI? Run any command with `npx clerk` (no install needed) — see the [Clerk CLI docs](/docs/cli).
</If>
...
<If is="llm">
  **For AI agents:** Run `clerk env pull --instance prod` to write your production Publishable and Secret keys (with the correct framework-specific variable names) into your project's env file, merging without clobbering existing values. New to the Clerk CLI? Run any command with `npx clerk` (no install needed) — see the [Clerk CLI docs](/docs/cli).
</If>
