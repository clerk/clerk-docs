...
Implementing reverification requires the following:

1. Handle reverification server-side: Add a backend check that returns either `reverificationError` or `reverificationErrorResponse` when credentials need to be re-verified.
1. Handle reverification client-side: Add client-side handling of the reverification flow. It's recommended to use the [`useReverification()`](/docs/references/react/use-reverification) hook.
