...
Triggers a prompt with passkeys listed. After a user selects a passkey it will execute a sign-in attempt. By default, renders a `<button>`.

### Usage {{ toc: false }}

```tsx filename="page.tsx"
<SignIn.Step name='start'>
  <SignIn.Passkey>Use passkey instead</SignIn.Passkey>
</SignIn.Step>
```
