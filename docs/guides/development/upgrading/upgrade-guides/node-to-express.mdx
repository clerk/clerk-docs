...
In the Node SDK, `requireAuth()` wrapped route handlers. With the Express SDK, use `clerkMiddleware()` to add authentication state to requests, then call `getAuth()` inside the route handler to protect the route.

```ts {{ del: [1, 4], ins: [2, [5, 14]] }}
...
If you'd like to retain the previous behavior of `requireAuth()` from the Node SDK by emitting an error, you can accomplish this as such:
...
To enforce strict authentication, replace your existing Node SDK strict middleware, `ClerkExpressRequireAuth`, with `clerkMiddleware()` and [`getAuth()`](/docs/reference/express/get-auth).

Use `clerkMiddleware()` to add authentication state to requests, then call `getAuth()` inside the route handler to decide how to handle unauthenticated users.

```ts {{ del: [1, [11, 13]], ins: [2, 8, [14, 22]] }}
...
If you were passing custom Clerk keys or a custom API URL, pass those to [`createClerkClient`](/docs/reference/backend/overview) and then pass the custom `clerkClient` into the middleware.
...
The [`clerkClient`](/docs/reference/backend/overview) exposes Clerk's Backend API resources. If you were importing `clerkClient` from the Node SDK, you can import it from the Express SDK.
...
If you were instantiating a custom `clerkClient` with `createClerkClient`, you can no longer import this method from the Express SDK. You must import it from [`@clerk/backend`](/docs/reference/backend/overview).
