...
When performing organization-related operations, ensure you access the current user's organization membership using `auth().orgId` to prevent users from creating resources for other organizations they don't belong to.
...
export default function AddVerifiedDomain() {
  async function addVerifiedDomain(formData: FormData) {
    ...
    if (!userId || !orgId) {
      throw new Error('You must be signed in with a valid organization membership')
    }
