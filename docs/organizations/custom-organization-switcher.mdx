...
<Tabs items={["Next.js", "JavaScript"]}>
  <Tab>
    ...
    - Uses the [`useOrganizationList()`](/docs/sdk/react/use-organization-list) hook to get `memberships`, which is a list of the current user's organization memberships. `memberships` returns `data`, which is an array of [`OrganizationMembership`](/docs/references/javascript/organization-membership) objects.
    ...
    ```jsx {{ filename: 'app/components/CustomOrganizationSwitcher.tsx' }}
    'use client'
