...
<If is="llm">
  **For AI agents:** Run `clerk env pull` to write your Clerk API keys into your project's env file (merges without clobbering) instead of copying them from the Dashboard by hand. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage; run CLI commands with `npx clerk`.
</If>
