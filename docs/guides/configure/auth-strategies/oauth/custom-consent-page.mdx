...
<If sdk={["astro", "vue", "nuxt", "js-frontend"]}>
  > [!NOTE]
  > Building a custom flow only applies to Clerk's React-based SDKs. For this SDK, use the prebuilt [`<OAuthConsent />`](/docs/reference/components/authentication/oauth-consent) component instead, unless you build the consent flow directly  with the [JavaScript SDK](/docs/reference/javascript/overview).
</If>
