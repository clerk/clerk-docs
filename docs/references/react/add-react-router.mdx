...
Learn how to add React Router to your application using React Router's new Data API router. This will cover configuring layouts and setting up protected routes.

<Steps>

### Install `react-router-dom`

React Router's `react-router-dom` is a mature, battle tested routing package for React that gives you many options. As it is the most popular routing option, we will use it for this guide. 
...
The exact routes your will need will depends on your application. For this guide you will create `/`, `/contact`, `/dashboard`, '/sign-in' and 'sign-up' routes. The `/dashboard` route will contain a default route (`/dashbard/`) and a invoices routes (`/dashboard/invoices`). The first step will be creating basic components for these routes. 

Use the tabs below to find the example components and recreate these files using the path from each tab.

<CodeBlockTabs type="installer" options={["Index", "Contact", "Sign-in", "Sign-up", "Dashboard",  "Invoices"]}>
...
The second layout will be non-rendering and will protect everything in the `/dashboard` route. This avoids the need for per page auth checks, or using control componnets.


<CodeBlockTabs type="installer" options={["Root Layout", "Dashboard Layout"]}>
...
- The new root-layout.tsx has a simple header that includes the `<UserButton />` and a link to the `/sign-in` page instead of a `<SignInButton />`. The rendering of these is controlled byt the `<SignedIn />` and `<SignedUp />` control components. This is very similar to the [Embed the `<UserButton />` and `<SignInButton />`](/docs/quickstarts/react#embed-the-user-button-and-sign-in-button) step from the [React Quickstart](/docs/quickstarts/react).

Both the layouts contain a `<Outlet />` componet from `react-router-dom`. This behaves similar to `{children}` in Next.js or more generic Rect componets. You will take advantage of this component in the next step.

### Wire layouts and routes up with `createBrowserRouter`

With all the routes and layouts you need created, you now need to wire up the layouts and the routes with the `createBrowserRouter` function from `reacr-router-dom`. This will use the Data API (aka Data Router) to configure your application.

You can start by removing `src/App.tsx` -- the contents of that file moved to `src/layouts/root-layout.tsx`. In `src/main.tsx` you will import `RouterProvider` and `createBrowserRouter()` from `react-router-dom`, as well as all of the default components from the layouts and routes you created. Replace the contents inside of `<React.StritMode />` with just ` <RouterProvider router={router} />`. Lastly you will build the `router` using `createBrowserRouter()`. 
...
- Several reoutes are direct children of the root layout. These are all public routes. You can use control components like `<SignedIn>` or check the `userId` from `useAuth()` if yo uwant to make content protected.
- The `<DashboardLayout` is child of the root layout, renders nothing and has a check to see if the `userId` exists. This will confirm that a user is logged in. If the userId is not truthy, then the user will be redirected to the `/sign-in` route to sign-in. That protects all pages in the `/dashboard` route. All children of `/dashboard` will be mounted inside the `<Outlet />` component in the dashboard layout.
...
Your application is setup with react-router and ready for you to add more layouts and routes as needed! If you want to get started using a template from this guide, please clone the following repository and then checkout the `integrate-react-router-dom-using-data-router-method` branch.
