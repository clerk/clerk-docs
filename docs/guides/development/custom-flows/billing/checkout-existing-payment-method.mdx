...
<Steps>
  ...
  <Tabs items={["Next.js"]}>
    <Tab>
      ...
      1. Uses the [`useCheckout()`](/docs/references/hooks/use-checkout) hook to initiate and manage the checkout session.
      1. Uses the [`usePaymentMethods()`](/docs/references/hooks/use-payment-methods) hook to fetch the user's existing payment methods.
