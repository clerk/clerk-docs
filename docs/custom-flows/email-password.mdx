...
export default function Page() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Once the sign-up is created, and email address verification is prepared, the sign-up form is no longer in a submission/loading state. The verification state is set to true and as a result, the verification form will be presented to the user so they can input their verification code.
      setVerifying(true)
      setLoading(false)
    }
    catch (err: any) {
