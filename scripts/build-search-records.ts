...
const OUTPUT_PATH = path.join("./.output/search-records.json");
...
function generateRecordsFromDoc(
  ...
  const availableSdksRaw = doc.frontmatter.availableSdks
    ?.split(",")
    .filter(Boolean);
  // Use ["all"] for non-SDK-scoped docs (no availableSdks in frontmatter)
  const availableSdksList =
  ...
  mdastVisit(doc.node, (node) => {
    ...
    if (node.type === "heading" && "depth" in node) {
      ...
      records.push(
        createRecord(`lvl${depth}` as SearchRecord["type"], null, anchor)
      );
      return;
    }
