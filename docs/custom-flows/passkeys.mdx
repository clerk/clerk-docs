...
- Create paths that host the [`<SignIn />`](/docs/components/sign-in) or [`<SignUp />`](/docs/components/sign-up) components in your application (e.g., `/sign-in` and `/sign-up`) instead of using Account Portal
