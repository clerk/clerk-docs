...
>
  ...
  <AccordionPanel>
    ...
    The `navigate` callback receives an object with the `session` property and the `decorateUrl` function. [Learn more about the `navigate` callback](/docs/references/javascript/types/set-active-params#navigate).
  </AccordionPanel>
