...
> [!WARNING]
> [**Email** must be enabled in the Clerk Dashboard](/docs/guides/configure/auth-strategies/sign-up-sign-in-options#email) to allow waitlist invitation emails to be sent to users after they are approved. Support for sending waitlist invitations when **Email** is disabled is being worked on.
