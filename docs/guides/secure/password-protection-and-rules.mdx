...
1. In the Clerk Dashboard, navigate to the [**Users**](https://dashboard.clerk.com/~/users) page and select the user you want to mark as compromised. You'll be redirected to the user's settings.
1. In the **Password** section, if a password is set, open the three-dot menu and select **Set password as compromised**. A modal will open asking you to confirm the action. Complete the instructions.

> [!IMPORTANT]
> Setting a user's password as compromised will prevent the user from signing in until they reset their password. If you are implementing [custom authentication flows](!custom-flow), you will need to handle the compromised password flow by yourself. See the [dedicated custom flow guide](/docs/guides/development/custom-flows/authentication/forgot-password) for more information.
