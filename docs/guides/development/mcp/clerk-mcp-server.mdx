...
<Accordion titles={["Why am I seeing an internal server error when trying to connect?", "Does the MCP server support SSE (Server-Sent Events)?", "When or why should I use the Clerk MCP server?"]}>
  <AccordionPanel>
  ...
  <AccordionPanel>
    Use the Clerk MCP server when building with Clerk and you want fast, accurate implementation guidance, with code examples for Clerk SDK features.
