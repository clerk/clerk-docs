...
<Tabs items={['<Show />', '<Show />', '<Show />']}>
  <Tab>
    The [`<Show />`](/docs/reference/components/control/show) component protects content or even entire routes based on a user's authentication state. It will render its children if the user's state is signed-in when using `when='signedIn'`. It accepts a `fallback` prop that will be rendered if the user's state is signed-out.
    ...
    export default function Page() {
      return (
        <Show fallback={<p>Signed-out and pending users can see this.</p>} when='signedIn'>
  ...
  <Tab>
    The [`<Show when='signedOut'>`](/docs/reference/components/control/show) component renders its children if the user's authentication state is signed-out.
    ...
    export default function Page() {
      return (
        <Show when='signedOut' treatPendingAsSignedOut={false}>
  ...
  <Tab>
    The [`<Show when='signedIn'>`](/docs/reference/components/control/show) component renders its children if the user's authentication state is signed-in.
    ...
    export default function Page() {
      return (
        <Show when='signedIn' treatPendingAsSignedOut={false}>
