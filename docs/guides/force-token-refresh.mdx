...
A user's session token is a short-lived JWT that Clerk [refreshes every 60 seconds](/docs/how-clerk-works/overview#token-refresh-mechanism). However, there are some cases where you might want to force a refresh.

There are two recommended approaches to force a user's session token to be refreshed:
...
- **Client-side:** Access `getToken()` from the [`useSession()`](/docs/hooks/use-session.mdx) or [`useAuth()`](/docs/hooks/use-auth.mdx) hooks.
- **Server-side:** Access `getToken()` from the [`Auth`](/docs/references/backend/types/auth-object.mdx) object.

The following example shows how to force a JWT refresh by using the [`useSession()`](/docs/hooks/use-session.mdx) hook to access the `getToken()` method.
...
The `user.reload()` method refreshes the current user's `User` object and their session token. It can be accessed from the [`useUser()`](/docs/hooks/use-user.mdx) hook.

The following example shows how to force a JWT refresh by using the [`useUser()`](/docs/hooks/use-user.mdx) hook to access the `user.reload()` method.
...
Say you're building a user profile page. You want to allow the user to update their information, and also allow them to set and update custom information. To store information about a user that Clerk doesn't collect, you can use [metadata](/docs/users/metadata.mdx), which will get stored on the user's `User` object as `User.publicMetadata`.
