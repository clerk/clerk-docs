...
{/* TODO: Jeff to remove per page and per layout auth protection in favor of middleware protection */}
...
For more details on route protection, check out the [`clerkMiddleware()`](/docs/references/nextjs/clerk-middleware) documentation.
...
Clerk strongly recommends migrating to the new [`clerkMiddleware()`](/docs/references/nextjs/clerk-middleware) for an improved DX and access to all present and upcoming features, but also want to note that `authMiddleware()`, while deprecated, will continue to work in v5 and will not be removed until the next major version, so you do not _need_ to make any changes to your Middleware setup this version.
...
Of course, in most cases you'll have a more complicated setup than this. You can find some examples below for how to migrate a few common use cases.
