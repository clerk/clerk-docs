...
<Steps>
  ...
  - `mcpAuthClerk`: Authentication middleware that automatically verifies the `Authorization` header using Clerk-issued OAuth tokens.
  - `protectedResourceHandlerClerk`: Express handler that serves the OAuth protected resource metadata, required for dynamic client registration.
  - `streamableHttpHandler`: Express handler that connects your MCP server to incoming requests using the HTTP streaming transport.
