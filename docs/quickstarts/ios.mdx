...
<Steps>
  ...
  Finally, provide users with a way to sign out of your app. In your `ContentView`, add the following code. It adds a button that calls Clerk's [`signOut()`](/docs/references/javascript/clerk#sign-out) method in order to end a user's session.
