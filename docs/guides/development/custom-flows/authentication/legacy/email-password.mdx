...
<Steps>
  ...
  <If notSdk={["js-frontend", "expo"]}>
    ...
    1. To verify the user's email address, send a one-time code to the provided email address with the [`SignUp.prepareEmailAddressVerification()`](/docs/reference/objects/sign-up#prepareemailaddressverification) method.
    1. Collect the user's one-time code and verify it with the [`SignUp.attemptEmailAddressVerification()`](/docs/reference/objects/sign-up#attemptemailaddressverification) method.
    1. If the email address verification is successful, the `SignUp.status` will be `complete`, and you can finish the sign-up flow by calling the [`setActive()`](/docs/reference/objects/clerk#setactive) method, which will set the newly created session as the active session. You may need to check for [session tasks](!session-tasks) that are required for the user to complete after signing up.
