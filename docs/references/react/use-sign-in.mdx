...
| `needs_factor_two` | The Second Factor verification is missing. The Second Factor is a optional step to provide additional verification and includes `phone_code` and `totp`. See [Second Factor](/docs/references/javascript/sign-in/second-factor) for details.|
