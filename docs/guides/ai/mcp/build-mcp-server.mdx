...
<Steps>
  ...
  <If sdk="expressjs">
    Run your server with the `--env-file=.env` flag, or call `process.loadEnvFile()` at the top of your entry file to ensure environment variables are loaded before importing Clerk modules.
  ...
  <If sdk="expressjs">
    ```ts {{ filename: 'index.ts', mark: [[14, 36]], collapsible: true }}
    // Load environment variables before importing Clerk
    process.loadEnvFile()
