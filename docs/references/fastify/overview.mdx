...
The Clerk Fastify SDK provides a powerful set of tools and utilities to seamlessly integrate authentication, user management, and organization management into your Fastify application. Refer to the [quickstart guide](/docs/getting-started/quickstart) to get started.
...
The `getAuth()` helper retrieves the current user's authentication state from the `request` object. It returns the [`Auth` object](/docs/references/backend/types/auth-object){{ target: '_blank' }}. See the [Next.js reference documentation](/docs/references/nextjs/pages-router/get-auth){{ target: '_blank' }} for more examples on how to use the returned `Auth` object.
