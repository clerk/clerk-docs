...
<Steps>
  ...
  1. In the `root-layout.tsx` file, paste the following code to create a layout for your app.
     ...
     - The footer includes Clerk's [`<UserButton />`](/docs/references/components/user/user-button) component and a link to the `/settings` page, which renders Clerk's [`<UserProfile />`](/docs/references/components/user/user-profile) component. Clerk's [`<SignedIn>`](/docs/references/components/control/signed-in) and [`<SignedOut>`](/docs/references/components/control/signed-out) control components determine what's displayed based on the user's authentication state.
