...
<Properties>
  ...
  **Required** if [Web3 authentication](/docs/authentication/configuration/sign-up-sign-in-options#web3-authentication) is enabled. The Web3 wallet address, made up of 0x + 40 hexadecimal characters.
  ...
  If `strategy` is set to `'email_link'`, this specifies the full URL or path that the user will be redirected to when they visit the email link. See the [custom flow](/docs/custom-flows/email-links) for implementation details.
...
<Properties>
  ...
  The full URL or path that the user will be redirected to when they visit the email link.
</Properties>
