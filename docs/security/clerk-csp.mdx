...
You can customize Content Security Policy (CSP) by adding your own directives to either the `default` or `strict` configurations. Simply provide a `directives` object, and your custom rules will be merged with Clerk's default security settings.
