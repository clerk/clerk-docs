...
<Properties>
  ...
  The current user's [session claims](/docs/guides/how-clerk-works/session-tokens).
  ...
  A function that gets the current user's [session token](/docs/guides/how-clerk-works/session-tokens) or a [custom JWT template](/docs/guides/development/jwt-templates).
...
`getToken()` retrieves the current user's [session token](/docs/guides/how-clerk-works/session-tokens) or a [custom JWT template](/docs/guides/development/jwt-templates).
...
- For React-based applications, you can use the `useAuth()` hook. See the [reference documentation](/docs/references/general/hooks/use-auth) for example usage.
...
<Tabs items={["Next.js", "Tanstack React Start", "Express", "Remix"]}>
  <Tab>
    ...
    - In App Router applications, use the [`auth()`](/docs/references/nextjs/app-router/auth) helper.
    - In Pages Router applications, use the [`getAuth()`](/docs/references/nextjs/pages-router/get-auth) helper.
  ...
  <Tab>
    To use the `getToken()` method in the backend, you can access it using the [`getAuth()`](/docs/references/nextjs/pages-router/get-auth) function.
...
The following is an example of the `Auth` object without an active organization. Notice that there is no `o` claim. Read more about token claims in the [guide on session tokens](/docs/guides/how-clerk-works/session-tokens).
...
The following is an example of the `Auth` object with an active organization. Notice the addition of the `o` claim. Read more about token claims in the [guide on session tokens](/docs/guides/how-clerk-works/session-tokens).
