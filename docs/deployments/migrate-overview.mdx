...
With Basic Export / Import, you are taking an export from your previous tool and importing data into Clerk. The most common way to handle this is by making use of the [Backend API](/docs/reference/backend-api/tag/Users#operation/CreateUser) and its `CreateUser` API endpoint. It's important to note that the `CreateUser` endpoint has a rate limit of 20 requests per 10 seconds.
...
> **Note:** If you are expecting to import 100k+ users, we recommend reaching out to [support@clerk.dev](mailto:support@clerk.dev) where we can work with you to coordinate a seamless import of your data.
