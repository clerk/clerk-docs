...
Any environment variable used in the context of Clerk should be prefixed with `CLERK_`. Learn more about [Clerk environment variables](/docs/guides/development/clerk-environment-variables).
...
Additionally, you need to make the environment variables for [API and SDK configuration](/docs/guides/development/clerk-environment-variables#api-and-sdk-configuration) available to your users.
...
Clerk recommends implementing the [sign-in and sign-up redirect](/docs/guides/development/clerk-environment-variables#sign-in-and-sign-up-redirects) environment variables.
...
- If you haven't already, [sign up for an account](https://dashboard.clerk.com/sign-up) and create a new application. Go to the [**API keys**](https://dashboard.clerk.com/~/api-keys) page to retrieve your [Publishable Key](!publishable-key) and/or [Secret Key](!secret-key).

<!-- Mention any other necessary steps a user has to perform before continuing. Mention any hard requirements (e.g., a minimum supported Node.js version). -->
...
Here's how the [`@clerk/astro`](/docs/reference/astro/overview) package is using subpath exports:
