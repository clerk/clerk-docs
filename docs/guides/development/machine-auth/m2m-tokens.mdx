...
> [!IMPORTANT]
> Loading environment variables is different depending on the framework you are using. For example, Next.js requires the `NEXT_PUBLIC_` prefix for environment variables that are available on the frontend. If you're running a Node server, you can load `.env` files natively with `node --env-file=.env` or `process.loadEnvFile()`. Check your framework's documentation for information on how to load environment variables.
