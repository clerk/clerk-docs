...
{/* TODO: check this pls */}
```typescript
function getOrganizationMemberships: (params: ClerkPaginationParams) => => Promise<ClerkPaginatedResponse<OrganizationMembershipResource>>;
```
...
| Type | Description |
| --- | --- |
| <code>Promise\<[PaginatedResponse](/docs/references/javascript/types/paginated-response)\<OrganizationMembershipResource\>\></code> | The list of organizations the user is a member of. |

{/* TODO: We might need to create a doc page that covers OrganizationMembershipResource */}

#### `getOrganizationMemberships()` example

{/* TODO: add a code example of this method being accessed and used, because that's how we're moving now. That's what we wanna do for methods now, we need to start providing examples on how they are accessed and used in real applications :~) */}
