...
Send an email link to the supplied email address. After the user opens the link, handle the deep link callback with `Clerk.auth.handle(intent.data)`.
...
Send an email link to verify the user's account. After the user opens the link, handle the deep link callback with `Clerk.auth.handle(intent.data)`.
