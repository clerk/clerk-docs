...
## Step 1: Install the Clerk Skills
...
## Step 3: Replace `<SignedIn>` and `<SignedOut>`
...
1. Re-run the search from Step 2 and confirm no `@clerk/nextjs` import still pulls in `SignedIn`, `SignedOut`, or `Protect`.
