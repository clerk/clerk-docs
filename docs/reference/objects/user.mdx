...
- If the connection failed, then the `externalAccount.verification.status` will not be `verified` and the `externalAccount.verification.error` will contain the error encountered, which you can present to the user. To learn more about the properties available on `verification`, see the [`VerificationResource`](/docs/reference/types/verification-resource) reference.
...
Creates a passkey for the signed-in user. Returns a [`PasskeyResource`](/docs/reference/types/passkey-resource) object.
