...
he React example below uses `react-router-dom` to define the required route. For NextJS apps, you only need to create a `pages/sso-callback` file.
...
To whitelist a redirect URL via the Clerk Dashboard, go to the [Social Connections](https://dashboard.clerk.com/last-active?path=user-authentication/social-connections?tab=Settings) tab of the **User & Authentication* section. On the **Social Connections** page, click on the **Settings* tab and add your redirect URL to the **Redirect URLs** list.
