...
<Properties>
  ...
  **Required** if `strategy` is set to `email_link`. The full URL or path that the user will be redirected to when they visit the email link. See the [custom flow](/docs/custom-flows/email-links) for implementation details.
</Properties>
