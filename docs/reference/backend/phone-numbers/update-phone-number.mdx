...
<Properties>
  ...
  Whether or not the phone number is reserved for [multi-factor authentication](/docs/guides/configure/auth-strategies/multi-factor-authentication). The phone number must also be verified. If there are no other reserved [second factors](!second-factor-verification), the phone number will be set as the default second factor.
</Properties>
