...
<Steps>
  ...
  1. Create a `middleware|proxy.ts` file.

     - If you're using the `/src` directory, create `middleware|proxy.ts` in the `/src` directory.
     - If you're not using the `/src` directory, create `middleware|proxy.ts` in the root directory alongside `.env`.

  1. In your `middleware|proxy.ts` file, export the `clerkMiddleware()` helper:
