...
<Tabs items={["Hooks", "JavaScript"]}>
  <Tab>
    ...
    ```tsx {{ filename: 'app/sign-in/[[...sign-in]]/page.tsx', mark: [[6, 7], 13, [21, 22], [56, 59], [90, 96]] }}
    'use client'
...
If you have enabled the [**Reject compromised passwords**](/docs/guides/secure/password-protection-and-rules#reject-compromised-passwords) setting, then sign-up/sign-in and password update attempts will be rejected with an error if the password is compromised. In this case, you can prompt the user to reset their password. See the [dedicated custom flow guide](/docs/guides/development/custom-flows/account-updates/forgot-password#reset-compromised-passwords) for more information.
