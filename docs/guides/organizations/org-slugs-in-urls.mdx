...
<Steps>
  ...
  The [`<OrganizationSwitcher />`](/docs/references/components/organization/organization-switcher) and [`<OrganizationList />`](/docs/references/components/organization/organization-list) components provide a robust set of options to manage organization slugs and IDs in your application's URLs.
  ...
  <Tabs items={["Server-side","Client-side"]}>
    ...
    <Tab>
      To get organization information on the client-side, use the [`useOrganization()`](/docs/references/hooks/use-organization) hook to access the [`organization`](/docs/references/javascript/organization) object.
