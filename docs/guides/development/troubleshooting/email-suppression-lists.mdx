...
In the Clerk Dashboard, navigate to the [**Email Logs**](https://dashboard.clerk.com/~/email-logs) page to view suppression-related events such as `bounce`, `blocked`, `spam_report`, or `dropped`.
...
Suppression state is scoped to the instance you're viewing in the Clerk Dashboard. When Email Logs show a suppression-list action for an event, the action applies only to that instance's current email provider account. It doesn't remove the same address from another application, another instance, or a previously used email provider account.
...
Clerk and its email providers add addresses to suppression lists automatically after delivery events such as bounces, blocks, or spam reports. You can't manually add an address to a suppression list from the Clerk Dashboard.

To remove an address from a suppression list:

1. In the Clerk Dashboard, navigate to the [**Email Logs**](https://dashboard.clerk.com/~/email-logs) page.
1. Select the suppression-related event for the recipient.
1. In the event details, select **Remove**.
