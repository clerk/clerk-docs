...
1. **Optional:** If you're configuring a provider for a native or mobile app, a single-page application (SPA), or another public client that can't securely store a client secret, enable the **Use PKCE** toggle. [PKCE (Proof Key for Code Exchange)](https://datatracker.ietf.org/doc/html/rfc7636) is a recommended best practice for modern OAuth clients. Once enabled, Clerk will use the Authorization Code with PKCE flow for authentication with that provider. Learn more about [PKCE](/docs/guides/configure/auth-strategies/oauth/how-clerk-implements-oauth#public-clients-and-pkce).
1. Select **Add connection**. You will be redirected to the connection's configuration page.

The provider is now configured but not yet enabled. On the connection's configuration page, find the **Authorized redirect URLs** to configure in your provider's settings. From there, you can enable the provider from the top of the page by selecting the **Enable connection** toggle.

> [!NOTE]
> If you didn't enable PKCE during creation, select the connection and enable the **Use PKCE** toggle under the **SSO** tab.
