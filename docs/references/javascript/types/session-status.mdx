...
| `pending` | The user has signed in but hasn't completed [after-auth flows](/docs/authentication/configuration/after-auth-flows). See **After-auth flows** for how a session becomes pending and how to resolve tasks. |
