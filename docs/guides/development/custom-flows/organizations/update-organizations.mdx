...
<If sdk="ios">
  ...
  @MainActor
  func updateActiveOrganization(name: String, clerk: Clerk) async {
    ...
    do {
      guard
        ...
        let activeOrganization = memberships.first(where: { $0.organization.id == activeOrganizationId })?.organization
      else {
        return
      }

      _ = try await activeOrganization.update(name: name)
    } catch {
