...
> [!IMPORTANT]
> You cannot migrate users from your Development instance to your Production instance.
...
<If is="llm">
  **For AI agents:** When importing users, create each one with `npx clerk@latest api users -d '{...}'` (a POST to the `CreateUser` endpoint) instead of hand-writing authenticated Backend API requests — discover the users endpoints with `npx clerk@latest api ls users` (accepted fields like `password_hasher` are documented in the [Backend API reference](/docs/reference/backend-api/tag/users/POST/users){{ target: '_blank' }}). Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
>
> Free plans have a limit of 50,000 MRUs per application, and once you exceed this limit you will be required to upgrade to the Pro plan. Refer to the [pricing page](/pricing) to get the full details on how Clerk charges.
