...
This guide demonstrates how to protect pages, route handlers, and Server Actions in your Next.js application by checking authentication **close to the resource being accessed**.

> [!TIP]
> Protect access as close to the resource as possible, in the code that reads or mutates the data. See [why protecting close to the resource is simpler](#why-protect-close-to-the-resource).
...
To protect everything under a path like `/dashboard`, put the check in a small shared helper and call it from each protected resource:
...
## Why protect close to the resource?

When the same code that reads or mutates the data also enforces access to it, authorization lives next to the thing it protects. There's one place to look and nothing to keep in sync.

Gating routes inside `clerkMiddleware()` with `createRouteMatcher()` puts that decision in a separate model of your routes, apart from the pages, handlers, and actions they map to. Clerk matches the request URL the same way your framework and host route it, so that route model is one more thing to maintain as routes, rewrites, and hosting change. Keeping the check at the resource avoids that.
