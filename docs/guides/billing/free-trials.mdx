...
By default, users must provide a payment method to start a free trial. This helps prevent abuse and ensures a smooth transition to paid service when the trial ends.

You can disable this requirement so that users can start a free trial without entering any payment details during checkout. To do so:

1. Navigate to [**Configure > Billing > Settings**](https://dashboard.clerk.com/~/billing/settings) in the Clerk Dashboard.
1. Disable **Require payment method for free trials**.

> [!WARNING]
> If you disable this requirement, users who don't add a payment method during checkout won't be automatically charged when their trial ends. They will need to add a payment method and manually subscribe to continue their access.
