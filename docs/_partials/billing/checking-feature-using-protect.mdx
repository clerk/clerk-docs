<If sdk="astro">
  > [!WARNING]
  > Astro SDK hasn't been updated to support "feature" in the `Protect` component yet.
...
<If sdk="chrome-extension">
  ```tsx {{ filename: 'src/screens/protected-premium-content.tsx' }}
  import { Protect } from '@clerk/chrome-extension'

  export default function ProtectedPremiumContentScreen() {
    return (
      <Protect
        feature="premium_access"
        fallback={<p>Only subscribers with the Premium Access feature can access this content.</p>}
...
<If sdk="nuxt">
  ...
  <template>
    <main>
      <Protect feature="premium_access" fallback="Only subscribers with the Premium Access feature can access this content.">
        <h1>Exclusive Premium Content</h1>
        <p>This content is only visible to users with Premium Access feature.</p>
      </Protect>
    </main>
...
<If sdk="expo">
  ...
  export default function ProtectedPremiumContentPage() {
    return (
      <Protect
        feature="premium_access"
        fallback={<Text>Only subscribers with the Premium Access feature can access this content.</Text>}
      >
        <View>
