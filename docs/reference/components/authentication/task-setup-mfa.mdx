...
<If notSdk="js-frontend">
  ...
  The following example demonstrates how to host the `<TaskSetupMFA />` component on a custom page. You first need to [set the `taskUrls` option on your Clerk integration](/docs/guides/configure/session-tasks#using-the-taskurls-option) so that users are redirected to the page where you host the `<TaskSetupMFA />` component when they have a pending `setup-mfa` session task.
...
<If sdk="js-frontend">
  ...
  You first need to set the `taskUrls` option on the [`clerk.load()`](/docs/reference/objects/clerk#load) method so that users are redirected to the page where you host the `<TaskSetupMFA />` component when they have a pending `setup-mfa` session task.
  ...
  The following methods available on an instance of the [`Clerk`](/docs/reference/objects/clerk) class are used to render and control the `<TaskSetupMFA />` component:

  - [`mountTaskSetupMFA()`](#mounttasksetupmfa)
  - [`unmountTaskSetupMFA()`](#unmounttasksetupmfa)
