...
Tasks are pending requirements that users must complete after authentication, such as selecting an organization. When enabled in the Clerk Dashboard, these tasks are handled automatically within the `<SignIn />` and `<SignUp />` components.
