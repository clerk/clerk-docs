...
## Next.js infinite redirect issues

Projects using `@clerk/nextjs@v4` will encounter infinite redirect loops when running a Next.js version between `>=13.4.13-canary.0` and `<=13.5.4`. This is because of [a bug](https://github.com/vercel/next.js/pull/56797) related to `x-forwarded-*` in those versions of Next.js.
