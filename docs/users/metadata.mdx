...
<Tabs items={["Frontend SDKs", "Express", "Go", "Ruby", "cURL"]}>
  <Tab>
...
On the frontend, it's available on the [`User`](/docs/references/javascript/user) object which can be accessed using the [`useUser()`](/docs/references/react/use-user) hook.
...
On the frontend, it is available on the [`User`](/docs/references/javascript/user) object, which you can access by using the [`useUser()`](/docs/references/react/use-user) hook.
