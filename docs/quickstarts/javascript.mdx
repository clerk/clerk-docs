...
<Tabs items={["NPM module", "<script>"]}>
  ...
  <Tab>
    <Steps>
      ...
      It's recommended to use environment variables to store your Clerk **Publishable Key** and **Frontend API URL**. A common way to use environment variables in a JavaScript project is putting the values in a `.env` file, then loading the values using the [`dotenv`](https://www.npmjs.com/package/dotenv) package. An example of how to set it up in this way is shown below:

      1. Create a `.env` file at the root of your project, and add your Clerk publishable key.
      ...
      2. Install the `dotenv` package using your package manager of choice:
      ...
      3. Use the `dotenv` package to load the environment variables from the `.env` file.
