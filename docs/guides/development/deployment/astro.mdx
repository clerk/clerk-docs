...
> [!WARNING]
> This guide assumes that you have already installed Clerk in your application locally and are ready to deploy. If you haven't installed Clerk yet, see [the quickstart guide](/docs/astro/getting-started/quickstart).

This guide will walk you through the steps to deploy your Astro app to production.

## Deploy your Clerk app to production

Follow the [guide on deploying your Clerk app to production](/docs/guides/development/deployment/production).
...
Due to Netlify's caching strategies, one of Clerk's core mechanisms is unable to work as expected, resulting in infinite redirects that cause an app to fail. This also affects preview environments that use **development** API keys. It's recommended to use [production API keys with a new domain](/docs/guides/development/managing-environments#acquire-an-additional-root-domain).

Additionally, read about [issues with Edge middleware](#issues-with-edge-middleware).

### Vercel

Read about [issues with Edge middleware](#issues-with-edge-middleware).
