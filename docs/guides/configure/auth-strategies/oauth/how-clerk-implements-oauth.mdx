...
To configure the token format for an OAuth application, navigate to the [**OAuth applications**](https://dashboard.clerk.com/~/oauth-applications) page in the Clerk Dashboard and select your application. Under **Token format**, choose between **JWT** (default) or **Opaque**.
