...
<Properties>
  ...
  - `passwordHasher?`
  - `'argon2i' | 'argon2id' | 'bcrypt' | 'bcrypt_sha256_django' | 'md5' | 'md5_salted' | 'pbkdf2_sha256' | 'pbkdf2_sha256_django' | 'pbkdf2_sha1' | 'pbkdf2_sha512' | 'pbkdf2_sha512_hex' | 'phpass' | 'scrypt_firebase' | 'scrypt_werkzeug' | 'sha256' | 'sha256_salted' | 'sha512_symfony'`
  ...
  **Deprecated. Use [`updateUserMetadata()`](/docs/reference/backend/user/update-user-metadata) for partial updates (deep-merges with the existing value) or [`replaceUserMetadata()`](/docs/reference/backend/user/replace-user-metadata) for full replacement.** This parameter will be removed in a future major version.
