...
Now with this function in hand, whenever a user does any kind of action, eg submitting a rental application on your next gen ai rental platform, just call this during it (or even in a [WaitUntil](https://vercel.com/docs/functions/functions-api-reference/vercel-functions-package#waituntil)) to ensure your database is in sync with Clerk. Removing any race condition issues.
...
If you are using a more tradition api, you could add to the `update()` function in our `getUser()` a line that attaches a header to the response say `x-refresh-token` and then client side you could check the response of all requests and run a `reload()` when the header is present. But in while using server actions we don't get such abilities. So we will need to be diligent and ensure that server actions that change clerk user data, custom or not, also `reload()` on the client side once resolved back to the user.