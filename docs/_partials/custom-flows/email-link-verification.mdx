...
1. Pass the [`User.createEmailAddress()`](/docs/reference/objects/user#createemailaddress) method to the [`useReverification()`](/docs/reference/hooks/use-reverification) hook to require the user to [reverify their credentials](/docs/guides/secure/reverification) before being able to add an email address to their account.
1. If the `createEmailAddress()` function is successful, a new [`EmailAddress`](/docs/reference/types/email-address) object is created and stored in `User.emailAddresses`. Use the newly created `EmailAddress` object to access the [`createEmailLinkFlow()`](/docs/reference/types/email-address#createemaillinkflow) method.
1. Use the `createEmailLinkFlow()` method to access the [`startEmailLinkFlow()`](/docs/reference/types/email-address#createemaillinkflow) method. Call the `startEmailLinkFlow()` method to send an email with a verification link to the user. This method requires specifying a `redirectUrl`, which is the URL that the user will be redirected to after visiting the link in their email.
1. On your verification page (the URL you set as the `redirectUrl` parameter), call the [`useClerk()`](/docs/reference/hooks/use-clerk) hook to access the [`handleEmailLinkVerification()`](/docs/reference/objects/clerk#handleemaillinkverification) method.
