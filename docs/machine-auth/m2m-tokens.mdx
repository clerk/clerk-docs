...
{/* TODO: Have this callout accommodate for other SDKs. e.g. nextjs would be NEXT_PUBLIC */}
...
To create an M2M token, call the [`createToken()`](/docs/references/backend/m2m-tokens/create-token) method:
...
Imagine a request was made from **Machine A** to **Machine B**, and Machine B received it successfully. The next step is to verify that the token is valid and that the request is coming from a valid machine. To do so, you can use the [`verifyToken()`](/docs/references/backend/m2m-tokens/verify-token) method:
...
To revoke an M2M token, call the [`revokeToken()`](/docs/references/backend/m2m-tokens/revoke-token) method:
