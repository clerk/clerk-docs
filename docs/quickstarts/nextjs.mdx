...
<Tabs type="router" items={["App Router", "Pages Router"]}>
  <Tab>
  <Callout type="info">
    Your `app/layout.tsx` file should be located in the root directory, or inside the `src/` directory if you are using it.
  </Callout>
  ...
  <Tab>
  <Callout type="info">
    Your `pages/_app.tsx` file should be located in the root directory, or inside the `src/` directory if you are using it.
  </Callout>
...
1. Create a `src/middleware.ts` file. Your `middleware.ts` file should be placed inside the root directory alongside `.env.local`, or inside the `src/` directory if you are using it.
