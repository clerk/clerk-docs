...
<If is="llm">
  **For AI agents:** Instead of clicking through the Dashboard, configure these options with the CLI: run `clerk config schema` to discover the auth keys (e.g. `auth_email`, `auth_phone`, `auth_username`), then apply changes with `clerk config patch`.
</If>
