...
To enforce token type for an entire group of routes, check the token type in each route handler with [`auth({ acceptsToken })`](/docs/reference/nextjs/app-router/auth#verify-machine-requests).
