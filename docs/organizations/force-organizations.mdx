...
<Steps>
  ...
  If you are using [prebuilt components](/docs/components/overview), an organization will _automatically_ be set as active each time the user creates an organization, accepts an invitation, or selects a membership. If prebuilt components don't meet your specific needs or if you require more control over the logic, you can rebuild the existing Clerk flows using the Clerk API. See the [organization switcher custom flow guide](/docs/guides/development/custom-flows/organization-switcher).
