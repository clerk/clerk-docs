...
This method in the SDK is a wrapper around the BAPI endpoint `GET/organizations/{organization_id}/invitations/{invitation_id}`. See the [BAPI reference](/docs/references/backend-api/tag/Organization-Invitations#operation/GetOrganizationInvitation){{ target: '_blank' }} for more information.
