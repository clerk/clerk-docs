...
- The `auth.protect()` helper is used extensively here. See its [reference doc](/docs/references/nextjs/app-router/auth#auth-protect) for more info.
...
>
  ...
  <AccordionPanel>
    The `CLERK_FRONTEND_API` environment variable was renamed to `CLERK_PUBLISHABLE_KEY`. To update, go to the [**API keys**](https://dashboard.clerk.com/last-active?path=api-keys) page in the Clerk Dashboard. After choosing your framework, copy and paste the new keys. Ensure this update is applied across all environments (e.g., development, staging, production). **Note:** The values are different, so this isn't just a key replacement. [More information](/docs/guides/development/deployment/production#api-keys-and-environment-variables).
  </AccordionPanel>
  ...
  <AccordionPanel>
    ...
    Make this change carefully as some behavior may have changed between our beta and stable releases. You can refer to [our documentation](/docs/getting-started/quickstart/app-router) and/or [approuter example](https://github.com/clerk/clerk-nextjs-app-quickstart) for up-to-date usage.
