...
<Steps>
  ...
  In addition to standard OIDC claims, you can sync custom attributes from your IdP (such as `department` or `employee_id`) into Clerk's `publicMetadata`. Custom attribute definitions are configured at the enterprise connection level and shared with your SCIM/Directory Sync connection if one is enabled.

  For a full walkthrough of defining custom attributes and mapping them for OIDC connections, refer to [Custom Attribute Mapping](/docs/guides/configure/auth-strategies/enterprise-connections/custom-attribute-mapping).

  > [!NOTE]
  > When [Directory Sync](/docs/guides/configure/auth-strategies/enterprise-connections/directory-sync) is enabled on the same connection, SCIM becomes the exclusive source for custom attribute values and overrides OIDC claims. Custom attributes become editable via OIDC again when Directory Sync is disabled.
