...
const TotpEnabled = () => {
  ...
  // This is not checking for MFA via SMS
  ...
  return (
    <div>
      <p>
        ...
        <Link href="/account/manage-app-mfa/add">
          <button>Add</button>
        </Link>
      </p>
    </div>
...
export default function ManageTOTP() {
  ...
  return (
    <>
      <h1>Current Authentication App Settings</h1>
    ...
    <>
      ...
      {totp && displayFormat === 'qr' && (
        <>
          ...
          <button onClick={() => setDisplayFormat('uri')}>Use URI</button>
        </>
      )}
      {totp && displayFormat === 'uri' && (
        <>
          ...
          <button onClick={() => setDisplayFormat('qr')}>Use QR Code</button>
        </>
      )}
...
function SuccessScreen() {
  return (
    <>
      <h1>SuccessScreen</h1>
      <p>You successfully added TOTP Mfa via an authentication application</p>
    </>
  );
}
...
export default function AddMfaScreen() {
  ...
  return (
    <>
      ...
      <Link href="/account/manage-app-mfa">Manage MFA</Link>
    </>
  );
}
```
