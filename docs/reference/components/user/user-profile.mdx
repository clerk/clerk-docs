...
<If notSdk="js-frontend">
  ...
  <If sdk="expo">
    ...
    Expo Router requires a non-platform route alongside a platform-specific route, so this example also includes `src/app/user-profile.tsx`, which renders the native [`<UserProfileView />`](/docs/reference/expo/native-components/user-profile-view) component on iOS and Android. Because users can sign out from `<UserProfileView />`, the example watches the auth state and redirects once they do.

    ```tsx {{ filename: 'src/app/user-profile.tsx' }}
    ...
    ```tsx {{ filename: 'src/app/user-profile.web.tsx' }}
...
<If sdk="js-frontend">
  ...
  - [`unmountUserProfile()`](#unmountuserprofile)
