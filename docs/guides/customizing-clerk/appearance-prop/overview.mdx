...
The `appearance` prop can be applied to [`<ClerkProvider>`](/docs/references/general/components/clerk-provider) to share styles across every component, or individually to any of the Clerk components.

This applies to all of the React-based packages, like [Next.js](/docs/getting-started/quickstart/app-router), as well as [the pure JavaScript ClerkJS package](/docs/references/javascript/overview).
