...
A _custom flow_ refers to a user flow created entirely from scratch using the Clerk API. If [prebuilt components](/docs/references/general/components/overview) don't meet your specific needs or if you require more control over the logic, you can rebuild the existing Clerk flows using the Clerk API.
...
1. Check for any pending tasks that need to be completed by accessing [`Session.currentTask`](/docs/references/javascript/types/session-task). Refer to [session tasks](/docs/guides/secure/session-tasks)
