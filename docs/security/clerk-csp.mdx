...
## Automatic CSP configuration with Clerk Middleware

Starting with `@clerk/nextjs` v6.14.0, Clerk provides built-in support for automatically injecting Content Security Policy headers through the middleware. This simplifies the process of setting up a secure CSP that works correctly with Clerk.

### Basic configuration

You can enable CSP injection by adding the `contentSecurityPolicy` option to your `clerkMiddleware` configuration:
...
export default clerkMiddleware(
  ...
  },
  {
    contentSecurityPolicy: {
      mode: "standard",
    }, // Enable default CSP configuration
  }
);
```
...
For more control, you can provide a detailed configuration object:
...
export default clerkMiddleware(
  ...
  {
    contentSecurityPolicy: {
      // Use 'strict-dynamic' mode for enhanced security or 'standard' for standard CSP
      mode: "strict-dynamic",
      
      // Add or override specific CSP directives
      directives: {
...
### Using strict-dynamic mode

When using the `strict-dynamic` mode, Clerk will:

1. Automatically generate a unique nonce for each request
3. Configure the CSP header with appropriate directives
4. Make the nonce available to your application through the `x-nonce` header
...
export default function Layout({ children }) {
  return (
    // When using middleware's CSP with strict-dynamic, 
