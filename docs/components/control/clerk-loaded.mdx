...
>
  ...
  <If sdk="astro">
    > [!NOTE]
    > Unlike other Clerk components for Astro, `ClerkLoaded` must be imported from `@clerk/astro/react`. This requires that your Astro app is set up with React. See [Use Clerk with Astro and React](/docs/references/astro/react) for guidance.
  ...
  <If sdk="nuxt">
    ...
    <template>
      <ClerkLoaded> <p>Clerk has loaded</p> </ClerkLoaded>>
    </template>
    ```
  </If>
</If>
