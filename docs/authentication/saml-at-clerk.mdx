...
A Clerk application can have multiple authentication strategies, but an organization that enables Enterprise SSO can not. Once Enterprise SSO is enabled for an organization, there can be no other authentication methods for that organization. This is in line with an organization’s intent to manage their users’ identity from one place. This will allow your Clerk application to enable Enterprise SSO connections for certain organizations while others use non-Enterprise SSO methods depending on each organization's needs.
