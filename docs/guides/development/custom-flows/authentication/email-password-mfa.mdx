...
## Support MFA with phone codes

The examples above verify MFA with authenticator app (TOTP) codes. To support SMS codes as a second factor, use the phone-code strategy instead.

<Tabs options={['JavaScript', 'Expo', 'iOS', 'Android']}>
  <Tab>
    ```ts
    // Prepare phone-code MFA after SignIn.status === "needs_second_factor"
    await signIn.prepareSecondFactor({ strategy: 'phone_code' })

    // Verify phone-code MFA
  ...
  <Tab>
    ```ts
    // Expo uses the same SignIn API shape as JavaScript
    await signIn.prepareSecondFactor({ strategy: 'phone_code' })
