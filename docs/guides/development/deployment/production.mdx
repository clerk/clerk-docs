...
After you update your proxy, in the Clerk Dashboard, return to the [**Domains**](https://dashboard.clerk.com/~/domains) page and select **Verify proxy** to confirm that Clerk can reach the `__clerk` proxy path. If `__clerk` is missing from the matcher, requests to Clerk's proxied Frontend API can be skipped by middleware and authentication can fail.
