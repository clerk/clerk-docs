...
<Steps>
  ...
  <Tabs items={["Client side rendering", "Server side rendering"]}>
    ...
    <Tab>
      The following example uses the [Next.js SDK](/docs/references/nextjs/overview) along with the [`getToken()`](/docs/references/nextjs/auth#use-auth-for-data-fetching) method of the [`auth()`](/docs/references/nextjs/auth#auth) helper.

      The `createClerkSupabaseClient()` is stored in a separate file so it can be accessed within `page.tsx` and `actions.ts`.

      ```jsx {{ prettier: false, filename: 'src/app/ssr/client.ts'}}
      ...
      The client can be used to query data on page load.

      ```jsx {{ prettier: false, filename: 'src/app/ssr/page.tsx'}}
      ...
      The client can also be used within server actions.

      ```jsx {{ prettier: false, filename: 'src/app/ssr/actions.ts'}}
      'use server'
