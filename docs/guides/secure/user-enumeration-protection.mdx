...
If that user later tries to sign in with the email on their social account and enters a password, strict enumeration protection prevents Clerk from revealing that no password is on file. The attempt fails with the same response as an incorrect password.

Clerk's prebuilt components handle this in two ways: the sign-in start screen highlights the user's most recently used strategy with a **Last used** badge when they return on the same device, and the password screen always offers a **Use another method** option to switch strategies. In a custom sign-in flow, after calling `signIn.create()`, read [`SignIn.supportedFirstFactors`](/docs/reference/objects/sign-in#supported-first-factors) to detect which strategies the user has, and always offer the social provider as an alternative.
