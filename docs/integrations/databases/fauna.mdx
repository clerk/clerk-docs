...
<Steps>
  ...
  1. Configure your template:
     ...
     - You can leave all other fields as their default settings or customize them to your needs. See the [JWT template guide](/docs/backend-requests/session-requests/making/jwt-templates#creating-a-template) to learn more about these settings.
     - In the **Claims** section, set the `aud` claim to the **Audience URL** you copied from Fauna in Step 2. The URL format should be `https://db.fauna.com/db/<YOUR_FAUNA_DB_ID>`. You can include additional claims if you’d like, but `aud` is the only required one. [Shortcodes](/docs/backend-requests/session-requests/making/jwt-templates#shortcodes) are available to make adding dynamic user values easy.
