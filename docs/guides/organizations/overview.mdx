...
Clerk measures Organization usage through **Monthly Retained Organizations (MROs)**. An MRO is an Organization with at least two members, where at least one member is a retained user. Free plans include up to 50 MROs in development and 100 in production. Pro plans offer unlimited development MROs and start at 100 free production MROs, then $1 per additional MRO. Refer to the [pricing page](/pricing){{ target: '_blank' }} for complete details.
