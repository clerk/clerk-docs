...
<Steps>
  ...
  1. Add a `.task` modifier that configures Clerk with your Clerk [Publishable Key](!publishable-key) and loads it when the app starts.
  ...
  One the app launches successfully, tap "Sign in" and the `AuthView` will appear, allowing you to sign up or sign in to create your first user.
</Steps>
...
<Cards>
  ...
  - [Clerk iOS SDK Reference](/docs/reference/ios/overview)
