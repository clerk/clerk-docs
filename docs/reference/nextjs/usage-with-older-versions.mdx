...
> [!WARNING]
> This page is only for apps still running Next.js 12 or older. It installs `@clerk/nextjs` v4, which is years behind the current release and no longer receives fixes. Don't follow it for a new app — start from the [Next.js quickstart](/docs/nextjs/getting-started/quickstart), which installs the current version.
