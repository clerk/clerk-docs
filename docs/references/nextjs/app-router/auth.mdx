...
If you need to send a JWT along to a server, `getToken()` retrieves the current user's [session token](/docs/guides/how-clerk-works/session-tokens) or a [custom JWT template](/docs/backend-requests/jwt-templates). See detailed examples in the [`Auth` object reference](/docs/references/backend/types/auth-object#get-token){{ target: '_blank' }}.
