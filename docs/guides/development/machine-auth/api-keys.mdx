...
To verify an API key secret, you can use the [`verifySecret()`](/docs/reference/backend/api-keys/verify-secret) method:

```ts
const apiKey = await clerkClient.apiKeys.verifySecret(secret)
```
