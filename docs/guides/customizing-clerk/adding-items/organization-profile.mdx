...
<If notSdk={["astro", "js-frontend", "vue"]}>
  ...
  <CodeBlockTabs options={["<OrganizationSwitcher />", "Dedicated page"]}>
    ```tsx {{ filename: 'app/components/Header.tsx', collapsible: true }}
    'use client'
    ...
    ```tsx {{ filename: 'app/organization-profile/[[...organization-profile]]/page.tsx', collapsible: true }}
    'use client'
...
To add an external link to the `<OrganizationProfile />` navigation sidenav, you will need to use one of the following components, depending on the use case mentioned in the [Before you start](#before-you-start) section.
