...
An important feature of modern OAuth 2.0 and OpenID Connect implementations is authorization server metadata, which is a standardized JSON document published by the authorization server. 

Clerk's FAPI natively supports this metadata, making it easier for clients to integrate and adhere to security best practices. Your can easily retrieve this metadata by visiting `<fapi>/.well-known/authorization-server-metadata`, where you’ll find all the relevant details automatically exposed in a standardized format. 

### Building a Model Context Protocol (MCP) service using Clerk’s OAuth server

Model Context Protocol (MCP) is an open standard that enables LLM applications to securely access user data from external services with proper authorization. For instance, MCP can enable AI applications like Claude, ChatGPT, or Cursor to ask a user for permission to access some of their private information such as emails or private GitHub repositories that would normally require signing in with an account. 
...
To support MCP integrations, Clerk provides a fully-featured OAuth server out of the box, making it easy to implement MCP in any app that uses Clerk for authentication. Additionally, Clerk offers an open-source library,`@clerk/mcp-tools`, which abstracts away the complexities of implementing auth providers with MCP. With just a few lines of code, you can expose an MCP service directly from your application or integrate MCP services from other applications into your LLM app. 
...
- [How to build an MCP service using Next.js]()
- [How to build an MCP service using Express]()
