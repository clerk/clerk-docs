...
You can use `auth()` to check if `isAuthenticated` is true. If it's false, then there is not an authenticated (signed in) user. See detailed examples in the [dedicated guide](/docs/references/nextjs/read-session-data).
