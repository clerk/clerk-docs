...
<If sdk={["fastify", "expressjs", "react-router"]}>
  ...
  - If the token is invalid or missing, `getAuth()` will return `false` for `isAuthenticated` and `null` for other properties, like `userId`.
  - If the token is an `api_key`, the code can access the associated user's data using the `userId`.
  - If the token is valid, `isAuthenticated` is `true` and `userId` is returned and available for use in the application logic. This example includes pseudo-code that uses the `userId` to get data from a database.
...
<If sdk="nuxt">
  ...
  - If the token is invalid or missing, `event.context.auth()` will return `false` for `isAuthenticated` and `null` for other properties, like `userId`.
  - If the token is an `api_key`, the code can access the associated user's data using the `userId`.
  - If the token is valid, `isAuthenticated` is `true` and `userId` is returned and available for use in the application logic. This example includes pseudo-code that uses the `userId` to get data from a database.
