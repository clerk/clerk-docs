  ...
  extension SignInOrSignUpView {
    ...
    func submit(emailAddress: String, password: String) async {
      do {
        ...
        // If the identifier is not found, the user is not signed up yet.
        // So swap to the sign-up flow.
        if String(describing: error).contains("form_identifier_not_found") {
          do {
...
<If sdk="android">
  ```kotlin {{ filename: 'SignInOrSignUp.kt', collapsible: true }}
  ...
  suspend fun signInOrSignUp(emailAddress: String, password: String) {
    when (
      val signInResult = Clerk.auth.signInWithPassword {
        identifier = emailAddress
        this.password = password
      }
    ) {
      is ClerkResult.Success -> {
        val signIn = signInResult.value
        ...
        when (signIn.status) {
          ...
          SignIn.Status.NEEDS_CLIENT_TRUST -> {
            signIn.prepareSecondFactor(SignIn.PrepareSecondFactorStrategy.EmailCode())
          }
          else -> Unit
        }
      }
      is ClerkResult.Failure -> {
        val identifierMissing =
          signInResult.error.errors.firstOrNull()?.code == "form_identifier_not_found"
      ...
      signUp
        ...
        .onSuccess { updatedSignUp ->
          if (
            ...
            Clerk.auth
              .setActive(sessionId = updatedSignUp.createdSessionId)
              .onFailure {
                // See https://clerk.com/docs/guides/development/custom-flows/error-handling
                // for more info on error handling
              }
    ...
    Clerk.auth.currentSignIn?.let { signIn ->
      signIn
        ...
        .onSuccess { updatedSignIn ->
          if (
            updatedSignIn.status == SignIn.Status.COMPLETE &&
            updatedSignIn.createdSessionId != null
          ) {
            Clerk.auth
              .setActive(sessionId = updatedSignIn.createdSessionId)
              .onFailure {
                // See https://clerk.com/docs/guides/development/custom-flows/error-handling
                // for more info on error handling
              }
