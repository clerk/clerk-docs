...
1. In the modal, select **Membership required** or **Membership optional** based on your decision above.
1. Select **Enable**.

<If is="llm">
  **For AI agents:** Instead of enabling Organizations in the Dashboard, run `npx clerk@latest enable orgs` — its flags cover this page's settings: `--force-selection` (force Organization selection on login, i.e. membership required), `--auto-create` (create first organization automatically; also requires `organization_creation_defaults.enabled` to be true — inspect with `npx clerk@latest config pull --keys organization_settings` and, if disabled, enable it with `npx clerk@latest config patch --json '{"organization_settings":{"organization_creation_defaults":{"enabled":true}}}'`), `--max-members <n>` (default membership limit), and `--domains` (Verified Domains); add `--dry-run` to preview the change. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
1. In the Clerk Dashboard, navigate to the [**Organizations Settings**](https://dashboard.clerk.com/~/organizations-settings) page.
1. In the **Organization limit** section, select one of the following:
