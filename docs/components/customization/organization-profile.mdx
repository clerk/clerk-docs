...
### `<OrganizationProfile.Page />` Usage
...
### `<OrganizationProfile.Link />` Usage
...
### `<OrganizationProfile.Link />` Props
...
If you want to reorder the default routes (`General` and `Members`) in the `OrganizationProfile` navigation sidebar, you can use the `<OrganizationProfile.Page />` component with the `label` prop set to `'general'` or `'members'`. This will target the existing default page and allow you to rearrange it.
...
It's important to note that the first page in the list will be rendered under the root path `/` (its `url` will be ignored) and the Clerk pages will be rendered under the path `/organitzation-members` and `/organization-general`. Also, the first item in the list cannot be a `<OrganizationProfile.Link />` component.
