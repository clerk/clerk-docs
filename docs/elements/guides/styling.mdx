...
When using the `asChild` prop, you must ensure that the component is open for extension. Meaning your component should spread the incoming props and forward refs to the underlying element. Here's an example of how you might implement a custom `Input` component:

```tsx
import {forwardRef} from 'react'

const CustomInput = forwardRef(function CustomInput(props, forwaredRef) {
  return <input ref={forwaredRef} {...props} className="..." />
})
```
...
If you need programmatic access to state for more complex styling, several components are provided that accept a prop with state as an argument. This is useful when dealing with animations, or for conditionally rendering elements based on state.

To access a field's state, use `<FieldState>`:
