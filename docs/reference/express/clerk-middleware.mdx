...
> [!TIP]
> Use `clerkMiddleware()` with [`getAuth()`](/docs/reference/express/get-auth) to protect routes while keeping control over unauthenticated responses. See the [example](#example-use-clerkmiddleware-and-getauth-together).
...
<Properties>
  - `clerkClient`
  - [`ClerkClient`](/docs/reference/backend/overview#createclerkclient-options)
