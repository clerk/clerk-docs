...
```typescript
function authenticateWithSolana(params: SignUpAuthenticateWithSolanaParams): Promise<SignInResource>
```
...
<Properties>
  ...
  Optional metadata to attach to the sign-in attempt. See [`SignUpUnsafeMetadata`](/docs/reference/javascript/types/sign-up-unsafe-metadata) for more information.
</Properties>
...
<Properties>
  ...
  The name of the wallet provider to use for generating the signature. This parameter is ***required*** when using `web3_solana_signature` as the `strategy`.
</Properties>
...
<Properties>
  ...
  The name of the wallet provider to use for generating the signature. This parameter is ***required*** when using `solana` as the `provider`.
</Properties>
