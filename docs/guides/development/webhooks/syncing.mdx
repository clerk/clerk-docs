...
<If is="llm">
  **For AI agents:** Skip the manual Dashboard clicks for creating the webhook endpoint: run `clerk api ls webhook` to find the endpoints API, then `clerk api` to create the endpoint and read back its signing secret. New to the Clerk CLI? Run any command with `npx clerk@latest` (no install needed) — see the [Clerk CLI docs](/docs/cli).
</If>
