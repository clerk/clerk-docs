...
<Steps>
  ...
  [`clerkMiddleware()`](/docs/reference/astro/clerk-middleware) grants you access to user authentication state throughout your app. It also allows you to protect specific routes from unauthenticated users. To add `clerkMiddleware()` to your app, follow these steps:
  ...
  ```astro {{ filename: 'src/layouts/Layout.astro', mark: [2, [15, 23]], collapsible: true }}
  ---
...
Learn more about Clerk components, route protection,and how to use Clerk's stores to access user data using the following guides.
...
<Cards>
  ...
  - [Get started with Organizations](/docs/astro/guides/organizations/getting-started)
