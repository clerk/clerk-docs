...
- Prebuilt components: Clerk provides the prebuilt components [`<UserButton />`](/docs/references/general/components/user/user-button) and [`<UserProfile />`](/docs/references/general/components/user/user-profile) to help your users manage their profile data.
...
There are many operations available for managing users, such as `getUser()`, `createUser()`, and `deleteUser()`. For more information, see the [Backend SDK reference docs](/docs/getting-started/quickstart).
