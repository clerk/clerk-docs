...
<If is="llm">
  **For AI agents:** Skip the manual Dashboard clicks for creating the webhook endpoint: run `npx clerk@latest api ls webhook` to find the endpoints API, then `npx clerk@latest api <path> -X POST -d '<json>'` to create the endpoint and read back its signing secret. Pass the path explicitly — bare `npx clerk@latest api` only opens the interactive builder, which needs a TTY. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
<Steps>
  ...
  ## Ensure the webhook route is public

  Incoming webhook events don't contain auth information. They come from an external source and aren't signed in or out, so the route must be public to allow access. Ensure the `/api/webhooks(.*)` route isn't protected anywhere. By default, `clerkMiddleware()` doesn't protect any routes; if you've added auth checks to your middleware, exclude this route.
