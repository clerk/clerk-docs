...
Use this method to initiate the sign-in process. It creates a new `SignIn` object and stores the sign-in lifecycle state.
...
The `create()` method will return a promise of the new `SignIn` object. This sign-in might be complete if you supply the required fields in one step. However, this is not mandatory. Clerk's sign-up process provides great flexibility and allows users to easily create multi-step sign-in flows.
