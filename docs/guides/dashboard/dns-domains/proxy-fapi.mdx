...
<Steps>
  ...
  > [!NOTE]
  > If you're using **Next.js** or **Express**, you can use the built-in `frontendApiProxy` option in [`clerkMiddleware()`](/docs/reference/nextjs/clerk-middleware#frontend-api-proxy) instead of manually configuring a proxy server. This handles all the proxy requirements automatically. See the [Next.js](#nextjs-clerk-middleware) and [Express](#express-clerk-middleware) tabs below.
