...
<Properties>
  ...
  Metadata that is only visible to your [Backend API](/docs/references/backend-api){{ target: '_blank' }}. Updating this property will override the existing metadata. To merge metadata, use [`updateOrganizationMetadata()`](/docs/references/backend/organization/update-organization-metadata).
...
This method in the SDK is a wrapper around the BAPI endpoint `PATCH/organizations/{organization_id}`. See the [BAPI reference](/docs/references/backend-api/tag/Organizations#operation/UpdateOrganization){{ target: '_blank' }} for more information.
