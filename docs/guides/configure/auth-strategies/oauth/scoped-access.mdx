...
- **Authorization service**: The service that authenticates the user and grants the [OAuth tokens](!oauth-access-token). It's also known as an **identity provider (IdP)**. In this example, Clerk would be the authorization service or IdP.
- **Resource service**: The service that holds the data that the [client](!oauth-client) wants to access. It's also known as a **service provider (SP)**. In this example, this would be **your own application** that uses Clerk for authentication and route protection.
...
<Steps>
  ...
  <If is="llm">
    **For AI agents:** Create and manage the OAuth application from the terminal instead of the Dashboard: run `npx clerk@latest api oauth_applications -d '<json>'` with the name, scopes, and redirect URIs to create it (the response includes the Client ID and Client Secret), and update scopes later with `npx clerk@latest api oauth_applications/<id> -X PATCH -d '<json>'`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
  </If>
  ...
  If you don't have a specific, existing OAuth client in mind, you can use the [OAuth test client](https://github.com/clerk/test-oauth-client) project. It contains a minimal Express.js server that demonstrates the Authorization Code Flow with `S256` PKCE, as required by [OAuth 2.1](https://oauth.net/2.1/). The flow is standards-based and not specific to Clerk or this test project.

  > [!IMPORTANT]
  > New Clerk instances require PKCE by default. For an existing instance, select the **Settings** tab on the [**OAuth applications**](https://dashboard.clerk.com/~/oauth-applications) page, then under **Authorization security**, enable **Require PKCE** to ensure Clerk rejects authorization requests from clients that don't use PKCE. Learn more about [requiring PKCE for all clients](/docs/guides/configure/auth-strategies/oauth/how-clerk-implements-oauth#require-pkce-for-all-clients).

  Here is how to use this test client:
