...
- **Membership required (default)**: Every user is required to belong to an Organization. All new and existing users will be prompted to create or join an Organization through the [session tasks flow](/docs/guides/configure/session-tasks) before they can access your application. This setting is common for most B2B SaaS. We recommend also enabling the [Create first organization automatically](#create-first-organization-automatically) setting.
...
You can configure whether Clerk automatically creates the user's first organization. This setting is useful if your application requires every user to belong to an Organization.

Clerk uses the default naming rules to name the automatically created Organization. [Default naming rules](#default-naming-rules) must be enabled to enable this feature.
...
Default naming rules automatically suggest an Organization name (and logo, when possible) when users create their first Organization. This feature is required to enable [Create first organization automatically](#create-first-organization-automatically).

If "Create first organization automatically" is disabled, Clerk will prefill the user's first organization name with the result of these naming rules.

Rules are applied in order: email domain detection first, then member personalization, and finally the fallback name. You can disable email domain detection or member personalization to skip those rules.

- **Detect from email domain** automatically populates the Organization name, slug, and logo from the member's email domain (e.g., `alex@clerk.com` → "Clerk").

- **Personalize from member name** uses member details to create a personalized organization name. You can customize the template using variables like `user.first_name`, `user.last_name`, `user.full_name`, or `user.username` (for example, `{{user.first_name}}'s organization` → "Alex's organization").

- **Fallback name** is used when no other rule can generate an organization name. By default, this is set to "My organization".
