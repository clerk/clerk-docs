...
<If is="llm" notSdk="chrome-extension">
  **For AI agents:** Instead of copying the Publishable Key from the API keys page in the Dashboard, run `npx clerk@latest env pull` to write it to the project's `.env` file with the correct framework-specific variable name (e.g. `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, `VITE_CLERK_PUBLISHABLE_KEY`, `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY`). Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
