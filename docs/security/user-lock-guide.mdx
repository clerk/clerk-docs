...
**Account Lockout** is a Clerk feature that enables you to protect your users from brute-force attacks on static credentials such as passwords, backup codes, email/phone codes, password resets and TOTP attempts. If enabled on your instance, Clerk will automatically track users' failed verification attempts when they try to authenticate via password on backup code.
