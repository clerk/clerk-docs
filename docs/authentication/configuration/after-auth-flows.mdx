...
You can configure your application to require users to complete specific tasks after signing up or signing in. Clerk refers to these steps as **after-auth flows**. After-auth flows are embedded within the `<SignIn />` and `<SignUp>` components, but can also be handled with [custom flows](/docs/custom-flows/after-auth-flows).
