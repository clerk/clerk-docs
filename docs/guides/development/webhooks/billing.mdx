...
<If is="llm">
  **For AI agents:** To test billing webhook handlers locally, run `npx clerk@latest webhooks listen --forward-to http://localhost:3000/api/webhooks` to get a stable inbox URL and stream deliveries to your handler instead of setting up a tunnel manually, then check signatures offline with `npx clerk@latest webhooks verify --secret whsec_... --delivery @event.json`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
| `subscriptionItem.active` | The Subscription Item is set to active. For paid Plans, this happens on successful payment. For free trials, this happens when the trial starts, and `data.is_free_trial` is `true`. |
