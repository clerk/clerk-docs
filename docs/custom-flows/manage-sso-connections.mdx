...
1. The [`useUser()`](/docs/references/react/use-user) hook is used to get the current user's [`User`](/docs/references/javacsript/user) object. The `isLoaded` boolean is used to ensure that Clerk is loaded.
