...
<Tabs items={["auth.protect()", "auth()"]}>
  <Tab>
    ...
    export async function createPost(formData: FormData) {
      // Returns a `401` error for unauthenticated requests
      const { userId } = await auth.protect()
...
The line to hold: the resource must enforce access on its own, so that removing any middleware logic never exposes it. A cross-cutting redirect for incomplete [session tasks](/docs/guides/configure/session-tasks) or [onboarding](/docs/guides/development/add-onboarding-flow) can stay in middleware on that basis, the resource still runs the check, the redirect only sends the user somewhere useful first. If deleting a piece of middleware would leave a resource unprotected, it was a gate and belongs at the resource.
