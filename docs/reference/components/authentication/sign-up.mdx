...
<If notSdk="js-frontend">
  ...
  <If sdk="expo">
    ...
    Expo Router requires a non-platform route alongside `src/app/sign-up.web.tsx`, so this example also includes `src/app/sign-up.tsx`, which renders the native [`<AuthView />`](/docs/reference/expo/native-components/auth-view) component on iOS and Android.
    ...
    For a complete walkthrough, including redirecting native users once authentication completes, see the [custom sign-up page guide](/docs/guides/development/web-support/custom-sign-up-page).
  </If>
...
<If sdk="js-frontend">
  ...
  The following methods available on an instance of the [`Clerk`](/docs/reference/objects/clerk) class are used to render and control the `<SignUp />` component:

  - [`mountSignUp()`](#mountsignup)
  - [`unmountSignUp()`](#unmountsignup)
