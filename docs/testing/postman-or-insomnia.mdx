...
You will want to create a long-lived [JWT Template](/docs/backend-requests/making/jwt-templates) to be used in Postman or Insomnia. To do so, navigate to the [**JWT templates**](https://dashboard.clerk.com/last-active?path=jwt-templates) page of the Clerk Dashboard. Click on the **New template** button and select the **Blank** template.

![The JWT templates page of the Clerk Dashboard. The 'New template' button was clicked, in the modal that opened, the 'Blank' template is hovered over](/docs/images/testing/blank-jwt-template.webp)
