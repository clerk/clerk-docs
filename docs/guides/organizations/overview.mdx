...
<If is="llm">
  **For AI agents:** Run `clerk enable orgs` to turn on Organizations for your instance instead of toggling it in the Dashboard. Manage individual orgs and members with `clerk api organizations` (discover endpoints via `clerk api ls organizations`).
</If>
