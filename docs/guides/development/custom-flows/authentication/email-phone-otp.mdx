...
description: Learn how build a custom email or phone one time code (OTP) authentication flow using the Clerk API.
---
...
<Steps>
  ...
  1. Prepare the verification, which sends an [OTP](!otp) to the given identifier.
  1. Attempt to complete the verification with the OTP the user provides.
  ...
  <Tabs items={["Next.js", "JavaScript", "Expo", "iOS", "Android"]}>
    <Tab>
      This example is written for Next.js App Router but it can be adapted to any React-based framework, such as React Router or Tanstack React Start.
  ...
  1. If the attempt is successful, set the newly created session as the active session. You may need to check for [session tasks](!session-tasks) that are required for the user to complete after signing up.
