...
<Tabs items={["Next.js", "JavaScript"]}>
  <Tab>
    ...
    1. Use the [`useOrganizationList()`](/docs/references/react/use-organization-list) hook to get `memberships`, which is a list of the current user's organization memberships. `memberships` returns `data`, which is an array of [`OrganizationMembership`](/docs/references/javascript/types/organization-membership) objects.
    1. Map over the `data` array to display the user's organization memberships in a table, providing a button that calls `setActive()` to set the selected organization as the active organization.
       - If there are no organizations, the [`<CreateOrganization />` component (custom-flow version, not the Clerk component)](/docs/custom-flows/create-organization) is rendered to allow the user to create an organization.
    ...
    <CodeBlockTabs options={["Paginated list", "Infinite list"]}>
      ...
      import CreateOrganization from '../components/create-organization' // See /docs/custom-flows/create-organization for this component
  ...
  <Tab>
    ...
    1. Maps over the `data` array to display the user's organization memberships in a list, providing a button that calls [`setActive()`](/docs/references/javascript/clerk/session-methods#set-active) to set the selected organization as the active organization.
