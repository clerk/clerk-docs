...
>
  <AccordionPanel>
    ...
    ```tsx {{ del: [1, 4, 7, 10, 13], ins: [2, 5, 8, 11, 14] }}
...
<Tabs items={['Next.js', 'React', 'React Router', 'Astro', 'TanStack React Start', 'Expo', 'Nuxt', 'Express']}>
  ...
  <Tab>
    ...
    ```html {{ del: [2, 6, 9, 12, 15, 18, 21], ins: [3, 7, 10, 13, 16, 19, 22] }}
    ---
  ...
  <Tab>
    ...
    ### `req.auth` object-access removed — use `getAuth()` {{ toc: false }}
