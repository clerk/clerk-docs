...
Clerk provides [composables](/docs/sdk/nuxt/methods#client-side-helpers) to access the session and user data in your Nuxt application.
...
In the following example, the [`auth`](/docs/sdk/nuxt/methods#auth-object) object is used to get the `userId` to check if the user is signed in. The `clerkClient()` helper retrieves the full [`User`](/docs/references/javascript/user/user) object.
