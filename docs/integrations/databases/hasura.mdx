...
The Hasura template will pre-populate the default claims required by Hasura. You can include additional claims as necessary. [Shortcodes](/docs/backend-requests/session-requests/making/jwt-templates#shortcodes) are available to make adding dynamic user values easy.
