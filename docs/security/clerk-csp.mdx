...
If you are using Next.js and your layout file is rendered on the server (as is the case with the example above), our SDK will automatically load the nonce into ClerkProvider, and the explicit passing of the nonce would be unnecessary. However, if you are rendering the provider on the client, you will need to explicitly pass the nonce value to ClerkProvider.