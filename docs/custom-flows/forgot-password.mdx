...
A user can reset their password by providing their email address or phone number and then following the instructions sent to them. The instructions include a [one-time code](/docs/authentication/configuration/sign-up-sign-in-options#authentication-strategy) that the user must then provide to complete the password reset process.
