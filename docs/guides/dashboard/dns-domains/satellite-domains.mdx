    ...
    <Tab>
      ...
      <Tabs items={["Primary app", "Satellite app"]}>
        <Tab>
          ...
          > [!NOTE]
          > In development, your [Publishable Key](!publishable-key) and [Secret Key](!secret-key) will start with `pk_test_` and `sk_test_` respectively. In production, they will start with `pk_live_` and `sk_live_` respectively.
