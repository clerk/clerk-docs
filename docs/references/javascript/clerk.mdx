...
Handles a 401 response from [Frontend API](/docs/references/frontend-api){{ target: '_blank' }} by refreshing the client and session object accordingly.
...
Initializes the `Clerk` object and loads all necessary environment configuration and instance settings from the [Frontend API](/docs/references/frontend-api){{ target: '_blank' }}.
