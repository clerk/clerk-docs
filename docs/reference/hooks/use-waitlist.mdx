...
The `useWaitlist()` hook provides access to the [`Waitlist`](/docs/reference/javascript/waitlist) object, which provides methods and properties to manage waitlist entries in your application. This hook is useful for building a [custom flow](!custom-flow) instead of using the prebuilt [`<Waitlist />`](/docs/reference/components/authentication/waitlist) component.
