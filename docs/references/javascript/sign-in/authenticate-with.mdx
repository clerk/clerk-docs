...
Starts a sign-in flow that uses the OKX Wallet to authenticate the user using their Web3 wallet address. Returns `Promise` which resolves to the current [`SignIn`][signin-ref].
