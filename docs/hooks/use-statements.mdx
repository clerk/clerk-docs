...
function StatementsList() {
  ...
  return (
    <ul>
      {data?.map((statement) => (
        <li key={statement.id}>
          Statement #{statement.number} - {statement.status}
          <br />
...
The following example demonstrates how to use `useStatements()` to display detailed statement information.

```tsx
import { useStatements } from '@clerk/nextjs/experimental'

function StatementsTable() {
  const { data, isLoading } = useStatements({ for: 'user' })
      ...
      <tbody>
        {data?.map((statement) => (
          <tr key={statement.id}>
            ...
            <td>
              <button onClick={() => window.open(statement.downloadUrl)}>Download PDF</button>
            </td>
          </tr>
        ))}
      </tbody>
