...
`<Show />` with `when='signedIn'` performs authentication checks. To perform [authorization checks](!authorization-check), you can pass different values to the `when` prop, like `when={{ role: '...' }}`, `when={{ permission: '...' }}`, `when={{ feature: '...' }}`, or `when={{ plan: '...' }}`.
...
`<Show when='signedIn'>` performs authentication checks. It will render its children if the user is signed-in, and its `fallback` prop if the user is signed-out.
...
<If sdk="nextjs">
  ...
  export default function Page() {
    return (
      <Show fallback={<p>Users that are signed-out can see this.</p>} when='signedIn'>
...
<If sdk="expo">
  ...
  export default function Screen() {
    return (
      <Show fallback={<Text>Users that are signed-out can see this.</Text>} when='signedIn'>
...
If you use `when='signedIn'` without any access control values, `<Show>` will render its children if the user is signed in, regardless of their Role or its Permissions.
...
While authorization by `permission` is **recommended**, for convenience, `<Show>` allows a `when={{ role: '...' }}` prop to be passed.
...
<If sdk="astro">
  ...
  <Show condition={(has) => has({ role: 'org:admin' }) || has({ role: 'org:billing_manager' })}>
