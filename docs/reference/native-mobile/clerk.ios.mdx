...
<Properties>
  ...
  The [Active Organization](!active-organization) for the current session. Returns `nil` when there is no active session, no selected organization, or the active user's memberships don't include the selected organization.
