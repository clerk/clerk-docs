...
> [!WARNING]
> A verified domain can't be added if it's already in use for the [organization's SSO](/docs/guides/organizations/add-members/sso).
...
Membership requests are requests from users who saw an organization suggestion and requested to join an organization. Membership requests are only available for organizations that have the **Verified domains** feature enabled and **Automatic suggestion** enabled in both the Dashboard and for the specific domain.
