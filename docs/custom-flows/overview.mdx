...
Sign-in's are initiated by creating a `SignIn` object on the current `Client`. If the sign-in is successfully authenticated, it will transform into an active session for that [`User`](/docs/references/javascript/user/user) on the current `Client`.
