...
Use `clerk.auth.setActive(sessionId:organizationId:)` to switch the [Active Organization](!active-organization) for a session. Omit `organizationId`, or pass `null`, to select the user's [Personal Account](!personal-account). The Personal Account option isn't allowed when `clerk.environment?.organizationSettings.forceOrganizationSelection` is `true`.
