...
<Tabs items={["NPM module", "<script>"]}>
  ...
  <Tab>
    <Steps>
      ...
      > [!IMPORTANT]
      > Any Clerk option passed as a script tag attribute must be prefixed with `data-clerk-`. For example, `publishableKey` becomes `data-clerk-publishable-key`, and `domain` becomes `data-clerk-domain`.
