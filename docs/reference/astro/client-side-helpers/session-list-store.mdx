...
The `$sessionListStore` store returns an array of [`Session`](/docs/reference/objects/session){{ target: '_blank' }} objects that have been registered on the client device.
