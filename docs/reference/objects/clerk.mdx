...
<Properties>
  ...
  The domain or proxy URL used to connect to Clerk. In a multi-domain setup, pass the satellite domain here.
</Properties>
...
<Properties>
  ...
  The domain used to connect to Clerk. If a function, will be called with a `URL` made from `window.location.href`. If you're configuring a satellite application with the JavaScript SDK, set `domain` here in the constructor rather than in `load()`.
</Properties>

### Using the SDK with a `<script>` tag

When using Clerk with a `<script>` tag, constructor options are passed as attributes on the ClerkJS script tag instead of through `new Clerk()`.

All options passed as script tag attributes must be prefixed with `data-clerk-`. For example:

- `publishableKey` becomes `data-clerk-publishable-key`
- `domain` becomes `data-clerk-domain`
- `proxyUrl` becomes `data-clerk-proxy-url`

Options that belong to [`load()`](#load), such as `isSatellite`, should still be passed to `Clerk.load()` after the script has loaded.
...
- Use `data-clerk-*` attributes only for constructor options such as `publishableKey`, `domain`, and `proxyUrl`.
- Use `Clerk.load()` for runtime options such as `isSatellite`, `signInUrl`, `signUpUrl`, and `appearance`.
- Place the Clerk script tags before any script that references `window.Clerk`.
...
> [!NOTE]
> For satellite applications, set `domain` in the [`constructor()`](#constructor) (or `data-clerk-domain` when using a `<script>` tag) and set `isSatellite` in `load()`.
