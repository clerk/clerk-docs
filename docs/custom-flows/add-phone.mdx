...
1. Uses the [`createPhoneNumber()`](/docs/references/javascript/user/create-metadata#create-phone-number) method on the `User` object to add the phone number to the user's account.
  - A new [`PhoneNumber`](/docs/references/javascript/phone-number) object is created and stored in the `phoneNumbers` property of the `User` object.
1. Uses the `prepareVerification()` method on the newly created `PhoneNumber` object to send a verification code to the user.
1. Uses the `attemptVerification()` method on the `PhoneNumber` object with the verification code provided by the user to verify the phone number.

<Tabs items={["Next.js"]}>
  <Tab>
    ```jsx {{ filename: 'app/account/add-phone/page.tsx' }}
    'use client';
    ...
    export default function Page() {
      ...
      const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
          ...
          // Find the phone number that was just added
