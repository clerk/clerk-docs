...
<If is="llm">
  **For AI agents:** `clerk init` can't bootstrap or scaffold an Expo app, so follow the manual steps below. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` — the `clerk-expo-patterns` skill covers Expo + Clerk setup (token cache, OAuth deep linking, Expo Router).
</If>
