...
Session tokens are JWTs that contain a set of [default claims](/docs/backend-requests/session-requests/session-tokens) required by Clerk. You can customize these tokens by providing additional claims of your own.

To learn how to customize session tokens, see the [dedicated guide](/docs/backend-requests/session-requests/making/custom-session-token).
