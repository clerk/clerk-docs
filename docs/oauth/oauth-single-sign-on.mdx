...
1. Select **Add**. A modal will open with your **Client Secret**. Make sure to copy this and store it somewhere secure.
...
1. You'll be redirected to your app's settings page. Under **Application credentials**, you can find your **Client ID**, which will be necessary for configuring the client to sign in using your Clerk users' credentials.
1. In the **Redirect URIs** field, add the redirect URI provided by your client in the earlier step. This is the URL that Clerk will redirect to after the user has authenticated.

If you'd like to know more about other OAuth application configuration settings, refer to the [dedicated reference documentation](/docs/oauth/how-clerk-implements-oauth).
...
In order to enable OIDC in your OAuth flow, you can follow the same steps outlined above, but ensure to select the `openid` scope when configuring your OAuth application in the Clerk Dashboard. This scope triggers OIDC and instructs the authorization server to issue an ID token alongside your access token.

![OAuth scopes openid](/docs/images/oauth/oauth-scopes.png)

You will also need to include this `openid` scope in the query parameters of your authorization endpoint when initiating the OAuth flow. For example:

```bash
https://clerk.<INSERT_YOUR_APP_DOMAIN>.com/oauth/authorize?client_id=x&response_type=code&scopes=email&state=y&code_challenge=z&redirect_uri=a&scope=openid%20profile%20email&
```
...
You must verify the signature on the ID token before using any of the user information it contains. This will require your instance's public key. You can find your instance's public key at `https://clerk.<INSERT_YOUR_APP_DOMAIN>.com/.well-known/jwks.json`.
...
- `none`: The user is guaranteed not to interact with the provider (i.e., no UI is shown). This requires the user to already have an active session **and** the consent screen must be disabled. If not, the provider responds with an error.
- `login`: The user is forced to re-authenticate, even if they already have an active session. If there was already an active session, the provider ends it and a a fresh login is required.
