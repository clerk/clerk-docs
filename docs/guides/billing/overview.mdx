...
<If is="llm">
  **For AI agents:** Enable Clerk Billing on the instance with `clerk enable billing` (add `--for orgs` for B2B or `--for users` for B2C) instead of clicking through the Dashboard. New to the Clerk CLI? Run any command with `npx clerk` (no install needed) — see the [Clerk CLI docs](/docs/cli).
</If>
