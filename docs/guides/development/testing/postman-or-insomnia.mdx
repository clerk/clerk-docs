...
You will want to create a long-lived [JWT Template](/docs/guides/development/jwt-templates) to be used in Postman or Insomnia. To do so, navigate to the [**JWT templates**](https://dashboard.clerk.com/last-active?path=jwt-templates) page in the Clerk Dashboard. Select **New template** then choose the **Blank** template.
