...
<Tabs items={["Version 2", "Version 1"]}>
  <Tab>
    ...
    The **`o` claim**, or organization claim, is only included if the user is part of an [organization](/docs/organizations/overview) and that organization is [active](!active-organization). Its value is an object that contains the following properties:
    ...
    The first bit position in the bitmask is the rightmost, least significant bit, while the first permission in list is the leftmost element.
    ...
    The first integer `3` represents the feature-permission mapping for the `dashboard` feature. Since `3` is `11` in binary, both permissions are allowed (`dashboard:read` and `dashboard:manage`).

    The second integer `2` represents the feature-permission mapping for the `teams` feature. Since `2` is `10` in binary, the _second_ permission is allowed (`teams:read`) because the second bit from the right is `1`.
