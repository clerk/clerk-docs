...
Retrieves a list of OAuth applications for an instance. Returns a [`PaginatedResourceResponse`](/docs/references/backend/types/paginated-resource-response) object with a `data` property that contains an array of [`OAuthApplication`](/docs/references/backend/types/backend-oauth-application) objects, and a `totalCount` property that indicates the total number of OAuth applications for the instance.
...
This method in the SDK is a wrapper around the BAPI endpoint `GET/oauth_applications`. See the [BAPI reference](https://clerk.com/docs/reference/backend-api/tag/oauth-applications/get/oauth_applications){{ target: '_blank' }} for more information.
