...
The following example demonstrates how to use the `$userStore` store to reload the current user's data on the client-side. This is only needed if you are updating the user object without using the `user.update()` method or the Clerk hooks, eg. if you are making changes to the user object via an API endpoint. 
