...
> [!IMPORTANT]
> Loading environment variables is different depending on the framework you are using. For example, Next.js requires the `NEXT_PUBLIC_` prefix for environment variables that are available on the frontend. In other frameworks, you can use a package like [dotenv](https://www.npmjs.com/package/dotenv) to load the environment variables from your `.env` file into your application. Check your framework's documentation for information on how to load environment variables.
