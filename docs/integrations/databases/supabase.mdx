...
To get started, you'll need to create a new Supabase project.

1. Navigate to the [Supabase dashboard](https://supabase.com/dashboard/projects).
1. Select **New project** and configure your project however you would like.
...
Next, you'll create a `tasks` table and enable RLS on that table. The `tasks` table will also contain a `user_id` column that will use the `requesting_user_id()` function as it's default value. This column will be used in the RLS policies to only return or modify records scoped to the user's account.
...
<Tabs items={["Supabase interface", "SQL Editor"]}>
  <Tab>
  In the sidebar, navigate to **Authentication** > **Policies**. You'll create four total policies, one for each of the four Policy Commands.
  ...
  1. Select **Create policy** to create the `INSERT` policy:
      ...
      ```
      - Select **Save policy**.
  1. Select **Create policy** to create the `UPDATE` policy:
      - Name: "Update task policy".
      - For **Policy Command**, select **UPDATE**.
      - For **Target roles**, select **authenticated**.
      - Replace the "-- Provide a SQL expression for the using statement" with the following:
      ```sql filename="Supabase policy editor"
      requesting_user_id() = user_id
      ```
      - Select **Save policy**.
  1. Select **Create policy** to create the `DELETE` policy:
      - Name: "Delete task policy".
      - For **Policy Command**, select **DELETE**.
      - For **Target roles**, select **authenticated**.
      - Replace the "-- Provide a SQL expression for the using statement" with the following:
      ```sql filename="Supabase policy editor"
      requesting_user_id() = user_id
...
Open the project in the code editor of your choice. All relevant code will be in the `src/app/page.tsx` file as shown below. Note the comments with the "👉" icon as they point out code of interest.

Most notably, the `createClerkSupabaseClient()` function in the code is what injects the Clerk Supabase token into the request headers sent to Supabase. The `requesting_user_id()` function that was created in Supabase will parse this token to use it when querying data from the `tasks` table in Supabase.
...
To run the project, you need to add your Clerk and Supabase credentials to the project.

1. Create a file in the root of the project named `.env.local` to store your environment variables.
1. Get your Clerk publishable and secret key.
    - Navigate to your [Clerk Dashboard](https://dashboard.clerk.com/last-active?path=api-keys). You may need to create a project if you haven't already.
    - In the navigation sidebar, select **API Keys**.
    - From the **Quick Copy** section, add your Clerk publishable and secret key to your `.env.local` file.
1. Find your Supabase credentials:
    - Go to your [Supabase dashboard](https://supabase.com/dashboard/projects).
    - In the sidebar, select **Settings > API**.
    - Add the **Project URL** to your `.env.local` file as `NEXT_PUBLIC_SUPABASE_URL`.
    - From the **Project API Keys** section, add the value beside `anon` `public` to your `.env.local` file as `NEXT_PUBLIC_SUPABASE_KEY`.
...
Check the table in the Supabase dashboard. You should see all the tasks between both users, but with differing values in the `user_id` column.
