---
title: Build a custom flow for viewing a user's organization memberships
description: Learn how to use the Clerk API to build a custom flow for viewing a user's organization memberships.
---

> [!CAUTION]
> This guide is for users who want to build a _custom_ user interface using the Clerk API. To update organizations using a _prebuilt_ UI, you should use the [prebuilt components](/docs/components/overview).
...
<Tabs items={["Next.js", "JavaScript"]}>
  <Tab>
    ...
    ```jsx {{ filename: 'app/components/JoinedOrganizations.tsx' }}
    'use client'
