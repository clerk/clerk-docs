...
| `has()` | `(isAuthorizedParams: CheckAuthorizationParamsWithCustomPermissions) => boolean` | A function that returns a boolean based on the permission or role provided as parameter. Can be used for authorization. See the [reference documentation](/docs/references/nextjs/auth-object#has) for more information. |
