...
Passkeys are tied to the domain they are created on and **cannot be used across different domains**. This means the same passkey cannot be used for both your Account Portal and your app's sign-in pages. For example:
...
If you use Account Portal in development and create a passkey through components such as `<UserButton />` or `<UserProfile />` mounted in your application (e.g., on `localhost:3000`), that passkey won't work for signing in through Account Portal. This is because the Account Portal domain (`your-app.accounts.dev`) does not match your local development domain.

To work around this in development, you can either:

- Create paths that host the [`<SignIn />`](/docs/components/authentication/sign-in) or [`<SignUp />`](/docs/components/authentication/sign-up) components in your application (e.g., `/sign-in` and `/sign-up`) instead of using Account Portal
- Create the passkey directly through Account Portal instead of your local application

This limitation only affects development. In production, your Account Portal will be hosted on a subdomain of your main domain (e.g., `accounts.your-domain.com`), enabling passkeys to work seamlessly across your application.
