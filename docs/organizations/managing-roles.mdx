...
<Tabs items={["Next.js", "JavaScript"]}>
  ...
  1. Uses the [`useOrganization()`](/docs/references/react/use-organization) hook to get `memberships`, which is a list of the active organization's memberships.
  1. `memberships` is an object with `data` that contains an array of [`OrganizationMembership`](/docs/references/javascript/organization-membership) objects.
  1. Each `OrganizationMembership` object has an [`update()`](/docs/references/javascript/organization-membership#update) and [`destroy()`](/docs/references/javascript/organization-membership#destroy) method to update the member's role and remove the member from the organization, respectively.
