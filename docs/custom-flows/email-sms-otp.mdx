...
<Callout type="danger">
  Clerk's [Account Portal pages](/docs/account-portal/overview) and [prebuilt components](/docs/components/overview) handle the entire one-time password (OTP) authentication flow out of the box. This guide is for users who want to build a custom user interface using the Clerk API.
</Callout>
