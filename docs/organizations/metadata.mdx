...
- Both **public** and **private** metadata are set and can be accessed from the [Backend API](/docs/reference/backend-api).
- Only **public** metadata can be accessed from the [Frontend API](/docs/reference/frontend-api/).
...
To ease the flow of setting metadata, Clerk provides the `updateOrganizationMetadata()` and `updateOrganizationMembershipMetadata()` methods from the [Backend SDK](/docs/reference/backend-api), which is a wrapper around the [Backend API](/docs/reference/backend-api).
