...
Testing magic links in E2E suites is an uphill task. We recommend turning on [email verification codes](/docs/authentication/configuration/sign-up-sign-in-options#verification-methods), and using that flow to authenticate your tests. The flows are very similar.
