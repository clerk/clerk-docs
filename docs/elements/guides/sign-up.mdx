...
In this guide you will learn how to build a complete sign-up flow using Clerk Elements components.
...
You may have noticed that none of the components displayed to this point include styling. This is intentional! With the exception of `<SignUp.Root>` and `<SignUp.Strategy>`, every component rendered so far can be styled. The `className` prop enables the addition of custom classes to each component. Check out the [styling guide](/docs/elements/guides/styling) for a more in-depth guide on styling Clerk Elements.
