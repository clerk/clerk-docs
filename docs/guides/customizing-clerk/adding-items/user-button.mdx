...
- [`<UserButton.Action>`](#userbutton-action) - A menu item that triggers an action when clicked.
- [`<UserButton.Link>`](#userbutton-link) - A menu item that navigates to a page when clicked.
...
<If sdk="js-frontend">
  To conditionally render menu items based on a user's Role or Custom Permissions, you can use the [`checkAuthorization()`](/docs/reference/objects/session#checkauthorization) method to check if a user is authorized.
</If>
