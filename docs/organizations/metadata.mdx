...
To ease the flow of setting metadata, Clerk provides the `updateOrganizationMetadata()` and `updateOrganizationMembershipMetadata()` methods from the [Backend SDK](/docs/reference/backend/overview), which is a wrapper around the [Backend API](/docs/reference/backend-api){{ target: '_blank' }}.
