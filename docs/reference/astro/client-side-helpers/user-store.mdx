...
The `$userStore` store provides a convenient way to access current [`User`](/docs/reference/objects/user){{ target: '_blank' }} data and helper methods for managing the active user.
...
For more information on the `update()` method, see the [`User` reference](/docs/reference/objects/user#update){{ target: '_blank' }}.
...
For more information on the `reload()` method, see the [`User` reference](/docs/reference/objects/user#reload){{ target: '_blank' }}.
