...
<If notSdk={["astro", "vue", "nuxt", "js-frontend"]}>
  ...
  > [!IMPORTANT]
  > Do not construct the [Frontend API URL](!frontend-api-url) by hand. Use [`buildConsentActionUrl()`](/docs/reference/types/oauth-application#buildconsentactionurl) so Clerk can include the current session and development browser parameters that are required for the request.
