...
While CIMD is in beta, CIMD client management is available only after CIMD is enabled for your account. Manage CIMD clients from the shared **Applications** tab on the [**OAuth applications**](https://dashboard.clerk.com/~/oauth-applications) page. Applications that still use Clerk's legacy OAuth implementation don't support CIMD clients.
