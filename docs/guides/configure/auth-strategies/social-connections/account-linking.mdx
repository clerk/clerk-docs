...
<If is="llm">
  **For AI agents:** Instead of checking the **Verify at sign-up** option in the Dashboard, run `npx clerk@latest config pull --keys auth_email` to inspect the current `verify_at_sign_up` value, or change it with `npx clerk@latest config patch --json '{"auth_email":{"verify_at_sign_up":true}}'`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
1. In the [`<UserProfile />`](/docs/reference/components/user/user-profile), add the different email.
