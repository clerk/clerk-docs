...
The `task` table needs to include the organization ID in order to support organizations.

{/* TODO: I don't know how to do migrations :( */}
...
Consider the two policies from the Supabase + Clerk integration guide that restrict access to the `tasks` table based on the requesting user's Clerk ID:
...
To update these to include the organization ID, you can use the `coalesce` function to check for the `o.id` value first, and fall back to the `sub` value if it's not available.

{/* TODO: I don't think these are correct, I think `o.id` has to get checked to be equal to the `org_id` column in the `tasks` table? */}

```sql
create policy "Users can view their own tasks"
on public.tasks
for select
using(coalesce(auth.jwt()->>'o.id', auth.jwt()->>'sub') = user_id);
...
to authenticated
with check (
coalesce(auth.jwt()->>'o.id', auth.jwt()->>'sub') = user_id
);
```

{/* TODO: I think there needs to be a migration? But I'm not familiar with that */}
