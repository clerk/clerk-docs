...
Use the [`useAuth()`](/docs/references/hooks/use-auth) hook's `getToken()` method to get the session token. Since `getToken()` returns a Promise, you'll need to await its resolution before making the request.
...
In React-based applications, use the [`useAuth()`](/docs/references/hooks/use-auth) hook to access the `getToken()` method.
