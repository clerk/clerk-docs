...
<Properties>
  ...
  - `factorAgeVerification`
  - `[number, number] | undefined`

  An array that describes when a first factor and second factor were used to verify the user's identity.
...
<Properties>
  ...
  - `reverification?`
  - <code>[ReverificationConfig](#reverification-config) | [ReverificationCustomConfig](#reverification-custom-config)</code>
...
The type is an alias for `“strict_mfa” | “strict” | “moderate” | "lax”`.

| Level | Description |
| - | - |
| strict\_mfa | User should have verified credentials in the past 10 minutes, otherwise ask for both first factor and second factor |
| strict | User should have verified credentials in the past 10 minutes, otherwise ask for second factor |
| moderate | User should have verified credentials in the past 1 hour, otherwise ask for second factor |
| lax | User should have verified credentials in the past 1 day, otherwise ask for second factor |

### `ReverificationCustomConfig`
...
<Properties>
  ...
  The age of the factor level to check for. Value should be ≥ 1 \< 99\_999.
</Properties>
...
- `has()` is used to check if the user has the verified their credentials in the last 10 minutes.
- If the user hasn’t then we return with a specific response that will trigger the re-verification flow on the client.
...
The following is an example of the `Auth` object with valid factor age:
...
## `Auth` object example of user without a 2fa method registered.

The following is an example of the `Auth` object of user without a 2fa method registered:
