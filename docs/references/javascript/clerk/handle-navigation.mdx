...
In case the magic link verification wasn't successful, the `handleMagicLinkVerification()` method will throw a [`MagicLinkError`](/docs/authentication/custom-flows/magic-links). You can check the error's `code` property to see if the magic link expired, or the verification simply failed.
