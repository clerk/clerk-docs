...
> [!IMPORTANT]
> This guide uses `useLocalCredentials()` to store and reuse password credentials after a local biometric check. [Biometric sign-in](/docs/guides/development/custom-flows/authentication/biometric-sign-in) instead registers a device-bound public key with Clerk and signs a one-time challenge. [`expo-local-authentication`](https://docs.expo.dev/versions/latest/sdk/local-authentication/) only performs the local check; by itself, it neither registers a biometric credential nor authenticates with Clerk.

This guide demonstrates how to use the [`useLocalCredentials()`](/docs/reference/expo/native-hooks/use-local-credentials) hook in your Expo app to securely store a user's password credentials on their device, enabling biometric sign-in for returning users.
...
<Steps>
  ...
  export default function Page() {
    ...
    return (
      <View>
        ...
        {userOwnsCredentials && (
          <Button title="Remove saved credentials" onPress={() => clearCredentials()} />
        )}
      </View>
    )
  }
  ...
  ```tsx {{ filename: 'src/app/update-user.tsx' }}
  import { useUser, useClerk } from '@clerk/expo'
  import { useLocalCredentials } from '@clerk/expo/local-credentials'
