...
<Tabs items={["NPM module", "<script>"]}>
  <Tab>
    <Steps>
      ...
      To initialize Clerk, add the following code to your `main.js` file. This code imports your [Publishable Key](!publishable-key), creates a new [`Clerk`](/docs/reference/objects/clerk) instance, and calls the [`load()`](/docs/reference/objects/clerk#load) method to initialize Clerk.
  ...
  <Tab>
    <Steps>
      ...
      Below the `<script`> tag that initializes the SDK, create another `<script>` tag that listens for the `load` event and call Clerk's [`load()`](/docs/reference/objects/clerk#load) method to load the SDK, as shown in the following example:
