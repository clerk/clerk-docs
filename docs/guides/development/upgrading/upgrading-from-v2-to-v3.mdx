...
> [!WARNING]
> As of April 4, 2025, RedwoodJS is no longer actively developed. [Read the RedwoodJS announcement](https://community.redwoodjs.com/t/the-future-of-redwood-launches-today/7938).
...
Users who have built custom auth flows using the SignIn and SignUp resources can consult the [SignUp and SignIn section below](#signup-and-signin).
...
If you want to fetch a token for an integration, you must prefix the integration name with `integration_`, e.g., `getToken({ template: 'integration_firebase' })`.
