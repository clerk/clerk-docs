...
If you would like to add custom claims to your session token, you can [customize it](/docs/backend-requests/session-requests/making/custom-session-token).

You can also create custom tokens using a [JWT template](/docs/backend-requests/session-requests/making/jwt-templates).
...
> [!NOTE]
> If your application encounters this issue, the Clerk Dashboard will display a warning: **"Some users are exceeding cookie size limits"**. To resolve this, update your [custom session token](/docs/backend-requests/session-requests/making/custom-session-token).
...
To learn how to manually verify a session token, refer to the [dedicated guide](/docs/backend-requests/session-requests/handling/manual-jwt).
