...
export default function SignInMFAForm() {
  ...
  const onPressTOTP = React.useCallback(async () => {
    ...
    try {
      ...
      // Attempt to sign in the user with email / password incase they haven't yet activated TOTP & Backup codes
      if (attemptFirstFactor.status === "complete") {

        // If the attempt is successful, re-route them past the MFA step to the dashboard
