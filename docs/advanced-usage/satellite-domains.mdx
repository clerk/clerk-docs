...
<Steps>
  ...
  > [!IMPORTANT]
  > To use a satellite domain in production, you will need to add a CNAME record for the `clerk` subdomain. Visit [https://dashboard.clerk.com/last-active?path=domains/satellites/](https://dashboard.clerk.com/last-active?path=domains/satellites/), select the satellite domain you are configuring and follow the directions for adding the CNAME record.
