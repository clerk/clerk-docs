...
<Properties>
  ...
  When the session's `actor` claim has `type: 'agent'`, this property exposes information about the agent and [Agent Task](TODO) that was used to create the session.
