...
The `requireAuth()` middleware is deprecated. For new route protection, use [`clerkMiddleware()`](#clerk-middleware) with [`getAuth()`](#get-auth) instead. See the [reference doc](/docs/reference/express/require-auth) for existing usage details.
