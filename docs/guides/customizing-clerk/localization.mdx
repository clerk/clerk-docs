...
- `|` — the pipe operator; it sends the value into the modifier to its right.
...
For example: `{{ date | weekday('en-US','long') | titleize }}` would take the `date` and apply the `weekday` formatter with the `'en-US'` locale and `'long'` style ('Monday' instead of 'Mon') to get a long English weekday name. Then, `titleize` would ensure the long English weekday name is properly capitalized. Some countries lowercase weekday names while others capitalize them. Overall, a rendered output would be something like `'Monday'` instead of `'Mon'` or `'monday'`.
