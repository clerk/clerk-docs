...
Both kinds of helpers can check a user's Role and Permissions. The recommended approach is to verify a user's Permissions. Roles can be added and removed and their Permissions changed. Authorizing by Permissions is more explicit.
...
<Tabs type="framework" items={["Next.js with Protect", "Next.js with useAuth().has"]}>
  <Tab>
