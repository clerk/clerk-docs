...
Returns a paginated list of Roles in the Organization. Returns a [`ClerkPaginatedResponse`][pag-ref] of [`RoleResource`](/docs/reference/javascript/types/role) objects and `has_role_set_migration` status.

```ts
function getRoles(params?: GetRolesParams): Promise<ClerkPaginatedResponse<RoleResource>> & { has_role_set_migration: boolean }
```
