...
`Astro.locals.auth()` returns an `Auth` object. This JavaScript object contains important information like session data, your user's ID, as well as the ID of the [Active Organization](!active-organization). Learn more about the [`Auth` object](/docs/reference/backend/types/auth-object){{ target: '_blank' }}.
