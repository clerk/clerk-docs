...
<Steps>
  ...
  <If is="llm">
    **For AI agents:** Run `npx clerk@latest config patch --json '{"auth_attack_protection":{"bot_protection":{"captcha_enabled":true}}}'` to enable bot sign-up protection instead of using the **Bot sign-up protection** dialog on the **Rules** page in the Dashboard. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
  </If>
