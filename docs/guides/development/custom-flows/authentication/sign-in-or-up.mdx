...
Because this guide uses email and password authentication, the example uses the code examples from the [email/password custom flow](/docs/guides/development/custom-flows/authentication/email-password) guide. If you are using a different authentication method, such as [email or phone OTP](/docs/guides/development/custom-flows/authentication/email-sms-otp) or [email links](/docs/guides/development/custom-flows/authentication/email-links), you will need to adapt the code accordingly.
...
export default function SignInForm() {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    if (error) {
      ...
      // If the identifier is not found, the user is not signed up yet
      // So this includes the flow for signing up a new user
      // signIn.password() returns { error: ClerkError | null }; API errors have error.error.errors (array)
      const apiError = error.error
      const firstErrorCode =
        apiError && isClerkAPIResponseError(apiError) ? apiError.errors[0]?.code : undefined
      if (firstErrorCode === 'form_identifier_not_found') {
        console.log('hello222')
        try {
          // Now the flow is a sign up flow
    ...
    if (signIn.status === 'complete') {
      await signIn.finalize({
        navigate: async ({ session, decorateUrl }) => {
          ...
          // When sign-in is complete, redirect to the home page
          const url = decorateUrl('/')
          if (url.startsWith('http')) {
            window.location.href = url
          } else {
    ...
    <>
      ...
      <form action={handleSubmit}>
        <div>
          ...
          You don't want to show the 'form_identifier_not_found' error because you handle that case in the sign-up flow */}
          {signInErrors.fields.identifier &&
            signInErrors.fields.identifier.code !== 'form_identifier_not_found' && (
              <p>{signInErrors.fields.identifier.message}</p>
            )}
          {signUpErrors.fields.emailAddress && <p>{signUpErrors.fields.emailAddress.message}</p>}
        </div>
        <div>
          <label htmlFor="password">Enter password</label>
          <input id="password" name="password" type="password" />
          {signInErrors.fields.password && <p>{signInErrors.fields.password.message}</p>}
          {signUpErrors.fields.password && <p>{signUpErrors.fields.password.message}</p>}
        </div>
        <button type="submit">Continue</button>
        {/* For your debugging purposes. You can just console.log errors, but we put them in the UI for convenience */}
        {signInErrors && <p>{JSON.stringify(signInErrors, null, 2)}</p>}
        {signUpErrors && <p>{JSON.stringify(signUpErrors, null, 2)}</p>}
      </form>
