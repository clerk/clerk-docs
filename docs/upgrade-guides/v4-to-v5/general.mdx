...
If you're using **Next.js**, you can read a more streamlined [Upgrading Next.js from v4 to v5](/docs/upgrade-guides/v4-to-v5/nextjs) guide.
...
As this package uses `@clerk/clerk-react` under the hood, you _potentially_ have to take [its Clerk v5 changes](#clerk-clerk-react) into account, too. `@clerk/nextjs` re-exports functions and components from `@clerk/clerk-react`.

<Callout type="info" >
  You can read a more streamlined guide at [Upgrading Next.js from v4 to v5](/docs/upgrade-guides/v4-to-v5/nextjs).
</Callout>
...
<Accordion titles={["<code>&lt;SignIn /&gt;</code> and <code>&lt;SignUp /&gt;</code>", "<code>&lt;UserButton /&gt;</code>", "<code>&lt;UserProfile /&gt;</code> and <code>&lt;OrganizationProfile /&gt;</code>"]} heading="h5">
  <AccordionPanel>
...
The [pagination response & parameters changes](#pagination-response-parameters) inside `@clerk/clerk-js` can also potentially affect you if you use SDKs like `@clerk/clerk-react`, `@clerk/nextjs`, or `@clerk/remix`. Double check if you're using any of the mentioned methods in your application.
...
You can also read the [How to migrate](/docs/upgrade-guides/v4-to-v5/nextjs#how-to-migrate) section of the Next.js upgrade guide for more examples.
