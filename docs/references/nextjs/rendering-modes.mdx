...
Next's upcoming [Partial Prerendering](https://nextjs.org/docs/app/building-your-application/rendering/partial-prerendering) (PPR) feature provides more granular control over static and dynamic rendering. Wrapping components that access auth data with `<Suspense>` will allow pages to be pre-rendered up to the Suspense boundaries.

## Accessing data with `auth()`
...
// This page will be dynamic rendered at request time
...
### (not recommended) Make auth data available globally with `<ClerkProvider dynamic>`

If you wish to make auth data available to all of your components, pass the `dynamic` prop to your root `<ClerkProvider>`. Note that this is not recommended as you lose the ability to granularly opt-in specific routes based on their data needs.
