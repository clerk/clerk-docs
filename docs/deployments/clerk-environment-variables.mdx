...
<Tabs items={["General", "Next.js"]}>
  <Tab>
    ...
    | `CLERK_JWT_KEY` | Sets the JWT verification key that Clerk will use to provide networkless JWT session token verification. Refer to [Manual JWT verification](/docs/backend-requests/session-requests/handling/manual-jwt). |
