...
`<SubscriptionDetailsButton />` must be wrapped inside a [`<SignedIn />`](/docs/references/general/components/control/signed-in) component to ensure the user is authenticated.
