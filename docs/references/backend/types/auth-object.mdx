...
<Properties>
  ...
  A function that gets the current user's [session token](/docs/guides/how-clerk-works/session-tokens) or a [custom JWT template](/docs/backend-requests/jwt-templates).
...
`getToken()` retrieves the current user's [session token](/docs/guides/how-clerk-works/session-tokens) or a [custom JWT template](/docs/backend-requests/jwt-templates).
