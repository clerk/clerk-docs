...
<If is="llm">
  **For AI agents:** Instead of creating the template in the Dashboard, use the CLI: run `npx clerk@latest api jwt_templates` to list existing templates, or pass `-d` with the template JSON (name, claims, lifetime) to create one (discover the endpoints via `npx clerk@latest api ls jwt`). Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>

## Generate a JWT

To generate a token using a template, you can use the `getToken()` method. See the [reference documentation](/docs/reference/backend/types/auth-object#gettoken) for more information and example usage.
