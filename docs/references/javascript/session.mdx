...
> For more details regarding the different session states, see our documentation on [Session management](/docs/advanced-usage/manual-jwt).
...
{cells: [<code>skipCache?</code>, <code>boolean</code>, <>Whether to skip the cache lookup and force a call to the server instead, even within the TTL. Useful if the token claims are time-sensitive or depend on data that can be updated (e.g. user fields).<br />Defaults to `false`.</>]},
  ]}
/>
...
{cells: [<code>revoked</code>, <>The application ended the session and the <code>Session</code> was removed from the <code>Client</code> object.</>]},  ]}
/>
