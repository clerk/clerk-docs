...
<Steps>
  ...
  <If notSdk={["ios", "android"]}>
    1) If the `SignUp.status` is `'complete'`, set the newly created session as the active session. You may need to check for [session tasks](!session-tasks) that are required for the user to complete after signing up.
  </If>

  <If sdk={["ios", "android"]}>
    To create a sign-up flow for **email** [OTP](!otp), the flow is the same except you'll swap `phone` for `email` and `phoneNumber` for `emailAddress` throughout the code. You can find all available methods in the [`auth`](/docs/ios/reference/native-mobile/auth#sign-up) documentation.
  </If>
  ...
  <If sdk={["ios", "android"]}>
    To create a sign-in flow for **email** OTP, it's the same except you'll swap `phone` for `email` and `phoneNumber` for `emailAddress` throughout the code. You can find all available methods in the [`auth`](/docs/ios/reference/native-mobile/auth#sign-in) documentation.
  </If>
