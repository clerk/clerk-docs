...
Enterprise Single Sign-On (SSO) can be configured at the organization level, allowing organizations to use their own Identity Provider (IdP) for authentication. When configured:
...
For instructions on how to set up and manage enterprise SSO for your organizations, see the [dedicated guide](/docs/organizations/enterprise-sso).
