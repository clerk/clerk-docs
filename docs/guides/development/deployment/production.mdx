...
1. A production domain — either a custom domain you own, or a Vercel-provided `*.vercel.app` domain.
...
If you use a `*.vercel.app` production domain and configure custom OAuth credentials, see [Custom OAuth provider credentials](#custom-oauth-credentials) for the required callback URL format.
...
### Vercel `*.vercel.app` domain {{ id: 'vercel-app-production-domains' }}
