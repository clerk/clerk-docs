...
You can customize the text of UI elements by passing a `localization` prop to the [`<ClerkProvider>`](/docs/components/clerk-provider) with the relevant keys. In the following example, the `socialButtonsBlockButton` key is customized to change the text of the social sign-in button.
...
You can customize Clerk's default error messages by adding an `unstable__errors` object inside the `localization` prop. You can then pass to this object specific error keys corresponding to different error types and assign them custom message strings. In the following example, the `form_identifier_not_found` error message is customized to display a more user-friendly message when a user tries to sign in with an account that does not exist in your Clerk instance.
...
const localization = {
  unstable__errors: {
    form_identifier_not_found: "We couldn't find an account with that email. Please try again.",
  },
}
