...
<TutorialHero
  ...
  exampleRepo={[
    {
      title: "Set up org slugs in URLs app",
...
<Steps>
  ...
  > [!TIP]
  > If your app doesn't use `clerkMiddleware()`, or you prefer to manually set the active organization, use the client-side [`setActive()`](/docs/references/javascript/clerk/session-methods) method to control the active organization via the client-side. See [this guide](/docs/guides/force-organizations#set-an-active-organization-based-on-the-url) to learn how to manually activate a specific organization based on the URL.
