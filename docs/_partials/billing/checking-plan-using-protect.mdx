<If sdk="astro">
  > [!WARNING]
  > Astro SDK hasn't been updated to support "plan" in the `Protect` component yet.
...
<If sdk="chrome-extension">
  ```tsx {{ filename: 'src/screens/protected-content.tsx' }}
  import { Protect } from '@clerk/chrome-extension'

  export default function ProtectedContentScreen() {
    return (
      <Protect
        plan="bronze"
        fallback={<p>Only subscribers to the Bronze plan can access this content.</p>}
