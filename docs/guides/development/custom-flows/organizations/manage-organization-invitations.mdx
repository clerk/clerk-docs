...
<If sdk="ios">
  ...
  extension ManageOrganizationInvitationsView {
    ...
    func activeOrganization() async throws -> Organization? {
    ...
    func inviteMember(emailAddress: String, role: String) async {
      do {
        guard let organization = try await activeOrganization() else { return }
    ...
    func fetchPendingInvitations() async {
      do {
        guard let organization = try await activeOrganization() else {
          invitations = []
          return
        }
    ...
    func revokeInvitation(_ invitation: OrganizationInvitation) async {
      do {
        ...
        invitations.removeAll(where: { $0.id == invitation.id })
      } catch {
        // See https://clerk.com/docs/guides/development/custom-flows/error-handling
        // for more info on error handling
        dump(error)
