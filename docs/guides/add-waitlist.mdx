...
<Steps>
  ...
  The following example demonstrates how to render the `<SignIn />` component. **Pass the `waitlistUrl` prop to the component to provide the target of the 'Waitlist' link that's rendered in the `<SignIn />` component.**
  ...
  For more information on building custom sign-in and sign-up pages, see the [dedicated guide](/docs/references/nextjs/custom-signup-signin-pages).
</Steps>
