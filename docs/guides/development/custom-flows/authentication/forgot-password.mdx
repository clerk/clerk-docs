...
<If sdk={["nextjs", "react", "expo", "js-frontend", "react-router", "tanstack-react-start"]}>
  ...
  >
  > If you're using an older version of one of these SDKs, or are using the legacy API, refer to the [legacy API documentation](/docs/guides/development/custom-flows/authentication/legacy/forgot-password).
</If>
...
> [!TIP]
> To allow users to update their password **after they've already signed in**, see the [custom flow for updating a user's password](/docs/guides/development/custom-flows/account-updates/update-password).
...
<If notSdk={["expo", "ios", "android"]}>
  ...
  ```tsx {{ filename: 'src/components/forgot-password.tsx', collapsible: true }}
  'use client'
