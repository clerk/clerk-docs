...
<Steps>
  ...
  <Tabs items={["Manage MFA page", "Add phone number page"]}>
    <Tab>
      ...
      <If sdk="ios">
        ```swift {{ filename: 'ManageSMSBasedMFAView.swift', collapsible: true }}
        import SwiftUI
        import ClerkKit

        struct ManageSMSBasedMFAView: View {
          @Environment(Clerk.self) private var clerk
          @State private var phoneId = ""

          var body: some View {
            VStack {
              TextField("Enter phone ID", text: $phoneId)
              Button("Reserve for MFA") {
                Task { await reservePhoneForMfa(phoneId: phoneId) }
              }

              Button("Generate backup codes") {
                Task { await generateBackupCodes() }
              }
            }
          }
        }

        extension ManageSMSBasedMFAView {

          func reservePhoneForMfa(phoneId: String) async {
            guard
              let phone = clerk.user?.phoneNumbers.first(where: { $0.id == phoneId })
            else { return }

            do {
              // Reserve the phone as a second factor for SMS MFA.
              let updatedPhone = try await phone.setReservedForSecondFactor(reserved: true)

              // Backup codes can be returned on the updated phone resource.
              print(updatedPhone.backupCodes ?? [])
            } catch {
              // See https://clerk.com/docs/guides/development/custom-flows/error-handling
              // for more info on error handling
              dump(error)
      ...
      <If sdk="android">
        ...
        suspend fun reservePhoneForMfa(phoneId: String) {
          val phone = Clerk.user?.phoneNumbers?.firstOrNull { it.id == phoneId } ?: return
        ...
        suspend fun makePhoneDefaultSecondFactor(phoneId: String) {
          val phone = Clerk.user?.phoneNumbers?.firstOrNull { it.id == phoneId } ?: return
        ...
        suspend fun generateBackupCodes() {
          Clerk.user
            ?.createBackupCodes()
