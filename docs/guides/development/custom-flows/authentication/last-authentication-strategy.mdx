...
<If sdk="ios">
  ...
  struct LastAuthenticationStrategyView: View {
    ...
    let providers: [ProviderOption] = [
      .init(provider: .google, strategy: "oauth_google", name: "Google"),
      .init(provider: .github, strategy: "oauth_github", name: "GitHub"),
    ]

    var body: some View {
      VStack {
        Button("Last Used Authentication") {
          if let lastStrategy = clerk.client?.lastAuthenticationStrategy?.rawValue {
            if let provider = providers.first(where: { $0.strategy == lastStrategy }) {
              print("Last used provider: \(provider.name)")
            } else {
              print("Last strategy '\(lastStrategy)' is not in your provider options.")
            }
          } else {
            print("No last authentication strategy found.")
          }
        }
      }
    }
