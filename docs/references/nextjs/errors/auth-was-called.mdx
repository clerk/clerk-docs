...
The most common cause of this error is when [`auth()`](/docs/references/nextjs/auth) is called **without** a [`<ClerkProvider>`](/docs/references/general/components/clerk-provider) wrapping the component tree where Clerk components are used.
