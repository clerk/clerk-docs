...
<If sdk="js-frontend">
  To apply a theme to all Clerk components, pass the `appearance` prop to the [`clerk.load()`](/docs/reference/objects/clerk#load) method. The `appearance` prop accepts the property `theme`, which can be set to a theme.
