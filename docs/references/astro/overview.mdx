...
To configure Clerk with Astro, you must pass [the `clerk()` integration](/docs/references/astro/integration) to the `integrations` array in your `astro.config.mjs` file. See the [quickstart](/docs/astro/getting-started/quickstart#update-astro-config-mjs) for more information on configuring the integration.
