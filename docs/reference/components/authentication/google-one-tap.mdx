...
sdk: astro, expo, nextjs, nuxt, react, react-router, tanstack-react-start, vue, js-frontend
---
...
<If notSdk="js-frontend">
  ...
  <If sdk="expo">
    ...
    ```jsx {{ filename: 'src/components/GoogleOneTap.web.tsx' }}
...
The methods in this section are available on instances of the [`Clerk`](/docs/reference/objects/clerk) class and are used to render and control the `<GoogleOneTap />` component.
...
Completes a Google One Tap redirection flow started by [`authenticateWithGoogleOneTap()`](#authenticatewithgoogleonetap). Also calls [`Clerk.setActive()`](/docs/reference/objects/clerk#setactive) and performs a custom navigation if given a custom navigation function.
...
<If sdk="js-frontend">
  See [`authenticateWithGoogleOneTap()` usage](#authenticatewithgoogleonetap-usage) for an example of how to use `handleGoogleOneTapCallback()`.
</If>
...
<Properties>
  - `signInOrUp`
  - <code>[SignInResource](/docs/reference/objects/sign-in) | [SignUpResource](/docs/reference/objects/sign-up)</code>
  ...
  - `params`
  - [`HandleOAuthCallbackParams`](/docs/reference/types/handle-o-auth-callback-params)
