...
if (clerk.user) {
  ...
  currentOrganization.update({ name: "New Name" })
  ...
  <Tab>
    ...
    if (clerk.user) {
      ...
      document.getElementById("upload-logo").addEventListener("click", () => {
        ...
        currentOrganization.setLogo({ file: orgLogo })
          .then((res) => console.log(res))
          .catch((error) => console.log("An error occurred:", error.errors));
      });
    } else {
...
if (clerk.user) {
  ...
  // Gets a list of roles in the organization
  currentOrganization.getRoles()
    .then((res) => console.log(res))
    .catch((error) => console.log("An error occurred:", error.errors));
} else {
  document.getElementById("app").innerHTML = `
...
In addition to the methods listed above, the `Organization` object also has the following methods:
