...
| `fontSmoothing` | `'auto' \| 'antialiased' \| 'never'` | The font smoothing used throughout the components. By default, this is not set. |
| `fontWeight` | `{normal: number, medium: number, bold: number}` | The font weight used throughout the components. By default, this is set to `{normal: 400, medium: 500, bold: 700}`. |
