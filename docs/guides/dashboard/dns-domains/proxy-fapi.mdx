...
If you use [satellite domains](/docs/guides/dashboard/dns-domains/satellite-domains) and are unable to add CNAME records, you can proxy the Frontend API instead. This replaces the CNAME-based DNS setup described in the satellite domains guide.

> [!NOTE]
> Before following these steps, make sure you have already [added your satellite domain](/docs/guides/dashboard/dns-domains/satellite-domains#add-your-first-satellite-domain) in the Clerk Dashboard and configured `allowedRedirectOrigins` on your primary domain app. See the [satellite domains guide](/docs/guides/dashboard/dns-domains/satellite-domains) for the full setup walkthrough.
...
<Steps>
  ...
  When using a proxy for a satellite domain, set `proxyUrl` / `CLERK_PROXY_URL` **instead of** `domain` / `CLERK_DOMAIN`. The `proxyUrl` replaces `domain` — you should not set both.
  ...
  <Tabs items={["Environment variables", "Properties"]}>
    <Tab>
      <CodeBlockTabs options={["Next.js", "Remix"]}>
        ...
        NEXT_PUBLIC_CLERK_PROXY_URL=https://satellite.dev/__clerk
        ...
        CLERK_PROXY_URL=https://satellite.dev/__clerk
    ...
    <Tab>
      If you prefer to use properties instead of environment variables, set `isSatellite`, `proxyUrl`, `signInUrl`, and `signUpUrl` on your `<ClerkProvider>` (and in your middleware, for Next.js):
          ...
          return (
            <html lang="en">
              <body>
                <ClerkProvider
                  isSatellite
                  proxyUrl="https://satellite.dev/__clerk"
