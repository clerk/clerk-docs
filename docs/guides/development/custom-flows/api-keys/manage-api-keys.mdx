...
<Tabs items={["Next.js", "JavaScript"]}>
  <Tab>
    ...
    1. Uses the [`useAPIKeys()`](/docs/reference/hooks/use-api-keys) hook from `@clerk/nextjs/experimental` to retrieve and manage API keys. It will use `subject` if provided; otherwise it falls back to the active Organization, then the current user.
    2. Displays the API keys in a table with options to create new keys and revoke existing ones.
    3. Provides a form to create new API keys using [`clerk.apiKeys.create()`](/docs/reference/javascript/api-keys#create) with an optional expiration (subject defaults to active org, then user).
    4. Allows revoking API keys using [`clerk.apiKeys.revoke()`](/docs/reference/javascript/api-keys#revoke).
    ...
    export default function ApiKeysManager() {
      ...
      const handleCreate = async (e) => {
        ...
        try {
          const expiration =
            formData.expirationSeconds.trim() === ''
              ? null
              : Number(formData.expirationSeconds)
              ...
              <div>
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, expirationSeconds: e.target.value })
                    }
                  />
                </label>
              </div>
              ...
              {!apiKeys || apiKeys.length === 0 ? (
                ...
                apiKeys.map((apiKey) => (
                  <tr key={apiKey.id}>
                    ...
                    <td>
                      {!apiKey.revoked && !apiKey.expired && (
                        <button onClick={() => handleRevoke(apiKey.id)}>
                          Revoke
                        </button>
                      )}
                    </td>
  ...
  <Tab>
    ...
    1. Calls [`clerk.apiKeys.getAll()`](/docs/reference/javascript/api-keys#getall) to retrieve the list of API keys for the active user or organization.
    2. Displays the API keys in a table.
    3. Provides a form to create new API keys using [`clerk.apiKeys.create()`](/docs/reference/javascript/api-keys#create).
    4. Allows revoking API keys using [`clerk.apiKeys.revoke()`](/docs/reference/javascript/api-keys#revoke).
