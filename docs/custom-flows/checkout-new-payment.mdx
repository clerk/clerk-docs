...
<Steps>
  ...
  <Tabs items={["Next.js"]}>
    <Tab>
      ...
      1. Uses the [`usePaymentElement()`](/docs/hooks/use-payment-element) hook to control the payment element, which is rendered by `<PaymentElementForm/>`.
      1. Assumes that you have already have a valid `planId`, which you can acquire in many ways.
         ...
         1. Use the new [`usePlans()`](/docs/hooks/use-plans) hook to get the plan details.

      This example is written for Next.js App Router but can be adapted for any React-based framework.

      ```tsx {{ filename: 'app/components/CheckoutPage.tsx', mark: [[3, 9], 13, 15, [20, 21], 23, 31,33, 39, 53, [63, 72], 82] }}
      'use client'
      ...
      export default function CheckoutPage() {
        return (
          <CheckoutProvider for="user" planId="cplan_xxxx" planPeriod="monthly">
            <CustomCheckout />
          </CheckoutProvider>
        )
      }
      ...
      function CheckoutInitialization() {
        ...
        return (
          <button onClick={start} disabled={fetchStatus === 'loading'} className="start-checkout-button">
            {fetchStatus === 'loading' ? 'Initializing...' : 'Start Checkout'}
          </button>
        )
      }
      ...
      function PaymentSection() {
        ...
        const { isFormReady, submit, isProviderReady } = usePaymentElement()
