...
> [!WARNING]
Clerk's JWT Template will not be applied if you're using the native Clerk integration. They are two different paths.

Here is an example if you're using the Clerk + Convex native integration (configured via Clerk Dashboard's "Convex" integration, not a JWT template). 

When aud === "convex", Convex's library calls getToken() with no template name — meaning it uses the native Clerk session token, not your JWT template.
The native Clerk session token only contains the base claims (sub, iss, sid, sts, fva, v, o) — your JWT template claims are never applied in this path.

The fix: You need to use a JWT Template approach, not the native Clerk integration. In your Clerk Dashboard:

1. Go to JWT Templates → create a template named exactly "convex" (not the built-in Convex integration)
2. Set the claims as you had them, with "aud": "convex"
3. Remove the native Convex integration from Clerk if you added it there

When a JWT template named "convex" exists with aud: "convex", the sessionClaims.aud on the session token will still not equal "convex" (session
tokens have a different aud), so the code will fall into the else branch and call getToken({ template: "convex" }), which will use your template
with all the custom claims.

The native Clerk "Convex" integration and JWT templates are two different paths. Your custom claims only work via the JWT Template path.
