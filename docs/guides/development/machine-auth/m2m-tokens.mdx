...
M2M tokens are a usage-based paid feature. The pricing is as follows:

- `$0.001` per token creation. The first 2,500 creations per month are free.
- `$0.00001` per token verification. The first 100,000 verifications per month are free.

Applications on the Hobby plan are limited to free tier usage. To use beyond the free tier limits, you must upgrade to the Pro plan.

Usage stats and monitoring are available in the Clerk Dashboard to make it easy to understand your usage and what you're being charged.
...
<If is="llm">
  **For AI agents:** Instead of creating and scoping machines in the Dashboard, use the CLI: run `npx clerk@latest api machines -d '{"name":"Machine A","scoped_machines":["mch_..."]}'` to create a machine, `npx clerk@latest api machines/<machine_id>/scopes -d '{"to_machine_id":"mch_..."}'` to grant an existing machine access to another, and `npx clerk@latest api machines/<machine_id>/secret_key` to fetch the machine secret key for your `.env`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
> [!IMPORTANT]
> Loading environment variables is different depending on the framework you are using. For example, Next.js requires the `NEXT_PUBLIC_` prefix for environment variables that are available on the frontend. If you're running a Node server, you can load `.env` files by starting your app with the `--env-file=.env` flag. Check your framework's documentation for information on how to load environment variables.
...
```json
{
  ...
  "expired": false,
  "expiration": 1754942036732, // Unix timestamp in milliseconds
  "createdAt": 1754938436732, // Unix timestamp in milliseconds
  "updatedAt": 1754938436732, // Unix timestamp in milliseconds
  "token": "mt_xxx"
}
```
...
```json
{
  ...
  "revocationReason": null,
  "expired": false,
  "expiration": 1754943177115, // Unix timestamp in milliseconds
  "createdAt": 1754939577115, // Unix timestamp in milliseconds
  "updatedAt": 1754939577568, // Unix timestamp in milliseconds
  "token": undefined
}
```
