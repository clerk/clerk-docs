...
Clerk supports password authentication, which allows users to sign up and sign in using their email address and password. This guide will walk you through how to build a custom email/password sign-up and sign-in flow using the [`useSignUp()`][use-signup-ref] and [`useSignIn()`][use-signin-ref] React hooks.
