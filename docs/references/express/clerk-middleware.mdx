...
The `clerkMiddleware()` function checks the request's cookies and headers for a session JWT and if found, attaches the [`Auth`](/docs/references/backend/types/auth-object) object to the `request` object under the `auth` key. **It's must be set before any other middleware.**

> [!TIP]
> Even if you are using [`requireAuth()`](/docs/references/express/require-auth) middleware, you should still use `clerkMiddleware()` as it will provide authentication state to routes that don't use `requireAuth()`. See the [example](#example-use-clerk-middleware-require-auth-and-get-auth-together).
