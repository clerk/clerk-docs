...
- [`getToken({ skipCache: true })`](#gettoken-skipcache-true)
...
<Tabs items={["API route", "Client-side page"]}>
  <Tab>
    The `/update-user-metadata` API route uses Clerk's [`updateUserMetadata()`](/docs/reference/backend/user/update-user-metadata) method from [`clerkClient`](/docs/reference/backend/overview) to update the user's birthday metadata.
