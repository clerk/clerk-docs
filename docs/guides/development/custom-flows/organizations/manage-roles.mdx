...
<If sdk="ios">
  ...
  struct ManageRolesView: View {
    ...
    var body: some View {
      VStack {
        Button("Load memberships") {
          Task { await fetchOrganizationMemberships() }
        }

        ForEach(Array(memberships.enumerated()), id: \.offset) { index, membership in
          HStack {
            Text("Member \(index + 1): \(membership.role)")
            Button("Toggle role") {
              Task {
            ...
            Button("Remove") {
              Task { await removeMembership(membership) }
            }
          }
        }
      }
  ...
  extension ManageRolesView {
    ...
    func fetchOrganizationMemberships() async {
      guard let activeOrganizationId = clerk.session?.lastActiveOrganizationId else {
        memberships = []
        return
      }
      ...
      do {
        guard
          let userMemberships = try await clerk.user?.getOrganizationMemberships().data,
          let activeOrganization = userMemberships.first(where: { $0.organization.id == activeOrganizationId })?.organization
        else {
          memberships = []
          return
        }

        memberships = try await activeOrganization.getMemberships().data
      } catch {
    ...
    func updateMembershipRole(_ membership: OrganizationMembership, role: String) async {
      do {
        let updatedMembership = try await membership.update(role: role)
        if let index = memberships.firstIndex(where: { $0.id == updatedMembership.id }) {
