...
The logo used in Clerk emails comes from your application's settings. To add or update it, navigate to your application's [**Settings**](https://dashboard.clerk.com/~/settings) in the Clerk Dashboard, and upload your application logo under **Branding**.
...
> [!NOTE]
> These partials render HTML and are intended for **email templates only**. They don't apply to SMS templates, which are plain text.
...
<If is="llm">
  **For AI agents:** Instead of editing templates in the Dashboard, use the CLI: run `npx clerk@latest api templates/email` (or `templates/sms`) to list templates, or update one by fetching it with `npx clerk@latest api templates/email/<slug>`, editing the JSON, and sending it back with `npx clerk@latest api templates/email/<slug> -X PUT --file template.json` — the update requires the full template (`name`, `subject`, and `body` are required; `from_email_name`, `reply_to_email_name`, and `delivered_by_clerk` are optional) — discover the related endpoints (preview, revert, `toggle_delivery`) via `npx clerk@latest api ls templates`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
