...
<Steps>
  ...
  1. In your `proxy.ts` file, export the `clerkMiddleware()` helper:
     ...
     ```
  1. By default, `clerkMiddleware()` will not protect any routes. All routes are public and you must opt-in to protection. To require a signed-in user, protect resources close to where they're used, as shown in the [Pages Router examples in the guide on reading user data](/docs/guides/users/reading#pages-router).
