...
You will want to create a long-lived [JWT templates](/docs/backend-requests/making/jwt-templates) to be used in Postman or Insomnia. To do so, go to the Clerk Dashboard and navigate to the [**JWT template**](https://dashboard.clerk.com/last-active?path=jwt-templates) page. Click on the **New template** button and select the **Blank** template.
