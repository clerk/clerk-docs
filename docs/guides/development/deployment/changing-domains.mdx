...
<SignedIn>
  2) Your Secret Key is required to authenticate the request. It is injected into the cURL command after `Authorization`. However, you can also find it on the [**API keys**](https://dashboard.clerk.com/~/settings/api-keys) page in the Clerk Dashboard.
</SignedIn>

<SignedOut>
  2. Your Secret Key is required to authenticate the request.
     - Navigate to the [Clerk Dashboard](https://dashboard.clerk.com/~/settings/api-keys) and sign in.
