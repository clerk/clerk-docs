...
**The client token is only used in production environments**. When [deploying your app to production with Clerk](/docs/guides/development/production), you configure a CNAME record to alias Clerk's Frontend API (FAPI) to a subdomain of your application (e.g., `clerk.example.com`). This ensures the client token cookie remains within the same site context and does not need to be sent cross-domain.
