...
<Steps>
  ...
  <If notSdk={["ios", "android"]}>
    ...
    1. If the `signIn.status` is `'needs_second_factor'`, the user has MFA enabled. See the [MFA custom flow guide](/docs/guides/development/custom-flows/authentication/multi-factor-authentication) for how to handle this status. If the status is `'needs_client_trust'`, see the [Client Trust custom flow guide](/docs/guides/development/custom-flows/authentication/client-trust).
