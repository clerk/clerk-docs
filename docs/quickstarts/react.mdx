...
- Use Clerk's control components to protect your content
...
```tsx filename="src/main.tsx" {16, 18}
import React from 'react'
import ReactDOM from 'react-dom/client'
import App from './App.tsx'
import './index.css'
...
You can use the `<SignedIn />` and `<SignedOut />` control components to restrict access to content, or even whole pages. The `<SignedIn />` component requires the user be signed in to view the content inside the tags. The `<SignedOut />` components requires the user to be signed out.
