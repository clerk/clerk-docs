...
2. Check if the user accessing a private route is signed in and if the user is then allow them to access that route.
3. Any use who is not accessing a public route will get access to the prublic.
...
```typescript {{ filename: "/middleware.ts", mark: [[8,14]]}}
import { clerkMiddleware, createRouteMatcher } from '@clerk/nextjs/server';
import { NextRequest, NextResponse } from 'next/server';
...
```typescript {{ filename: "/middleware.ts", mark: [[12, 13]]}}
import { clerkMiddleware, createRouteMatcher } from '@clerk/nextjs/server';
import { NextRequest, NextResponse } from 'next/server';
...
``` {{ filename: "/app/organization-selection", mark: [[6, 10], [13,21]]}}
