...
<Tabs items={["NPM module", "<script>"]}>
  <Tab>
    <Steps>
      ...
      To initialize Clerk, add the following code to your `main.js` file. This code imports your [Publishable Key](!publishable-key), dynamically loads the Clerk UI bundle, creates a new [`Clerk`](/docs/reference/javascript/clerk) instance, and calls the [`load()`](/docs/reference/javascript/clerk#load) method to initialize Clerk.
