...
<Steps>
  ...
  <If sdk="expo">
    You can customize the appearance of the CAPTCHA widget by passing the `captcha` property to the [`appearance`](/docs/guides/customizing-clerk/appearance-prop/captcha) prop on `<ClerkProvider>`.

    ```tsx {{ filename: 'src/app/_layout.tsx', mark: [[10, 16]] }}
