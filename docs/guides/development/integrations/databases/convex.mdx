...
<Steps>
  ...
  <Tabs items={["Next.js", "React"]}>
    ...
    <Tab>
      The following example demonstrates how to configure Clerk and Convex's providers. Clerk's [`useAuth()`](/docs/references/hooks/use-auth) hook must be passed to Convex's `<ConvexProviderWithClerk>` and Clerk's `<ClerkProvider>` must be wrapped around it.
