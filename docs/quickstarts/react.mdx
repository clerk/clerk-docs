...
<Callout type='info'>
  You're reading the quickstart guide for the **beta** of the next major version of the React SDK. For more information about the changes from the current version, [check out the upgrade guide](/docs/upgrade-guides/core-2/react)!
</Callout>
...
<CodeBlockTabs type="installer" options={["npm", "yarn", "pnpm"]}>
  ...
  npm create vite@latest clerk-react@beta -- --template react-ts
  cd clerk-react
  npm install
  npm run dev
  ```
  ...
  ```bash filename="terminal"
  yarn create vite clerk-react@beta --template react-ts
  cd clerk-react
  yarn install
  yarn dev
  ```
  ...
  ```bash filename="terminal"
  pnpm create vite clerk-react@beta --template react-ts
  cd clerk-react
  pnpm install
  pnpm dev
  ```
...
2. Find your publishable key. If you're signed into Clerk, the `.env.local` snippet below will contain your key. Otherwise:
