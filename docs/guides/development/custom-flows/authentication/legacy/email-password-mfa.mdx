...
<Steps>
  ...
  1. If the verification is successful, set the newly created session as the active session. You may need to check for [session tasks](!session-tasks) that are required for the user to complete after signing up.
