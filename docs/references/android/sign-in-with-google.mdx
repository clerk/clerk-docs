...
<Steps>
  ...
  1. Complete the required fields.
     ...
     - **SHA-1 certificate fingerprint**: To get your SHA-1, run the `keytool -keystore path-to-debug-or-production-keystore -list -v` command where `path-to-debug-or-production-keystore` needs to be replaced with the path to your debug or production keystore. By default, the debug keystore is located in `~/.android/debug.keystore`. It may ask for a keystore password, which is `android`.
  1. Select **Create**.
