...
Clerk's Next.js and Node SDKs expose an instance of the Clerk Backend SDK for use in server environments.

<Tabs items={["Next.js", "Node"]}>
<Tab>
The Backend SDK is accessible via the `clerkClient` object if you are using Next.js.

This example shows how to update a user's profile information. See the [`updateUser()`](/docs/references/backend/user/update-user) documentation for more information.
