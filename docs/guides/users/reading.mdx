...
To retrieve a token from a [JWT template](/docs/guides/development/jwt-templates) and fetch data from an external source, use the [`getToken()`](/docs/references/backend/types/auth-object#get-token){{ target: '_blank' }} method from the `auth()` local.
