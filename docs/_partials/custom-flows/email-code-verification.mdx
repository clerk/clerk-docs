...
<If notSdk={["ios", "android"]}>
  1. Every user has a [`User`](/docs/reference/objects/user) object that represents their account. The `User` object has a `emailAddresses` property that contains all the email addresses associated with the user. The [`useUser()`](/docs/reference/hooks/use-user) hook is used to get the `User` object.
  1. The [`User.createEmailAddress()`](/docs/reference/objects/user#create-email-address) method is passed to the [`useReverification()`](/docs/reference/hooks/use-reverification) hook to require the user to reverify their credentials before being able to add an email address to their account.
  1. If the `createEmailAddress()` function is successful, a new [`EmailAddress`](/docs/reference/types/email-address) object is created and stored in `User.emailAddresses`.
  ...
  <If sdk="expo">
    ```tsx {{ filename: 'src/app/(account)/add-email/page.tsx', collapsible: true }}
    ...
    export default function Page() {
      ...
      // Step 1: Add the email address to the user's User object
      const handleSubmit = async () => {
        try {
          ...
          // Create a reference to the new email address, to use related methods
          const emailAddress = user?.emailAddresses.find((a) => a.id === res?.id)
          setEmailObj(emailAddress)
      ...
      // Step 3 UI: Display a success message if the email was added successfully
      if (successful) {
        return (
          <ThemedView style={styles.container}>
      ...
      // Step 1 UI: Display the initial form to capture the email address
      return (
        <ThemedView style={styles.container}>
          <ThemedText type="title" style={styles.title}>
            Add Email
