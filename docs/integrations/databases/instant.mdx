...
> [!IMPORTANT]
> Check out the [demo repo](https://github.com/clerk/clerk-instant-nextjs) for a full example of how to integrate Instant with Clerk in a Next.js app.

Integrating Instant with Clerk gives you the benefits of using an Instant database while leveraging Clerk's authentication, prebuilt components, and webhooks.
This guide will walk you through the steps to integrate Instant with Clerk in your Next.js app.
...
<Steps>
  ...
  - Calls Instant's `db.auth.signInWithIdToken` with the session, to sign into Instant

  When you call `db.auth.signInWithIdToken`, Instant will verify that the session was signed by your Clerk app. If verified, Instant use the email in the session's claims to log in, lookup your user or create a new one.
