...
2. In the navigation sidebar, select **Customization**.
3. In the dropdown, select the **[Email](https://dashboard.clerk.com/last-active?path=customization/email)** or **[SMS](https://dashboard.clerk.com/last-active?path=customization/sms)** template listing page.
4. Select the desired template card.

Once changes are made, select the **Apply Changes** button at the bottom right. Your changes will then take immediate effect for the corresponding application instance.
...
The email template editor is comprised of [template settings](#template-settings) & the WYSIWYG editor.
...
The toolbar contains two types of controls:

1. Buttons - expose a sub-menu with more settings you can apply to the currently selected block or text.
...
Select the mobile view toggle again to return to desktop view.
...
To exit HTML/source mode, select the toggle again.
...
Exposes a color picker so that you can select the color of the fonts in the affected block or text fragment. For the button to appear, you will need to select inside a text node.
...
Shows a dropdown with the available variables for the current template. For the button to appear, you will need to select inside a text node. Upon insertion, the variable will be automatically wrapped in curly braces; there is no need to add them yourself.
...
Permits setting a border width, color and radius on a given block. Using the radius you can give a block rounded corners. For the button to appear, you will need to select an element that allows borders, such as a button.
...
The email editor text area is intended mainly for authoring content. Though, if you select a block, a set of tools will also appear:
...
To insert a variable at the current cursor position, select the corresponding variable badge in the variable widget that is located adjacent to the text area.
