...
<Tabs items={["API route", "Client-side page"]}>
  <Tab>
    The `/update-user-metadata` API route uses Clerk's [`updateUserMetadata()`](/docs/references/backend/user/update-user-metadata) method from the [JS Backend SDK](/docs/js-backend/getting-started/quickstart) to update the user's birthday metadata.
