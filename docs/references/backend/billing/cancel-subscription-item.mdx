...
description: Use Clerk's Backend SDK to cancel a subscription item.
sdk: js-backend
---
...
> [!WARNING]
> This API is experimental and subject to change. Pin the SDK version to avoid breaking changes.
...
This method in the SDK is a wrapper around the BAPI endpoint `DELETE/commerce/subscription_items/{id}`. See the [BAPI reference](/docs/reference/backend-api/tag/Billing#operation/CancelSubscriptionItem){{ target: '_blank' }} for more information.
