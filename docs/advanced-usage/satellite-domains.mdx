...
<Steps>
  ...
  To use a satellite domain in production, you will need to add a **CNAME record** for the `clerk` subdomain.

  1. If the page is no longer open, navigate to the [**Domains**](https://dashboard.clerk.com/last-active?path=domains) page in the Clerk Dashboard.
  1. Select the **Satellites** tab.
  ...
  Once your CNAME record is set up correctly, you should see a **Verified** label next to your satellite domain in the Dashboard.

  > [!NOTE]
  > It can take up to 48hrs for DNS Records to fully propagate, so be patient.
