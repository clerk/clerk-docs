...
**Session tasks** are pending requirements that users must complete after authentication, such as choosing an Organization. When a session task is enabled, it is handled automatically within the `<SignIn />` and `<SignUp />` components and Clerk's [Account Portal](/docs/guides/account-portal/overview).
...
Once enabled, the following session task components are **embedded by default** within the `<SignUp />` and `<SignIn />` components. However, if you want to customize the route where the task components are rendered or customize their appearance, you can host them within your application.
...
- [The `taskUrls` option](#using-the-task-urls-option): Redirect across your entire application

<If sdk="nextjs">
  * [Middleware-based redirects](#using-middleware-based-redirects): Redirect for specific routes or route groups
</If>

<If sdk={["chrome-extension", "expo", "nextjs", "nuxt", "react", "react-router", "tanstack-react-start", "vue"]}>
  - [The `<RedirectToTasks />` component](#using-the-redirect-to-tasks-control-component): Redirect from routes, route groups (in a layout), or individual pages
</If>
...
<If sdk="nextjs">
  ...
  In your `clerkMiddleware()`, read the session status and redirect users with pending tasks to the task page. This example matches the request path to decide which routes require completed tasks.

  {/* TODO: Add example for Astro, Nuxt, React Router, and Tanstack React Start. */}

  ```tsx {{ filename: 'proxy.ts' }}
  import { clerkMiddleware } from '@clerk/nextjs/server'
  import { NextRequest, NextResponse } from 'next/server'

  // Set the routes that require completed session tasks
  const sessionTaskRoutes = ['/dashboard', '/forum']

  export default clerkMiddleware(async (auth, req: NextRequest) => {
    // Use the `auth()` helper to get the session status
  ...
  > [!WARNING]
  > Like any middleware path match, this redirect is a navigation convenience, not a security boundary: it controls where pending users land, not access to data. A `pending` session is already treated as signed-out, so enforce protection in the page, Route Handler, or Server Action that reads protected data.
</If>
