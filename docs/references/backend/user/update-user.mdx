...
<Properties>
  ...
  The hashing algorithm that was used to generate the password digest. Each of the supported hashers expects the incoming digest to be in a particular format. See the [Clerk Backend API reference](/docs/references/backend-api/tag/Users#operation/CreateUser){{ target: '_blank' }} for more information.
  ...
  Metadata that can be read from the Frontend API and [Backend API](/docs/references/backend-api){{ target: '_blank' }} and can be set only from the Backend API. Updating this property will override the existing metadata. To merge metadata, use [`updateUserMetadata()`](/docs/references/backend/user/update-user-metadata).
  ...
  Metadata that can be read and set only from the [Backend API](/docs/references/backend-api){{ target: '_blank' }}. Updating this property will override the existing metadata. To merge metadata, use [`updateUserMetadata()`](/docs/references/backend/user/update-user-metadata).
...
This method in the SDK is a wrapper around the BAPI endpoint `PATCH/users/{user_id}`. See the [BAPI reference](/docs/references/backend-api/tag/Users#operation/UpdateUser){{ target: '_blank' }} for more information.
