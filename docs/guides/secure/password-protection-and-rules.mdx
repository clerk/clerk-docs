...
- Before enabling rejection of compromised passwords on sign-in, ensure your app has support for the password reset flow. You can do so by using Clerk components or [implementing a custom flow](/docs/guides/development/custom-flows/forgot-password#prompting-users-to-reset-compromised-passwords-during-sign-in).
