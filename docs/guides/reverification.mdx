...
<Tabs items={["Next.js Server Action", "Next.js Route Handler", "Ruby on Rails", "Other"]}>
  ...
  <Tab>
    The following example uses the `clerk_user_needs_reverification` and `clerk_render_reverification` helpers to check if the user has verified their credentials withing a specific time period. The `moderate` configuration sets the time period to 1 hour. If the user hasn't verified their credentials within 1 hour, the `clerk_render_reverification` utility is used to return a 403 JSON response that the client reads to initiate the reverification flow. Once the user completes the reverification, they can access the `foo` action, which returns a success response.
