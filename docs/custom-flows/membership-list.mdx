...
<Tabs items={["Next.js", "JavaScript"]}>
  <Tab>
    ...
    1. Uses the [`useOrganizationList()`](/docs/references/react/use-organization-list) hook to get `memberships`, which is a list of the current user's organization memberships.
       - `memberships` returns `data`, which is an array of [`OrganizationMembership`](/docs/references/javascript/types/organization-membership) objects.
    ...
    ```jsx {{ filename: 'app/components/JoinedOrganizations.tsx', collapsible: true }}
    'use client'
