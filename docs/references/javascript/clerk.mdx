...
Another example is when a user switches organizations in a multi-organization application. `setActive()` needs to be used to set the new organization as the [active organization](!active-organization). See the implementation of this in the [Custom organization switcher](/docs/custom-flows/organization-switcher) guide.
