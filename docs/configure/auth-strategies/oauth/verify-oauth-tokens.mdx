...
If you are building an application that uses Clerk and would like to incorporate OAuth, you'll want to ensure that after the [**client**](/docs/configure/auth-strategies/oauth/overview#key-terminology) gets an OAuth access token, they can use it to make authenticated requests into your app (the [**resource service**](/docs/configure/auth-strategies/oauth/overview#key-terminology)) using the token.
