...
On the backend, it's available on the [Backend `User`](/docs/references/backend/types/backend-user) object which can be accessed using the JavaScript Backend SDK's [`getUser()`](/docs/references/backend/user/get-user) method. However, this makes a network request, which is subject to [rate limits](/docs/backend-requests/resources/rate-limits.mdx). To avoid a network request, you can attach the metadata as a claim in the user's session token. Then, the metadata can be retrieved from the `sessionClaims` on the [`Auth`](/docs/references/backend/types/auth-object) object. See the [guide on customizing your session token](/docs/backend-requests/custom-session-token.mdx).
