...
The following example shows how to force a JWT refresh by using the [`useSession()`](/docs/hooks/use-session) hook to access the `getToken()` method.
...
The following example shows how to force a JWT refresh by using the [`useUser()`](/docs/hooks/use-user) hook to access the `user.reload()` method.
