...
PKCE is currently **optional** for public clients. If a `code_challenge` is sent during the authorization request, then PKCE will be enforced for that flow. A `code_verifier` must then be provided to successfully complete the token exchange.
