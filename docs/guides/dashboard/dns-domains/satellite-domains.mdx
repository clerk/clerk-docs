...
By default (`satelliteAutoSync: false`), satellite domains do not automatically sync authentication state with the primary domain when a user visits the page. This means there is no upfront performance cost for users visiting a satellite domain. Authentication state is only synced when a user initiates a sign-in or sign-up action:

1. When a user clicks sign in on the satellite domain, they are redirected to the primary domain.
...
If you set `satelliteAutoSync: true`, the satellite domain will automatically redirect to the primary domain on first load to sync authentication state, even if the user has no session. This is the legacy behavior from Core 2 and is useful if you want users who are already signed in on the primary domain to be automatically recognized on the satellite without needing to click sign in. However, this comes with a performance cost since every first visit triggers a redirect.
...
<Steps>
      ...
      <Tabs items={["Next.js", "Remix"]}>
        <Tab>
          ...
          const options = {
            ...
            // Set to true to automatically sync auth state on first load (not recommended for performance)
  ...
  1. Visiting the satellite domain. You will appear as signed out.
  1. Clicking sign in. You will be redirected to the primary domain.
...
| Sign-in flow | Same as Core 3 | User clicks sign in → redirected to primary → signs in → redirected back |
| Already signed in on primary | Automatically synced on first visit | Only synced after user clicks sign in (redirect is instant, no user action needed) |
...
If your application relies on users being automatically recognized on the satellite domain without clicking sign in, set `satelliteAutoSync` to `true`:
...
For best performance, keep `satelliteAutoSync` set to `false` (the default). With this setting, satellite domains load instantly without any upfront redirect cost. Users will only be redirected to the primary domain when they initiate a sign-in or sign-up action. If they are already signed in on the primary domain, the redirect back is immediate and requires no user interaction.
