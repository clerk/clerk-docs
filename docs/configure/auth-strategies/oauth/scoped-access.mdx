...
> [!NOTE]
> If you would like to know more about the terminology around OAuth, check out the [OAuth terminology guide](/docs/configure/auth-strategies/oauth/overview#key-terminology).
...
<Steps>
  ...
  1. Complete the following fields:
     ...
     - `Scopes` - The scopes that you would like to leverage. [Learn more about the scopes currently supported by Clerk](/docs/configure/auth-strategies/oauth/how-clerk-implements-oauth#scopes).
  ...
  To learn more about the other OAuth application configuration settings, like **Dynamic client registration**, see the [dedicated guide](/docs/configure/auth-strategies/oauth/how-clerk-implements-oauth).
  ...
  >
  > By default, the consent screen is shown for all newly created OAuth apps, but this can be disabled in each app's settings [in the Clerk Dashboard](https://dashboard.clerk.com/last-active?path=oauth-applications), although not recommended. [Learn more about how Clerk's OAuth consent screen works](/docs/configure/auth-strategies/oauth/how-clerk-implements-oauth#consent-screen-management).
