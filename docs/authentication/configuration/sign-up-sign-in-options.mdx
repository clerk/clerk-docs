...
- Passkeys are domain-specific. In development, this means passkeys only work on the domain where they were created. Refer to [the passkeys custom flow guide](/docs/custom-flows/passkeys#domain-restrictions-for-passkeys-in-development) for more information.
