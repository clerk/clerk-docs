---
title: User & Organization Pages | Account Portal
description: In addition to the sign-in/sign-up pages, the Account Portal also provides the User Profile, Organization Profile and Create Organization flow out of the box. 
---

# User & Organization Pages

In addition to the sign-in/sign-up pages, the Account Portal also provides the User Profile, Organization Profile, and Create Organization flow out of the box. 

You can use Clerk's Control Components to navigate your users to the appropriate page.

<Tabs items={["User Profile Page", "Organization Profile Page","Create Organization Flow"]}>
<Tab>
# [`<RedirectToUserProfile/>`](/docs/components/control/redirect-to-userprofile)

<Images 
  width={3248}
  height={1716}
  src="/docs/images/account-portal/account_portal_user_profile.png"
  alt="User Profile in Account Portal"
/>
</Tab>

<Tab>
# [`<RedirectToOrganizationProfile/>`](docs/components/control/redirect-to-organization-profile)

<Images 
  width={3248}
  height={1716}
  src="/docs/images/account-portal/account_portal_org_profile.png"
  alt="Organization Profile in Account Portal"
/>
</Tab>

<Tab>
# [`<RedirectToCreateOrganization/>`](docs/components/control/redirect-to-create-organization)

<Images 
  width={3248}
  height={1716}
  src="/docs/images/account-portal/account_portal_create_org.png"
  alt="Create Organization flow in Account Portal"
/>
</Tab>
</Tabs>