...
`clerkMiddleware()` doesn't protect routes by default, and adding auth checks to it is no longer recommended. Enforce authentication and authorization as close to the resource as possible, in the page, Route Handler, Server Action, or other server-side code that reads or mutates the data.

- To require a signed-in user, see [Protect content from unauthenticated users](/docs/guides/secure/protect-content).
