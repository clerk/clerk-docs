...
1. **Handle the task**: If a task exists, build UI to guide the user through completing the required action based on the task type. See the [available task types](/docs/authentication/configuration/after-auth-flows#available-flows) to understand what actions may be required.
