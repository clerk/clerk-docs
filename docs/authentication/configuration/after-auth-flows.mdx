...
A user's authentication state can be considered signed-in, signed-out, or pending. **By default, `pending` sessions are treated as signed-out** across Clerk's authentication context. This affects features that rely on the user's authentication state, like control components, the `useAuth()` hook, and server utilities that access the `auth` object, such as `getAuth()` or `request.auth`. However, you can set `treatPendingAsSignedOut` to `false` to treat `pending` sessions as signed-in. This is useful for cases where you may need to access the user's data, like their user ID, regardless if they have completed the after-auth flow.
