...
> [!IMPORTANT]
> This guide uses `useLocalCredentials()` to store and reuse password credentials after a local biometric check. [Biometric trusted-device sign-in](/docs/guides/development/custom-flows/authentication/biometric-sign-in) instead registers a device-bound public key with Clerk and signs a one-time challenge. [`expo-local-authentication`](https://docs.expo.dev/versions/latest/sdk/local-authentication/) only performs the local check; by itself, it neither registers a trusted device nor authenticates with Clerk.
