...
- [The "Light/Dark" theme](#light-dark-theme)
...
To use the light/dark theme, set `theme` to `lightDark`:

```tsx {{ mark: ['lightDark'] }}
import { lightDark } from '@clerk/themes'
...
To enable the light/dark theme, you need to set the `color-scheme` property to `light dark` in your `global.css` file.
...
You can also enable dark mode via a class by specifying `color-scheme: dark` in your `global.css` file.
...
You can also enable light mode via a class by specifying the `light` class in your `global.css` file.
