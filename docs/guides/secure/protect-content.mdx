...
- Use [`auth.protect()`](/docs/reference/nextjs/app-router/auth#auth-protect) to:
  - Redirect unauthenticated users to the sign-in route for document requests.
  - Return a `404` error for authenticated users who are not authorized to access the route or content.
- Use [`auth()`](/docs/reference/nextjs/app-router/auth) and check `isAuthenticated` yourself if you want more control over the response.
...
<Tabs items={["auth.protect()", "auth()"]}>
  <Tab>
    Inside a Server Action, [`auth.protect()`](/docs/reference/nextjs/app-router/auth#auth-protect) doesn't redirect. Because a Server Action isn't a document request, it rejects unauthenticated requests with a `401` error instead. To handle the unauthenticated case yourself, use `auth()`.
...
>
> Protect the data instead: guard the Route Handlers, Server Actions, and external API endpoints the client calls. Protecting the Server Components that render these Client Components also guards any data you pass down as props (see [Server-side](#server-side)).
