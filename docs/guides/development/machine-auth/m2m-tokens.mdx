...
M2M tokens can be created as either [opaque tokens](!opaque-token) (the default) or [JSON Web Tokens (JWTs)](/docs/guides/how-clerk-works/tokens-and-signatures#json-web-tokens-jwts). JWTs don't require a network request for verification and incur charges only for creation, not verification. See [Token formats](/docs/guides/development/machine-auth/token-formats) for more details on the differences between the two formats.
...
To create a JWT token instead of the default opaque token, pass `tokenFormat: 'jwt'`:
...
> [!NOTE]
> JWT tokens are not stored by Clerk, so they cannot be fetched via the list endpoint (`clerkClient.m2m.getTokens()`). The list endpoint will only return opaque tokens. Additionally, since JWT verification happens client-side, Clerk cannot track `last_used_at` for JWT tokens.
