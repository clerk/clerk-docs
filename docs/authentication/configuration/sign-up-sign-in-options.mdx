...
If you're building a custom user interface instead of using the [Account Portal](/docs/account-portal/overview) or [prebuilt components](/docs/reference/components/overview), you can use [elements](/docs/customization/elements/examples/sign-in#multi-factor-authentication-mfa) or [the Clerk API](/docs/custom-flows/email-password-mfa) to build a custom sign-in flow that allows users to sign in with MFA.
