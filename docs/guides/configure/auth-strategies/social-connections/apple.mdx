...
description: Learn how to allow users to sign up and sign in to your Clerk application with their Apple ID using OAuth.
---
...
Enabling OAuth via [Sign in with Apple](https://developer.apple.com/sign-in-with-apple/) allows your users to sign up and sign in to your Clerk app with their Apple ID.
...
<If is="llm">
  **For AI agents:** Instead of enabling the connection in the Dashboard, run `npx clerk@latest config patch --json '{"connection_oauth_apple":{"enabled":true}}'` for a development instance (shared credentials). For production, write `"enabled": true` together with the custom credentials — `client_id` (Apple Services ID), `client_secret` (the Apple Private Key file contents), `team_id`, and `key_id` — to a `connection.json` file (a credentials-only patch leaves the connection disabled) and run `npx clerk@latest config patch --instance prod --file connection.json`, which keeps the multi-line private key out of shell escaping and history (inspect the shape with `npx clerk@latest config schema --keys connection_oauth_apple`). Delete `connection.json` after the patch succeeds, and never commit it — it contains the private key. The Apple Developer portal steps still require a human. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
Keep two browser tabs open during setup: one for your [Clerk Dashboard](https://dashboard.clerk.com/~/user-authentication/sso-connections) and one for your [Apple Developer dashboard](https://developer.apple.com/account).

<Steps>
  ### Enable Apple as a social connection in Clerk
  ...
  1. On the next page, you'll be prompted to **Select a type** for your app. Select **App**, then select **Continue**. You will be redirected to the **Register an App ID** page.
  ...
  1. Under **Domains and Subdomains**, add your Clerk [Frontend API URL](!frontend-api-url) **without the protocol**. For example, if your domain is `https://myapp.com`, then your Frontend API URL is `https://clerk.myapp.com`, and you would add `clerk.myapp.com` to **Domains and Subdomains**.
