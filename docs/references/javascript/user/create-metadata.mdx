...
<Callout type="warning">
  **Email address** must be enabled as an identifier in your Clerk settings for this method to work. See the [Identifiers](/docs/authentication/configuration/sign-in-sign-up-options#identifiers) section to learn more.
</Callout>
...
| Type | Description |
| --- | --- |
| <code>Promise\<[EmailAddress][email-ref]\></code> | This method returns a `Promise` which resolves to the the email address created for the user. |
...
if (clerk.user) {
  ...
  clerk.user
    ...
    .then((response) => {
      ...
      console.log("Email added to user successfully");
    })
    .catch((error) => {
...
<Callout type="warning">
  **Phone number** must be enabled as an identifier in your Clerk settings for this method to work. See the [Identifiers](/docs/authentication/configuration/sign-in-sign-up-options#identifiers) section to learn more.
</Callout>
...
| Type | Description |
| --- | --- |
| `Promise<PhoneNumber>` | This method returns a `Promise` which resolves to the [`PhoneNumber`][phone-ref] created for the user. |
...
if (clerk.user) {
  ...
  clerk.user
    ...
    .then((response) => {
      console.log(response);
      console.log("Phone number added to user successfully");
    })
    .catch((error) => {
      console.log("An error occurred:", error.errors);
    });
...
<Callout type="warning">
  A Web3 provider must be enabled in your Clerk settings for this method to work. See the [Web3 authentication](/docs/authentication/configuration/sign-in-sign-up-options#web3-authentication) section to learn more.
</Callout>
...
| Type | Description |
| --- | --- |
| `Promise<Web3Wallet>` | This method returns a `Promise` which resolves to the [`Web3Wallet`][web3-ref] created for the user. |

### `createWeb3Wallet()` example
...
if (clerk.user) {
  ...
  clerk.user
    ...
    .then((response) => {
      console.log(response);
      console.log("Web3 wallet added to user successfully");
    })
    .catch((error) => {
      console.log("An error occurred:", error.errors);
    });
...
<Callout type="warning">
  The social provider that you want to connect to must be enabled in your Clerk settings for this method to work. See the [Social connections (OAuth)](/docs/authentication/configuration/sign-in-sign-up-options#social-connections-o-auth) section to learn more.
</Callout>
...
| Type | Description |
| --- | --- |
| `Promise<ExternalAccount>` | This method returns a `Promise` which resolves to the [`ExternalAccount`][exacc-ref] created for the user. |

### `createExternalAccount()` example
...
- If the connection was not successful, then the `externalAccount.verification.status` should not be `verified` and the `externalAccount.verification.error` will contain the error encountered so that you can present corresponding feedback to the user.

The following example demonstrates how to add a Notion account as an external account for the user. When the user clicks the "Add Notion as a social connection" button, the user will be redirected to Notion to connect their account. After connecting their account, they will be redirected back to the `/` route of your application and the status of the connection will be displayed.
