...
You can programmatically unlock a user using the [`UnlockUser`](https://clerk.com/docs/reference/backend-api/tag/Users#operation/UnlockUser) Backend API endpoint.
...
If you'd like to manually lock a user from signing in, you can use the [`LockUser`](https://clerk.com/docs/reference/backend-api/tag/Users#operation/LockUser) Backend API endpoint. Keep in mind that Clerk will still lock the user based on failed verification attempts.
