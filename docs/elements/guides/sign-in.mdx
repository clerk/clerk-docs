...
Make it functional by adding fields. Use the Clerk Elements field components to render an `identifier` field, as well as the `<Provider>` component to allow users to sign in with a social provider, like Google:
