...
The most common cause of this error is when [`auth()`](/docs/references/nextjs/app-router/auth) is called **without** a [`<ClerkProvider>`](/docs/references/components/clerk-provider) wrapping the component tree where Clerk components are used.

<Include src="_partials/clerk-provider/explanation" />

#### Solution

Ensure that `<ClerkProvider>` is set up at a higher level in your component hierarchy than any Clerk components like [`<UserButton />`](/docs/references/components/user/user-button), [`<SignedIn />`](/docs/references/components/control/signed-in), or others. [The Next.js quickstart](https://clerk.com/docs/quickstarts/nextjs) shows how you can do this in your root layout.
