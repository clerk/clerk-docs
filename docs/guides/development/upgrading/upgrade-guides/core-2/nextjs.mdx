...
> [!NOTE]
> This section describes the Core 2 Middleware migration. `createRouteMatcher()` has since been deprecated, and performing auth checks inside `clerkMiddleware()` is no longer recommended. For current guidance on protecting routes, see [Protect content from unauthenticated users](/docs/guides/secure/protect-content).
