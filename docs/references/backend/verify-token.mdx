...
Verifies a Clerk-generated token signature. Networkless if the `jwtKey` is provided. Otherwise, performs a network call to retrieve the JWKS from the [Backend API](/docs/references/backend-api/tag/JWKS#operation/GetJWKS){{ target: '_blank' }}.
...
<Properties>
  ...
  The [Clerk Backend API](/docs/references/backend-api){{ target: '_blank' }} endpoint. Defaults to `'https://api.clerk.com'`.
