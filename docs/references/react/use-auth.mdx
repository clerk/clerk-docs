...
> [!NOTE]
> For Next.js applications, it's recommended to use the [`auth()`](/docs/references/nextjs/auth) helper instead of the `useAuth()` hook. `auth()` must be used in server components, so you will need to pass the auth data to Client Components as needed. _If_ you would like to use the `useAuth()` hook, you will need to pass the `dynamic` prop to the `<ClerkProvider>` component, but doing so will opt the application into dynamic rendering. Learn more [here](/docs/references/nextjs/rendering-modes).
