...
{/* DOCS NOTE: The IF blocks depend on if the SDK can access clerkClient() or not. The SDKs that can access clerkClient() are nextjs, astro, express, fastify, hono, nuxt, react-router, and tanstack-react-start. Everything else must install the `@clerk/backend` package separately. */}
...
<If notSdk={["nextjs", "astro", "expressjs", "fastify", "hono", "nuxt", "react-router", "tanstack-react-start"]}>
  Some of Clerk's SDKs, like Next.js and React Router, provide access to `clerkClient` from their package. If you are not using one of these SDKs, install the `@clerk/backend` package using the following command:
...
<If sdk={["nextjs", "astro", "expressjs", "fastify", "hono", "nuxt", "react-router", "tanstack-react-start"]}>
...
<If sdk={["nextjs", "astro", "expressjs", "fastify", "hono", "nuxt", "tanstack-react-start"]}>
  ...
  <If sdk="hono">
    In Hono applications, the `clerkMiddleware()` helper attaches the Clerk Backend API client to the context. Use `c.get('clerk')` inside a route handler after `clerkMiddleware()` has run.
...
<If sdk={["nextjs", "astro", "expressjs", "fastify", "hono", "nuxt", "react-router", "tanstack-react-start"]}>
  ...
  <If sdk="hono">
    For Hono, the `Auth` object is accessed using the `getAuth()` function. [Learn more about using `getAuth()`](/docs/reference/hono/get-auth).
