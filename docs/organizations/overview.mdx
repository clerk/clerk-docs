...
The number of organizations you can create depends on your [Clerk plan](/pricing), and is measured by "monthly active organizations" (MAOs) - this means an organization with more than one user that has signed in that month.

With the Free plan, you can have up to 100 MAOs on any given Clerk app. Each MAO can have up to 5 members.

For a Clerk app with the Pro plan, you can create up to 100 MAOs free. Each MAO after the first 100 costs $1.00 per month. Each MAO can have an unlimited number of members. As an example, if you have 120 MAOs on your app, it would cost an extra $20/month, on top of the base Pro plan pricing at $25/month, for a total of $45/month.
...
Users can create organizations within your application. When a user creates an organization, they become the organization's admin. As the organization's admin, they have full control over the organization, including the ability to update the organization's settings, invite users to join the organization, and manage the organization's members.

A user within one of your applications can create up to 100 organizations in that application. This limit is separate from the number of [organizations *you* can create](#application-owner) across your entire account.

The easiest way to allow users to create organizations is to use Clerk's [`<CreateOrganization />`](/docs/components/organization/create-organization) component. This component provides a form for users to create a new organization.
...
If Clerk's prebuilt components don't meet your specific needs or if you require more control over the logic, you can rebuild the existing Clerk flows using the Clerk API. Some useful guides include:
