...
<Properties>
  ...
  If your web application only uses [control components](/docs/references/general/components/overview#control-components), you can set this value to `headless` and load a minimal ClerkJS bundle for optimal page performance.
  ...
  **Chrome Extension only** To enable, pass the URL of the web application that the extension will sync the authentication state from. See the [dedicated guide](/docs/guides/development/sync-host) for more information.
  ...
  The URL paths users are redirected to after sign-up or sign-in when specific session tasks need to be completed. For example, `{ 'choose-organization': '/onboarding/choose-organization' }` redirects users to `/onboarding/choose-organization` after sign-up if they need to select an organization.
