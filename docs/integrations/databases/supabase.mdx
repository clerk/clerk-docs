...
<Steps>
  ...
  <Tabs items={["Client side rendering", "Server side rendering"]}>
    ...
    <Tab>
      ...
      The `client` can be used to both query data on page load and within server actions, which are callable from server and client components.

      <CodeBlockTabs options={['page.tsx', 'actions.ts', 'AddTaskForm.tsx', 'TaskRow.tsx']}>
        ...
        function TaskRow({ id, name, is_done }: Props) {
          ...
          return (
            <div
              className={`group flex items-center transition-all w-full${is_done ? 'text-slate-500' : ''}`}
            >
              <Button
                variant="link"
