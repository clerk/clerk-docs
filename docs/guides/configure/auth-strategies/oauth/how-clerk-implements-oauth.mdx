...
Some OAuth clients, including ChatGPT and Claude, may omit the `scope` parameter when they dynamically register. Whether a client sends `scope` is determined by the client on a per-client basis and is outside the control of both Clerk and a Clerk user.

When a client omits `scope`, you can configure the default scopes Clerk should use for the registration. This provides some control over the access requested by clients that don't specify their scopes. Clerk doesn't override the `scope` value when a client provides one.

You can configure default scopes for your instance in either of these ways:

- In the Clerk Dashboard, open [**OAuth applications**](https://dashboard.clerk.com/~/oauth-applications), configure **Default scopes**, and save your changes. The **Default scopes** setting is only available when **Dynamic client registration** is enabled.
