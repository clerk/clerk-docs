---
description: Learn how to build a custom sign-up and sign-in flow using Clerk's API that supports OAuth connections.
---
...
<Tabs items={["Next.js", "Expo", "iOS (Beta)"]}>
  ...
  <Tab>
    ```swift {{ filename: 'OAuthView.swift' }}
    import SwiftUI
    import ClerkSDK
    ...
    extension OAuthView {
        ...
        func signInWithOAuth(provider: ExternalProvider) async {
            do {
                // Start the sign-in process using the selected OAuth provider.
                let signIn = try await SignIn.create(strategy: .externalProvider(provider))

                // Start the OAuth process
                let oauthResult = try await signIn.authenticateWithRedirect()
                ...
                // Therefore, an OAuthResult can contain either a SignIn or SignUp.

                // Check if the result of the OAuth was a sign in
                if let signIn = oauthResult?.signIn {
                ...
                if let signUp = oauthResult?.signUp {
...
<Tabs items={["Next.js", "iOS (Beta)"]}>
  ...
  <Tab>
    ...
    // authenticateWithRedirect() will return the orginal sign in to you
