...
<Steps>
  ...
  <If notSdk={["ios", "android"]}>
    ...
    1. If the `signIn.status` is `'needs_second_factor'`, send a one-time code to the user's email address with the [`signIn.mfa.sendEmailCode()`](/docs/reference/objects/sign-in-future#mfa-send-email-code) method.
