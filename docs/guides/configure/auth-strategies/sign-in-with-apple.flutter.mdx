...
> [!NOTE]
> Native Sign in with Apple is only available on **iOS**. On Android, use the browser-based OAuth flow instead — call `ssoSignIn(context, Strategy.oauthApple)` as described in the [authentication flows reference](/docs/reference/flutter/auth#sign-in-with-oauth).
...
If you only need browser-based OAuth (a WebView flow that doesn't require additional packages), you can call `ssoSignIn(context, Strategy.oauthApple)` directly — see the [authentication flows reference](/docs/reference/flutter/auth#sign-in-with-oauth). The steps below cover the native ID token approach.
