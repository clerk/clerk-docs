...
#### Per-page auth config

<Callout type='danger'>
    It is recommended not to rely on Layouts for auth protection. NextJS layouts do not rerender on client-side navigations and are not executed under certain conditions, so it is not guaranteed that your auth logic will be executed always. If you want to protect entire routes, use per-page auth or  Middleware-based protection instead.
    For more information, [refer to the NextJS Layout docs](https://nextjs.org/docs/app/building-your-application/routing/pages-and-layouts#layouts).
</Callout>

Sometimes, code speaks louder than words. Let's get right into an example:

<Tabs type="auth-protect-page" items={["Page"]}>
  <Tab>
    ...
    export default async function DashboardPage() {
        auth().protect();
        // 👆 This one line is all it takes - this page is protected and
...
In this example, you add [`auth().protect()`](/docs/references/nextjs/auth#protect) to a page and that page is gated by auth. If you want to protect entire routes, use Middleware-based auth as described in the next section.
