...
Before uprading, it's highly recommended that you update your Clerk SDKs to the latest Core 1 version (`npm i @clerk/clerk-expo@core-1`). Some changes required for Core 2 can be applied incrementally to the latest v1 release, which should contribute to a smoother upgrading experience.
...
<Accordion titles={["<code>User.update({ password: &#39;x&#39; })</code> -&gt; <code>User.updatePassword(&#39;x&#39;)</code>", "<code>CLERK_FRONTEND_API</code> replaced by <code>CLERK_PUBLISHABLE_KEY</code>"]}>
  <AccordionPanel>    
