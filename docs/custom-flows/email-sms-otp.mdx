...
This guide will walk you through how to build a custom email or SMS OTP sign-up and sign-in flow using the [`useSignUp()`](/docs/references/react/use-sign-up) and [`useSignIn()`](/docs/references/react/use-sign-up) React hooks.

<Steps>

### Enable email and SMS OTP

To use email and SMS OTP, you first need to enable these authentication strategies in the Clerk Dashboard. Go to **User & Authentication > [Email, Phone, and Username](https://dashboard.clerk.com/last-active?path=user-authentication/email-phone-username)**. In the **Authentication factors** section of this page, enable the **Email verification code** and **SMS verification code** strategies.
