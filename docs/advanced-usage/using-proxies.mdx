...
Setting up CNAME records and using DNS is the simplest way. However, if you're unable to use the CNAME approach, or would like more control over your integration with Clerk, you can use a proxy.
...
For this example, we've chosen `/__clerk` as the path for our proxy. Your proxy server must be on the same domain as your application. 
  ...
  <Tab>
    <CodeBlockTabs options={["Handler", "Wrangler Config", "Environment", "Types"]}>
      ...
      ```env filename=".dev.vars"
        ...
        CLERK_PROXY_URL="http://localhost:8787/__clerk"
...
<Callout type="info">Every proxy configuration will be different and we're here to help. Please reach out to support@clerk.com if there's a specific use-case you're looking to solve.</Callout>
