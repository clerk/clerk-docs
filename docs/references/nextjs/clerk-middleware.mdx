...
| `secretKey?` | `string` | The Clerk secret key for your instance. This can be found in your Clerk Dashboard on the **[API Keys](https://dashboard.clerk.com/last-active?path=api-keys)** page. `CLERK_ENCRYPTION_KEY` must be defined when providing `secretKey` as an option, refer to [Server-side options propagation](#server-side-options-propagation). |

### Server-side options propagation

The `clerkMiddleware` options listed below are propagated to the Next.js application server, making them accessible for server-side helpers like [auth](/docs/references/nextjs/auth):
- `signUpUrl`
- `signInUrl`
- `secretKey`

To pass options securely from the middleware to the Next.js application server, the options are encrypted using the [AES encryption algorithm](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard). This algorithm requires a key to encrypt and decrypt a value. When providing a `secretKey` as a middleware option, the `CLERK_ENCRYPTION_KEY` environment variable is mandatory and used as the encryption key. If no `secretKey` is provided as a runtime option to `clerkMiddleware`, the encryption key defaults to `CLERK_SECRET_KEY`.

When providing `CLERK_ENCRYPTION_KEY`, a 128-bit, pseudorandom value should be used.

The server-side options passed to middleware are not accessible in client-side components. When passing custom values, make sure they are passed to both your middleware and [ClerkProvider](/docs/components/clerk-provider) or [environment variables](/docs/deployments/clerk-environment-variables).
