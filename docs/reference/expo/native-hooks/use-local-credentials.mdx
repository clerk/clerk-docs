...
> [!IMPORTANT]
> `useLocalCredentials()` stores and reuses password credentials after a local biometric check. [Biometric sign-in](/docs/guides/development/custom-flows/authentication/biometric-sign-in) instead registers a device-bound public key with Clerk and signs a one-time challenge. [`expo-local-authentication`](https://docs.expo.dev/versions/latest/sdk/local-authentication/) only performs the local check; by itself, it neither registers a biometric credential nor authenticates with Clerk.
...
<Properties>
  ...
  - `setCredentials()`
  - <code>(params: [LocalCredentials](#localcredentials)) => Promise\<void></code>
  ...
  - `authenticate()`
  - <code>() => Promise\<[SignInResource](/docs/reference/objects/sign-in)></code>
