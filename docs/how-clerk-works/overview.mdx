...
{/* TODO: Add tooltip for hashing and session */}
...
1. The server validates the credentials against a database. This is normally done by [hashing](TODO) the password and comparing it with a stored password hash. Upon successful validation, it creates a new [session](TODO) in the database associated with the user.
...
{/* TODO: comprehension check: talk about horizontal scaling and why jwt auth is easier to scale */}

<Details title="Why JWT auth is easier to scale">
  Lorem ipsum...
</Details>
...
1. The server responds to the browser's request by sending the client token in a [`Set-Cookie` header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Set-Cookie) in the response, which is set on the FAPI domain. Clerk's client-side SDK then makes a request to FAPI to get a session token and sets it on your app's domain.
   ...
   <Details
     title="Why doesn't Clerk set a session token using the set-cookie header when its setting the client token?"
   >
     Client cookie is set by a header in the servers response. The session cookie is not bc it cant be - bc the domain of a cookie is the domain of the server that set the cookie, and the server that sets the cookie for the client cookie is FAPI. And the client cookie is set on FAPI, thats fine. But it cant set a cookie with a domain value of your-app bc its not your-app, its a different app that we made that lives at a diff url, so it cant do that. So it sends back the value of the jwt (but not as a header!). And the client side api picks it up and uses javascript to set the session cookie. This guarantees that its not http-only (bc http-only cookies cannot be touched by javascript).
   </Details>
   <Video
...
{/* TODO: comprehension check: why does the handshake flow work? */}

<Details title="Why does the handshake flow work?">
  Lorem ipsum...
</Details>
