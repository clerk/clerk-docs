...
- **`script-src 'unsafe-eval'`** is a [requirement for Next.js](https://nextjs.org/docs/app/building-your-application/configuring/content-security-policy#without-nonces) to run **in development mode**. **For production environment** CSPs, it should be removed.
- **`script-src 'unsafe-inline'`** depends on your Next.js routing structure.
  - **App Router:** Required in **both dev and prod** environments, unless you are using the `strict-dynamic` approach.
  - **Pages Router:** Can be safely removed.
- **`style-src 'unsafe-inline'`**  is a requirement for Clerk's components to inject their styles. Removing this requirement is on on Clerk's roadmap. If you'd like to see this implemented sooner, [contact support](/contact/support){{ target: '_blank' }}.
