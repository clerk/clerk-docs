...
<Properties>
  ...
  Metadata that can be read from both the Frontend API and [Backend API](/docs/references/backend-api){{ target: '_blank' }}, but can be set only from the Backend API.
  ...
  Metadata that is only visible to your [Backend API](/docs/references/backend-api){{ target: '_blank' }}.
</Properties>
...
This method in the SDK is a wrapper around the BAPI endpoint `PATCH/organizations/{organization_id}/memberships/{user_id}/metadata`. See the [BAPI reference](/docs/references/backend-api/tag/Organization-Memberships#operation/UpdateOrganizationMembershipMetadata){{ target: '_blank' }} for more information.
