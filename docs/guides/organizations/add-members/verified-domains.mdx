...
description: Automatically invite or suggest users to join an Organization based on their email domain, using Clerk's Verified Domains feature.
metadata:
  title: Verified Domains within Clerk Organizations
---

Once a domain is [verified](/docs/guides/organizations/domain-verification) for an Organization, Clerk's **Verified Domains** feature can automatically invite or suggest users with that domain to join. For example, if the domain `@clerk.com` is verified, any user with an email address ending in `@clerk.com` can be automatically invited or be suggested to join an Organization with that domain. Clerk assigns users the [**Default** Role](/docs/guides/organizations/control-access/roles-and-permissions#the-default-role-for-members) set in the Organization settings page.
...
If you need precise control over specific people and their Roles, use [invitations](/docs/guides/organizations/add-members/invitations). If customers require authentication through their identity provider (IdP), use [Enterprise SSO](/docs/guides/organizations/add-members/sso).
...
Enrollment relies on [domain affiliation](/docs/guides/organizations/domain-verification#domain-affiliation) verification: Clerk sends a verification code to an email that matches the domain, and the user enters it. If the user adding the domain already has a verified email using that domain in their account, Clerk verifies the domain automatically. For the other verification methods and how they differ, see [Domain verification](/docs/guides/organizations/domain-verification).
...
<Cards>
  ...
  - Learn how to set up Enterprise SSO Connections for centralized authentication through an IdP.
