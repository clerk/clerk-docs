...
Users must complete both the sign-in and sign-up flows on the primary domain by using the [`<SignIn />`](/docs/reference/general/components/authentication/sign-in) component or [`useSignIn()`](/docs/reference/general/hooks/use-sign-in) hook for sign-in and [`<SignUp />`](/docs/reference/general/components/authentication/sign-up) component or [`useSignUp()`](/docs/reference/general/hooks/use-sign-up) hook for sign-up.
...
<Steps>
  ...
  <Tabs items={["Environment variables", "Properties"]}>
    ...
    <Tab>
      ...
      <Tabs items={["Next.js", "Remix"]}>
        <Tab>
          In a Next.js application, you must set the properties in the [`<ClerkProvider>`](/docs/reference/general/components/clerk-provider) component _and_ in your [`clerkMiddleware()`](/docs/references/nextjs/clerk-middleware).
  ...
  1. You now have an active session in the satellite domain, so you can see the [`<UserProfile />`](/docs/reference/general/components/user/user-profile) component and update your information.
