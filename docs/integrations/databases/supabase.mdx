...
<Steps>
  ...
  You can access Clerk session token data in Supabase using the built-in `auth.jwt()` function. Supabase uses this function to create custom RLS policies to restrict database access based on the requesting user.
