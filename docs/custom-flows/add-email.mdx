...
1. Uses the [`createEmailAddress()`](/docs/references/javascript/user/create-metadata#create-email-address) method on the `User` object to add the email address to the user's account.
  - A new [`EmailAddress`](/docs/references/javascript/email-address/email-address) object is created and stored in the `emailAddresses` property of the `User` object.
1. Calls the `prepareVerification()` method on the newly created `EmailAddress` object to send a verification code to the user.
1. Uses the `attemptVerification()` method on the `EmailAddress` object with the verification code provided by the user to verify the email address.
