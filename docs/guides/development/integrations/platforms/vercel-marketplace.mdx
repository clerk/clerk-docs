...
Connecting to a project will automatically sync your Clerk development API keys to your Vercel project's development environment variables. Configure preview and production environment variables in Vercel separately. The following development environment variables will be automatically synced:
...
When the connected Vercel project has a production `*.vercel.app` domain, Clerk can configure the production instance to use that domain. Make sure your Vercel production environment uses your production Clerk API keys (`pk_live_` and `sk_live_`). Production apps on `*.vercel.app` use Clerk's Vercel auto-proxy support at `https://<your-app>.vercel.app/__clerk`, so Next.js middleware or proxy matchers must include Clerk's proxy path, such as `/(api|trpc|__clerk)(.*)`.
...
For production apps using a `*.vercel.app` domain, core auth emails such as sign-in, sign-up, verification codes, and password reset codes are delivered through Clerk's shared `accounts.dev` email infrastructure. Custom email templates, email link authentication, application invitations, Organization invitations, email template customization, and the hosted Account Portal require adding a custom domain.
