...
The `$signInStore` store provides a convenient way to access the [`SignIn`](/docs/reference/objects/sign-in){{ target: '_blank' }} object, which allows you to check the current state of a sign-in. This is also useful for creating a custom sign-in flow.
...
The possible values for the `status` property of the `SignIn` resource are listed in the [`SignIn` object properties](/docs/reference/objects/sign-in#properties).
