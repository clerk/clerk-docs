...
Removes metadata fields from the user and Organization update endpoints. User metadata fields (`public_metadata`, `private_metadata`, `unsafe_metadata`) are removed from `PATCH /v1/users/{user_id}` and `unsafe_metadata` from `PATCH /v1/me`. Organization metadata fields (`public_metadata`, `private_metadata`) are removed from `PATCH /v1/organizations/{organization_id}`. Metadata must now be set through the dedicated metadata endpoints. See the [migration guide](/docs/guides/development/upgrading/upgrade-guides/2026-05-12) for more details.

The following SDKs are compatible with this version:

| SDK | Version |
| - | - |
| Next.js | @clerk/nextjs v7.5.2 or higher |
| React | @clerk/react v6.9.1 or higher |
| JavaScript | @clerk/clerk-js v6.16.1 or higher |
| Expo | @clerk/expo v3.4.2 or higher |
| TanStack React Start | @clerk/tanstack-react-start v1.4.2 or higher |
| React Router | @clerk/react-router v3.4.2 or higher |
| Express | @clerk/express v2.1.26 or higher |
| Astro | @clerk/astro v3.4.2 or higher |
| C# | Clerk.BackendAPI v2.0.0 or higher |
| Chrome Extension | @clerk/chrome-extension v3.1.35 or higher |
| Fastify | @clerk/fastify v3.1.36 or higher |
| Go | clerk-sdk-go v2.7.0 or higher |
| Java | com.clerk:backend-api v5.0.0 or higher |
| JavaScript Backend | @clerk/backend v3.7.0 or higher |
| Nuxt.js | @clerk/nuxt v2.6.2 or higher |
| PHP | clerkinc/backend-php v1.0.0 or higher |
| Python | clerk\_backend\_api v6.0.0 or higher |
| Ruby | clerk-sdk-ruby v7.0.0 or higher |
| Vue | @clerk/vue v2.4.2 or higher |
