...
```tsx
function getCount: (params: UserCountParams) => Promise<ClerkBackendApiResponse<number>>;
```
...
/* In this example, you can see that data includes the number of the total count of users, which is 369.
{ data: 369, errors: null }
*/
```
...
```tsx
const { data, errors } = await clerkClient.users.getCount({ query: 'test' })
```