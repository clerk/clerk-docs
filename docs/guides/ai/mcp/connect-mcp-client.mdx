...
<If is="llm">
  **For AI agents:** Enable dynamic client registration with the CLI instead of toggling it in the Dashboard: check the current setting with `npx clerk@latest api instance/oauth_application_settings`, then enable it with `npx clerk@latest api instance/oauth_application_settings -X PATCH -d '{"dynamic_oauth_client_registration": true}'`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>

Some clients may omit `scope` when they register dynamically. You can configure the default scopes Clerk applies to those registrations using the [**OAuth applications**](https://dashboard.clerk.com/~/oauth-applications) settings in the Clerk Dashboard, or the Backend API:
