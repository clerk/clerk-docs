...
- The [`<OrganizationSwitcher />` component](/docs/references/general/components/organization/organization-switcher) provides a way for your users to select which organization is active. The [`useOrganizationList()` hook](/docs/guides/development/custom-flows/organizations/organization-switcher) can be used for more control.
- The [`useOrganization()` hook](/docs/references/general/hooks/use-organization) can be used to fetch the current, active organization.
- The [`<Protect>` component](/docs/references/general/components/control/protect) enables you to limit who can view certain pages based on their role. Additionally, Clerk exposes a number of helper functions, such as [`auth()`](/docs/references/nextjs/app-router/auth), and hooks, such as [`useAuth()`](/docs/references/general/hooks/use-auth), to check the user's authorization throughout your app and API endpoints.
