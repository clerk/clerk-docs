...
Allowlist identifiers are case-insensitive. For example, allowing `JOHN.DOE@clerk.dev` has the same effect as blocking `john.doe@clerk.dev`, since identifiers are normalized to lowercase.
