...
> [!DEPRECATED]
> `createRouteMatcher()` is deprecated. If you're currently using it to protect routes, migrate to resource-based auth checks. See the [guide on migrating away from `createRouteMatcher()`](/docs/guides/development/upgrading/upgrade-guides/migrate-from-create-route-matcher) for more information.
...
## `createRouteMatcher()` <DeprecatedTag />

> [!DEPRECATED]
> `createRouteMatcher()` is deprecated and logs a runtime deprecation warning when called. To protect routes, migrate to resource-based auth checks (see the [migration guide](/docs/guides/development/upgrading/upgrade-guides/migrate-from-create-route-matcher)). For non-auth path logic, use the framework's native matching instead.
