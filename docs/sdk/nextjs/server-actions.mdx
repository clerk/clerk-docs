...
You can use the [`auth()`](/docs/sdk/nextjs/auth) helper to protect your server actions. This helper will return the current user's ID if they are signed in, or `null` if they are not.
...
Current user data is important for data enrichment. You can use the [`currentUser()`](/docs/sdk/nextjs/current-user) helper to fetch the current user's data in your server actions.
