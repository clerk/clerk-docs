...
<Steps>
  ...
  To handle form submissions for adding and deleting user messages, create two Server Actions in `app/actions.ts`. Use the [`auth()` helper](/docs/references/nextjs/auth) to obtain the user ID, which will be used to interact with the database.
  ...
  To retrieve the user's messages, use the [`auth()` helper](/docs/references/nextjs/auth) to obtain the user's ID. Then, use this ID to query the database for the user's messages.
