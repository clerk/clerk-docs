...
Make sure environment variables are loaded before importing any Clerk modules. With Node, run your app with `node --env-file=.env` so env vars are available during module initialization.Alternatively, you can load the file programmatically using `process.loadEnvFile()` before your app starts.
