...
<Steps>
  ...
  You can then accesss the authenticate state in any route by accessing the `auth` property on the request object.
  ...
  To protect your routes, use the [`requireAuth`](/docs/references/express/overview#require-auth) middleware. This middleware functions the same as `clerkMiddleware` except it redirects you to the configured sign in URL when unauthenticated.

  In this example, if the user is authenticated, it retrieves the `userId` from the `req.auth` object and uses it with `clerkClient.users.getUser()` to fetch the current user's `User` object. If the user is not authenticated, they are redirected to the '/sign-in' route.

  ```ts {{ filename: 'index.ts', mark: [3, [7, 8], [10, 13], [15, 17]] }}
  import 'dotenv/config'
  import express from 'express'
