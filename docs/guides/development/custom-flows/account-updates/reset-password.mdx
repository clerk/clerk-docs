...
description: Learn how to build a custom reset password flow using the Clerk API.
search:
  keywords:
    - reset password
---
...
This guide demonstrates how to build a custom user interface that allows users to reset their password once they're already signed in.
...
<If notSdk={["expo", "ios", "android"]}>
  ...
  ```tsx {{ filename: 'app/account/reset-password/page.tsx', collapsible: true }}
  'use client'
...
<If sdk="expo">
  ```tsx {{ filename: 'app/(account)/reset-password.tsx', collapsible: true }}
...
<If sdk="ios">
  ```swift {{ filename: 'ResetPasswordView.swift', collapsible: true }}
    import SwiftUI
    import ClerkKit
...
<If sdk="android">
  <CodeBlockTabs options={["ResetPasswordViewModel.kt", "ResetPasswordActivity.kt"]}>
    ```kotlin {{ filename: 'ResetPasswordViewModel.kt', collapsible: true }}
          ...
          viewModelScope.launch {
            ...
            user
              ...
              .onFailure {
                ...
                // for more info on error handling
                Log.e(
                  ResetPasswordViewModel::class.simpleName,
                  it.errorMessage,
                  it.throwable,
                )
    ...
    ```kotlin {{ filename: 'ResetPasswordActivity.kt', collapsible: true }}
      ...
      fun ResetPasswordScreen(
        ...
        Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
          when (state) {
            ResetPasswordViewModel.UiState.Loading -> CircularProgressIndicator()

            ResetPasswordViewModel.UiState.SignedOut -> {
              Text("You must be signed in to access this page")
            }

            ResetPasswordViewModel.UiState.PasswordUpdated -> {
              Text("Password updated!")
            }

            ResetPasswordViewModel.UiState.Form -> {
              Column(
