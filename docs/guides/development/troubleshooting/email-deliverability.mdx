...
During production instance set up, Clerk will have you set up the required records to configure [SPF](!spf) and [DKIM](!dkim) for security and deliverability. It's also recommended to set up [DMARC](!dmarc) ([instructions below](#setup-dmarc-email-authentication)) for additional security and deliverability.
