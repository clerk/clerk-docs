...
If you need to send a JWT along to a server, `getToken()` retrieves the current user's [session token](/docs/backend-requests/resources/session-tokens) or a [custom JWT template](/docs/guides/development/jwt-templates). See detailed examples in the [`Auth` object reference](/docs/references/backend/types/auth-object#get-token){{ target: '_blank' }}.
