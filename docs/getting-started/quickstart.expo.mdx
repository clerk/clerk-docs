...
<Tabs items={["JavaScript (Native sign-in optional)", "Native components"]}>
  ...
  <Tab>
    ...
    <Steps>
      ...
      > [!CAUTION]
      > If Google or Apple are enabled as social connections but the steps below are not completed, users will see the OAuth buttons in `<AuthView />` but get an error when they tap them.
      ...
      1. Add the Google Client IDs as environment variables in your `.env` file. Follow the `.env.example` in the [Sign in with Google](/docs/guides/configure/auth-strategies/sign-in-with-google) guide.
