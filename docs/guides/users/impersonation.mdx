...
When creating actor tokens, the object that you pass as the `actor` parameter will end up in the authentication token's `act` claim. You can read more details in the [guide on session tokens](/docs/guides/how-clerk-works/session-tokens#default-claims).
...
Clerk also adds an `act` claim on the [Clerk session token](/docs/guides/how-clerk-works/session-tokens), which contains information like the impersonated session ID and the actor/impersonator's user ID.
...
<Tabs items={["React-based SDKs", "JavaScript"]}>
  <Tab>
    You can use the [`useAuth()`](/docs/references/general/hooks/use-auth) hook to get access to the authentication context, which includes the `actor` object.
