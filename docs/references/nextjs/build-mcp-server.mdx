...
<Steps>
  ...
  Add the following code to your `route.ts` file. This uses Vercel's `withMcpAuth()` function to wrap the MCP handler in authentication logic and uses Clerk's [`auth()`](/docs/references/nextjs/auth) helper to parse the incoming OAuth token and extract the session context. This token is then passed Clerk's `verifyClerkToken` method, which verifies it and extracts key metadata such as `scopes`, `clientId`, and `userId`. To learn more about verifying OAuth tokens in Next.js apps, see the [dedicated guide](/docs/references/nextjs/verifying-oauth-access-tokens).
