...
To authenticate requests when using a Clerk SDK, you must pass Clerk's short-lived [session token](/docs/guides/how-clerk-works/session-tokens) to your server. The session token contains cryptographically signed claims about the user's identity and authentication state. [Read more about making requests](/docs/backend-requests/making-requests).
