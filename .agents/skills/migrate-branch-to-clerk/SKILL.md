...
`--clerk-path` points at a `clerk` checkout — a separate repository from the `clerk-docs` checkout (or worktree) you run the command in — and the script uses that checkout as its workspace: it checks out the migration branch there (`git checkout --no-track -b …` in create mode, `-B …` in update mode), cherry-picks the branch's commits, commits into it, and runs `pnpm install` in its `clerk-docs/` — leaving the checkout on the migration branch (and, on a Tier 3 conflict, with a conflicted working tree and the filter-repo remote still configured — a Tier 2 conflict is cleaned up on the clerk side and resolved in `clerk-docs` instead). Its branch and working tree change, so point `--clerk-path` at a dedicated `clerk` checkout or worktree whenever your main checkout's state must be preserved. Omit the flag entirely and the script clones `clerk` into a temp directory instead — slower, but it leaves your local `clerk` checkouts alone. It still uses the `clerk-docs` checkout you run from either way: by default it merges `origin/main` into your current `clerk-docs` branch (skip with `--no-merge-main`), and a Tier 2 conflict commits to that branch and leaves conflict markers in its working tree (see the conflict tiers below). (The temp filter-repo duplicate is a copy of `clerk-docs`, not the `clerk` checkout — a separate thing.)

## Triaging an external (fork) PR — accept, refine, or decline

External contributions arrive as fork PRs on the clerk-docs mirror — the reliable signal is the PR head repo being a fork, not `clerk/clerk-docs` itself. A bot comment and the `external-contribution` label usually flag them too, but treat those as hints, not the test: a first-time contributor's PR can sit in `action_required` before that automation runs, so the fork head is what to go on. Acceptance is a maintainer call — work it in order: **validate → sweep → decide (as-is / refine / decline) → migrate.** The script below only handles the last step; the first three — validate, sweep, and decide — are on you, and none is optional.

**Validate first — confirm the change is correct.** A docs PR can read plausibly, pass `build:tsx`/`lint`, and still be factually wrong — an accept you'll regret. Before accepting, verify every claim about external behavior against its source of truth, not the PR's own description and not memory: SDK method names, types, and signatures → `clerk/javascript`; API behavior, endpoints, versions → `clerk/clerk_go`; how docs pages or the API reference render → `clerk/clerk` (see clerk-docs `AGENTS.md` → "Verifying technical claims"). If a load-bearing claim is wrong, or you can't check one, that's a decline or a request for changes — not an accept.

**Sweep for what the contributor missed.** A contributor fixes the one page they hit; the same bug — or a related inaccuracy in the same files — often sits on sibling pages too. Once the fix validates, look for that class of issue across the docs (a scoped grep plus a read of the neighbors covers most of it; a subagent is worth it when the surface is large). What you find decides as-is vs refine: fold the extra fixes onto the branch before migrating (see **Accept with refinements**), or, if the sweep comes back clean, record that in the migrated PR so the reviewer needn't re-check.
...
**Decline.** Don't run the migration. Be kind, thank them, and state the reason factually in as few words as possible — then close the PR. Unlike the accept path's notice comment (script-generated, uniform by construction), a decline reply is prose you write, and it posts under the maintainer's GitHub account — so draft it in the maintainer's own writing voice and check it reads as them before posting, the same care you'd give any message that goes out under their name. Template:
