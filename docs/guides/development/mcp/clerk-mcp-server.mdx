...
<Tabs items={["Cursor", "Claude", "VSCode", "Windsurf", "Zed", "Codex", "Other clients"]}>
  <Tab>
    ...
    To add Clerk's MCP server to Cursor, navigate to the [Tools & MCP](cursor://anysphere.cursor-deeplink/mcp/install?name=Clerk\&config=eyJ1cmwiOiJodHRwczovL21jcC5jbGVyay5jb20vbWNwIn0=){{ target: '_blank' }} page in Cursor Settings, and select **Install** in the window presented to you.
  ...
  <Tab>
    ...
    <Tabs items={["Configuration via CLI", "Configuration through the config file"]}>
      <Tab>
        ...
        > [!NOTE]
        > If this is the first time you are using an MCP in Codex, you will need to enable the `rmcp` feature. To do so, add the following into your `~/.codex/config.toml` file:
        >
        > ```toml
        > [beta]
        > rmcp = true
...
Use Clerk's MCP server when building with Clerk and you want fast, accurate implementation guidance, with code examples for Clerk SDK features. It's ideal for getting code snippets, usage patterns, and best practices directly in your AI coding assistant.
