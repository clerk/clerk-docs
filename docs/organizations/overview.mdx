...
> [!IMPORTANT]
> Applications created after \[RELEASE DATE]
...
With Clerk Organizations, there are two types of workspaces:

- **Personal workspace**: A personal workspace is a user's unique, individual space, and is not tied to any organization.
- **Organization workspace**: An organization workspace is owned and managed by an organization, which can have multiple members. The organization workspace that a user is currently viewing is called the [active organization](#active-organization).

Most multi-tenant applications want all users to be part of an organization rather than operating in individual personal workspaces. Therefore, **by default, personal workspaces are disabled** for applications using organizations. This means that after signing up, users must create or join an organization, rather than having access to a personal workspace.

To enable personal workspaces for your application, toggle **Allow personal workspaces** in the [**Organizations Settings**](https://dashboard.clerk.com/last-active?path=organizations-settings) page.

> [!IMPORTANT]
> Personal workspaces being disabled by default was released on \[RELEASE DATE]. Applications created before this date will not be able to see the **Allow personal workspaces** setting, because personal workspaces were enabled by default.

{/* TODO: commenting this out for now as I don't think we need this. We can definitely add something about a custom flow though, when those are ready.
When personal workspaces are disabled, you'll need to implement logic to handle users who don't have an active organization. For detailed implementation guidance including middleware setup, custom flows, and handling pending sessions, see the [force organizations guide](/docs/organizations/force-organizations). */}
...
When personal workspaces are enabled, users initially sign in to their personal workspace with no active organization set. When personal workspaces are disabled (the default setting), users must select or create an organization to access the application. Even if a user is a member of only one organization, they must explicitly set it as active or \[the application can have logic to set this automatically]\(TODO: link to force organizations guide? does doing this overrule the after-auth flow?).
...
{/* For comprehensive implementation details on organization-first flows, including middleware setup, custom redirects, and handling users without active organizations, see the [force organizations guide](/docs/organizations/force-organizations). */}
