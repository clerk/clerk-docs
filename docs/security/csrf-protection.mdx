...
- The cookie *will* be sent when `bar.example.com` initiates a request to `foo.example.com`
- The cookie *will not* be sent when `bar.example.org` initiates a request to `foo.example.com`

*Lax* alleviates this restriction slightly and allows for the cookie to be sent when the user is navigating from a third party site. In the example above, if the user is on `bar.example.org` and clicks a link to `foo.example.com`, then the initial request to load `foo.example.com` will include the cookie.
