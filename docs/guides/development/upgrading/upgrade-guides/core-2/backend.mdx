...
description: "Learn how to upgrade the @clerk/backend package to the latest version."
---
...
>
  <AccordionPanel>
    The `logoUrl` property of any [`Organization` object](/docs/reference/objects/organization) has been changed to `imageUrl`.
  </AccordionPanel>
  ...
  <AccordionPanel>
    The `picture` property of any [`ExternalAccount` object](/docs/reference/types/external-account) has been changed to `imageUrl`.
  </AccordionPanel>
...
>
  <AccordionPanel>
    If you are updating a user's password via the [`User.update` method](/docs/reference/objects/user#update), it must be changed to [`User.updatePassword`](/docs/reference/objects/user#update-password) instead. This method will require the current password as well as the desired new password. We made this update to improve the security of password changes. Example below:
