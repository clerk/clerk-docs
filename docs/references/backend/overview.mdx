...
You will need your `CLERK_SECRET_KEY` to initialize the SDK, which you can find in the [API Keys](https://dashboard.clerk.com/last-active?path=api-keys) section in the Clerk Dashboard.

This example shows how to access the `users` sub-api from the Backend SDK. The [`getUser`](/docs/references/backend/user/get-user) method is then used to retrieve a user by their user ID.
