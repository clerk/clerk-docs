...
> [!TIP]
> Using [native components](/docs/reference/expo/native-components/overview)? Google Sign-In is built-in to [`<AuthView />`](/docs/reference/expo/native-components/auth-view) automatically when Google is enabled as a social connection in your Clerk Dashboard. This guide is for apps using custom sign-in UI with the [`useSignInWithGoogle()`](/docs/reference/expo/native-hooks/use-sign-in-with-google) hook.
