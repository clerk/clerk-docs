<If sdk="astro">
  > [!WARNING]
  > Astro SDK hasn't been updated to support "plan" in the `has()` function yet.
...
<If sdk="expo">
  ...
  export default function BronzeContentPage() {
    ...
    if (!hasBronzePlan) return <Text>Only subscribers to the Bronze plan can access this content.</Text>
