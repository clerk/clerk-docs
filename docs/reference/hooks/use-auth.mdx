...
<If sdk="astro">
  ```tsx {{ filename: 'src/components/external-data-page.jsx' }}
  ...
  ---
  import ExternalDataPage from '../components/external-data-page'
  ---

  <ExternalDataPage client:load />
  ```
</If>
...
<If sdk="expo">
  ```tsx {{ filename: 'app/(auth)/external-data.tsx' }}
