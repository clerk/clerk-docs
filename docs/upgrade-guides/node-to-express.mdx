...
To enforce strict authentication, replace your existing Node SDK strict middleware, [`ClerkExpressRequireAuth`](/docs/backend-requests/session-requests/handling/nodejs), with the new Express SDK middleware, [`requireAuth()`](/docs/backend-requests/session-requests/handling/nodejs#require-auth).
