...
Metadata is stored on the [`Organization`](/docs/reference/objects/organization) and [`OrganizationMembership`](/docs/reference/types/organization-membership) objects.
...
Both the [`Organization`](/docs/reference/objects/organization) and [`OrganizationMembership`](/docs/reference/types/organization-membership) objects have the metadata fields: `publicMetadata` and `privateMetadata`.
...
You can set Organization metadata in the [Clerk Dashboard](https://dashboard.clerk.com/~/organizations) or using Clerk's Backend API. See the [`updateOrganizationMetadata()`](/docs/reference/backend/organization/update-organization-metadata) and [`updateOrganizationMembershipMetadata()`](/docs/reference/backend/organization/update-organization-membership-metadata) methods for more information. If you need to fully replace Organization metadata rather than deep-merging, use [`replaceOrganizationMetadata()`](/docs/reference/backend/organization/replace-organization-metadata).

## Access public metadata

To access public metadata on the frontend, it's available on the [`Organization`](/docs/reference/objects/organization) object, which can be accessed using the [`useOrganization()`](/docs/reference/hooks/use-organization) hook.
