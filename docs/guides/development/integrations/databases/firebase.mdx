...
<Steps>
  ...
  1. Toggle the **Firebase** integration on. The configuration modal will open. Keep it open while you configure your Firebase project.
  ...
  <Tabs items={["Configure automatically (Recommended)", "Configure manually"]}>
    <Tab>
      ...
      1. Near the bottom of the page, select **Generate new private key**.
      1. In the modal that pops up, select **Generate key** to download the JSON file that contains your service account key.
      1. In the Clerk Dashboard, the Firebase configuration modal should still be open. Select **Upload service account key** and upload the JSON file you downloaded.
