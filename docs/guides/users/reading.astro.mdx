...
To retrieve a token from a [JWT template](/docs/guides/sessions/jwt-templates) and fetch data from an external source, use the [`getToken()`](/docs/reference/backend/types/auth-object#gettoken) method from the `auth()` local.
