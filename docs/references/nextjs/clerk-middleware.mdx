...
The `organizationSyncOptions` property on the <code>[clerkMiddleware()](#clerk-middleware-options)</code> options
...
<code>[auth()](/docs/references/nextjs/auth)</code>) and the organization as indicated by the URL is detected, an attempt
