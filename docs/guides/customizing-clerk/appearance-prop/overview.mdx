...
The `appearance` prop can be applied to [`<ClerkProvider>`](/docs/references/components/clerk-provider) to share styles across every component, or individually to any of the Clerk components.

This applies to all of the React-based packages, like [Next.js](/docs/nextjs/getting-started/quickstart), as well as [the pure JavaScript ClerkJS package](/docs/references/javascript/overview).
...
The `layout` property is used to adjust the layout of the [`<SignIn/>`](/docs/references/components/authentication/sign-in) and [`<SignUp/>`](/docs/references/components/authentication/sign-up) components, as well as set important links to your support, terms, and privacy pages. See the [Layout](/docs/guides/customizing-clerk/appearance-prop/layout) docs for more information.
