...
You can use the `isAuthenticated` property from the [`auth()`](/docs/references/nextjs/auth) helper to protect your Server Actions. If `isAuthenticated` is `false`, then the user is not signed in.
...
For more examples on how to use the `auth()` helper, see the [`auth()`](/docs/references/nextjs/auth) reference.
