The following example demonstrates how to use the [`$userStore`](/docs/reference/astro/client-side-helpers/user-store) to access the [`User`](/docs/reference/objects/user) object, which contains the current user's data such as their ID. It returns `undefined` while Clerk is still loading and `null` if the user is not signed in.
