---
title: Build a custom sign-in flow with Device Trust
description: Learn how to build a custom sign-in flow that requires Device Trust.
---
...
If you have [Device Trust](/docs/guides/secure/device-trust) enabled for your application, when a user is signing in **with a password** on a new device, **the sign-in attempt will return a status of `needs_client_trust`**. Your custom sign-in flow needs to support handling an email code, SMS code, or email link, depending on the settings you've enabled in the Clerk Dashboard. This guide demonstrates the email code and SMS code strategies.
...
If Device Trust **and** MFA are enabled, MFA will take precedence and the sign-in attempt will return a status of `needs_second_factor`. See the [MFA custom flow guide](/docs/guides/development/custom-flows/authentication/multi-factor-authentication) for more information.
...
<Tabs items={["Email code", "SMS code"]}>
  ...
  <Tab>
    ...
    1. Select the **Password** tab and enable **Sign-up with password**. **Device Trust** is enabled by default (applications created before November 14, 2025 will need to [enable Device Trust](/docs/guides/secure/device-trust#enable-device-trust)).
    ...
    <If sdk="android">
      <CodeBlockTabs options={["PhonePasswordSignInViewModel.kt", "PhonePasswordSignInActivity.kt"]}>
        ...
        +        _uiState.value == PhonePasswordSignInUiState.NeedsDeviceTrust ->
        +          PhonePasswordSignInUiState.NeedsDeviceTrust
        ...
        +              .onSuccess { _uiState.value = PhonePasswordSignInUiState.NeedsDeviceTrust }
        +              .onFailure {
        +                // See https://clerk.com/docs/guides/development/custom-flows/error-handling
        +                // for more info on error handling
        +              }
        ...
        data object NeedsDeviceTrust : PhonePasswordSignInUiState
        ...
        +  PhonePasswordSignInViewModel.PhonePasswordSignInUiState.NeedsDeviceTrust -> {
        +    Column(
        +      verticalArrangement = Arrangement.spacedBy(16.dp, Alignment.CenterVertically),
        +      horizontalAlignment = Alignment.CenterHorizontally,
        +    ) {
