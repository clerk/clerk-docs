...
Create a `middleware|proxy.ts` file at the root of your project, or in your `src/` directory if you have one.
...
<CodeBlockTabs options={["auth.protect()", "auth().isAuthenticated()"]}>
  ...
  ```tsx {{ filename: 'app/middleware|proxy.ts' }}
  import { clerkMiddleware, createRouteMatcher } from '@clerk/nextjs/server'
...
```tsx {{ filename: 'middleware|proxy.ts', mark: [[4, 7]] }}
import { clerkMiddleware } from '@clerk/nextjs/server'
...
```js {{ filename: 'middleware|proxy.ts' }}
import { clerkMiddleware, createRouteMatcher } from '@clerk/nextjs/server'
