---
title: Handle invitations
description: Learn how to use Clerk's API to build a custom flows for handling application and organization invitations.
---
...
When a user visits an [invitation](/docs/users/invitations) link, and no custom redirect URL was specified, then they will be redirected to Clerk's [Account Portal sign-up page](/docs/customization/account-portal/hosted-pages#sign-up) and **their email address will be automatically verified.**

{/* TODO: What happens if the user is already signed up? */}

However, if you specified [a redirect URL when creating the invitation](/docs/users/invitations#redirect-url), you must handle the sign-up flow in your code for that page.

This guide will demonstrate how to build a custom sign-up flow to handle application invitations.

## Create the sign-up flow

Once the user visits the invitation link and is redirected to the specified URL, the URL will contain the query parameter `__clerk_ticket`, which contains the invitation token.

To create a sign-up flow using the invitation token, you need to extract the token from the URL and pass it to the [`signUp.create`](/docs/references/javascript/sign-up/sign-up#create) method.
