    ...
    <Tab>
      ...
      <If sdk="ios">
        ...
        struct ManageSMSBasedMFAView: View {
          ...
          var body: some View {
            VStack {
              ForEach(clerk.user?.phoneNumbers ?? []) { phone in
                Button(phone.phoneNumber) {
                  Task { await reservePhoneForMfa(phone: phone) }
                }
              }
            }
          }
        ...
        extension ManageSMSBasedMFAView {
          ...
          func reservePhoneForMfa(phone: PhoneNumber) async {
            do {
              // Reserve the phone as a second factor for SMS MFA.
              try await phone.setReservedForSecondFactor(reserved: true)
            } catch {
              // See https://clerk.com/docs/guides/development/custom-flows/error-handling
              // for more info on error handling
              dump(error)
