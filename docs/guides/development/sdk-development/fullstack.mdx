...
- User only needs to provide their [Publishable Key](!publishable-key) and [Secret Key](!secret-key)
...
- User should be able to enforce authentication on individual routes (e.g., with a [`requireAuth`](/docs/guides/development/sdk-development/backend-only#create-a-requireauth-helper) helper)
