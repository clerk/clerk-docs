...
> [!WARNING]
> The Google Analytics (GA) integration is no longer supported. All new applications **cannot** enable the integration. Existing applications with the integration previously enabled will continue to function, but once disabled, it cannot be re-enabled.
> As a workaround, you can implement your own analytics tracking by observing user state changes via Clerk's `useUser` hook and firing events to your analytics provider.
