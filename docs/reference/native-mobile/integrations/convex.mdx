...
<Steps>
  ...
  <Include src="_partials/convex-configure-auth" />
  ...
  <If sdk="android">
    ...
    Initialize Clerk first with your Clerk [Publishable Key](!publishable-key), then create a `ConvexClientWithAuth` using `ClerkConvexAuthProvider().createConvexClientWithAuth()`, passing in your Convex deployment URL. Refer to the [Convex deployment docs](https://docs.convex.dev/dashboard/deployments/deployment-settings) for more info.
  ...
  <Include src="_partials/convex-deploy-production" />
</Steps>
