...
Authenticates the user using a Google token from Google Identity Services (e.g. Google One Tap). Returns the resulting [`SignIn`](/docs/reference/objects/sign-in) or [`SignUp`](/docs/reference/objects/sign-up) resource; use [`handleGoogleOneTapCallback()`](#handlegoogleonetapcallback) to complete the flow after redirect.
...
Completes a Google One Tap redirect flow started by [`authenticateWithGoogleOneTap()`](#authenticatewithgoogleonetap). Call this when the user returns from the Google One Tap redirect.
