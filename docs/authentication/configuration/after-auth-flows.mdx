...
Each after-auth flow is mapped to a unique [`SessionTask['key']`](/docs/references/javascript/types/session-task) that identifies the specific task. You can use these task keys to conditionally handle different after-auth requirements in your application logic.

The following table lists the available after-auth flows and their corresponding task keys.

| Name | Key | Description |
| - | - | - |
| [Personal accounts disabled (default)](/docs/organizations/overview#allow-personal-accounts) | `choose-organization` | Requires users to choose an organization after authenticating. |
...
Once enabled in the Clerk Dashboard, after-auth flows are **embedded by default** within the `<SignUp />` and `<SignIn />` components. For more customization, you can opt out of using the `<SignUp />` and `<SignIn />` components and [create custom pages for your after-auth flows](/docs/custom-flows/overview#after-auth-flow).

The following table lists the available after-auth flows and their corresponding after-auth components. See the linked reference guide for usage instructions.
...
If the prebuilt components don't meet your specific needs or if you require more control over the logic, you can also build your own UI using the `Session.currentTask` property to check if the user has pending after-auth tasks. Refer to the [custom flows documentation](/docs/custom-flows/overview) for specific implementation guides.
...
<CodeBlockTabs options={["React", "JavaScript"]}>
  ...
  if (session?.currentTask) {
    ...
    // to complete after-auth flow. You can display custom UI to help
    // users resolve these tasks, see https://clerk.com/docs/custom-flows/overview#after-auth-flow
...
### Redirecting to after-auth tasks

When users have pending after-auth tasks, you can redirect them to specific pages or components to complete these requirements. This is useful for creating custom user experiences or when you want to handle after-auth flows outside of the default `<SignIn />` and `<SignUp />` components.

#### Using the `redirectTo` prop

You can redirect users to a custom page after authentication by using the `redirectTo` prop on the `<SignIn />` and `<SignUp />` components:
...
export default function AuthPage() {
  return (
    <div>
      <SignIn redirectUrl="/complete-profile" />
      <SignUp redirectUrl="/complete-profile" />
    </div>
  )
}
```

#### Custom redirect logic with `useSession`

For more control, you can use the `useSession` hook to check for pending tasks and redirect users programmatically:
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (isLoaded && session?.currentTask) {
      // Redirect based on the specific task type
      switch (session.currentTask.type) {
...
You can also handle redirects at the middleware level by checking for pending sessions:
...
export default clerkMiddleware(async (auth, req) => {
  if (isProtectedRoute(req)) {
    ...
    if (session?.currentTask) {
      // Redirect to appropriate completion page
      switch (session.currentTask.type) {
        case 'organization_required':
...
For API routes, you can return redirect responses when users have pending tasks:
    ...
    return Response.json(
      {
        ...
        task: session.currentTask.type,
      },
      { status: 200 },
    )
  }

  // User is fully authenticated, proceed with API logic
...
- **Always check for pending tasks** before allowing access to protected content
- **Provide clear user feedback** about what needs to be completed
- **Use consistent redirect URLs** across your application
- **Handle edge cases** like users navigating back to protected pages
- **Consider user experience** - don't redirect too aggressively or create redirect loops
