...
Subscription plans are what your customers subscribe to. There is no limit to the number of plans you can create. If your Clerk instance has existing custom permissions, these permissions will automatically be added to the free plan for orgs. This ensures that organization members get the same set of permissions when billing is enabled, because all organizations start on the free plan.
