...
After your Clerk application has been created, in the Clerk Dashboard, navigate to the **[API keys](https://dashboard.clerk.com/last-active?path=api-keys)** page, click **Show API URLs**, and copy your **Frontend API URL**.

![The API URLs modal from the API keys page of the Clerk Dashboard. There is a red rectangle around the 'Frontend API URL'.](/docs/images/integrations/fauna/frontend-api-url.webp)
...
Paste the **Frontend API URL** you copied from the Clerk Dashboard into the **Issuer** field.
...
You can specify multiple `role` properties if needed. Don’t change the `issuer` or `jwks_uri` fields values.
