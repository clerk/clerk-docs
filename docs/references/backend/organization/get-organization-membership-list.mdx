...
This method in the SDK is a wrapper around the BAPI endpoint `GET/organizations/{organization_id}/memberships`. See the [BAPI reference](/docs/references/backend-api/tag/Organization-Memberships#operation/ListOrganizationMemberships){{ target: '_blank' }} for more information.
