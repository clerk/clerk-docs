...
1. Users who are already signed in shouldn't be sent through a sign-in or sign-up flow. Create a route group with a layout that redirects users if they're already signed in, as shown in the following example:
    > The example on this page must be created in the same route group where this layout is defined.
