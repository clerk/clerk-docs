...
The Hasura template will pre-populate the default claims required by Hasura. You can include additional claims as necessary. [Shortcodes](/docs/guides/sessions/jwt-templates#shortcodes) are available to make adding dynamic user values easy.
...
You can find the **JWKS URL** on the [**API keys**](https://dashboard.clerk.com/~/api-keys) page in the Clerk Dashboard.
...
Replace `<YOUR_FRONTEND_API>` with the Frontend API value. This value can be found on the [**API keys**](https://dashboard.clerk.com/~/api-keys) page in the Clerk Dashboard.
...
// This component needs to be a child of <ClerkProvider> and <Show when="signed-in">
