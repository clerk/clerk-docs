...
For example, an organization member with role `org:admin` has custom permission `org:teams:manage` if the plan that organization is subscribed to has the `teams` feature **and** if that permission is assigned to the `org:admin` role.

If your Clerk instance has existing custom organization permissions, these permissions will automatically be added to the free plan for orgs.
