...
<Steps>
  ...
  - [`@clerk/backend`](/docs/references/backend/overview): Clerk's JavaScript Backend SDK provides a `MachineAuthObject` that can be used to authenticate machine requests.
  ...
  1. Inside this file, add the following imports:
  ...
  The `createMcpHandler()` function is used to create a new MCP server. Inside it, you can define tools that external LLM-based apps can invoke. Each tool includes:
  ...
  1. The `withMcpAuth()` function is used to wrap the MCP handler in authentication logic. It uses Clerk's [`auth()`](/docs/references/nextjs/auth) helper to verify the incoming OAuth token [in accordance with the mcp specification](https://modelcontextprotocol.io/specification/2025-06-18/basic/authorization#authorization-server-location). [Learn more about verifying OAuth tokens in Next.js apps](/docs/references/nextjs/verifying-oauth-access-tokens).
  ...
  To comply with the MCP specification, your server must expose two `well-known` metadata endpoints: `.well-known/oauth-protected-resource` and \`.well-known/oauth-authorization-server. These metadata endpoints allow clients to discover where to authenticate and some details about how the authentication service works. [OAuth protected resource metadata](https://datatracker.ietf.org/doc/html/rfc9728#section-4.1).

  Clerk provides prebuilt helpers that handle this for you:

  - `authServerMetadataHandlerClerk`: Next.js route handler that serves OAuth 2.0 authorization server metadata in the format expected by MCP clients.
  - `protectedResourceHandlerClerk`: Next.js route handler that serves OAuth protected resource metadata. This handler automatically uses your `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` environment variable.

  {/* TODO: Ensure your have it in your .env file */}
  ...
  1. Inside it, create two subfolders:
     - `oauth-authorization-server`
     - `oauth-protected-resource`.
  1. In each folder, create a `route.ts` file.
  1. Add the following code in each corresponding file:
