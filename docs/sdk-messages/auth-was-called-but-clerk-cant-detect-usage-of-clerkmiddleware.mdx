...
The most common cause of this error is when [`auth()`](/docs/references/nextjs/auth) is called **without** a [`<ClerkProvider>`](/docs/references/nextjs/clerk-provider) wrapping the component tree where Clerk components are used. This is a common pattern with React context and also used by many libraries.
