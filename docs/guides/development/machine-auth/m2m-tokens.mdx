...
> [!IMPORTANT]
> Loading environment variables is different depending on the framework you are using. For example, Next.js requires the `NEXT_PUBLIC_` prefix for environment variables that are available on the frontend. If you're running a Node server, you can load `.env` files by starting your app with the `--env-file=.env` flag or by calling `process.loadEnvFile()` at the top of your entry file. Check your framework's documentation for information on how to load environment variables.
