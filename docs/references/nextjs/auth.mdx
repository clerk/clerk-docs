...
`auth` returns the [`Authentication`][auth-object] object with a few extra properties

## Properties

| Name | Type | Description |
| --- | --- | --- |
| `protect` | Function | Accepts a permission or a role key or a function with `has` as the first parameter. It will throw a `notFound` Nextjs error if the user is unauthorized, otherwise it returns a the Authentication object |
