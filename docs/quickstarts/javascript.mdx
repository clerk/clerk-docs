...
<Tabs items={["NPM module", "<script>"]}>
  ...
  <Tab>
    <Steps>
      ...
      It's recommended to use environment variables to store your Clerk **Publishable Key**. A common way to use environment variables in a JavaScript project is by putting the values in a `.env` file and then loading them into your application using a package like [`dotenv`](https://www.npmjs.com/package/dotenv). However, wite Vite, environment variables are exposed through the [`import.meta.env` object](https://vitejs.dev/guide/env-and-mode.html#env-variables).
