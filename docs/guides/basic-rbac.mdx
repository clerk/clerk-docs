...
<Steps>
  ...
  1. Apply `clerkMiddleware` to intercept requests to the `/admin` route, and use the `checkRole()` function to verify that the user has the `admin` role. If the user does not have the `admin` role, redirect them to the home page.
  ...
  With the server action set up, now build the `<SearchUsers />` component. This component includes a form for searching users, and when submitted, appends the search term to the URL as a search parameter. The page component, which you'll refactor next as a server component, will then perform a query based on the updated URL.
  ...
  With the server action and the search form set up, it's time to refactor the page for the `app/admin` route to be a server component. It will check whether a search parameter has been appended to the URL by the search form. If a search parameter is present, it will query for users matching the entered term.
