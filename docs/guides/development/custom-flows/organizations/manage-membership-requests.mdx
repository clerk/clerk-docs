...
<If sdk="ios">
  ...
  struct ManageMembershipRequestsView: View {
    ...
    var body: some View {
      VStack {
        Button("Load membership requests") {
          Task { await fetchMembershipRequests() }
        }

        ForEach(Array(membershipRequests.enumerated()), id: \.offset) { index, request in
          HStack {
  ...
  extension ManageMembershipRequestsView {
    ...
    func fetchMembershipRequests() async {
      ...
      do {
        guard
          ...
          let organization = memberships.first(where: { $0.organization.id == activeOrganizationId })?.organization
        else {
          membershipRequests = []
          return
        }
    ...
    func rejectMembershipRequest(_ request: OrganizationMembershipRequest) async {
      do {
        ...
        membershipRequests.removeAll(where: { $0.id == request.id })
      } catch {
        // See https://clerk.com/docs/guides/development/custom-flows/error-handling
        // for more info on error handling
        dump(error)
