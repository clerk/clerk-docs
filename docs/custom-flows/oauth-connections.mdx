...
<Tabs items={["Next.js", "Expo", "iOS (beta)"]}>
  ...
  <Tab>
    ...
    1. Calls the `startSSOFlow()` method with the `strategy` param set to `oauth_google`, but you can use any of the [supported OAuth strategies](/docs/references/javascript/types/sso#oauth-strategy). A `redirect_url` param is also set to redirect the user once they finish the authentication flow.
