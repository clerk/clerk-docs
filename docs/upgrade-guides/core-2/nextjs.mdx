...
Want to get more specific with permission gating? You can add an organization's role as a param to `protect()` to ensure that only users with a specific role can access the page or layout. Here's an example of how you can protect `/admin` routes to only users with the `org:admin` role:
