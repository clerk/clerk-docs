...
- The [`<Protect>` component](/docs/reference/components/protect) enables you to limit who can view certain pages based on their role. Additionally, Clerk exposes a number of helper functions, such as [`auth()`](/docs/references/nextjs/auth), and hooks, such as [`useAuth()`](/docs/reference/hooks/use-auth), to check the user's authorization throughout your app and API endpoints.
