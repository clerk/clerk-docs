...
> [!WARNING]
> This guide is for users who want to build a _custom_ user interface using the Clerk API. To authenticate users with one-time passwords using a _prebuilt_ UI, you should use Clerk's [Account Portal pages](/docs/customization/account-portal//overview) or [prebuilt components](/docs/components/overview).
