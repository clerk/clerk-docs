...
[`clerkClient`](/docs/reference/backend/overview) is a wrapper around the [Backend API](/docs/reference/backend-api){{ target: '_blank' }} that makes it easier to interact with the API. For example, to update a user's first and last name, you can use the [`users.updateUser()`](/docs/reference/backend/user/update-user) method.
