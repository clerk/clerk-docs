...
- If you are using organizations combined with billing, custom permissions will only appear in the session token (JWT) and in API responses (including the result of the `has()` check) if the feature part of the permission key (`org:<feature>:<action>`) **is a feature included in the organization's active plan**. If the feature is not part of the plan, the `has()` check for permissions using that feature will return `false`, and those permissions will not be represented in the session token. For example, an organization member with role `org:admin` has custom permission `org:teams:manage` if the plan that organization is subscribed to has the `teams` feature **and** if the `org:teams:manage` permission is assigned to the `org:admin` role.
  - If your Clerk instance has existing custom organization permissions, these permissions will automatically be added to the free plan for orgs. This ensures that organization members get the same set of permissions when billing is enabled, because all organizations start on the free plan.
