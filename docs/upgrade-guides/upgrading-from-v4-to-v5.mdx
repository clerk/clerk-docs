...
<Tabs type="v4-to-v5-sdk" items={["Next.js", "React", "Remix"]}>
  <Tab>
    <CodeBlockTabs type="installer" options={["npm", "yarn", "pnpm"]}>
      ```bash filename="terminal"
...
You can use your package manager to find more possible upgrades:
...
As this package uses [`@clerk/clerk-react`](#clerk-clerk-react) under the hood you _potentially_ have to take its changes into account, too. Reason is that this package re-exports functions and components from `@clerk/clerk-react`. In most cases you probably only need to check if the [change in default routing behavior](#routing) affects you.

A couple of subpath imports have been removed but functionality stayed the same. You should use `@clerk/nextjs` instead.
...
The `@clerk/clerk-react` package is used by all SDKs based on React and as such any change in this package can _potentially_ affect other packages, too. SDKs based on React like `@clerk/nextjs` re-export certain components and functions from `@clerk/clerk-react`. Please check your code for the changes below, even if you didn't explicitly install `@clerk/clerk-react`.
...
- The `withUser`, `withSession`, and `withClerk` higher order functions (HOF) have been removed. The `WithUser`, `WithSession`, and `WithClerk` higher order components (HOC) have been removed. In both instances, please use [React hooks](/docs/references/react/overview) instead.
...
<Accordion titles={["sessions.getToken()", "signJwt, hasValidSignature, decodeJwt, verifyJwt, and verifyToken functions",]} heading="h5">
  <AccordionPanel>
    ```diff
  ...
  <AccordionPanel>
    Their `errors` array will only ever have one item so you can [destructure](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment) it directly.
    ...
    const { data, errors: [error] } = decodeJwt()
    if (error) throw error
...
If you're using the `navigate` prop on the `<ClerkProvider>` you'll need to [replace `navigate` with `routerPush` & `routerReplace`](#replace-navigate-with-router-push-router-replace)
...
<Accordion titles={["New localization keys", "Deleted localization keys", "Changed values"]} heading="h4">
  ...
  <AccordionPanel>
    ...
    const localization = merge({}, yourOldOject, newValues)
    ```
  </AccordionPanel>
</Accordion>
...
As mentioned in the [v5 announcement post](https://clerk.com/changelog/2024-01-16) Clerk's prebuilt components got a new look. These changes may impact your style customization applied through the `appearance` prop or class overrides. Internally some changes were made that might be a breaking change for you:
...
<Accordion titles={["Next.js", "Fastify", "Gatsby", "@clerk/backend", "@clerk/clerk-sdk-node"]} heading="h5">
  <AccordionPanel>
