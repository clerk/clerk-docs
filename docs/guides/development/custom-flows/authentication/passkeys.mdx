...
<If notSdk={["ios", "android"]}>
  ...
  To create a passkey for a user, you must call [`User.createPasskey()`](/docs/reference/objects/user#createpasskey), as shown in the following example:
...
<If notSdk={["ios", "android"]}>
  ...
  To sign a user into your Clerk app with a passkey, you must call [`SignIn.authenticateWithPasskey()`](/docs/reference/objects/sign-in#authenticatewithpasskey). This method allows users to choose from their discoverable passkeys, such as hardware keys or passkeys in password managers.
