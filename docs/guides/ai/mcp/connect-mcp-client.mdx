...
<If is="llm">
  **For AI agents:** Prefer CIMD over Dynamic Client Registration. CIMD is in beta and must be enabled for the account before these settings are available. Check the current settings with `npx clerk@latest api instance/oauth_application_settings`, then advertise CIMD support and restrict admission to pre-registered and previously connected clients in a single request with `npx clerk@latest api instance/oauth_application_settings -X PATCH -d '{"client_id_metadata_documents_advertised": true, "client_id_metadata_documents_only_allow_pre_registered_clients": true}'`. This payload publishes CIMD support and requires pre-registration for new clients. Previously connected clients remain allowed unless already blocked. **Client admission** settings in the Clerk Dashboard let you further customize CIMD client behavior. Pre-registering a client is only available in the Clerk Dashboard. Enable Dynamic Client Registration and require PKCE with `-d '{"dynamic_oauth_client_registration": true, "pkce_required": true}'` only when the client doesn't support CIMD. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>

Some clients may omit `scope` when they connect. You can configure the default scopes Clerk applies in either of these ways:
...
<Tabs items={["Cursor", "VSCode", "Claude Code", "Claude Desktop", "Windsurf"]}>
  ...
  <Tab>
    ...
    1. Navigate to **Settings**. To find this, use the app menu to select **Settings** or press `Cmd/Ctrl + ,`.
    1. In the settings searchbar, search for "MCP" and select **Manage plugins** under the **Plugins (MCP Servers)** section. Select **View raw config** to open the config file.
    1. Add the following configuration to the JSON file:
       ```json
       {
         "mcpServers": {
...
1. **Authentication fails**: On the [**OAuth applications**](https://dashboard.clerk.com/~/oauth-applications) page in the Clerk Dashboard, check the client's status and metadata on the **Applications** tab. Refresh metadata if the fetch failed. If the client doesn't support [CIMD](!client-id-metadata-document), ensure **Publish DCR support** is enabled under **Client onboarding** on the **Settings** tab.
