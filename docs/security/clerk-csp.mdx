...
For more control, both `standard` and `strict-dynamic` modes allow you to add additional CSP directives by providing a `directives` object.
...
export default clerkMiddleware(
  ...
  {
    contentSecurityPolicy: {
      // Both 'standard' and 'strict-dynamic' modes allow you to override specific CSP directives
      mode: 'standard',
      directives: {
...
With this configuration, the CSP header will include both the additional directive values you specified and the default values provided by the standard mode.
