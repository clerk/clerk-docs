...
With Clerk, we recommend pinning your Clerk SDK in two ways:

1. Pin your Clerk SDK in your `package.json` file to a specific version (no range operators).
```json {{ filename: 'package.json' }}
"@clerk/nextjs": "1.1.0"
```
1. Set the `clerkJsVersion` property when you initialize the Clerk integration. For most SDKs, this is done in the `<ClerkProvider>` component.
```tsx
