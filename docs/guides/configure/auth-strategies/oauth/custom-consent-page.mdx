...
Keep the consent route focused on the consent decision. If your application uses a shared layout with navigation, account menus, sign-in or sign-out controls, or other links, use a minimal layout for this route instead. Unrelated navigation can take the user away from the OAuth flow and cause the authorization request to fail or lose its original parameters.
