...
> [!IMPORTANT]
> This guide is for **companion watch apps** that pair with an iPhone app. **Standalone watch apps** must authenticate separately and do not use Watch Connectivity sync. To synchronize Clerk sessions between sibling iOS apps on the same device, use [shared session sync](/docs/reference/native-mobile/shared-session-sync) instead.
