...
By default, new Clerk instances lock user accounts after 10 failed sign-in attempts and require a one-hour cooldown period before anyone can attempt to sign in to that account again. The attempt limit default changed from 100 to 10 on **July 6, 2026** — existing instances kept their limit, and the one-hour cooldown is unchanged.
...
> [!NOTE]
> Currently, users cannot unlock their own account or submit a request to the admin to be unlocked ("self-service unlock"). This is an upcoming feature, so see the [changelog](/changelog) periodically to find out when it is available. Until then, you can [lock and unlock user accounts programmatically](#unlock-a-user-programmatically).

## Customize max sign-in attempts and lockout duration

You can customize the number of times a sign-in can be attempted before the account is locked, and how long lockouts last. To configure this feature, in the Clerk Dashboard, navigate to the [**Rules**](https://dashboard.clerk.com/~/protect/rules) page under **Protect**. In the **Lockout policy** row, select **Enable** if the policy is disabled or **Manage** if it's enabled. In the dialog, you can enable or disable **Lockout policy**, or configure the following settings:

- The **Maximum attempt limit** setting controls the number of failed sign-in attempts before a user is locked out. To change this setting, enter a new number of failed attempts allowed. The minimum is 5.
...
1. Open the three-dot menu on the right of that user's row.
