...
<Steps>
  ...
  - Adds the [`<ClerkProvider>`](/docs/reference/components/clerk-provider) component to your app, providing Clerk's authentication context to your app. Pass `loaderData` to the provider so it can access the authentication data loaded by `rootAuthLoader()`.
  - Creates a header with Clerk's [prebuilt components](/docs/reference/components/overview) to allow users to sign in and out, and display different content for signed-in and signed-out users. The components used in this example are explained below.
