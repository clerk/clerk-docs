...
<If notSdk={["ios", "android"]}>
  1. The [`useUser()`](/docs/reference/hooks/use-user) hook is used to get the current user's [`User`](/docs/reference/objects/user) object. The `isLoaded` boolean is used to ensure that Clerk is loaded.
  ...
  1. The `addSSO()` function is used to add a new external account using the `strategy` that is passed in.
     - It uses the `user` object to access the [`createExternalAccount()`](/docs/reference/objects/user#create-external-account) method.
  ...
  <If sdk="expo">
    ```tsx {{ filename: 'src/app/(account)/manage-external-accounts/page.tsx', collapsible: true }}
    import { ThemedText } from '@/components/themed-text'
    import { ThemedView } from '@/components/themed-view'
    import { useUser } from '@clerk/expo'
    import * as AuthSession from 'expo-auth-session'
    import type { ExternalAccountResource, OAuthStrategy } from '@clerk/expo/types'
...
<If sdk="ios">
  The following example provides basic examples of how to add, reverify, and remove an external account.
...
<If sdk="android">
  The following example provides basic examples of how to add and remove an external account.
