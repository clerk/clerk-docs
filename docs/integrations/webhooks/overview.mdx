...
3. Select the **Event Catalog** tab.
...
4. In the **Message Attempts** section, next to the message you want to replay, select the menu icon on the right side, and then select **Replay**.
5. The **Replay Messages** menu will appear. You can choose to:
...
To begin developing webhooks, start by discovering all types of event payloads. For this, you need an HTTPS endpoint. Beeceptor provides free HTTPS endpoints for payload discovery and routing to a service running locally on your development box. This allows you to test locally and iterate quickly without regular deployment.
Here is how you can get started:

1. Create a free endpoint on Beeceptor: Visit [Beeceptor](https://beeceptor.com/webhook-integration/?utm_source=social&utm_campaign=clerk-docs) to create a free endpoint.
2. Copy the endpoint URL and configure webhooks: Copy the endpoint URL and enter it in the webhooks configuration screen.
3. Trigger events and discover payloads: Trigger some events and discover the incoming payloads on Beeceptor. These are shown as they happen.
4. Route requests to a localhost port like 3000 or 8080: Use the Beeceptor CLI to route requests to a localhost port for local testing.
