...
4. The user clicks the email link. This can happen on a different device from where they entered their email address if the [**Require the same device or browser**](/docs/security/email-link-protection) setting is _off_.
