...
- The [`<SignUp />`](/docs/reference/components/authentication/sign-up) is accessible only to users who have been invited and have a valid invitation link. Users who don't have access will see a message indicating to join the waitlist.

- The [`<Waitlist />`](/docs/reference/components/authentication/waitlist) component provides a form where users can submit their details to join the waitlist. Once [approved](#manage-waitlist-entries), users will receive an email with instructions on how to join your app.
...
Users will get sent an email confirming their request to join the waitlist (the **Waitlist confirmation** email). Once approved by admins, users will receive an email with instructions on how to join your app (the **Waitlist invitation** email).

You can disable the **Waitlist confirmation** email, but you cannot disable the **Waitlist invitation** email. By default, both emails are enabled. You can also customize the emails to better match your brand.
