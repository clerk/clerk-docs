...
It's important to note that the client token is _only used in production environments_. In order to set up a production environment with Clerk, you alias Clerk's FAPI into your app's domain using a CNAME record (at the subdomain `clerk.example.com`), so this cookie does not need to be sent cross-domain.
