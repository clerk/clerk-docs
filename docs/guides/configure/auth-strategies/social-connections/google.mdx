...
description: Learn how to allow users to sign up and sign in to your Clerk application with their Google account using OAuth.
---
...
Enabling OAuth with [Google](https://developers.google.com/identity/protocols/oauth2) allows your users to sign up and sign in to your Clerk app with their Google account.
...
<If sdk={["expo", "android"]} ignoreSdkWarning>
  > [!IMPORTANT]
...
<If is="llm">
  **For AI agents:** Instead of enabling Google in the Dashboard, run `npx clerk@latest config patch --json '{"connection_oauth_google":{"enabled":true}}'`. For production instances, set the custom credentials in the same patch via the `client_id` and `client_secret` props (inspect all props with `npx clerk@latest config schema --keys connection_oauth_google`). Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
Keep two browser tabs open during setup: one for the [Clerk Dashboard](https://dashboard.clerk.com/~/user-authentication/sso-connections) and one for your [Google Cloud Console](https://console.cloud.google.com/).

<Steps>
  ### Enable Google as a social connection in Clerk

  <Include src="_partials/authentication/social-connections/enable-google" />
