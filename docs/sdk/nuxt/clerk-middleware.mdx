...
> [!NOTE]
> To learn how to protect pages, see the [dedicated guide](/docs/sdk/nuxt/protect-pages).
...
To protect routes based on user authentication status, you can check if the user is signed in by checking the `userId` on the [`auth`](/docs/sdk/nuxt/methods#auth-object) object.
...
To protect routes based on user authorization status, you can use the `has()` helper to check if the user has the required [organization roles or custom permissions](/docs/organizations/roles-permissions). It is available on the [`auth`](/docs/sdk/nuxt/methods#auth-object) object.
