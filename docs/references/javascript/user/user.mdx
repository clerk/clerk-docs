...
| Type | Description |
| --- | --- |
| <code>Promise\<[PaginatedResponse](/docs/references/javascript/types/clerk-paginated-response)\<OrganizationMembershipResource\>\></code> | The list of organizations the user is a member of or rejects with `ClerkAPIResponseError` or Network error. |
