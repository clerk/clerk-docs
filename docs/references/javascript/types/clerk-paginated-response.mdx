---
title: ClerkPaginatedResponse
description: An interface that describes the response of a method that returns a paginated list of resources.
---

# `ClerkPaginatedResponse`

An interface that describes the response of a method that returns a paginated list of resources.

## Properties

| Properties | Type | Description |
| --- | --- | --- |
| `data` | `T[]` |  An array that contains the fetched data. |
| `total_count` | `number` |  The total count of data that exist remotely. |

## Error handling

Clerk's SDKs always use `Promise<ClerkPaginatedResponse<T>>`. If the promise resolves you'll get back the [properties](#properties), if it's rejected you'll receive a `ClerkAPIResponseError` or network error.