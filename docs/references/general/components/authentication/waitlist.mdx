...
> [!WARNING]
> Before using the `<Waitlist />` component, you must provide the `waitlistUrl` prop either in the [`<ClerkProvider>`](/docs/references/general/components/clerk-provider#properties) or [`<SignIn />`](/docs/references/general/components/authentication/sign-in#properties) component to ensure proper functionality.
