...
<Steps>
  ...
  1. The [`useSignInWithGoogle()`](/docs/reference/expo/native-hooks/use-sign-in-with-google) hook requires the `@clerk/expo-google-signin` package for the native Google Sign-In module and `expo-crypto` for generating secure nonces during the authentication flow.
     ```bash {{ filename: 'terminal' }}
     npx expo install @clerk/expo-google-signin expo-crypto
     ```
  1. Add the `@clerk/expo-google-signin` [config plugin](https://docs.expo.dev/config-plugins/introduction/) to your app config, alongside the `@clerk/expo` plugin:
     ```json {{ filename: 'app.json' }}
     {
       "expo": {
