...
<Properties>
  ...
  - `error`
  - <code>null | ClerkAPIResponseError</code>
  ...
  - `setData`
  - `CacheSetter<undefined | ClerkPaginatedResponse<CommerceStatementResource>[]>` when `infinite` is `true`, or `CacheSetter<undefined | ClerkPaginatedResponse<CommerceStatementResource>>` when `infinite` is `false`
