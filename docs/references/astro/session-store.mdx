...
The `$sessionStore` store provides a convenient way to access the current user's [`Session`](/docs/references/javascript/session){{ target: '_blank' }} object, as well as helpers for setting the session.
