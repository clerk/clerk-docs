...
Clerk also adds an `act` claim on the [Clerk session token](/docs/session-requests/session-tokens), which contains information like the impersonated session ID and the actor/impersonator's user ID.
