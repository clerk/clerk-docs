...
description: Learn how to configure your Content Security Policy to work with Clerk and its abuse and fraud protections.
---
...
For Clerk to work correctly and protect your application against abuse and fraud, the following CSP directives are required:

1. `script-src` - This value should include the host application's FAPI hostname, such as `https://clerk.your-domain.com`, Cloudflare's bot protection host, `https://challenges.cloudflare.com`, and Clerk's abuse and fraud protection hosts through `https://*.protect.clerk.com`.
1. `connect-src` - This value should include the host application's FAPI hostname, such as `https://clerk.your-domain.com`, and Clerk's abuse and fraud protection hosts through `https://*.protect.clerk.com:*`. The trailing `:*` is required: Clerk's abuse and fraud protection hosts are served on ports other than `443`, and a CSP source with no port matches port `443` only.
...
1. `frame-src` - This value should include Cloudflare's bot protection host, `https://challenges.cloudflare.com`, and Clerk's abuse and fraud protection hosts through `https://*.protect.clerk.com`.
...
> [!WARNING]
> Automatic CSP configuration is available only for `@clerk/nextjs >=6.14.0`. Upgrade to `@clerk/nextjs >=7.7.8` to include Clerk's abuse and fraud protection domains automatically. Versions `>=7.5.21 <7.7.8` include those domains but only on port `443`, so requests to them on other ports are blocked.
...
Both modes automatically include Clerk's abuse and fraud protection domains for every application, regardless of plan or enabled features.

### `default` configuration

The `default` configuration will apply the following CSP directives:

- `connect-src` - `'self' https://clerk-telemetry.com https://*.clerk-telemetry.com https://api.stripe.com https://maps.googleapis.com https://img.clerk.com https://images.clerkstage.dev https://*.protect.clerk.com:* https://{{fapi_url}}`
- `default-src` - `'self'`
- `form-action` - `'self'`
...
- `script-src` - `'self' 'unsafe-inline' https: http: https://*.js.stripe.com https://js.stripe.com https://maps.googleapis.com https://*.protect.clerk.com`
- `style-src` - `'self' 'unsafe-inline'`
- `worker-src` - `'self' blob:`

In development environments, both modes also include `'unsafe-eval'` in the `script-src` directive, which [Next.js requires in development](https://nextjs.org/docs/app/building-your-application/configuring/content-security-policy#without-nonces). It is not included in production.
...
const cspHeader = `
  default-src 'self';
  script-src 'self' 'unsafe-inline' 'unsafe-eval' https://{{fapi_url}} https://challenges.cloudflare.com https://*.protect.clerk.com;
  connect-src 'self' https://{{fapi_url}} https://*.protect.clerk.com:*;
  img-src 'self' https://img.clerk.com;
  worker-src 'self' blob:;
  style-src 'self' 'unsafe-inline';
