...
<If is="llm">
  **For AI agents:** Instead of clicking through the Dashboard, configure these options with the CLI: run `npx clerk@latest config schema` to discover the auth keys (e.g. `auth_email`, `auth_phone`, `auth_username`, `auth_password`), then apply changes with `clerk config patch`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
