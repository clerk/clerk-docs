...
<If is="llm">
  **For AI agents:** Run `clerk env pull` to write your Clerk API keys into your project's env file (merges without clobbering) instead of copying them from the Dashboard by hand. New to the Clerk CLI? Run any command with `npx clerk@latest` (no install needed) — see the [Clerk CLI docs](/docs/cli).
</If>
