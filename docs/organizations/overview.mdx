...
Most multi‑tenant applications want every user to be part of an organization rather than operating in an isolated personal workspace. Accordingly, **personal workspaces are disabled by default** once you enable organizations. After signing up, a user must create or join an organization before they can proceed.

> [!TIP]
> Curious what the user sees? The after‑auth flow handles it automatically—check out [Steps after sign‑up / sign‑in](/docs/authentication/configuration/after-auth-flows).
...
When personal workspaces are enabled, users initially sign in to their personal workspace with **no** active organization set.\
When personal workspaces are **disabled** (the default), users must select or create an organization to continue.\
Even if the person belongs to a single organization, they still need to make it active via the `<OrganizationSwitcher />`, or programmatically with `setActive()` if you prefer to handle it in code.
This is handled automatically in the after‑auth flow. For more information, see [Steps after sign‑up / sign‑in](/docs/authentication/configuration/after-auth-flows).
