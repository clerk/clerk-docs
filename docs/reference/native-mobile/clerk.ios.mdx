...
<Properties>
  ...
  The [Active Organization](!active-organization) for the current active session. Returns `nil` when there is no active session, no selected Organization, or the active user's memberships don't include the selected Organization.
