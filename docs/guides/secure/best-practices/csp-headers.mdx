...
> [!IMPORTANT]
> You must pass the [`dynamic` prop](/docs/reference/general/components/clerk-provider#properties) to `<ClerkProvider>` for `strict` CSP configuration to work correctly. This is because the nonce value is generated on the server and passed to the client, which requires dynamic rendering.
...
> [!IMPORTANT]
> You must pass the [`dynamic` prop](/docs/reference/general/components/clerk-provider#properties) to `<ClerkProvider>` for `strict-dynamic` CSPs to work correctly. This is because the nonce value is generated on the server and passed to the client, which requires dynamic rendering.
