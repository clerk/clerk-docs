...
Each after-auth flow is mapped to a unique [`SessionTask['key']`](/docs/references/javascript/types/session-task.mdx) that identifies the specific task. For example:
...
After-auth tasks are automatically displayed and handled when using the `<SignIn />` and `<SignUp />` components.

For more customization, you can opt out of using the built-in `<SignIn />` and `<SignUp />` components and create custom pages for your after-auth flows. This can be done by configuring the `taskUrls` option in your Clerk options and using dedicated Task components like [`<TaskSelectOrganization />`](/docs/components/authentication/task-select-component.mdx).
