...
You've added two imports, which you'll use as you build out the rest of the flow. `<SignIn.Root>` manages the sign-in state and handles connecting the components to Clerk's APIs.
...
Make it functional by adding input fields. Utilize Clerk Elements' common components to render an `identifier` field, as well as the `<Connection>` component to allow users to sign in with a social connection, like Google:
