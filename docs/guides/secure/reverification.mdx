...
The simplest way to require reverification for a sensitive action is to use the [`useReverification()`](/docs/references/hooks/use-reverification) hook. This hook will automatically check if the user has verified their credentials within 10 minutes and if not, displays a modal that prompts the user to verify their credentials. See the [reference doc](/docs/references/hooks/use-reverification) for more information and comprehensive examples.
