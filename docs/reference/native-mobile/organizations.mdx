...
The [Active Organization](!active-organization) is stored on `Session.lastActiveOrganizationId` for the current session. Use [`Clerk.organization`](/docs/reference/native-mobile/clerk#active-organization-state) to read the Active Organization and `Clerk.organizationMembership` to read the signed-in user's Active Organization Membership.
