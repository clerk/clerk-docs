...
Actor tokens are similar to [sign-in tokens](/docs/references/backend-api/tag/Sign-in-Tokens#operation/CreateSignInToken){{ target: '_blank' }}. They can be used for one-time sign-ins, but they result in impersonated sessions.

### Create an actor token

To create an actor token, you can use the [Backend API](/docs/references/backend-api/tag/Actor-Tokens#operation/CreateActorToken){{ target: '_blank' }}.
...
To revoke an actor token, you can use the [Backend API](/docs/references/backend-api/tag/Actor-Tokens#operation/RevokeActorToken){{ target: '_blank' }}.
...
The [Frontend API](/docs/references/frontend-api/){{ target: '_blank' }} will redirect you to the `/sign-in` page of your application, where the flow will continue by consuming the `__clerk_ticket` parameter.
