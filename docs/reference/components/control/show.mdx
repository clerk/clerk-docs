...
`<Show />` with `when='signed-in'` performs authentication checks. To perform [authorization checks](!authorization-check), you can pass different values to the `when` prop, like `when={{ role: '...' }}`, `when={{ permission: '...' }}`, `when={{ feature: '...' }}`, or `when={{ plan: '...' }}`.
