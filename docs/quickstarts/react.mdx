...
The [`<ClerkProvider />`](/docs/components/clerk-provider) component provides active session and user context to Clerk's hooks and other components. Import it into your app by adding `import { ClerkProvider } from '@clerk/clerk-react'` at the top of your file. You will need to import your Publishable Key and provide it to your `<ClerkProvider />` using the `publishableKey` prop. 
