...
To support legal acceptance in your sign-up custom flow, pass the `legalAccepted` boolean to whichever method is initializing the sign-up process (e.g., [`SignUp.password()`](/docs/reference/javascript/sign-up#password), [`SignUp.create()`](/docs/reference/javascript/sign-up#create), etc.). Because `legalAccepted` is tracked through a simple boolean, this means that you can add any kind of "legal acceptance" fields to your sign-up form that you want - you are not limited to only "Terms of Service" or "Privacy Policy" fields.
