...
Clerk’s Next.js SDK provides a built-in `auth()` function that supports token validation via the `acceptsToken` parameter. This lets you specify which type(s) of token your API route should accept.
...
Below are two examples of verifying machine tokens in a Next.js API route using Clerk’s SDK:
...
You can also protect entire route groups using [`clerkMiddleware()`](/docs/references/nextjs/clerk-middleware). See how to implement this in this [dedicated section](/docs/references/nextjs/clerk-middleware#protect-routes-based-on-token-types).
