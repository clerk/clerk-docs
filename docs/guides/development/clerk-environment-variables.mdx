...
<Tabs items={["General", "Next.js", "Vite"]}>
  <Tab>
    ...
    | - | - |
    | `CLERK_PUBLISHABLE_KEY` | Your Clerk app's [Publishable Key](!publishable-key). It will be prefixed with `pk_test_` in development instances and `pk_live_` in production instances. |
