...
description: Learn how to manage SMS based multi-factor authentication, to enable the same features as in the User Profile component.
---

# Manage SMS based Multi-factor Authentication


```typescript {{ filename: 'app/account/manage-mfa/page.tsx' }}
'use client';
...
const ManageMfaPhoneNumbers = () => {
  ...
  // Check is for any phone numbers are reserved for second factor (2fa)
  ...
  // If the array is a length of 0, then no phone numbers have been reserved for 2fa
  if (mfaPhones.length === 0) {
    return <p>No phone numbers configured for MFA</p>;
  }

  // Render a table of 2fa phone numbers, with buttons to make default and remove
  // Also show an indicator of which number is the default
  return (
    <>
      <h2>2fa Number</h2>
      <ul>
        {mfaPhones.map((phone) => {
          return (
            ...
            >
              ...
              <div>{phone.defaultSecondFactor && 'Default'}</div>
              <div>
                <button
                  onClick={() =>
              ...
              {/* This button is not strictly needed here -- this could be part of the user profile */}
              <div>
...
```typescript {{ filename: 'app/account/manage-mfa/add/page.tsx' }}
'use client';
