...
export default function Page() {
  ...
  // This function will handle the user submitting their email and passowrd
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (completeSignIn.status !== 'complete') {
        // The status can also be `needs_factor_on'm 'needs_factor_two' or 'needs_identifier'
        // Please see https://clerk.com/docs/references/react/use-sign-in#result-status for  more information
        console.log(JSON.stringify(completeSignIn, null, 2));
      }

      if (completeSignIn.status === 'complete') {
        // If complete, user exists and porivded password match -- set session active
