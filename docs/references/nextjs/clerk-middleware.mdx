...
The following example requires a user to be signed in to view the content for any `/dashboard` route. If the user is not signed in, `auth.protect()` will redirect the user to the sign-in route.
...
If you want to do something other than redirecting the user to the sign-in route, or you want to run custom logic before the redirect happens,  you can use `auth().userId()` to check if the user is signed in and then perform the action you want based on the user's authentication status.

The following example requires a user to be signed in to view the content on any `/dashboard` route. If the user is not signed in, custom logic will run, and [`auth().redirectToSignIn()`](/docs/references/nextjs/auth#redirect-to-sign-in) will redirect the user to the sign-in route.
...
The following example requires a user to be signed in to view the content for any `/dashboard` route. If the user is not signed in, `auth.protect()` will return a `404` error.
...
If you want to do something other than redirecting the user to the sign-in route, or you want to run custom logic before the redirect happens, you can use `auth().has()` to check if the user has the required roles or permissions and then perform the action you want based on the user's authorization status.

The following example requires a user to have the `org:sys_memberships:manage` or `org:sys_domains_manage` permission to view the content on any `/admin` route. If the user does not have the required permissions, custom logic will run, and [`auth().redirectToSignIn()`](/docs/references/nextjs/auth#redirect-to-sign-in) will redirect the user to the sign-in route.
