...
In non-browser environments, you’ll need to re-implement the [Clerk class](/docs/reference/objects/clerk) in the SDK’s programming language, interacting with the [FAPI](/docs/guides/development/sdk-development/terminology).
...
- Give users access to [`Client`](/docs/reference/objects/client), [`Session`](/docs/reference/objects/session), [`User`](/docs/reference/objects/user), and [`Organization`](/docs/reference/objects/organization) properties through the framework’s choice of state management
- User should be able to use [ClerkJS options](/docs/reference/objects/clerk#clerkoptions)
...
<Steps>
  ...
  In order to make `Clerk` available on the `window` object, your SDK needs to load ClerkJS. Call `loadClerkJSScript()` from `@clerk/shared/loadClerkJsScript`.
  ...
  Expose properties to the internal state (e.g., a `useState` in React) through adding [event listeners](/docs/reference/objects/clerk#addlistener).
  ...
  Build out the components that your users will utilize in their app. Call the `mount()` function when the component is in view/mounts and the `unmount()` function when the component is unmounted. These functions are described in the [`Clerk` class](/docs/reference/objects/clerk) reference (switch the page to the JavaScript SDK to see the component methods).
