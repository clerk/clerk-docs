...
The `appearance` prop can be applied to [`<ClerkProvider>`](/docs/reference/components/clerk-provider) to share styles across every component, or individually to any of the Clerk components.

This applies to all of the React-based packages, like [Next.js](/docs/nextjs/getting-started/quickstart), as well as [the pure JavaScript ClerkJS package](/docs/reference/javascript/overview). For other SDKs, you can pass the `appearance` prop to the SDK's corresponding configuration method.
