...
The [JavaScript Backend SDK](/docs/references/backend/quickstart) exposes the Backend API resources and low-level authentication utilities for JavaScript environments.

For information about the `User` operations available, such as `getUser()`, `createUser()`, and `deleteUser()`, see the [Backend SDK reference docs](/docs/references/backend/quickstart).
