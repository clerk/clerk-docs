...
description: Learn what OAuth and OpenID Connect are, why they're used, and how the different OAuth flows work.
---
...
1. The user clicks a "Connect" or "Sign in with" button in your application.
1. They’re redirected to the OAuth provider’s authorization page.
1. After they approve the request, the provider redirects them back to your application with a short-lived authorization code.
...
>
> Additionally, any attacker with only the Client ID, which is **not** a sensitive or secret value that's already exposed to the browser, could obtain an OAuth token without any client verification, significantly compromising the security of your application.
>
...
>
> In environments without a secure server such as mobile apps or SPAs, a Client Secret can’t be safely stored. In this case, OAuth uses PKCE to improve security by ensuring only the client that requested the authorization code can exchange it. To learn more, see our [dedicated section on PKCE](/docs/oauth/how-clerk-implements-oauth#public-clients-and-pkce).
