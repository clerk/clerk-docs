...
<Tab>
From the sidebar, select **SQL Editor** and run the following query, replacing `tasks` with the name of the table you wish to add `user_id` to:

```sql
alter table tasks add user_id text not null default (requesting_user_id());
```
...
Next, you'll create a `tasks` table and enable RLS on that table.
...
and select the desired table. Click the **RLS disabled** button in the top navigation, then **Enable RLS for this table** in the menu that appears.
</Tab>
<Tab>
...
The final result should be similar to this:
    ...
    CLERK_SECRET_KEY={{secret_key}} // TODO: Is this right???
...
- Try integrating Supabase into your own application by implementing RLS and leveraging the sample code shown in this guide.
