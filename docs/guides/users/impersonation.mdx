...
Clerk's frontend SDKs handle this automatically for active sessions. If you use the Frontend API directly, call the [`/v1/client/sessions/{session_id}/touch`](/docs/reference/frontend-api/tag/sessions/post/v1/client/sessions/\{session_id}/touch){{ target: '_blank' }} endpoint or create a session token with the [`/v1/client/sessions/{session_id}/tokens`](/docs/reference/frontend-api/tag/sessions/post/v1/client/sessions/\{session_id}/tokens){{ target: '_blank' }} endpoint while the impersonator is active. These refresh the session's last active state, but they don't extend the session beyond the actor token's configured maximum session duration.
