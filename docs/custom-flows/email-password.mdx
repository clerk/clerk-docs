...
<Steps>
  ...
  <Tabs items={["Next.js", "JavaScript", "Expo", "iOS", "Android (beta)"]}>
    <Tab>
      <CodeBlockTabs options={["app/sign-in/[[...sign-in]]/page.tsx", "app/tasks/[[...task]]/page.tsx"]}>
                ...
                await setActive({
                  ...
                  navigate: async ({ session }) => {
                    if (session?.currentTask) {
                      // Navigate to task page (see https://clerk.com/docs/custom-flows/overview#after-auth-flows)
                      await router.push(`/tasks/${session.currentTask.key}`)
                      return
                    }
        ...
        ```tsx {{ filename: 'app/tasks/page.tsx', collapsible: true }}
        import { TaskChooseOrganization, useSession } from '@clerk/nextjs'
          ...
          try {
            ...
            if (signUpAttempt.status === 'complete') {
              await setActive({
                ...
                navigate: async ({ session }) => {
                  if (session?.currentTask) {
                    // If there's a pending session task, navigate the user to your custom task page to complete it. See https://clerk.com/docs/custom-flows/overview#after-auth-flows
                    await router.push(`/sign-in/tasks/${session.currentTask.key}`)
                    return
                  }
    ...
    <Tab>
      <CodeBlockTabs options={["app/sign-in/[[...sign-in]]/page.tsx", "app/tasks/[[...task]]/page.tsx"]}>
        ...
        export default function Page() {
          ...
          if (currentTask?.key === 'choose-organization') {
            // For building custom task UI instead of using TaskChooseOrganization,
            // see our guide on handling tasks:
            // https://clerk.com/docs/custom-flows/overview#after-auth-flows
            return <TaskChooseOrganization redirectUrlComplete="/" />
          }
              ...
              await setActive({
                ...
                navigate: async ({ session }) => {
                  if (clerk.session?.currentTask) {
                    // If there's a pending session task, navigate the user to your custom task page to complete it. See https://clerk.com/docs/custom-flows/overview#after-auth-flows
                    await router.push(`/tasks/${clerk.session.currentTask.key}`)
                    return
                  }
