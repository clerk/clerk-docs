...
### Make a header with Clerk components

You can control which content signed in and signed out users can see with Clerk's [prebuilt components](/docs/components/overview). To get started, we'll make a header where users can sign in or out. To do this, we'll use:
