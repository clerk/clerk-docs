...
1. **Optional:** If you're configuring a provider for a native or mobile app, a single-page application (SPA), or another public client that can't securely store a client secret, enable the **Use PKCE** toggle. [PKCE (Proof Key for Code Exchange)](https://datatracker.ietf.org/doc/html/rfc7636) is also a recommended best practice for modern OAuth clients more broadly. Once enabled, Clerk will use the Authorization Code with PKCE flow for authentication with that provider. Learn more about [PKCE](/docs/guides/configure/auth-strategies/oauth/how-clerk-implements-oauth#public-clients-and-pkce).
