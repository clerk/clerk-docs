...
Next.js's upcoming [Partial Prerendering (PPR)](https://nextjs.org/docs/app/building-your-application/rendering/partial-prerendering) feature provides more granular control over static and dynamic rendering. Wrapping components that access auth data with `<Suspense>` will allow pages to be pre-rendered up to the Suspense boundaries.
