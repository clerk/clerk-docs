...
> [!NOTE]
> If password is used for sign-up, but a user signs up with a social provider, they will have an account with no password set. To avoid revealing that no password is on file, Clerk still prompts for a password when the user tries to sign in, but the attempt will always fail with an error response indicating that the password is incorrect or another sign-in strategy should be used. Clerk's components also surface a **Use another method** option so the user can easily sign in with another strategy (such as the social provider they used at sign-up, or an email code if enabled).
