...
> [!IMPORTANT]
> You can perform up to **5 user impersonations per month for free**. This allows you to try user impersonation before purchasing the **Enhanced Administration Add-on**, which is required for higher usage. Head to the [pricing page](/pricing) to get the full details on how Clerk charges for user impersonation.
...
The `url` attribute is a Clerk [Frontend API URL](!frontend-api-url) that will use the token to sign out existing users and prepare the [`SignIn`](/docs/reference/javascript/sign-in) object for impersonation. You can directly visit the `url` provided in the response to consume the actor token.
