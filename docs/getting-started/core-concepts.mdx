...
Clerk's SDKs are built on top of many key objects, like the `Clerk`, `User`, and `Session` objects. As you're building your application, you'll likely interact with these objects, either directly or through hooks and helpers. Refer to the [reference docs](/docs/reference/objects/overview) for more information.
