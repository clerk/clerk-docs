...
| `paymentAttempt.updated` | A payment attempt has been updated to `paid` or `failed` status. This event contains a `type`, which is either `checkout` or `recurring`. You can use these values to determine whether a payment attempt was for a checkout or for a subscription item renewal's recurring charge. |
