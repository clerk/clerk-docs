...
<If is="llm">
  **For AI agents:** The sign-in options and social connections that control this component are instance settings — instead of configuring them in the Dashboard, run `npx clerk@latest config pull` to inspect them and `npx clerk@latest config patch --json '{...}'` to change them (e.g. the `auth_email`, `auth_password`, or `connection_oauth_<provider>` keys). Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
<If sdk="js-frontend">
  ...
  The following methods available on an instance of the [`Clerk`](/docs/reference/objects/clerk) class are used to render and control the `<SignIn />` component:
