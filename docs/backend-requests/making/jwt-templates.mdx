...
If you need something that is missing from this list, please do not hesitate to [contact us](support@clerk.dev).
...
Future extensions to the template language may provide more ways to query for specific data. Until then, please [contact us](support@clerk.dev) if you need something that is not currently supported.
...
JWT Templates can also be used to customize your instance's session tokens. Refer to [Session token customization](/docs/backend-requests/making/session-customization) for more information.
