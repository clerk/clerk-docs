...
<If notSdk={["js-frontend", "ios", "android"]}>
  ...
  1. Uses the [`useOrganization()`](/docs/reference/hooks/use-organization) hook to get `memberships`, which is a list of the [Active Organization's](!active-organization) memberships.
     - `memberships` is an object with `data` that contains an array of [`OrganizationMembership`](/docs/reference/types/organization-membership) objects.
     - Each `OrganizationMembership` object has an [`update()`](/docs/reference/types/organization-membership#update) and [`destroy()`](/docs/reference/types/organization-membership#destroy) method to update the member's Role and remove the member from the Organization, respectively.
  ...
  <If sdk="expo">
    ```tsx {{ filename: 'src/components/manage-roles.tsx', collapsible: true }}
...
<If sdk="ios">
  ...
  struct ManageRolesView: View {
    ...
    @State private var membershipToRemove: OrganizationMembership?
