...
<Properties>
  ...
  **(Deprecated)** Use `lastActiveAtAfter` instead. Filters users that had session activity since the given date (day precision, in ms since epoch).
  ...
  Filters users with memberships to the given Organization IDs. For each ID, prefix with `+` to include or `-` to exclude users with memberships in that Organization. Up to 100 accepted.
