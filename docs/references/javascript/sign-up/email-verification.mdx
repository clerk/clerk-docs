...
These are all methods on [the `SignUp` class](/docs/references/javascript/sign-up/sign-up) that allow you to verify a user's sign-up request via email.
...
This is equivalent to calling [`SignUp.attemptVerification({strategy: "email_code", ...params})`](/docs/references/javascript/sign-up/verification#attemptverification).
