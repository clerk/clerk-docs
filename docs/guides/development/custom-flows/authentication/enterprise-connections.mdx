...
<If sdk="expo">
  ...
  1. Calls the `startSSOFlow()` method with the `strategy` param set to `enterprise_sso` and the `identifier` param set to the user's email address that they provided. The optional `redirect_url` param is also set in order to redirect the user once they finish the authentication flow. The redirect URL **must** be registered in the [Clerk Dashboard](https://dashboard.clerk.com/~/native-applications) under **Redirect URLs**. Without this, the Enterprise SSO provider will complete authentication but the session will not be created.
     ...
     - If authentication is not successful, you can handle the missing requirements, such as MFA, using the [`signIn`](/docs/reference/objects/sign-in) or [`signUp`](/docs/reference/objects/sign-up) object returned from `startSSOFlow()`, depending on if the user is signing in or signing up. These objects include properties, like `status`, that can be used to determine the next steps. See the section on [handling missing requirements](/docs/guides/development/custom-flows/authentication/oauth-connections#handle-missing-requirements) for more information.
