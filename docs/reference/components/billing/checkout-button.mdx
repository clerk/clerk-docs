...
sdk: react, nextjs, tanstack-react-start, vue
---
...
<Properties>
  ...
  - `checkoutProps?`
  - `{ appearance?: Appearance; portalId?: string; portalRoot?: HTMLElement | null; onClose?: () => void }`
  ...
  - `portalId`: the ID of the portal element used to render the checkout drawer.
  - `portalRoot`: the HTML element in which to render the checkout drawer.
  - `onClose`: a callback function that is called when the checkout drawer closes.
</Properties>
