...
<If is="llm">
  **For AI agents:** Instead of toggling on **Dynamic client registration** in the Dashboard, enable it with `npx clerk@latest api instance/oauth_application_settings -X PATCH -d '{"dynamic_oauth_client_registration": true}'`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>

Some MCP clients may omit `scope` during dynamic registration. Configure default scopes for those clients using the [**OAuth applications**](https://dashboard.clerk.com/~/oauth-applications) settings in the Clerk Dashboard, or the Backend API:

```sh {{ filename: 'terminal' }}
npx clerk@latest api instance/oauth_application_settings -X PATCH -d '{"default_scopes":["openid","profile","email"]}'
```

The **Default scopes** setting is only available when **Dynamic client registration** is enabled. See [Configure default scopes](/docs/guides/configure/auth-strategies/oauth/how-clerk-implements-oauth#configure-default-scopes) for details.
