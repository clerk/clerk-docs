The following example demonstrates how to use the [`$userStore`](/docs/references/astro/client-side-helpers/user-store) to access the `User` object. It returns `undefined` while Clerk is still loading and `null` if the user is not signed in.
