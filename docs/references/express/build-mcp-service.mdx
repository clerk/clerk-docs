...
<Steps>
  ...
  - The `mcpAuth` middleware runs first to validate the OAuth token and attach the authenticated user's info to the request.
  - A new `StreamableHTTPServerTransport` instance is created to support streaming requests from LLM clients.
  - `server.connect(transport)` connects the MCP server to the transport layer.
  ...
  The `generateClerkProtectedResourceMetadata()` helper from `@clerk/mcp-tools` creates the required metadata object using your Clerk Publishable Key and your app's resource URL. You can retrieve the Clerk Publishable Key from the [**API keys**](https://dashboard.clerk.com/last-active?path=api-keys) page in the Clerk Dashboard.
