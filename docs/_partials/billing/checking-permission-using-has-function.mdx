<If sdk="astro">
  > [!WARNING]
  > Astro SDK hasn't been updated to support "permission" in the `has()` function yet.
...
<If sdk="chrome-extension">
  ```tsx {{ filename: 'src/screens/manage-premium-content.tsx' }}
  import { useAuth } from '@clerk/chrome-extension'

  export default function ManagePremiumContentScreen() {
    const { isLoaded, has } = useAuth()
