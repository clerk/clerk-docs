...
<Tabs type="framework" items={["General", "Next.js"]}>
  ...
  <Tab>
    ...
    | `NEXT_CLERK_AFTER_SIGN_IN_URL` | Full URL or path to navigate to after successful sign in. Defaults to `/`. `NEXT_PUBLIC_CLERK_SIGN_IN_FALLBACK_REDIRECT_URL` and `NEXT_CLERK_SIGN_IN_FORCE_REDIRECT_URL` have priority and should be used instead. |
  </Tab>
</Tabs>
