...
Pass an array to [`acceptsToken`](/docs/reference/backend/authenticate-request#authenticaterequestoptions) to verify more than one Clerk token type in the same call. Switch on `auth.tokenType` to branch on what authenticated.
