...
#### Social/OAuth provider authentication caveat

If **password** is enabled and a user signs up with a social provider, they will have an account with no password set. Then, if the user tries to sign in with the email on their OAuth account, Clerk still prompts for a password but to avoid revealing that no password is on file, the attempt will always fail with an error response indicating that the password is incorrect or another sign-in strategy should be used. We understand this is a confusing experience and are working on a solution. In the meantime, Clerk's components surface a **Use another method** option so the user should try to sign in with another strategy, such as the social provider they used at sign-up.
