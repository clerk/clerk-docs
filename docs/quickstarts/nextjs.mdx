...
1. Create a `middleware.ts` file.
    - If you are using the `/src` directory, place `middleware.ts` in the `/src` directory.
    - If you are not using the `/src` directory, place `middleware.ts` in the root directory alongside `.env.local`.
