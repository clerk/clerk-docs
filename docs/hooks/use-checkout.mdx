...
1.  In conjunction with the [`<CheckoutProvider />`](#checkout-components-and-hooks), which provides the checkout context to its children. This is useful when multiple components in a tree need to access the same checkout state.
2.  As a standalone hook by passing configuration options directly to it. This is ideal for self-contained components that manage their own checkout flow.
...
<Properties>
  * `options?`
  * [`UseCheckoutOptions`](#usecheckoutoptions)
        ...
        export function CheckoutFlow() {
          ...
          return (
            <div>
              ...
              {error && (
                <p style={{ color: 'red' }}>Error: {error.errors[0].message}</p>
              )}
