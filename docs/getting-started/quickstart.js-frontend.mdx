...
<Tabs items={["NPM module", "<script>"]}>
  <Tab>
    <Steps>
      ...
      - Imports your [Publishable Key](!publishable-key) and derives the [Frontend API URL](!frontend-api-url) from it.
      ...
      const frontendApiUrl = atob(publishableKey.replace(/^pk_(test|live)_/, '')).replace(/\$$/, '')
