...
<If sdk="ios">
  ...
  @MainActor
  func fetchOrganizationMemberships(clerk: Clerk) async -> [OrganizationMembership] {
    do {
      return try await clerk.user?.getOrganizationMemberships().data ?? []
    } catch {
      // See https://clerk.com/docs/guides/development/custom-flows/error-handling
      // for more info on error handling
      dump(error)
  ...
  @MainActor
  func setActiveOrganization(organizationId: String, clerk: Clerk) async {
