...
<Properties>
  ...
  - `awaiting_initialization`: The checkout hasn't started but the hook is mounted. Call `start()` to continue.
  - `awaiting_confirmation`: The checkout has been initialized and is awaiting confirmation. Call `confirm()` to continue.
    ...
    <Tabs items={["<SubscriptionPage />", "<CheckoutFlow />"]}>
      <Tab>
        ...
        export function SubscriptionPage() {
          ...
          return (
            <CheckoutProvider for="user" planId="cplan_123" planPeriod="month">
              <div>
