...
**Version 2026-05-12** of Clerk's Frontend and Backend APIs removes metadata fields from the general-purpose user and organization update endpoints. Metadata must now be set through the dedicated metadata endpoints, which provide deep-merge semantics.
...
If you are using a Clerk SDK, use the [`updateOrganizationMetadata()`](/docs/reference/backend/organization/update-organization-metadata) method (which wraps `PATCH`) or the [`replaceOrganizationMetadata()`](/docs/reference/backend/organization/replace-organization-metadata) method (which wraps `PUT`) instead of passing metadata fields to `updateOrganization()`.
