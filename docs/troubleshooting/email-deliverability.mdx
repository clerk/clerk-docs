...
Email providers will check if there's an actual mailbox behind the "from address" of an email. By default, Clerk will send your emails from [notifications@yourdomain.com](mailto:notifications@yourdomain.com). You should make sure that you set up a mailbox at this email address. If you'd like to use a different "from address", you can change its value via our [Backend API](/docs/references/backend-api/tag/Beta-Features#operation/UpdateInstanceAuthConfig){{ target: '_blank' }}.
...
Once Clerk delivery is disabled, in order to handle the delivery of a email or SMS message, you will need to enable [webhooks](/docs/guides/configure/webhooks/overview) and listen for the `email.created` or `sms.created` events, respectively.
