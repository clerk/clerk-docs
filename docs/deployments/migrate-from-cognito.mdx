...
> [!CAUTION]
> As usual, rate limits apply to the [`CreateUser`](/docs/reference/backend-api/tag/users/post/users){{ target: '_blank' }} endpoint. For development instances, a rate limit rule of **100 requests per 10 seconds** is applied. For production instances, that limit goes up to **1000 requests per 10 seconds**. For more information, see the [guide on rate limits](/docs/backend-requests/resources/rate-limits#backend-api-requests).
