...
2. Find your secret keys. If you're signed into Clerk, the `.env.local` snippet below will contain your secret key. Otherwise:
...
<CodeBlockTabs type="router" options={["App Router", "Pages Router"]}>
    ```tsx filename="/src/app/layout.tsx" {1,12-14}
...
```tsx filename="pages/index.tsx" {1,9}
import { UserButton } from "@clerk/nextjs";
...
Finally, use the `<UserButton />` by selecting your avatar, and select **Sign out**. You should be redirected to the url you specified with `afterSignOutUrl`.
