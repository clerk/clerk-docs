...
Current user data is important for data enrichment. The `currentUser()` local fetch the current user's [Backend `User`](/docs/reference/backend/types/backend-user){{ target: '_blank' }} object, which includes all of the user's information and provides a set of methods to manage their account.
...
For more information on `currentUser()`, see the [reference](/docs/reference/nextjs/app-router/current-user){{ target: '_blank' }}.
