...
>
  ...
  <AccordionPanel>
    The `avatarUrl` property of any [`ExternalAccount` object](/docs/reference/types/external-account) has been changed to `imageUrl`.
  </AccordionPanel>
...
>
  ...
  <AccordionPanel>
    The param `redirect_url` of [`User.createExternalAccount`](/docs/reference/objects/user#create-external-account) has been updated to `redirectUrl`. This is a simple text replacement without any signature changes.
  </AccordionPanel>

  <AccordionPanel>
    The `generatedSignature` param to [`Signup.attemptWeb3WalletVerification()`](/docs/reference/objects/sign-up#attempt-web3-wallet-verification) has been removed. Instead, use the `signature` param. Note that this param takes a string, where the `generatedSignature` param took a function, so both key and value will need to change.
...
>
  ...
  <AccordionPanel>
    The `profileImageUrl` property of any [`User` object](/docs/reference/objects/user) or [`OrganizationMembershipPublicData` object](https://github.com/clerk/javascript/blob/37f36e538d8879981f76f4a433066e057afb06de/packages/backend/src/api/resources/OrganizationMembership.ts#L31) has been changed to `imageUrl`.
  </AccordionPanel>

  <AccordionPanel>
    The `getOrganizationMemberships` [method on the `Clerk` class](/docs/reference/objects/clerk) has been removed. Instead, use `getOrganizationMemberships` on a user instance.
  ...
  <AccordionPanel>
    If you are using [`Clerk.addListener`](/docs/reference/objects/clerk#add-listener) or `OrganizationContext` and utilizing either the `lastOrganizationInvitation` and/or `lastOrganizationMember` emitted events, these properties have been removed, as they were only relevant for internal use. We are not providing a way to backfill this functionality at the moment. If you are using it and this is an issue for you, [contact support](https://clerk.com/contact/support){{ target: '_blank' }} and we can find a way to resolve the issue!
  </AccordionPanel>
