...
<Steps>
  ...
  <Tabs items={['Dashboard', 'Backend API']}>
    ...
    <Tab>
      ...
      The above request will update the domain to use the proxy URL `https://app.dev/__clerk`. In doing so, it will trigger checks to validate the proxy URL. If the proxy URL is invalid, the request will fail.
    </Tab>
  </Tabs>
