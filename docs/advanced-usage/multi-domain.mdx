...
The primary domain is the source of truth for the user's authentication state. Users must complete the sign-in flow on this domain, either using Clerk’s `<SignIn/>` component or our `useSignIn()` hook.
...
### Ready to go 🎉 - What to expect

Your satellite application should now sync with your primary! 

Let's run through a hypothetical scenario.
* User visits the primary domain and signs in.
* User visits the satellite domain.
* User has a session in the satellite domain, so he can see the `<UserProfile />` component and update his information.
