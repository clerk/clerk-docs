...
<Steps>
  ...
  When TanStack Start's `srvx` dependency automatically updates to version `0.8.16`, it introduces changes to cookie handling that can lead to the following error:

  ```
  TypeError: Cannot read properties of undefined (reading 'get') at AuthenticateContext.getCookie
  ```

  To fix this error, override Tanstack's `srvx` version and manually install the version before that change:

  ```npm
  npm install srvx@0.8.15
  ```
