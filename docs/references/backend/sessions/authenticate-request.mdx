...
```tsx
function authenticateRequest: (request: Request, options: AuthenticateRequestOptions) => Promise<ClerkBackendApiResponse<RequestState>>;
```
...
| `afterSignInUrl?` | `string` | The URL to navigate after sign-in completion. |
