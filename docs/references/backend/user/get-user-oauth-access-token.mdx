...
> [!IMPORTANT]
> When using custom OAuth providers, prefix the provider name with `oauth_custom_`. For example, if your custom provider is named "whoop," use `oauth_custom_whoop` as the provider parameter. For standard providers, use the `oauth_` prefix (e.g., `oauth_google`). Refer to the [OAuth Types](/docs/references/javascript/types/oauth#oauthstrategy) for type definitions.
