...
The **Security** tab appears only when [self-serve SSO](/docs/guides/configure/auth-strategies/enterprise-connections/self-serve-sso) is enabled for the Organization in the Clerk Dashboard. From this tab, admins with the `org:sys_enterprise_connections` permission can configure and manage the Organization's enterprise SSO connection.
