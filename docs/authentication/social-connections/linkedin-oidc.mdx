...
To make the development flow as smooth as possible, Clerk uses preconfigured shared OAuth credentials and redirect URIs for *development instances* - no other configuration is needed. Navigate to the Clerk Dashboard and go to **User & Authentication -> [Social Connections](https://dashboard.clerk.dev/last-active?path=user-authentication/social-connections)**. Scroll to find the LinkedIn option, toggle it on, and click **Save**.
