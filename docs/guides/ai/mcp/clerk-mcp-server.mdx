...
## Set up with the Clerk CLI

The fastest way to connect is with the [Clerk CLI](/docs/cli). Run the following command to detect supported AI clients installed on your machine and register Clerk's MCP server:
...
```sh {{ filename: 'terminal' }}
# Register in specific clients, or every detected client, without prompting
clerk mcp install --client claude --client cursor
clerk mcp install --all

# Show every Clerk MCP entry the CLI has registered
clerk mcp list
...
<Tabs items={["Claude", "Codex", "Cursor", "fx", "VSCode", "Windsurf", "Zed", "Other clients"]}>
  <Tab>
    ...
    <Tabs items={["Claude Code", "Claude.ai", "Claude Desktop"]}>
      ...
      <Tab>
        1. In Claude Desktop, navigate to **Settings** → **Developer**, then select **Edit Config** to open the `claude_desktop_config.json` file.
  ...
  <Tab>
    ### Connecting fx to Clerk's MCP server

    [fx](https://fx.sh) is Vercel's open-source coding agent for the terminal.

    <Tabs items={["Configuration via CLI", "Configuration through the config file"]}>
      <Tab>
        1. Run the following command in the terminal:
           ```sh {{ filename: 'terminal' }}
           fx mcp add --transport http clerk https://mcp.clerk.com/mcp
           ```
           This saves the server to `~/.fx/mcp.json`. The same command is available inside an fx session as `/mcp add`.
        1. Run `fx mcp list --connect` to verify the connection.
      </Tab>

      <Tab>
        1. Open the `~/.fx/mcp.json` config file in your preferred editor. To confirm the exact file location, run `/mcp path` in an fx session.
        1. Add the following configuration:
           ```json
           {
             "mcp": {
           ...
           ```
        1. Run `/mcp reload` in an fx session to apply the change without restarting fx.
      </Tab>
    </Tabs>
  </Tab>
  ...
  <Tab>
    ...
    1. Depending on your preferred transport method, follow the instructions below:
       ...
       <Tabs items={["HTTP", "Stdio"]}>
         <Tab>
           Enter the following URL and hit enter: `https://mcp.clerk.com/mcp`
         </Tab>
