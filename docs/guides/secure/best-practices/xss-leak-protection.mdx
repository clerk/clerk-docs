...
Clerk works to minimize the surface area by using `HttpOnly` cookies for authenticated requests to our [Frontend API](/docs/references/frontend-api){{ target: '_blank' }}, so that credentials cannot be leaked during XSS attacks.
