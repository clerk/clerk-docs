...
<Tabs items={["Next.js", "Expo"]}>
  <Tab>
    ...
    - The Server Action that generates the actor token using [Clerk's Backend API](/docs/reference/backend-api/tag/Actor-Tokens#operation/CreateActorToken)
  ...
  <Tab>
    ...
    <Steps>
      ...
      With your hook setup you can now create an API route that will call Clerk's [retrieve user endpoint](/docs/reference/backend-api/tag/Users#operation/GetUserList) at `/users` and get back the Impersonated user's full [`User` object](/docs/references/javascript/user/user#user-object).
