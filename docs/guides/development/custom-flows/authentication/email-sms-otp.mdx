...
<Steps>
  ...
  <If notSdk={["ios", "android"]}>
    ...
    1. Initiate the sign-up process by collecting the user's phone number with the [`signUp.create()`](/docs/reference/objects/sign-up-future#create) method.
    1. Send a one-time code to the provided phone number for verification with the [`signUp.verifications.sendPhoneCode()`](/docs/reference/objects/sign-up-future#verifications-send-phone-code) method (or [`signUp.verifications.sendEmailCode()`](/docs/reference/objects/sign-up-future#verifications-send-email-code) if you're modifying this example for email OTP).
    1. Collect the one-time code and verify it with the [`signUp.verifications.verifyPhoneCode()`](/docs/reference/objects/sign-up-future#verifications-verify-phone-code) method (or [`signUp.verifications.verifyEmailCode()`](/docs/reference/objects/sign-up-future#verifications-verify-email-code) if you're modifying this example for email OTP).
       <Include src="_partials/custom-flows/phone-number" />
    1. If the phone number verification is successful, finalize the sign-up with the [`signUp.finalize()`](/docs/reference/objects/sign-up-future#finalize) method to create the user and set the newly created session as the active session.
    ...
    <If notSdk="expo">
      ...
      ```tsx {{ filename: 'src/app/sign-up/page.tsx', collapsible: true }}
      'use client'
  ...
  <If notSdk={["ios", "android"]}>
    ...
    1. Initiate the sign-in process by collecting the user's phone number with the [`signIn.phoneCode.sendCode()`](/docs/reference/objects/sign-in-future#phone-code-send-code) method.
    1. Send a one-time code to the provided phone number for verification with the [`signIn.phoneCode.verifyCode()`](/docs/reference/objects/sign-in-future#phone-code-verify-code) method (or [`signIn.emailCode.verifyCode()`](/docs/reference/objects/sign-in-future#email-code-verify-code) if you're modifying this example for email OTP).
    1. Collect the one-time code and verify it with the [`signIn.phoneCode.verifyCode()`](/docs/reference/objects/sign-in-future#phone-code-verify-code) method (or [`signIn.emailCode.verifyCode()`](/docs/reference/objects/sign-in-future#email-code-verify-code) if you're modifying this example for email OTP).
       <Include src="_partials/custom-flows/phone-number" />
    1. If the phone number verification is successful, finalize the sign-in with the [`signIn.finalize()`](/docs/reference/objects/sign-in-future#finalize) method to set the newly created session as the active session.
