...
Session tasks are pending requirements that users must complete after authentication, such as selecting an organization. When enabled in the Clerk Dashboard, these tasks are handled automatically within the `<SignIn />` and `<SignUp />` components.

## Available tasks

Each session task is identified by a unique [`SessionTask['key']`](/docs/references/javascript/types/session-task). You can use these task keys to conditionally handle different requirements in your application logic.

The following table lists the available session tasks and their corresponding keys.
...
Once enabled in the Clerk Dashboard, session tasks are **embedded by default** within the `<SignUp />` and `<SignIn />` components. For more customization, you can opt out of using the `<SignUp />` and `<SignIn />` components and [create custom pages for your session tasks](/docs/custom-flows/overview#session-tasks).

The following table lists the available session tasks and their corresponding components. See the linked reference guide for usage instructions.
...
<CodeBlockTabs options={["React", "JavaScript"]}>
  ...
  if (session?.currentTask) {
    ...
    // to complete additional tasks. You can display custom UI to help
    // users resolve these tasks, see https://clerk.com/docs/custom-flows/overview#session-tasks
...
When users have pending session tasks, you can redirect them to specific pages or components to complete these requirements. This is useful for creating custom user experiences or when you want to handle session tasks outside of the default `<SignIn />` and `<SignUp />` components.
