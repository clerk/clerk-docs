...
<Properties>
  ...
  - `({ complete, cancel, level }: [NeedsReverificationParameters](NeedsReverificationParameters)) => void`
...
<Properties>
  - `{ complete: () => void, cancel: () => void, level: SessionVerificationLevel | undefined }`
