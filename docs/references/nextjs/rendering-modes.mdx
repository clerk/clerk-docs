...
`useAuth()` provides authentication data in Client Components. Due to Next.js's default behavior, these components will be statically rendered. If that's fine with you, see the [reference docs](/docs/reference/general/hooks/use-auth) for more information, like code examples.
