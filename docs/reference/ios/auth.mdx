...
Use the Clerk iOS SDK to handle authentication flows. Choose the method that best fits your UX.

## Sign in

### Sign in with Password

Sign in directly with an identifier and password:
...
```swift
var result = try await clerk.auth.signInWithIdToken(idToken, provider: .apple)
```
...
signIn = try await signIn.verifyMfaCode("123456", type: .phoneCode)
```
...
signIn = try await signIn.verifyMfaCode("123456", type: .emailCode)
```
...
```swift
signIn = try await signIn.verifyMfaCode("123456", type: .totp)
```
...
```swift
signIn = try await signIn.verifyMfaCode("backup123", type: .backupCode)
```
...
signIn = try await signIn.verifyCode("123456")
signIn = try await signIn.resetPassword(newPassword: "newpassword", signOutOfOtherSessions: true)
```
...
```swift
var token = try await clerk.auth.getToken()
```
