...
Both `auth()` and `getAuth()` return an `Authentication` object. This JavaScript object contains important information like session data, your user's ID, as well as their organization ID. Learn more about the `Authentication` object [here](/docs/references/nextjs/authentication-object).
