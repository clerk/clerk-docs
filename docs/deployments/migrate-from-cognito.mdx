...
the [Backend API](/docs/references/backend-api/tag/Users#operation/CreateUser){{ target: '_blank' }}.
...
[^1]: [`CreateUser`](/docs/references/backend-api/tag/Users#operation/CreateUser){{ target: '_blank' }} has a rate limit rule of 20 requests per 10 seconds.
...
<Steps>
  ...
  update your application to use Clerk’s managed [Account Portal](/docs/guides/customizing-clerk/account-portal) or the [`<SignIn />`](/docs/references/general/components/authentication/sign-in) component.
