...
<Properties>
  ...
  - [`has()`](#has)
  - <code>(isAuthorizedParams: [CheckAuthorizationParamsWithCustomPermissions](#checkauthorizationparamswithcustompermissions)) => boolean</code>
...
<Properties>
  ...
  - `reverification?`
  - <code>[ReverificationConfig](#reverificationconfig)</code>
...
<If is="llm">
  **For AI agents:** Calling `getToken({ template })` requires that custom JWT template to already exist — instead of creating it in the Dashboard, run `npx clerk@latest api jwt_templates` to list existing templates, or pass `-d` with the template JSON to create one. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
- For JavaScript applications, see the [reference documentation](/docs/reference/objects/session#gettoken) for example usage.
...
<If sdk="astro">
  ```ts {{ filename: 'src/pages/api/example.ts' }}
