...
<Steps>
  ### Add `PLASMO_PUBLIC_SYNC_HOST` to your environment variables

  The `PLASMO_PUBLIC_SYNC_HOST` environment variable defines the host that the Chrome Extension will sync with.

  The values for `PLASMO_PUBLIC_SYNC_HOST` will differ between development and production environments. Using separate `.env.development` and `.env.production` files allows you to seamlessly pass the appropriate values to your builds.

  Use the following tabs to view the instructions for development versus production instances.

  <Tabs items={["Development", "Production"]}>
    <Tab>
      Add `PLASMO_PUBLIC_SYNC_HOST` to your `.env.development` file. The value should be `http://localhost`.

      ```env {{ filename: '.env.development', mark: [3] }}
      PLASMO_PUBLIC_CLERK_PUBLISHABLE_KEY={{pub_key}}
      CLERK_FRONTEND_API={{fapi_url}}
      PLASMO_PUBLIC_SYNC_HOST=http://localhost
      ```
    </Tab>

    <Tab>
      Add `PLASMO_PUBLIC_SYNC_HOST` to your `.env.production` file. The value should be the domain your web app's production server runs on. For example, `https://clerk.com`.
  ...
  if (!PUBLISHABLE_KEY || !SYNC_HOST) {
    throw new Error(
      'Please add the PLASMO_PUBLIC_CLERK_PUBLISHABLE_KEY and PLASMO_PUBLIC_SYNC_HOST to the .env.development file',
    )
  }
  ...
  In the `package.json` file, in the `manifest` object, update the `host_permissions` array. Remove `http://localhost/*` and replace with `$PLASMO_PUBLIC_SYNC_HOST/*`, as shown in the following example:
  ...
  ```json {{ filename: 'package.json', mark: [6] }}
  {
    ...
    "manifest": {
      ...
      "host_permissions": ["$PLASMO_PUBLIC_SYNC_HOST/*", "$CLERK_FRONTEND_API/*"]
    }
  }
  ```
  ...
  > [!NOTE]
  > If you have not [configured a consistent key](/docs/references/chrome-extension/configure-consistent-key), you will have to repeat this step every time your extension's ID changes.
  ...
  <SignedOut>
    1. In the [Clerk Dashboard](https://dashboard.clerk.com/last-active?path=api-keys), navigate to the **API Keys** page.
