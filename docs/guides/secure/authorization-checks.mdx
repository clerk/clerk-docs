...
- The [`<Protect>`](/docs/references/general/components/control/protect) component: prevents content from rendering if the user is unauthorized.
...
The [`<Protect>`](/docs/references/general/components/control/protect) component prevents content from rendering if the user does not have the correct access control. If they aren't authorized, you can pass a fallback UI to the `fallback` prop. Under the hood, it uses the [`has()`](/docs/references/backend/types/auth-object#has) helper so it can only check for custom permissions. It can be used both client-side and server-side (in Server Components).
