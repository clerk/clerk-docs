...
When a session has a pending task (accessible via [`session.currentTask`](/docs/references/javascript/session#properties)), calling `navigateToTask()` will automatically redirect users to default route paths.

Here's the mapping of default paths for each task type:
...
<Steps>
  ### Build custom page to complete task

  Create a custom page that completes the user's pending session task. For this example, we'll build a page where users can create and select an organization:

  <Tabs items={["Next.js", "JavaScript"]}>
    <Tab>
      ```tsx {{ filename: 'app/tasks/add-organization/page.tsx', mark: [[17, 21]], collapsible: true }}
      'use client'
        ...
        const handleSubmit: FormEventHandler<HTMLFormElement> = async (e) => {
          ...
          createOrganization({ name: organizationName })
            .then((organization) => {
              ...
              // Check for and navigate to any pending tasks after selecting the organization
    ...
    <Tab>
      <CodeBlockTabs options={["index.html", "main.js"]}>
        ...
        ```js {{ filename: 'main.js', mark: [[28, 32]], collapsible: true }}
  ...
  ### Handle `pending` sessions after sign-up

  <Tabs items={["Next.js", "JavaScript"]}>
    <Tab>
      This example is written for Next.js App Router but it can be adapted for any React-based framework.

      ```tsx {{ filename: 'app/sign-up/[[...sign-up]]/page.tsx', mark: [[28, 31]], collapsible: true }}
      'use client'
        ...
        const handleSubmit = async (e: React.FormEvent) => {
          ...
          try {
            ...
            if (signUpAttempt.status === 'complete') {
              ...
              // Navigate to page where tasks will get completed
  ...
  <Tabs items={["Next.js", "JavaScript"]}>
    <Tab>
      ...
      ```tsx {{ filename: 'app/sign-in/[[...sign-in]]/page.tsx', mark: [[28, 31]], collapsible: true }}
      'use client'
      ...
      <CodeBlockTabs options={["index.html", "main.js"]}>
        ...
        if (clerk.isSignedIn) {
          ...
          document.getElementById('sign-in-form').addEventListener('submit', async (e) => {
            ...
            try {
              // Start the sign-in process using the email and password provided
              const signInAttempt = await clerk.client.signIn.create({ emailAddress, password })
