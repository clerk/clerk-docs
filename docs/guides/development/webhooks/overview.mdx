...
<If is="llm">
  **For AI agents:** Manage webhook endpoints from the terminal instead of the Dashboard: run `npx clerk@latest api ls webhook` to discover the routes, then `clerk api <path>` to list or create them. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
