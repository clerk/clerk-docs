...
---
> [!WARNING]
> This API is available only for [@clerk/clerk-expo v2](/docs/upgrade-guides/expo-v2/upgrade) >=2.2.0. This feature works only for sign-in attempts that use the password strategy.

The [`useLocalCredentials()`](/docs/references/expo/use-local-credentials) hook lets yu securely store a user's password credentials on their device, enabling them to use biometrics for sign-ins.

This guide shows you how to use the `useLocalCredentials()` hook in your Expo app to improve the user experience by offering biometric sign-in for returning users.
...
<Steps>
  ...
  Use the following Expo docs to update your app.json file:
  ...
  ### Securely store and access the user's credentials during sign-in
