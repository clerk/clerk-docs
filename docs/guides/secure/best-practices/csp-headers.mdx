...
1. `connect-src` - This value should include the host application's FAPI hostname, such as `https://clerk.your-domain.com`, and Clerk's abuse and fraud protection hosts through `https://*.protect.clerk.com`.
...
> [!WARNING]
> Automatic CSP configuration is available only for `@clerk/nextjs >=6.14.0`. Upgrade to `@clerk/nextjs >=7.5.21` to include Clerk's abuse and fraud protection domains automatically.
