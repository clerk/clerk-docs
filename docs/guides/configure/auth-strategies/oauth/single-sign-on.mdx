...
> [!NOTE]
> For JWT-formatted tokens (the default), introspection is not available and will return an error. JWTs are self-contained and do not require introspection to validate.
>
> If you are using [opaque tokens](!opaque-token), you can use this endpoint to validate the token and retrieve additional token metadata.
