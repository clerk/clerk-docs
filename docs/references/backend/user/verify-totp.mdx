---
title: verifyTOTP()
description: Use Clerk's Backend SDK to verify a TOTP or backup code for a user.
---

{/* clerk/javascript file: https://github.com/clerk/javascript/blob/main/packages/backend/src/api/endpoints/UserApi.ts#L236 */}

# `verifyTOTP()`

Verify that the provided TOTP or backup code is valid for the user. Verifying a backup code will result it in being consumed (i.e. it will become invalid). Useful for custom auth flows and re-verification.

```tsx {{ prettier: false }}
function verifyTOTP: (params: VerifyTOTPParams) => Promise<{ verified: true; code_type: 'totp' }>;
```

## `verifyTOTPParams`

| Name | Type | Description |
| - | - | - |
| `userId` | `string` | The ID of the user to verify the TOTP for. |
| `code` | `string` | The TOTP or backup code to verify |

## Example

```tsx {{ prettier: false }}
const response = await clerkClient.users.verifyTOTP({
  userId: "user_123",
  code: "123456"
})

console.log(response);
/*
{
  verified: true,
  code_type: 'totp'
}
*/
```

## Backend API (BAPI) endpoint

This method in the SDK is a wrapper around the BAPI endpoint `POST/users/{user_id}/verify_totp`. See the [BAPI reference](/docs/reference/backend-api/tag/Users#operation/VerifyTOTP) for more details.
