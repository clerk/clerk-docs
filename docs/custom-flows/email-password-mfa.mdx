---
title: Build a custom multi-factor sign-in flow
description: Learn how to build a custom email/password sign-in flow that requires multi-factor authentication (MFA).
---

# Build a custom multi-factor sign-in flow

<Callout type="danger">
  This guide is for users who want to build a *custom* user interface using the Clerk API. To sign in users with multi-factor authentication (MFA) using a *prebuilt* UI, you should use Clerk's [Account Portal pages](/docs/account-portal/overview) or Clerk's [`<SignIn />`](/docs/components/authentication/sign-in) and [`<SignInButton />`](/docs/components/unstyled/sign-in-button) components.
</Callout>
...
When MFA is enabled in an application, Clerk's [`<SignIn />`](/docs/components/authentication/sign-in) and [`<SignInButton />`](/docs/components/unstyled/sign-in-button) components will automatically prompt the user to complete the second factor verification, if they have MFA enabled for their account. However, if you're building a custom sign-in flow, you'll need to handle the second factor verification yourself.

This guide will walk you through how to build a custom email/password sign-in flow that requires multi-factor authentication (MFA) using the [`useSignIn()`](/docs/references/react/use-sign-in) hook.
...
For the sake of this guide, users will sign in using their email and password before being prompted to complete the second factor verification.
...
3. In the **Contact information** section, enable **Email address** and **Password**.
...
1. Navigate to your Clerk Dashboard.
2. Select your application, then select **User & Authentication > [Multi-factor](https://dashboard.clerk.com/last-active?path=user-authentication/multi-factor)** in the sidebar menu.
3. For the sake of this guide, toggle on the **Backup codes** and **Authenticator application** strategy.
...
6. If the verification is successful, set the newly created session as active.
