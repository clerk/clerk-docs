...
There are two ways to limit access to users with active organizations only:
  - [Limit access using the `authMiddleware()` helper](#limit-access-using-the-auth-middleware-helper)
...
Clerk's [`authMiddleware()`](/docs/references/nextjs/auth-middleware) helper can be used in both App Router and Pages Router applications to limit access to users with active organizations only.
