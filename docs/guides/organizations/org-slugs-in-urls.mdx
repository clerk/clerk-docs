...
<Steps>
  ...
  The [`<OrganizationSwitcher />`](/docs/reference/general/components/organization/organization-switcher) and [`<OrganizationList />`](/docs/reference/general/components/organization/organization-list) components provide a robust set of options to manage organization slugs and IDs in your application's URLs.
  ...
  <Tabs items={["Server-side","Client-side"]}>
    ...
    <Tab>
      To get organization information on the client-side, use the [`useOrganization()`](/docs/reference/general/hooks/use-organization) hook to access the [`organization`](/docs/references/javascript/organization) object.
