...
If not canceled, the customer is charged using the default card on file (which may differ from the card used at signup).
...
You have full control over active trials:

- **Cancel with access**: Cancel the trial while allowing the customer to keep access until the trial period ends
