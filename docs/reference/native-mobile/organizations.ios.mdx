...
To switch Organizations, call `clerk.auth.setActive(sessionId:organizationId:)`. Pass `organizationId: nil`, or omit it, to select the [Personal Account](!personal-account). Selecting a Personal Account isn't allowed when `clerk.environment?.organizationSettings.forceOrganizationSelection` is `true`.
