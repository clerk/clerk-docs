...
<Properties>
  ...
  **Chrome Extension only** To enable, pass the URL of the web application that the extension will sync the authentication state from. See the [dedicated guide](/docs/references/chrome-extension/sync-host) for more information.
  ...
  The URL paths users are redirected to after sign-up or sign-in when specific session tasks need to be completed. For example, `{ 'select-organization': '/onboarding/select-organization' }` redirects users to `/onboarding/select-organization` after sign-up if they need to select an organization.
