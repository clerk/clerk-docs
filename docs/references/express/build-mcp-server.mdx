...
<Steps>
  ...
  Add the following code to your `index.ts` file. This registers a single tool called `get_clerk_user_data` that retrieves information about the authenticated Clerk user.

  The `McpServer()` function is used to create a new MCP server with a name and version. The `server.tool()` function is used to define a tool that external LLM-based apps can invoke. Each tool includes:
