...
Some notes about the `unsafe-eval` and `unsafe-inline` directives:
...
- Within `script-src`, `unsafe-inline` is a requirement for next.js in both dev and prod environments if you're using app router. If you are using pages router, it should be removed.
- Within `style-src`, `unsafe-inline` is a requirement for Clerk's components. Removing this requirement is on our roadmap - please [reach out to support](https://clerk/.com/support) if you'd like to see this implemented sooner.