...
<Properties>
  ...
  The current `Session`, which is guaranteed to be one of the sessions in `Client.sessions`. If there is no current session, this field will be `null`. If the session is loading, this field will be `undefined`.
