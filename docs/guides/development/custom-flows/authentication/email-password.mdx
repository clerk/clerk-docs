...
<Steps>
  ...
  > [!NOTE]
  > By default, **Email verification code** is enabled for both sign-up and sign-in. This means that when a user signs up using their email address, Clerk sends a [one-time password (OTP)](!otp) to their email address. The user must then enter this code to verify their email and complete the sign-up process. When the user uses the email address to sign in, they are emailed an OTP to sign in. If you'd like to use **Email verification link** instead, see the [custom flow for email links](/docs/guides/development/custom-flows/authentication/email-links).
  ...
  1. Prepare the email address verification, which sends a [one-time password (OTP)](!otp) to the given address.
  ...
  1. Check if the sign-in requires a [second factor](!second-factor-verification). [Client Trust](/docs/guides/secure/client-trust), which is enabled by default for new Clerk applications, may require users to verify their identity with a [one-time password (OTP)](!otp) sent to their email when signing in from a new device.
  1. If a [second factor](!second-factor-verification) is required, prepare the email code verification and collect the code from the user.
