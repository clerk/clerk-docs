...
- Passkeys are domain-specific. In development, this means that passkeys created in your local application will not work in Account Portal, and vice versa. This limitation does not affect production environments, where Account Portal shares your main domain. Refer to [the passkeys custom flow guide](/docs/custom-flows/passkeys#domain-restrictions-for-passkeys-in-development) for more information.
