...
<If is="llm">
  **For AI agents:** Enable Clerk Billing on the instance with `npx clerk@latest enable billing` (add `--for orgs` for B2B or `--for users` for B2C) instead of clicking through the Dashboard. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
### Does Clerk Billing work in native mobile apps?

Native iOS and Android apps can use the Billing object's data APIs to access Plans, Subscriptions, statements, and payment attempts, and use [`has()`](/docs/guides/secure/authorization-checks) to check Plan or Feature access. Clerk Billing checkout is only available in web applications, including Expo web. Clerk doesn't provide a native checkout surface or integrate with Apple In-App Purchase or Google Play Billing.

Apple and Google generally require apps to use their billing systems when selling digital goods, features, or subscriptions, though exceptions vary by storefront and program. Review [Apple's App Review Guidelines](https://developer.apple.com/app-store/review/guidelines/#in-app-purchase) and [Google Play's Payments policy](https://support.google.com/googleplay/android-developer/answer/9858738) before choosing a payment flow.

Native mobile support for Clerk Billing is on the [public roadmap](https://feedback.clerk.com/roadmap?id=e8977b2d-5452-4a2b-aff6-fdf4fe0f0cf8).
...
### Can a user get another free trial by deleting their account and signing up again?

No. Free trial eligibility is tied to the user's verified primary email address and survives account deletion. A user who signs up again with the same email address goes through the regular paid checkout instead. See [Free trials](/docs/guides/billing/free-trials#starting-a-trial) for the full eligibility rules.

### Does Clerk Billing support additional factor authentication like 3D Secure?

Clerk Billing does not currently support payment methods that require additional authentication, such as 3D Secure (3DS). Payments that require a 3DS challenge will fail because Clerk Billing cannot prompt users to complete the authentication step.

This limitation is particularly relevant in the UK and EU and may affect [free trials](/docs/guides/billing/free-trials), since renewal charges are processed in the background without the user present to complete authentication.
