...
> [!NOTE]
> Clerk Billing does not currently support additional factor authentication such as 3D Secure (3DS), which is commonly required by banks in the UK and EU. Even if you disable the payment method requirement so users can start a trial without entering payment details, renewals at the end of the trial will still fail if the user's payment method requires a 3DS challenge — because the charge happens in the background with no user present to complete it. See the [Billing overview](/docs/guides/billing/overview#does-clerk-billing-support-additional-factor-authentication-like-3-d-secure) for details.
