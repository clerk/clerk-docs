...
<Tabs items={["Next.js", "iOS", "Android"]}>
  ...
  <Tab>
    ```kotlin {{ filename: 'AddEmailView.kt', collapsible: true }}
    @Composable
    fun AddEmailView(modifier: Modifier = Modifier, viewModel: EmailAddressViewModel = viewModel()) {
      val state by viewModel.uiState.collectAsState()
      when (state) {
        EmailAddressViewModel.AddEmailUiState.NeedsVerification -> {
          InputContentView(buttonText = "Continue", onClick = { viewModel.createEmail(it) })
        }

        EmailAddressViewModel.AddEmailUiState.Verified -> {
          Text("Verified!")
        }

        EmailAddressViewModel.AddEmailUiState.Verifying -> {
          InputContentView(buttonText = "Verify") {
            viewModel.verifyCode(it, state.emailAddress)
          }
        }
      }
    }
    ...
    @Composable
    fun InputContentView(buttonText: String, modifier: Modifier = Modifier, onClick: (String) -> Unit) {
    ...
    class AddEmailViewModel: ViewModel() {
      private val _uiState = MutableStateFlow<AddEmailState>(AddEmailState.Idle)
      val uiState = _uiState.asStateFlow()

      fun createEmail(email: String) {
        val user = requireNotNull(Clerk.user)

        // Add an unverified emailaddress to the user,
        // then send the user an email with the verification code
        viewModelScope.launch {
            ...
            .onSuccess { 
              // Update the state to show that the email has been created
              // and that the user needs to verify the email
              _uiState.value = AddEmailState.NeedsVerification(it) 
            }
            .onFailure { Log.e("AddEmailViewModel", "Failed to create email: $it.longErrorMessageOrNull") }
        }
      }
      ...
      fun verifyCode(code: String, newEmailAddress: EmailAddress) {
        viewModelScope.launch {
          newEmailAddress
            ...
            .onSuccess { 
              // Update the state to show that the email has been verified
              _uiState.value = AddEmailState.Verified 
            }
            .onFailure { Log.e("AddEmailViewModel", "Failed to verify code: $it.longErrorMessageOrNull") }
        }
      }

      sealed interface AddEmailState {
        data object NeedsVerification: AddEmailState
        data class Verifying(val emailAddress: EmailAddress) : AddEmailState
        data object Verified : AddEmailState
      }
    }
    ```
  </Tab>
