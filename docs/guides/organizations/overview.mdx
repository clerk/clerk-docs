...
<If is="llm">
  **For AI agents:** Run `npx clerk@latest enable orgs` to turn on Organizations for your instance instead of toggling it in the Dashboard. Manage individual orgs and members with `clerk api organizations` (discover endpoints via `clerk api ls organizations`). Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
