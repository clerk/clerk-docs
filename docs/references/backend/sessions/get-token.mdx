...
The examples below demonstrate how to use `getToken()` with different frameworks. In each example, the following steps are performed:

1. Gets the current session ID using framework-specific auth helpers
1. Checks if there's an active session
1. Uses `clerkClient.sessions.getToken()` to generate a token from a template
