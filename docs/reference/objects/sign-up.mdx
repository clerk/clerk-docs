---
title: '`SignUp` object'
description: The SignUp object holds the state of the current sign-up and provides helper methods to navigate and complete the sign-up flow.
sdk: js-frontend, chrome-extension, expo, nextjs, react, react-router, tanstack-react-start
tag: legacy
---

> [!LEGACY]
> This page describes our legacy API, which will be removed in a future release. We recommend upgrading to [Core 3](/docs/guides/development/upgrading/upgrade-guides/core-3) and using the [`SignUpFuture`](/docs/reference/objects/sign-up-future) object instead.
...
See the [custom flow guides](/docs/guides/development/custom-flows/authentication/legacy/email-password) for comprehensive examples of using the `SignUp` object to build custom user interfaces with the Clerk API.
...
<Properties>
  ...
  The Web3 wallet address, made up of 0x + 40 hexadecimal characters or a `base58` encoded `ed25519` public key (for Solana wallets). Only supported if [Web3 authentication](/docs/guides/configure/auth-strategies/sign-up-sign-in-options#web3-authentication) is enabled in the instance settings.
  ...
  - `unsafeMetadata`
  - [`SignUpUnsafeMetadata`](/docs/reference/types/metadata#signupunsafemetadata)
  ...
  The epoch time when the user accepted the [legal compliance](/docs/guides/secure/legal-compliance) documents. `null` if not applicable or not yet accepted.
  ...
  The locale for the sign-up (e.g., from the browser or passed to `create()`). Used for localized emails and copy.
</Properties>
...
Attempts to verify an email address by validating the [OTP](!otp) provided by the user against the code sent during the prepare verification step. This is a convenience method that wraps [`SignUp.attemptVerification()`](/docs/reference/objects/sign-up#attemptverification) with the `'email_code'` strategy.
...
<Properties>
  ...
  The [OTP](!otp) that was sent to the user via email.
</Properties>

### `attemptPhoneNumberVerification()`

Attempts to verify a phone number by validating the [OTP](!otp) provided by the user against the OTP sent during the prepare verification step. This is a convenience method that wraps [`SignUp.attemptVerification()`](#attemptverification) with the `'phone_code'` strategy.
...
Attempts to complete a pending verification process for the specified verification strategy. This method must be called after initiating verification via [`SignUp.prepareVerification()`](#prepareverification). The verification attempt will validate the provided verification parameters (code, signature, etc.) against the pending verification request.
...
<Properties>
  - `strategy`
  - `'phone_code' | 'email_code' | 'web3_base_signature' | 'web3_metamask_signature' | 'web3_coinbase_wallet_signature' | 'web3_okx_wallet_signature' | 'web3_solana_signature'`
  ...
  - `'web3_solana_signature'`: The verification will attempt to be completed using the user's Web3 wallet address via their [Solana wallet provider](https://solana.com/solana-wallets). The `web3_wallet_id` parameter can also be specified to select which of the user's known Web3 wallets will be used.
  ...
  **Required** if `strategy` is set to `'phone_code'` or `'email_code'`. The [OTP](!otp) that was sent to the user.
...
Attempts to verify a Web3 wallet address by validating the cryptographic signature generated by the wallet against the nonce provided during the prepare verification step. This is a convenience method that wraps [`SignUp.attemptVerification()`](#attemptverification) with Web3 wallet strategies.
...
<Properties>
  ...
  The signature that was generated after [`prepareVerification`](#prepareverification) was called.
  ...
  The Web3 verification strategy. Defaults to `'web3_metamask_signature'` if omitted.
</Properties>
...
<Properties>
  ...
  Optional metadata to attach to the sign-in attempt. See [`SignUpUnsafeMetadata`](/docs/reference/types/metadata#signupunsafemetadata) for more information.
</Properties>
...
<Properties>
  ...
  The full URL or path that the OAuth provider should redirect to after successful authorization on their part. Typically, this will be a simple `/sso-callback` route that either calls [`Clerk.handleRedirectCallback`](/docs/reference/objects/clerk#handleredirectcallback) or mounts the [`<AuthenticateWithRedirectCallback />`](/docs/reference/components/control/authenticate-with-redirect-callback) component. See the [custom flow](/docs/guides/development/custom-flows/authentication/oauth-connections) for implementation details.
...
<Properties>
  ...
  - `generateSignature`
  - <code>(opts: [GenerateSignatureParams](#generatesignatureparams)) => Promise\<string></code>
...
> [!WARNING]
> Once the sign-up process is complete, pass the `createdSessionId` to the [`setActive()`](/docs/reference/objects/clerk#setactive) method on the `Clerk` object. This will set the newly created session as the active session.
...
<Properties>
  ...
  **Required** if [Web3 authentication](/docs/guides/configure/auth-strategies/sign-up-sign-in-options#web3-authentication) is enabled. The Web3 wallet address, made up of 0x + 40 hexadecimal characters or a `base58` encoded `ed25519` public key (for Solana wallets).
  ...
  If `strategy` is set to `'oauth_<provider>'` or `'enterprise_sso'`, this specifies full URL or path that the OAuth provider should redirect to after successful authorization on their part. Typically, this will be a simple `/sso-callback` route that either calls [`Clerk.handleRedirectCallback()`](/docs/reference/objects/clerk#handleredirectcallback) or mounts the [`<AuthenticateWithRedirectCallback />`](/docs/reference/components/control/authenticate-with-redirect-callback) component. See the [custom flow](/docs/guides/development/custom-flows/authentication/oauth-connections) for implementation details.
...
<Properties>
  - `startEmailLinkFlow`
  - <code>(params: [StartEmailLinkFlowParams](#startemaillinkflowparams)) => Promise\<SignUp></code>
...
Initiates an email verification process by sending a [OTP](!otp) to the email address associated with the current sign-up attempt. This is a convenience method that wraps [`SignUp.prepareVerification()`](#prepareverification) with the `'email_code'` strategy.
...
<Properties>
  ...
  - `'email_code'`: Send an email with an [OTP](!otp) to input.
...
Initiates a phone number verification process by sending an [OTP](!otp) via SMS to the phone number associated with the current sign-up attempt. This is a convenience method that wraps [`SignUp.prepareVerification()`](#prepareverification) with the `'phone_code'` strategy.
...
<Properties>
  ...
  - `'phone_code'`: Send an SMS with an [OTP](!otp) to input.
  ...
  - `'sms'`: Send an SMS with an [OTP](!otp) to input.
  - `'whatsapp'`: Send a WhatsApp message with an [OTP](!otp) to input.
</Properties>
...
<Properties>
  - `strategy`
  - `'phone_code' | 'email_code' | 'email_link' | 'oauth_<provider>' | 'enterprise_sso' | 'web3_base_signature' | 'web3_metamask_signature' | 'web3_coinbase_wallet_signature' | 'web3_okx_wallet_signature' | 'web3_solana_signature'`

  The verification strategy to validate the user's sign-up request. The following strategies are supported:

  - `'phone_code'`: User will receive an [OTP](!otp) via SMS.
  - `'email_code'`: Send an email with an [OTP](!otp) to input.
  - `'email_link'`: Send an email with a link which validates sign-up.
  - `'oauth_<provider>'`: The user will be authenticated with their [social connection account](/docs/guides/configure/auth-strategies/social-connections/overview). See a list of [supported values for `<provider>`](/docs/reference/types/sso).
  - `'enterprise_sso'`: The user will be authenticated either through SAML or OIDC depending on the configuration of their [enterprise SSO account](/docs/guides/configure/auth-strategies/enterprise-connections/overview).
  ...
  If `strategy` is set to `'oauth_<provider>'` or `'enterprise_sso'`, this specifies the full URL or path that the OAuth provider should redirect to after successful authorization. Typically, this will be a simple `/sso-callback` route that either calls [`Clerk.handleRedirectCallback()`](/docs/reference/objects/clerk#handleredirectcallback) or mounts the [`<AuthenticateWithRedirectCallback />`](/docs/reference/components/control/authenticate-with-redirect-callback) component. See the [custom flow](/docs/guides/development/custom-flows/authentication/oauth-connections) for implementation details.
...
Initiates a verification process for a Web3 wallet by sending the wallet address to the server and retrieving a nonce that must be cryptographically signed by the wallet. This is a convenience method that wraps [`SignUp.prepareVerification()`](#prepareverification) with Web3 wallet strategies.
...
Validates a password against the instance's password policy (e.g., minimum length, complexity). Uses the same rules as the Clerk Dashboard configuration. Useful for client-side validation before submitting a sign-up. Accepts a callback that receives validation errors.
...
`SignUpUpdateParams` is a mirror of [`SignUpCreateParams`](#signupcreateparams) with the same fields and types, depending on the configuration of the instance.
...
### `__experimental_getEnterpriseConnections()` <ExperimentalTag />

> [!EXPERIMENTAL]

Fetches the list of enterprise connections available for this sign-up. Returns an array of enterprise connection resources (e.g., for enterprise SSO).

```typescript
function __experimental_getEnterpriseConnections(): Promise<SignUpEnterpriseConnectionResource[]>
```
