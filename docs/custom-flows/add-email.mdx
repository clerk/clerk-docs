...
<Callout type="danger">
  Clerk's [User Profile page](/docs/account-portal/user-profile-org-profile#user-profile) and [`<UserProfile />`](/docs/components/user/user-profile) component allows users to add and verify new email addresses out of the box. This guide is for users who want to build a custom user interface using the Clerk API. 
</Callout>
