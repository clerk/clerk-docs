...
To store information about a user that Clerk doesn't collect, you can use metadata, which will get stored on the user's [`User` object](/docs/reference/objects/user).
...
To access public metadata on the frontend, it's available on the [`User`](/docs/reference/objects/user) object, which can be accessed using the [`useUser()`](/docs/reference/hooks/use-user) hook.
...
<Tabs items={["Client-side", "Server-side"]}>
  <Tab>
    Use the [`User.updateMetadata()`](/docs/reference/objects/user#updatemetadata) method to update unsafe metadata from the frontend. The provided value is **deep-merged** with the existing `unsafeMetadata`, and any key set to `null` is removed.
...
<If is="llm">
  **For AI agents:** Instead of adding claims in the Claims editor on the Sessions page in the Dashboard, run `npx clerk@latest config patch --json '{"session": {"claims": {...}}}'` to store metadata in the session token. You can also set a user's metadata directly with `npx clerk@latest api users/<user_id>/metadata -X PATCH -d '{"public_metadata": {...}}'` (values are merged with existing metadata). Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
