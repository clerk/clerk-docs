...
<If sdk={["astro", "vue", "nuxt", "js-frontend"]}>
  > [!IMPORTANT]
  > The custom flow documented in this section uses the `useOAuthConsent()` hook, which is only available for Clerk's React-based SDKs. For other SDKs, use the prebuilt [`<OAuthConsent />`](/docs/reference/components/authentication/oauth-consent) component, or build the consent flow with the low-level [`OAuthApplication`](/docs/reference/types/oauth-application) methods.
</If>
