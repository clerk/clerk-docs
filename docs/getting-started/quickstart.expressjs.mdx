...
<Steps>
  ...
  Node can load `.env` files natively. When running your app, use the `--env-file=.env` flag so environment variables can be accessed at import time. Alternatively, you can load environment variables programmatically using `process.loadEnvFile()` at the top of your entry file before importing any Clerk modules.
  ...
  ```ts {{ filename: 'index.ts', mark: [5, 10] }}
  // Load environment variables before importing Clerk
  process.loadEnvFile()
