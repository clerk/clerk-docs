...
`<CheckoutButton />` will throw an error if the `subscriberType` property is set to `'org'` and no [active organization](/docs/guides/organizations/overview#active-organization) is set.
