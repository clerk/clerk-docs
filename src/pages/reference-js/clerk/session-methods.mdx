...
<Tables
  ...
  <>A <code>Session</code> object or <code>Session ID</code> string to be set as the current session, or <strong>null</strong> to simply remove the active session, without setting a new one.</>,
  ],
  },
    {
  cells: [
  <code>beforeEmit?</code>,
  <code>(session: Session | null) {"=>"} Promise{"<any>"}</code>,
...
<Tables
  ...
  cells: [
  <code>beforeEmit</code>,
  <code>(session?: Session | null) {"=> void | Promise<any>"}</code>,
