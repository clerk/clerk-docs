...
<Callout type="danger">
  Clerk's [`<SignIn />`](/docs/components/authentication/sign-in) and [`<SignInButton />`](/docs/components/unstyled/sign-in-button) components handle multi-factor authentication (MFA) out of the box. This guide is for users who want to build a custom user interface using the Clerk API. 
</Callout>
