...
To switch Organizations, call `Clerk.auth.setActive(sessionId = sessionId, organizationId = organizationId)`. Pass `organizationId = null`, or omit it, to select the [Personal Account](!personal-account). Selecting a Personal Account isn't allowed when `Clerk.organizationSelectionIsForced` is `true`.
...
`OrganizationMembership` includes `hasPermission(...)` and convenience properties for [Clerk system permissions](/docs/guides/organizations/control-access/roles-and-permissions#system-permissions):
