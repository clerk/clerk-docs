---
title: The Backend `CommerceSubscriptionItem` object
description: The Backend CommerceSubscriptionItem object holds information about a subscription item of your application.
sdk: js-backend
---

<Include src="_partials/billing/api-experimental" />

The Backend `CommerceSubscriptionItem` object is similar to the [`CommerceSubscriptionItem`](/docs/references/javascript/types/commerce-subscription-item-resource) object as it holds information about a subscription item, as well as methods for managing it. However, the Backend `CommerceSubscriptionItem` object is different in that it is used in the [Backend API](TODO){{ target: '_blank' }} and is not directly accessible from the Frontend API.

## Properties

<Properties>
  - `amount`
  - <code>undefined | null | [CommerceMoneyAmount](/docs/references/javascript/types/commerce-money)</code>

  The current amount for the subscription item.

  ---

  - `canceledAt?`
  - `number`

  Unix timestamp (milliseconds) when the subscription item was canceled.

  ---

  - `id`
  - `string`

  The unique identifier for the subscription item.

  ---

  - `lifetimePaid?`
  - <code>null | [CommerceMoneyAmount](/docs/references/javascript/types/commerce-money)</code>

  The lifetime amount paid for the subscription item.

  ---

  - `nextPayment`
  - `null | { amount: number; date: number; }`

  The next payment information.

  ---

  - `pastDueAt?`
  - `number`

  Unix timestamp (milliseconds) when the subscription item became past due.

  ---

  - `periodEnd?`
  - `number`

  Unix timestamp (milliseconds) when the current period ends.

  ---

  - `periodStart`
  - `number`

  Unix timestamp (milliseconds) when the current period starts.

  ---

  - `plan`
  - <code>[CommercePlan](/docs/references/backend/types/backend-commerce-plan)</code>

  The plan associated with this subscription item.

  ---

  - `planId`
  - `string`

  The plan ID.

  ---

  - `planPeriod`
  - `'month' | 'annual'`

  The plan period for the subscription item.

  ---

  - `status`
  - `'active' | 'past_due' | 'upcoming' | 'canceled' | 'ended' | 'expired' | 'incomplete' | 'abandoned'`

  The status of the subscription item.
</Properties>
