...
1. Optionally, if you have enabled [multi-factor (MFA)](/docs/guides/configure/auth-strategies/sign-up-sign-in-options) for your application, you will need to prepare the [second factor verification](!second-factor-verification) by calling [`SignIn.prepareSecondFactor()`](#prepare-second-factor).
1. Attempt to complete the second factor verification by calling [`SignIn.attemptSecondFactor()`](#attempt-second-factor).
1. If verification is successful, set the newly created session as the active session by passing the `SignIn.createdSessionId` to the [`setActive()`](/docs/reference/objects/clerk#set-active) method on the `Clerk` object. You may need to check for [session tasks](!session-tasks) before the session can be set as active.
...
<Properties>
  ...
  - `'needs_client_trust'`: The user is signing in from a new device and must complete a [second factor verification](!second-factor-verification) to establish [Client Trust](/docs/guides/secure/client-trust). See the [Client Trust custom flow guide](/docs/guides/development/custom-flows/authentication/client-trust) for more information.
  ...
  **Optional** if the `strategy` is set to `'oauth_<provider>'` or `'enterprise_sso'`. **Required** otherwise. The authentication identifier value for the current sign-in.
  ...
  The [Client Trust](/docs/guides/secure/client-trust) state for the sign-in when the user is signing in from a new device. Populated when the sign-in requires establishing client trust (e.g., status is `'needs_client_trust'`).
</Properties>
...
<Properties>
  - `strategy`
  - <code>[OAuthStrategy](/docs/reference/types/sso#o-auth-strategy) | 'enterprise\_sso'</code>

  The strategy to use for authentication. The following strategies are supported:

  - [`OAuthStrategy`](/docs/reference/types/sso#o-auth-strategy): The user will be authenticated with their [social connection account](/docs/guides/configure/auth-strategies/social-connections/overview).
  ...
  The full URL or path that the OAuth provider should redirect to, on successful authorization on their part. Typically, this will be a simple `/sso-callback` route that calls [`Clerk.handleRedirectCallback`](/docs/reference/objects/clerk#handle-redirect-callback) or mounts the [`<AuthenticateWithRedirectCallback />`](/docs/reference/components/control/authenticate-with-redirect-callback) component. See the [custom flow](/docs/guides/development/custom-flows/authentication/oauth-connections) for implementation details.
...
<Properties>
  ...
  The Web3 provider to generate the signature with.  See [`Web3Provider`](/docs/reference/types/web3-wallet#web3-provider) for more information.
...
> [!WARNING]
> Once the sign-in process is complete, pass the `createdSessionId` to the [`setActive()`](/docs/reference/objects/clerk#set-active) method on the `Clerk` object. This will set the newly created session as the active session.
...
<Properties>
  ...
  **Required** if `strategy` is set to `'ticket'`. The [ticket _or token_](/docs/guides/development/custom-flows/authentication/embedded-email-links#generate-a-sign-in-token) generated from the Backend API.
  ...
  If `strategy` is set to `'oauth_<provider>'` or `'enterprise_sso'`, this specifies the full URL or path that the OAuth provider should redirect to after successful authorization on their part. Typically, this will be a simple `/sso-callback` route that either calls [`Clerk.handleRedirectCallback`](/docs/reference/objects/clerk#handle-redirect-callback) or mounts the [`<AuthenticateWithRedirectCallback />`](/docs/reference/components/control/authenticate-with-redirect-callback) component. See the [custom flow](/docs/guides/development/custom-flows/authentication/oauth-connections) for implementation details.
...
<Properties>
  ...
  **Required** if `strategy` is set to `'oauth_<provider>'` or `'enterprise_sso'`. The full URL or path that the OAuth provider should redirect to after successful authorization on their part. Typically, this will be a simple `/sso-callback` route that either calls [`Clerk.handleRedirectCallback()`](/docs/reference/objects/clerk#handle-redirect-callback) or mounts the [`<AuthenticateWithRedirectCallback />`](/docs/reference/components/control/authenticate-with-redirect-callback) component. See the [custom flow](/docs/guides/development/custom-flows/authentication/oauth-connections) for implementation details.
