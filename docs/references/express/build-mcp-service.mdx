...
<Steps>
  ...
  - `mcpAuthClerk`: Verifies the Authorization header and ensures the request is authenticated with a Clerk-issued OAuth token.
  - `protectedResourceHandlerClerk`: Exposes metadata required for dynamic client registration.
  - `streamableHttpHandler`: Sets up a handler that connects your MCP server to incoming requests using the HTTP streaming transport.
