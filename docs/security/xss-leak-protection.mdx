...
Clerk works to minimize the surface area by using HttpOnly cookies for authenticated requests to our [Frontend API](/docs/reference/frontend-api), so that credentials cannot be leaked during XSS attacks.
