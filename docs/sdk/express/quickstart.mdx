...
<Steps>
  ...
  Clerk's [Express SDK](/docs/sdk/express/methods) ships with a variety of helpers for the backend to make user authentication easier.
  ...
  The [`clerkMiddleware()`](/docs/sdk/express/methods#clerk-middleware) function checks the request's cookies and headers for a session JWT and, if found, attaches the [`Auth`](/docs/references/backend/types/auth-object#auth-object){{ target: '_blank' }} object to the `request` object under the `auth` key.
  ...
  To protect your routes, use the [`requireAuth()`](/docs/sdk/express/methods#require-auth) middleware. This middleware functions similarly to `clerkMiddleware()`, but also protects your routes by redirecting unauthenticated users to the sign-in page.
...
<Cards>
  - [Use middleware to protect routes](/docs/sdk/express/methods#require-auth)
  - Learn how to protect specific routes from unauthenticated users.

  ---

  - [Protect routes based on authorization status](/docs/sdk/express/methods#get-auth)
  - Learn how to protect a route based on both authentication and authorization status.

  ---

  - [Express SDK reference](/docs/sdk/express/methods)
