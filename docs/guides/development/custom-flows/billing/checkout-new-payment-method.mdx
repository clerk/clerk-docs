...
<Steps>
  ...
  <Tabs items={["Next.js"]}>
    <Tab>
      ...
      1. Uses the [`useCheckout()`](/docs/reference/general/hooks/use-checkout) hook to get to initiate and manage the checkout session.
      1. Uses the [`usePaymentElement()`](/docs/reference/general/hooks/use-payment-element) hook to control the payment element, which is rendered by `<PaymentElement />`.
      1. Assumes that you have already have a valid `planId`, which you can acquire in many ways.
         ...
         - Use the new [`usePlans()`](/docs/reference/general/hooks/use-plans) hook to get the plan details.
