...
You can configure your application to require users to complete specific actions after signing up or signing in. Clerk refers to these steps as [after-auth flows](/docs/authentication/configuration/after-auth-flows)
...
## Navigating to tasks after sign-in or sign-up
...
By default, `clerk-js` has default route paths for tasks. Here's the mapping for each task type:
...
<Steps>
  ...
  Create a custom page that handles the user's pending session task. For this example, we'll build a page where users can create and select an organization - a required step before they can access the application:
