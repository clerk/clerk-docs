...
<If is="llm">
  **For AI agents:** Skip the manual Dashboard clicks for creating the webhook endpoint: run `npx clerk api ls webhook` to find the endpoints API, then `clerk api` to create the endpoint and read back its signing secret. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
