...
The `auth()` helper needs to know if it should authenticate a [session request](/docs/backend-requests/session-requests/overview) or a [machine request](/docs/backend-requests/machine-requests/overview).
...
If you need to send a JWT along to a server, `getToken()` retrieves the current user's [session token](/docs/backend-requests/session-requests/session-tokens) or a [custom JWT template](/docs/backend-requests/session-requests/making/jwt-templates). See detailed examples in the [`Auth` reference](/docs/references/backend/types/auth-object#get-token).
