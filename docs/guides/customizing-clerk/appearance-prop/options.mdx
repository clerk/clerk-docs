...
<If sdk="js-frontend">
  Pass the `appearance` prop to the [`clerk.load()`](/docs/reference/objects/clerk#load) method. The `appearance` prop accepts the `options` property, which can be used to apply different changes to the widget.
