...
This guide demonstrates how to hide a user's personal account in order to appear as if they only have access to organizations, and how to limit access to your application to only users with [active organizations](!active-organization), further enforcing organization-centric access. This is useful for applications that are built for organizations only, such as B2B applications.
...
<Steps>
  ...
  A user can have many organization memberships, but only one of them can be active at a time. This is called the [active organization](!active-organization).
  ...
  Now that you have hidden personal accounts from the UI and can detect and set an [active organization](!active-organization), you can limit access to your application to users with active organizations only. This will ensure that users without active organizations cannot access your application.
