...
After configuring your app, return to the **Domains** page in the Clerk Dashboard and select **Verify proxy** to confirm that Clerk can reach the `__clerk` proxy path. If `__clerk` is missing from the matcher, requests to Clerk's proxied Frontend API can be skipped by middleware and authentication can fail.
