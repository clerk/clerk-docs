...
- The [`<OrganizationSwitcher />` component](/docs/reference/general/components/organization/organization-switcher) provides a way for your users to select which organization is active. The [`useOrganizationList()` hook](/docs/custom-flows/organization-switcher) can be used for more control.
