...
- Both **public** and **private** metadata are set and can be accessed from the [Backend API](/docs/references/backend-api){{ target: '_blank' }}.
- Only **public** metadata can be accessed from the [Frontend API](/docs/references/frontend-api/){{ target: '_blank' }}.
...
To ease the flow of setting metadata, Clerk provides the `updateOrganizationMetadata()` and `updateOrganizationMembershipMetadata()` methods from the [Backend SDK](/docs/references/backend/overview), which is a wrapper around the [Backend API](/docs/references/backend-api){{ target: '_blank' }}.
