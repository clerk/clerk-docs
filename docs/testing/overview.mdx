...
Testing is an important part of every application. Each framework may require a slightly different setup. If you're having trouble getting testing to work properly, [reach out to support](/contact/support).

## Testing with one time passcodes

To avoid sending an email or SMS message with a one time passcode (OTP) during testing, you can use a faux email address or phone number that has a fixed code. Read the complete documentation [here](/docs/testing/test-emails-and-phones).
...
Obtained via the [Backend API](/docs/reference/backend-api/tag/Testing-Tokens), Testing Tokens are short-lived and valid only for the specific instance for which they are issued. Testing Tokens are only available in development instances.
