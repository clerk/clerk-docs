...
```tsx
function createOrganizationMembership: (params: CreateOrganizationMembershipParams) => Promise<ClerkBackendApiResponse<OrganizationMembership>>;
```
...
## `createOrganizationMembership()` Example
...
/* In this example, you can see that data is an OrganizationMembership object.
{
  data: _OrganizationMembership {
