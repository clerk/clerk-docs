...
In the following example, the `userId` is passed to the JS Backend SDK's [`getUser()`](/docs/references/backend/user/get-user){{ target: '_blank' }} method to get the user's full `User` object. For information on how to use the JS Backend SDK, see the [JS Backend SDK documentation](/docs/js-backend/getting-started/quickstart){{ target: '_blank' }}.
