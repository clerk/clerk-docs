...
Your Chrome Extension has its own unique CRX Id. This key can change while you are developing, unless you take steps to configure the extension. This guide will cover those steps so that your extension will have a stable, unchanging key.

> [!NOTE]
> If you following the [Chrome Extension Quickstart](/docs/quickstarts/chrome-extension) you have already completed this configuration.
...
<Steps>
  ...
  1. Select **View public key**. A dialog will appear with your public key.
  1. Copy the contents that are between `-----BEGIN PUBLIC KEY-----` and `-----END PUBLIC KEY-----`.
