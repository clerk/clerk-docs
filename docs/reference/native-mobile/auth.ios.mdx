...
```swift
var signIn = try await clerk.auth.signInWithTrustedDevice()
```
...
Send an email link to the supplied email address. After the user opens the link, handle the callback with [`clerk.handle(url)`](#deep-linking).
...
Send the user an email link to verify their sign-in. After the user opens the link, handle the callback with [`clerk.handle(url)`](#deep-linking).
...
Send the user an email link to verify their account. After the user opens the link, handle the callback with [`clerk.handle(url)`](#deep-linking).
