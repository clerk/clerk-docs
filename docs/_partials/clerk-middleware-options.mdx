...
<Properties>
  ...
  Used to activate a specific [organization](/docs/organizations/overview) or [personal workspace](/docs/organizations/overview#allow-personal-workspaces) based on URL path parameters. If there's a mismatch between the active organization in the session (e.g., as reported by [`auth()`](/docs/references/nextjs/auth)) and the organization indicated by the URL, the middleware will attempt to activate the organization specified in the URL.
