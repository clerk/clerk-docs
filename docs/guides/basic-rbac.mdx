...
The `/admin/dashboard` route now requires that the user is signed into the application. It also requires that the user have a `publicMetadata` of `{"role": "admin" }`.
...
```tsx filename="src/app/admin/dashboard/page.tsx" {3-5, 7-9, 14, 16, 23, 25-55}
import { redirect } from "next/navigation";
import { checkRole } from "@/utils/roles";
