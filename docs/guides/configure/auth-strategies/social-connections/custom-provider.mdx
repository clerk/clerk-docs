...
1. **Optional:** If your provider requires [PKCE (Proof Key for Code Exchange)](https://datatracker.ietf.org/doc/html/rfc7636), enable the **Use PKCE** toggle. Once enabled, Clerk will use the Authorization Code with PKCE flow for authentication with that provider. To learn more, see the [dedicated section on PKCE](/docs/guides/configure/auth-strategies/oauth/how-clerk-implements-oauth#public-clients-and-pkce).
