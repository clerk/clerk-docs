...
<Steps>
  ...
  Then, in the same folder, create an `index.tsx` file with the following code. The [`useClerk()`](/docs/reference/hooks/use-clerk) hook provides the `signOut()` function. If the user is signed in, it displays their email and a sign-out button. If they're not signed in, it displays sign-in and sign-up links.
