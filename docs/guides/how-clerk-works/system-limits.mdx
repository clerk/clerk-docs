...
Backend API requests are rate-limited per application instance which is identified by the [Secret Key](!secret-key) that is provided when making Backend API requests.
...
<Properties order="manual">
  ...
  `PATCH /v1/users/{user_id}`<br />`PATCH /v1/users/{user_id}/metadata`<br />`PUT /v1/users/{user_id}/metadata`
  ...
  `PATCH /v1/organizations/{organization_id}`<br />`PATCH /v1/organizations/{organization_id}/metadata`<br />`PUT /v1/organizations/{organization_id}/metadata`
  ...
  `PATCH /v1/organizations/{organization_id}/memberships/{user_id}`<br />`PATCH /v1/organizations/{organization_id}/memberships/{user_id}/metadata`
