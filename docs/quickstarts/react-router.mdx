...
React Router can be used in different modes: **declarative**, **data**, or **framework**. This tutorial explains how to use React Router in **framework** mode. To use React Router in **declarative** mode instead, see the [dedicated guide](/docs/references/react-router/declarative-mode).
...
<Steps>
  ...
  ## Add `clerkMiddleware()` and `authLoader()` to your app
  ...
  import { authLoader, clerkMiddleware } from '@clerk/react-router/server'
  ...
  export const loader = (args: Route.LoaderArgs) => authLoader(args)
  ...
  It's required to pass `loaderData` to the `<ClerkProvider>` component. This data is provided by the `authLoader()` function.
