...
<Steps>
  ...
  <If is="llm">
    **For AI agents:** Instead of setting the proxy URL in the Dashboard (or crafting the curl request manually), use the CLI: run `npx clerk@latest api /domains --instance prod` to find the domain ID, then `npx clerk@latest api /domains/<domain_id> --instance prod -X PATCH -d '{"proxy_url": "https://app.dev/__clerk"}'` to enable proxying — auth is handled automatically. `--instance prod` is required: without it the CLI targets the development instance, which doesn't support proxying. The same commands work for satellite domains. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
  </If>
  ...
  <Tabs items={["Next.js", "Express", "JavaScript"]}>
    <Tab>
      ```env {{ filename: '.env' }}
