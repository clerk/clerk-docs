...
<Steps>
  ...
  Clerk's Next.js SDK gives you access to prebuilt [components](/docs/references/general/components/overview), [hooks](/docs/references/nextjs/overview#client-side-helpers), and [helpers](/docs/references/nextjs/overview) for Next.js Server Components, Route Handlers and Middleware. Run the following command to install it:
  ...
  **Pro tip!** If you are signed into the [Clerk Dashboard](https://dashboard.clerk.com/last-active?path=api-keys), your Secret Key should become visible by clicking on the eye icon.
  ...
  To use the Account Portal, remove the routes that mount the Auth.js sign-in and sign-up UI. Replace the links to those routes with the [`<SignInButton>`](/docs/references/general/components/unstyled/sign-in-button) or [`<SignUpButton>`](/docs/references/general/components/unstyled/sign-out-button) components.
  ...
  - use the [prebuilt components](/docs/guides/development/custom-sign-in-or-up-page), such as the [`<SignIn />`](/docs/references/general/components/authentication/sign-in) and [`<SignUp />`](/docs/references/general/components/authentication/sign-up) components
  - build a [fully custom UI using the Clerk API](/docs/guides/development/custom-flows/overview), leveraging Clerk's React hooks such as [`useSignIn()`](/docs/references/general/hooks/use-sign-in) and [`useSignUp()`](/docs/references/general/hooks/use-sign-up)
  ...
  To conditionally render UI when the user is signed in, wrap it with [`<SignedIn>`](/docs/references/general/components/control/signed-in).

  To conditionally render UI when the user is _not_ signed in, wrap it with [`<SignedOut>`](/docs/references/general/components/control/signed-out).
  ...
  <Tabs items={["App Router", "Pages Router"]}>
    <Tab>
      You can replace Auth.js's `setServerSession()` with Clerk's [`auth()`](/docs/references/nextjs/app-router/auth) helper in order to read your user data.
    ...
    <Tab>
      You can replace Auth.js's `setServerSession()` with Clerk's [`getAuth()`](/docs/references/nextjs/pages-router/get-auth) helper in order to read your user data.
  ...
  The [`useAuth()`](/docs/references/general/hooks/use-auth) hook can be used to retrieve basic authentication information. The [`useUser()`](/docs/references/general/hooks/use-user) hook can be used to retrieve the full [`User`](/docs/references/javascript/user) object, which includes information about the user, such as their first name, emails, phone numbers, and more.
  ...
  To retain the user data in your database for easy querying, see the [guide on data synchronization with webhooks](/docs/guides/configure/webhooks/syncing).
