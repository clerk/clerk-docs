...
`createRouteMatcher()` is a Clerk helper function that lets you match a set of routes. It accepts an array of paths and returns a function that, given the `req` object from your Middleware, returns `true` when the request matches one of those paths. The paths use the same format as the paths provided to the Next.js Middleware `matcher`.
...
In the following example, `createRouteMatcher()` matches all `/dashboard` and `/forum` routes.

```tsx
const isDashboardRoute = createRouteMatcher(['/dashboard/:path*', '/forum/:path*'])
```
...
- To require a signed-in user, see [Protect pages and routes](/docs/guides/secure/protect-pages).
...
If you currently gate routes inside `clerkMiddleware()`, see [Migrating from middleware checks](/docs/guides/secure/protect-pages#migrating-from-middleware-checks).
