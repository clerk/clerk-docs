...
When you or a user of your application performs certain actions, a webhook can be triggered. You can see the full list of [webhook events](/docs/guides/configure/webhooks/overview#supported-webhook-events) for a list of the actions that could result in a Webhook. Depending on the events subscribed to in the [**Webhooks**](https://dashboard.clerk.com/last-active?path=webhooks) page in the Clerk Dashboard, a webhook event will be triggered and sent to the specified endpoint in your application.
...
> [!IMPORTANT]
> Your webhook needs to return a success code like `200` or `201` when it has been successfully handled. This will mark the webhook as successful in the Dashboard and prevent [retries](/docs/guides/configure/webhooks/overview#retry).
...
| `404` | The URL for the webhook was not found. Check that your application is running and that [the endpoint is correct in the Clerk Dashboard](/docs/guides/configure/webhooks/debugging#check-your-configuration-in-the-clerk-dashboard). |
...
To verify the webhook, see the [guide on webhooks](/docs/guides/configure/webhooks/syncing) for a detailed code example. You can also visit the [Svix guide on verifying payloads](https://docs.svix.com/receiving/verifying-payloads/how).
