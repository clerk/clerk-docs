...
<Tabs items={["NPM module", "<script>"]}>
  ...
  <Tab>
    <Steps>
      ...
      It's recommended to use environment variables to store your Clerk **Publishable Key**. A common way to use environment variables in a JavaScript project is putting the values in a `.env` file, then loading the values using the [`dotenv`](https://www.npmjs.com/package/dotenv) package. An example of how to set it up in this way is shown below:
