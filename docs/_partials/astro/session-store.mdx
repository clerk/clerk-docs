The following example demonstrates how to use the `$sessionStore` store to access the `Session` object. The `Session` object is used to access the `lastActiveAt` property, which is a `Date` object used to show the time the session was last active.
