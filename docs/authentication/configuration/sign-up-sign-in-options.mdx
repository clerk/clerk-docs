...
Clerk offers several [social providers](/docs/configure/auth-strategies/social-connections/oauth) for use during sign-up and sign-in. This authentication option is appealing because users often don't need to enter additional contact information since the provider already has it.
...
1. For development instances, simply select the social providers that you would like to enable. For production instances, you'll need to configure credentials for each social provider. See [the social provider's dedicated guide](/docs/configure/auth-strategies/social-connections/oauth) to learn how to configure credentials.

## Web3 authentication

Clerk provides Web3 authentication with either [MetaMask](/docs/configure/auth-strategies/web3/metamask), [Coinbase Wallet](/docs/configure/auth-strategies/web3/coinbase-wallet), or [OKX Wallet](/docs/configure/auth-strategies/web3/okx-wallet). As part of validating the accuracy of the returned Web3 account address, Clerk handles the signing of a message and verifying the signature. Because sign-in with Web3 uses the same abstraction as our other authentication factors, like passwords or email links, other Clerk features like multi-factor authentication and profile enrichment work for Web3 users out-of-the-box.
