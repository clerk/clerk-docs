...
<Properties>
  ...
  Determines what type of authentication to perform. If set to `session_token`, the function will authenticate a user session. If set to a machine token type, the function will authenticate a machine request. If set to `any`, the function will authenticate both user sessions and machine requests. Defaults to `session_token`.
