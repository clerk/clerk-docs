...
In this guide, **you** configure the connection in the Clerk Dashboard, while **the customer** (whoever manages the IdP) configures the IdP.
...
<Steps>
  ...
  <Copyable as="html">
    Here are the instructions for setting up SAML SSO with your IdP:

    **Step 1: Create a SAML application**

    1. In your IdP's dashboard, create a new **SAML 2.0** application.
    1. Add the **Assertion Consumer Service (ACS) URL** and **Entity ID** values provided to you. Some IdPs label these differently, for example **Single sign-on URL** and **Audience URI (SP Entity ID)**.
    ...
    **Step 3: Assign users or groups**
    ...
    Assign users or groups to the application before they can sign in with SSO. The exact steps depend on your IdP.

    **Step 4: Share the IdP details**

    Provide the application's **Metadata URL**. If your IdP doesn't support a metadata URL, share the **SSO URL**, **Entity ID (Issuer)**, and **Signing certificate** instead.
  </Copyable>
...
> [!NOTE]
> Custom SAML connections don't send a login hint to the IdP by default. To send one, see [Configure a SAML login hint](/docs/guides/configure/auth-strategies/enterprise-connections/overview#configure-a-saml-login-hint).
