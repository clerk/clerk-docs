...
> [!CAUTION]
> This method is now deprecated and was removed in `@clerk/nextjs@v6.0.0`. Please use [`clerkMiddleware()`](/docs/references/nextjs/clerk-middleware) instead.
...
<CodeBlockTabs options={["middleware.ts", ".env.local"]}>
  ```ts {{ filename: 'middleware.ts', mark: [3, 4, 5] }}
  import { authMiddleware } from '@clerk/nextjs/server'
...
```ts {{ filename: 'middleware.ts', mark: [4] }}
import { authMiddleware } from '@clerk/nextjs/server'
export default authMiddleware({
...
If you are having issues getting your Middleware dialed in, or are trying to narrow down auth-related issues, you can use the debugging feature in `authMiddleware`. Add `debug: true` to `authMiddleware` and you will get debug logs in your terminal.
...
<Properties>
  ...
  Called after the authentication middleware is executed. This function has access to the [`Auth`](/docs/references/nextjs/auth-object) object and can be used to execute logic based on the auth state.
  ...
  A list of routes that should be accessible without authentication. You can use glob patterns to match multiple routes or a function to match against the request object. For example: `['/foo', '/bar(.*)']` or `[/^\/foo\/.*$/]`<br />The sign in and sign up URLs are included by default, unless a function is provided.
