...
- If the [.env based settings](/docs/references/nextjs/custom-signup-signin-pages#update-your-environment-variables) for the sign-in and sign-up routes are present, add those to any routes listed in `publicRoutes`.
...
Assuming that the .env based settings are for sign-in and sign-up are set to /sign-in and /sign-up resepctively, the following `authMiddleware` would make the routes `/`, `/contact`, `/sign-in` and `/sign-up` public, no routes ignored and all remaining routes protected.
...
export default authMiddleware({
  afterAuth(auth, req, evt) {
    ...
    // redirect logged in user to organization selection page if they are not active in an organization
...
When using `afterAuth` the checks must handle all cases -- a custom `afterAuth` completely overrides the default behaviour covered earlier aside from adding sign-in and sign-up to `publicRoutes`. `afterAuth` does not allow you to add just one check and rely on the default behaviour otherwise. 
...
```
2. If the user is signed in but is not currently active in an organization and is not visiting the route for selecting an org, then redirect them to the organization selector. This will force users to select and be active in an organization to visit other parts of the site.
```ts 
if(auth.userId && !auth.orgId && req.nextUrl.pathname !== "/org-selection"){
      const orgSelection = new URL('/org-selection', req.url)
      return NextResponse.redirect(orgSelection)
...
### Controlling public routes using `publicRoutes`
...
You can use the `req` object to match against urls. The following example will make all routes but `/dashboard` public.
...
You can also use regex to match routes. The following is an example of a negative assertion that makes only the `/admin` protected.
...
## Debugging your middleware

If you are having issues getting your Middleware dialed in, or are trying to narrow down auth related issues you can use the debugging feature in authMiddlware. Add `debug: true` to `authMiddlware` and you will get a debug log to your development terminal or your server logs. 
