...
> [!NOTE]
> By default, roles and permission types like `OrganizationCustomRoleKey` and `OrganizationCustomPermissionKey` are assigned `string`. However, if a `ClerkAuthorization` type is defined, it will be utilized instead.
