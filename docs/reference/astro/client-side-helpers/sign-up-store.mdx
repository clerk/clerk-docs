...
The `$signUpStore` store provides a convenient way to access the [`SignUp`](/docs/reference/javascript/sign-up){{ target: '_blank' }} object, which allows you to check the current state of a sign-up. This is also useful for creating a custom sign-up flow.
...
The possible values for the `status` property of the `SignUp` resource are listed [here](/docs/reference/javascript/sign-up#properties){{ target: '_blank' }}.
