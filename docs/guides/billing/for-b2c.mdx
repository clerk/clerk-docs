...
You can use Clerk's Features and Plans to gate access to the content. There are a few ways to do this, but the recommended approach is to either use the [`has()`](/docs/reference/backend/types/auth-object#has) method or the [`<Show>`](/docs/reference/components/control/protect) component.

The `has()` method is available for any JavaScript framework, while `<Show>` is only available for React-based frameworks.
