...
<TutorialHero
  beforeYouStart={[
    {
      title: "A Clerk + your choice of SDK app is required.",
...
In this guide, you'll set up a webhook in your app to listen for the `user.created` event, create an endpoint in the Clerk Dashboard, build a handler, and test it locally using ngrok and the Clerk Dashboard.
...
<Steps>
  ...
  Set up a Route Handler that uses the `verifyWebhook` function to verify the incoming Clerk webhook and process the payload.
  ...
  <Tabs items={["Next.js", "React Router", "Astro", "Nuxt", "TanStack React Start", "Express", "Fastify"]}>
    <Tab>
