...
The users will not be deleted, so your app will not break. However, they will need to use another [sign-in option](/docs/authentication/configuration/sign-up-sign-in-options) such as password or email codes to authenticate themselves moving forward.
