...
> [!NOTE]
> This section describes the Core 2 Middleware migration. Auth checks inside `clerkMiddleware()`, including `createRouteMatcher()`, have since been deprecated. For current guidance on protecting routes, see [Protect content from unauthenticated users](/docs/guides/secure/protect-content).
