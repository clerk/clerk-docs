...
<Steps>
  ...
  1. Complete the required fields:
     ...
     - **SHA-1 certificate fingerprint**: To get your SHA-1, run the following command in your terminal:
       ...
       > [!NOTE]
       > Replace `path-to-debug-or-production-keystore` with the path to your debug or ßproduction keystore. By default, the debug keystore is located in `~/.android/debug.keystore`. It may ask for a keystore password, which is `android`. **Please note that Java is required to run the `keytool` command.**
  1. Select **Create**.
