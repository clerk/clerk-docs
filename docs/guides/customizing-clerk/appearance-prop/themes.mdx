...
To apply a theme to all Clerk components, pass the `theme` property to the `appearance` prop, which is available wherever you initialize the Clerk integration. For most SDKs, this is done via the [`<ClerkProvider>`](/docs/reference/components/clerk-provider) component. For other SDKs, it's configured through the SDK's Clerk integration or plugin.
...
You can apply a theme to all instances of a Clerk component by passing the component to the `appearance` prop, which is available wherever you initialize the Clerk integration. For most SDKs, this is done via the [`<ClerkProvider>`](/docs/reference/components/clerk-provider) component. For other SDKs, it's configured through the SDK's Clerk integration or plugin.
