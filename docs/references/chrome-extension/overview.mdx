...
A Chrome Extension can be identified by its unique, in a similar way to how a website can be identified by its domain. You will need to explicitly configure your Chrome Extension's key or it will change often. If the key changes it can cause the Chrome Extension to fail.
