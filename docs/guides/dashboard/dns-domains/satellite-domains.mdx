...
With `satelliteAutoSync: true` (the default), satellite domains automatically redirect to the primary domain on first load to sync authentication state, even if the user has no session. This means users who are already signed in on the primary domain are automatically recognized on the satellite without needing to select "Sign in". However, this comes with a performance cost since every first visit triggers a redirect.

If you set `satelliteAutoSync: false`, satellite domains will not automatically sync authentication state with the primary domain when a user visits the page. This means there is no upfront performance cost for users visiting a satellite domain. Authentication state is only synced when a user initiates a sign-in or sign-up action:
      ...
      <Tabs items={["Next.js", "Remix"]}>
        <Tab>
          ...
          const options = {
            ...
            // Auth state is automatically synced on first load (satelliteAutoSync defaults to true).
            // Uncomment the following to disable automatic syncing and improve performance for apps where most visitors are anonymous:
