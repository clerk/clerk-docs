...
<If notSdk={["js-frontend"]}>
  ...
  <If sdk={["vue", "nuxt"]}>
    The `Client` object is available on the [`Clerk`](/docs/reference/objects/clerk) object. Use the [`useClerk()`](/docs/reference/hooks/use-clerk) composable to access the `clerk` object, as shown in the following example.
