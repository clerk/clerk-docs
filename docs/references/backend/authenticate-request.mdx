...
Authenticates a token passed from the frontend. Networkless if the `jwtKey` is provided. Otherwise, performs a network call to retrieve the JWKS from the [Backend API](/docs/references/backend-api/tag/JWKS#operation/GetJWKS){{ target: '_blank' }}.
