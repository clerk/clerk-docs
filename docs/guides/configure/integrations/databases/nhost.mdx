...
The Nhost template will pre-populate the default claims required by Nhost and Hasura. You can include additional claims as necessary. [Shortcodes](/docs/guides/development/jwt-templates#shortcodes) are available to make adding dynamic user values easy.
...
Here is an example of using [Apollo Client](https://github.com/apollographql/apollo-client) in conjunction with the [`useAuth()`](/docs/references/general/hooks/use-auth) hook in a Next.js application to make a request to the Hasura GraphQL endpoint:
