...
Renders a social connection button based on the provided `name`. Throws an error in development if the provided social connection is not enabled in your instance. See [Social connections (OAuth)](/docs/configure/auth-strategies/social-connections/overview) to learn how to enable them in the Clerk Dashboard.
