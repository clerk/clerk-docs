...
If you've built a custom sign-in flow using the Clerk API or SDKs that allows password-based sign-ins, you'll need to handle the `needs_second_factor` status that Client Trust can trigger.
...
> [!TIP]
> If your application supports both Client Trust and user-enabled MFA, make sure tocheck for all possible second factor strategies when handling the `needs_second_factor` status.
