...
1. Every user has a [`User`](/docs/references/javascript/user/user) object that represents their account. The `User` object has a `emailAddresses` property that contains all the email addresses associated with the user. The [`useUser()`](/docs/references/react/use-user) hook is used to get the `User` object.
1. The [`User.createEmailAddress()`](/docs/references/javascript/user/create-metadata#create-email-address) method is used to add the email address to the user's account. A new [`EmailAddress`](/docs/references/javascript/types/email-address) object is created and stored in `User.emailAddresses`.
...
1. On the `/account/add-email/verify` page, the [`useClerk()`](/docs/references/react/use-clerk) hook is used to get the `handleEmailLinkVerification()` method.
