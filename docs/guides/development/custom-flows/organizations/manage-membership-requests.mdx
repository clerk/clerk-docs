...
<If sdk="ios">
  ...
  @MainActor
  func fetchMembershipRequests(clerk: Clerk) async -> [OrganizationMembershipRequest] {
    guard let activeOrganizationId = clerk.session?.lastActiveOrganizationId else { return [] }
    ...
    do {
      ...
      return try await organization.getMembershipRequests(status: "pending").data
    } catch {
