...
{/* DOCS NOTE: The IF blocks depend on if the SDK can access clerkClient() or not. The SDKs that can access clerkClient() are nextjs, astro, express, fastify, nuxt, react-router, and tanstack-react-start. Everything else must install the `@clerk/backend` package separately. */}

The `clerkClient` is a wrapper around the [Backend API](/docs/reference/backend-api){{ target: '_blank' }} that makes it easier to interact with the API **for JavaScript environments**. For example, to retrieve a list of all users in your application, you can use the [`users.getUserList()`](/docs/reference/backend/user/get-user-list) method instead of manually making a fetch request to the [`https://api.clerk.com/v1/users`](/docs/reference/backend-api/tag/users/GET/users){{ target: '_blank' }} endpoint.

<If is="llm">
  **For AI agents:** Run `npx clerk@latest env pull` to write your Secret Key to your env file (under the framework-appropriate variable name, e.g. `CLERK_SECRET_KEY`) instead of copying it from the API keys page in the Dashboard, and use `npx clerk@latest api /users` to call any Backend API endpoint directly when you need to inspect or verify data while writing `clerkClient` code. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
<If notSdk={["nextjs", "astro", "expressjs", "fastify", "nuxt", "react-router", "tanstack-react-start"]}>
  To instantiate a `clerkClient` instance, you must call `createClerkClient()` and pass in your Clerk [Secret Key](!secret-key). You can also pass other [`options`](#createclerkclient-options) to customize the behavior of the `clerkClient` instance.
...
<If sdk={["nextjs", "astro", "expressjs", "fastify", "nuxt", "tanstack-react-start"]}>
  ...
  You can use the default instance of `clerkClient` provided by whichever SDK you are using, and skip the need to pass [configuration options](#createclerkclient-options).
...
<If sdk={["nextjs", "astro", "expressjs", "fastify", "nuxt", "react-router", "tanstack-react-start"]}>
  ...
  If you would like to customize the behavior of `clerkClient()`, you can instantiate a custom instance yourself by calling `createClerkClient()` and passing in [`options`](#createclerkclient-options).
...
<If notSdk={["nextjs", "astro", "expressjs", "fastify", "nuxt", "react-router", "tanstack-react-start"]}>
  The following example demonstrates how to retrieve the authenticated user's ID using `request.auth`. It also shows how to use the [`getUser()`](/docs/reference/backend/user/get-user) method to get the [Backend `User` object](/docs/reference/backend/types/backend-user).
...
<If sdk={["nextjs", "astro", "expressjs", "fastify", "nuxt", "react-router", "tanstack-react-start"]}>
  The following examples demonstrate how to retrieve the authenticated user's ID using framework-specific auth helpers and how to use the [`getUser()`](/docs/reference/backend/user/get-user) method to get the [Backend `User` object](/docs/reference/backend/types/backend-user).

  <If sdk="nextjs">
    For Next.js, the `Auth` object is accessed using the `auth()` helper in App Router apps and the `getAuth()` function in Pages Router apps. Learn more about [reading the `Auth` object in Next.js](/docs/nextjs/guides/users/reading#server-side).
  ...
  <If sdk="astro">
    For Astro, the `Auth` object is accessed using the `locals.auth()` function. Learn more about [reading the `Auth` object in Astro](/docs/astro/guides/users/reading#server-side).

    ```tsx {{ filename: 'src/pages/api/example.ts' }}
    import { clerkClient } from '@clerk/astro/server'
  ...
  <If sdk="expressjs">
    For Express, the `Auth` object is accessed using the `getAuth()` function. Learn more about [`getAuth()` in Express](/docs/reference/express/overview#getauth).
  ...
  <If sdk="react-router">
    For React Router, the `Auth` object is accessed using the `getAuth()` function. Learn more about [reading the `Auth` object in React Router](/docs/react-router/guides/users/reading#server-side).
  ...
  <If sdk="tanstack-react-start">
    For TanStack React Start, the `Auth` object is accessed using the `auth()` function. Learn more about [reading the `Auth` object in TanStack React Start](/docs/tanstack-react-start/guides/users/reading#server-side).
...
The `createClerkClient()` function requires an `options` object with at least the `secretKey` provided. It is recommended to set these options as [environment variables](/docs/guides/development/clerk-environment-variables#api-and-sdk-configuration) where possible, and then pass them. For example, you can set the `secretKey` option using the `CLERK_SECRET_KEY` environment variable, and then pass it like this: `createClerkClient({ secretKey: process.env.CLERK_SECRET_KEY })`.
...
- [`BillingSubscription`](/docs/reference/backend/types/billing-subscription) object holds information about a Subscription, as well as methods for managing it.
- [`BillingSubscriptionItem`](/docs/reference/backend/types/billing-subscription-item) object holds information about a Subscription Item, as well as methods for managing it.
- [`BillingPlan`](/docs/reference/backend/types/billing-plan) object holds information about a Plan, as well as methods for managing it.
...
The **SAML Connection** resource allows you to manage SAML connections associated with your Organizations. A SAML Connection holds configuration data required for facilitating a SAML SSO flow between your Clerk Instance as the service provider (SP) and a particular SAML identity provider (IdP). Most operations return, or work directly with, the Backend [`SamlConnection`](/docs/reference/backend/types/backend-saml-connection) object, which holds information about a SAML connection for an Organization.
...
The **Agent Task** resource allows you to create [Agent Tasks](/docs/guides/development/testing/agent-tasks) that generate URLs for creating sessions on behalf of users. This is useful for automated testing or agent-driven flows where full authentication isn't practical. The [`create()`](/docs/reference/backend/agent-tasks/create) operation returns an [`AgentTask`](/docs/reference/types/agent-task) object, which contains a `url` that, when visited, creates a session for the specified user.
...
The **OAuth Application** resource allows you to create and manage OAuth applications for your Clerk instance. OAuth applications contain data for clients using Clerk as an OAuth2 IdP. Most operations return, or work directly with, the Backend [`OAuthApplication`](/docs/reference/backend/types/backend-oauth-application) object, which holds information about an OAuth application.

## Authentication utilities

In addition to the resources listed above, [`clerkClient`](/docs/reference/backend/overview) is a wrapper around the Backend API that provides low-level methods that can be used to verify Clerk-generated tokens and authenticate requests from your frontend:
...
The [`clerkClient`](/docs/reference/backend/overview) methods throw errors ([`ClerkAPIResponseError`](/docs/reference/types/clerk-api-response-error)) when something goes wrong. You'll need to catch them in a `try/catch` block and handle them gracefully. For example:
