...
<Tabs items={["Next.js", "JavaScript"]}>
  <Tab>
    The following example uses the [`useOrganizationList()`](/docs/references/hooks/use-organization-list) hook to access the `createOrganization()` method. This method is used to create a new organization with the provided name.
