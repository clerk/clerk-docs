...
The [`<ClerkProvider />`][clerk-provider] component provides active session and user context to Clerk's hooks and other components. For this project, it makes sense to wrap the `<html/>` element with `<ClerkProvider />`. This makes the active session and user context accessible anywhere within the app.
...
### Require authentication to access you app
