...
Make it functional by adding input fields. The following example uses the `<Clerk.Field>` component to render the `identifier` and `username` fields, as well as the `<Connection>` component to allow users to sign up with a social connection, like Google:
