...
You must verify the signature on the ID token before using any of the user information it contains. This will require your instance's public key. You can find this your instance's public key by loading your **Frontend API URL** (FAPI) with `/.well-known/jwks.json` appended to it. For example:
