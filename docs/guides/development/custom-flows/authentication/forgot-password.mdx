---
title: Build a custom flow for forgot password
description: Learn how to build a custom forgot password flow using the Clerk API.
search:
  keywords:
    - forgot password
---
...
1. Clerk verifies the code, and if successful, updates the user's password and signs them in. In the following examples, `signOutOfOtherSessions` is set to `true` so the user will be signed out of all other authenticated sessions.
...
> [!TIP]
> Looking for how to build a custom user interface that allows users to reset their password **after they've already signed in**? See the [custom flow for updating a user's password](/docs/guides/development/custom-flows/account-updates/update-password).
