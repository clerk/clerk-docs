...
<If sdk="ios">
  ...
  @MainActor
  func signInOrSignUp(emailAddress: String, password: String, clerk: Clerk) async {
    do {
      ...
      // If the identifier doesn't exist yet, create a sign-up and send an email code.
      if String(describing: error).contains("form_identifier_not_found") {
        do {
          var signUp = try await clerk.auth.signUp(emailAddress: emailAddress, password: password)
  ...
  @MainActor
  func verifyEmailCode(code: String, clerk: Clerk) async {
    do {
      if let signUp = clerk.auth.currentSignUp {
        let updatedSignUp = try await signUp.verifyEmailCode(code)
        if updatedSignUp.status == .complete, let sessionId = updatedSignUp.createdSessionId {
          try await clerk.auth.setActive(sessionId: sessionId)
          return
        }
      }
      ...
      if let signIn = clerk.auth.currentSignIn {
        let updatedSignIn = try await signIn.verifyMfaCode(code, type: .emailCode)
        if updatedSignIn.status == .complete, let sessionId = updatedSignIn.createdSessionId {
          try await clerk.auth.setActive(sessionId: sessionId)
        }
      }
    } catch {
  ...
  suspend fun signInOrSignUp(emailAddress: String, password: String) {
    when (
      ...
      is ClerkResult.Failure -> {
        val identifierMissing = signInResult.error.errors.firstOrNull()?.code == "form_identifier_not_found"
        if (!identifierMissing) return
    ...
    Clerk.auth.currentSignUp?.let { signUp ->
      signUp
        ...
        .onSuccess { updatedSignUp ->
          if (updatedSignUp.status == SignUp.Status.COMPLETE && updatedSignUp.createdSessionId != null) {
    ...
    Clerk.auth.currentSignIn?.let { signIn ->
      signIn
        ...
        .onSuccess { updatedSignIn ->
          if (updatedSignIn.status == SignIn.Status.COMPLETE && updatedSignIn.createdSessionId != null) {
            Clerk.auth.setActive(sessionId = updatedSignIn.createdSessionId)
          }
        }
        .onFailure {
          // See https://clerk.com/docs/guides/development/custom-flows/error-handling
