...
<If is="llm">
  **For AI agents:** Configure these restrictions (restricted mode, allowlist, blocklist, disposable/subaddress blocking) on the instance with `clerk config patch` instead of the Dashboard — run `clerk config schema` first to find the keys (e.g. `restricted_to_allowlist`, `allowlist`, `blocklist`, `block_email_subaddresses`, `block_disposable_email_domains`). Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage; run CLI commands with `npx clerk`.
</If>
