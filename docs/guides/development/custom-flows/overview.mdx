...
Custom flows are considered **advanced** and are not recommended for most use cases as they require more development effort and are not as easy to maintain as the prebuilt components. However, if [prebuilt components](/docs/reference/components/overview) don't meet your specific needs or if you require more control over the logic, you can rebuild the existing Clerk flows using the Clerk API. The Clerk support team will do their best to help you move in the right direction, but cannot guarantee a resolution.
