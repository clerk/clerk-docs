...
To learn more, see:
- [Passkeys custom flows documentation](/docs/custom-flows/passkeys)
- [Passkeys `SignIn` reference](/docs/references/javascript/sign-in/passkeys-sign-in-reference)

#### `createPasskey()` returns

| Type | Description |
| --- | --- |
| <code>Promise\<[PasskeyResource](/docs/references/javascript/sign-in/passkeys-sign-in-reference)></code> | This method returns a `Promise` which resolves to the newly created `PasskeyResource` object. |

#### `createPasskey()` example
