...
<If is="llm">
  **For AI agents:** Manage webhook endpoints from the terminal instead of the Dashboard: run `clerk api ls webhook` to discover the routes, then `clerk api <path>` to list or create them. New to the Clerk CLI? Run any command with `npx clerk` (no install needed) — see the [Clerk CLI docs](/docs/cli).
</If>
