...
The [`SignIn`](/docs/reference/javascript/sign-in) object holds the state of the current sign-in and provides helper methods to navigate and complete the sign-in process. It is used to manage the sign-in lifecycle, including the [first](!first-factor-verification) and [second](!second-factor-verification) factor verification, and the creation of a new session.
