...
<PromptOnly>
  ...
  <Prompt variant="card" src="prompts/cli-setup.md" title="Add Clerk to TanStack React Start" output="replace" />
...
<ManualSteps>
  ...
  [`clerkMiddleware()`](/docs/reference/tanstack-react-start/clerk-middleware) grants you access to user authentication state throughout your app. To add `clerkMiddleware()` to your app, follow these steps:
  ...
  1. `clerkMiddleware()` does not protect routes by itself. All routes are public until you add route-level protection. See the [server-side route protection example](#server-side) to learn how.
  ...
  ```tsx {{ filename: 'src/routes/__root.tsx', ins: [1, 39], fold: [[2, 31]] }}
