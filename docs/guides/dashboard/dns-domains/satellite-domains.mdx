...
<Steps>
  ...
  <Tabs items={["Environment variables", "Properties"]}>
    ...
    <Tab>
      ...
      <Tabs items={["Next.js", "JavaScript SDK", "Script tag"]}>
        <Tab>
        ...
        <Tab>
          For apps that initialize Clerk with `@clerk/clerk-js`, set `domain` in the `new Clerk()` constructor and `isSatellite` in `clerk.load()`.
          ...
          On your primary app, configure `allowedRedirectOrigins` to allow redirects back to the satellite domain.
        </Tab>

        <Tab>
          When using the SDK via a `<script>` tag, set `domain` as `data-clerk-domain` on the ClerkJS script, while `isSatellite` still belongs in `Clerk.load()`.
          ...
          If you need to pass other constructor options via attributes, prefix them with `data-clerk-`. For example, `proxyUrl` becomes `data-clerk-proxy-url`.
        </Tab>
      </Tabs>
    </Tab>
  </Tabs>
