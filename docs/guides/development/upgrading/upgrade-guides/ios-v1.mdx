...
<Steps>
  ...
  v1 splits imports by product: use `ClerkKit` for core APIs, or `ClerkKitUI` for [`AuthView`](/docs/reference/views/authentication/auth-view), [`UserProfileView`](/docs/reference/views/user/user-profile-view), and other [prebuilt views](/docs/reference/views/overview).
  ...
  // OR
  import ClerkKitUI // (includes Clerk UI)
  ```
  ...
  - Replace `import Clerk` with `import ClerkKit` or `import ClerkKitUI`.
