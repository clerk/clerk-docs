...
When you create a new application through the [Clerk Dashboard](https://dashboard.clerk.com/), Clerk provisions a dedicated frontend API (FAPI) instance for your application. It is hosted at `https://<slug>.accounts.dev` in development environments, where `<slug>` is a unique identifier generated for your application. You can find your application's FAPI URL in the [**Domains**](https://dashboard.clerk.com/last-active?path=domains) page of the Clerk Dashboard.

When configuring your Clerk app, you'll need to provide a [Publishable Key](/docs/deployments/clerk-environment-variables#clerk-publishable-and-secret-keys).
...
Like FAPI, while you can interact directly with BAPI, most developers opt to use Clerk's SDKs for smoother integration with their preferred language or framework. Documentation for Clerk's SDKs is available in [the left sidebar of the docs](https://clerk.com/docs).

Although the administrative features of BAPI are vital for many applications, the most common server-side task is verifying a user's authentication state when processing requests from your app's frontend. For instance, if a user submits a request to update their email address, **your server must ensure the user is authenticated and authorized to make this change.** Without proper validation, malicious actors could potentially take over user accounts. Let's explore how Clerk handles this scenario.
...
1. The server validates the credentials against a database. Upon successful validation, it creates a new session in the database associated with the user, typically containing information about the session such as the session ID, user ID, expiration time, and session status.
1. The server responds to the browser's request by setting the session ID in a [`Set-Cookie`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Set-Cookie) header in the response. This cookie will be automatically included in future requests from the browser in order to authenticate the user.
   <Video
     ...
     playsInline
   />
1. The next time the browser sends a request to the server, it automatically includes the session cookie. The server checks the database for the session ID and retrieves the associated user ID and session metadata. If the session is valid and active, the server can then use the user ID to fetch any required user data and process the request.
   <Video
...
> [!NOTE]
> If an attacker obtains your session ID, they can fully impersonate your session. To mitigate this risk, Clerk implements the following security measures: 1. [HTTPS](https://developer.mozilla.org/en-US/docs/Glossary/HTTPS) is enforced to prevent [man-in-the-middle attacks](https://en.wikipedia.org/wiki/Man-in-the-middle_attack) that could intercept session IDs in transit, and 2. the [`HttpOnly` flag](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Set-Cookie#httponly) is set on session cookies to prevent malicious JavaScript from accessing them through cross-site scripting (XSS) attacks. [Read more about Clerk's security measures](/docs/security/overview).
...
An alternative approach is "stateless" authentication, which addresses the scalability and latency challenges of stateful authentication while introducing different tradeoffs. The stateless authentication flow operates as follows:

1. The user initiates authentication by navigating to `example.com/sign-in`, entering their credentials (e.g. username/password), and submitting the form, usually by clicking a "submit" button. This makes a request to the server with the credentials.
1. The server validates the credentials against a database. Upon successful validation, it generates a cryptographically [signed token](/docs/how-clerk-works/tokens-signatures) containing essential user data like the user ID and any relevant claims.
...
While more complex to implement, this stateless model offers significant advantages. Most notably, it eliminates the latency overhead of database lookups required in the stateful model, leading to faster request processing. It also provides better horizontal scalability since there's no need to coordinate database access across multiple server instances.

However, this approach comes with important technical tradeoffs. The most significant limitation is that **JWTs cannot be revoked** due to their self-contained nature. Since JWT validation happens locally without consulting a central authority (i.e., they never "phone home"), there's no direct mechanism to invalidate them before their natural expiration.

This creates challenges for session management. To forcibly terminate a user's session, you have two suboptimal options:

1. Wait for the JWT to expire naturally.
1. Rotate the signing keys, which effectively invalidates all active sessions across your entire user base, signing out all of your users.
...
- Short-lived session tokens (60 seconds) combined with a background token refresh mechanism.
- The use of a client token to verify the user's authentication state, in conjunction with the session token.
...
1. The server validates the credentials against a database and, upon successful validation:
   ...
   - Generates a [signed JWT](/docs/how-clerk-works/tokens-signatures) containing essential user data like the user ID and any relevant claims (stateless component)
   - Sets the token's expiration to 60 seconds
1. The server responds to the browser's request by sending the token in a [`Set-Cookie` header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Set-Cookie) in the response.
   <Video
...
So far, this is the same as stateless auth, with one key distinction: the session token's expiration time. This is because normally, in stateless authentication implementations, the token's expiration is typically set to match the intended session duration - commonly ranging from one week to one month. Since tokens can't be revoked, if a token is compromised, the attacker has the entirety of the session lifetime to be able to take over the user's account. This will be several days at least on average, if not several weeks. Clerk's model mitigates this issue by setting an extremely short session token lifetime of 60 seconds. Normally, a Clerk token will have already expired before an attacker gets the chance to even try to use it. This is great for security, but it does create a complication in the authentication flow: **it decouples token expiration from session lifetime**.

<Video src="/docs/images/how-clerk-works/invalidation.mp4" width="1600" height="900" autoPlay muted loop playsInline />

Setting the session token lifetime to 60 seconds also creates another complication. When a token expires, it signifies the end of the session, requiring the user to re-authenticate. You have likely experienced this in many apps you have used, where every once in a while you need to re-authenticate. Modern browsers also enforce their own limitations on session persistence - for instance, Chrome implements a maximum cookie lifetime of 400 days, after which users are automatically signed out regardless of application-level session settings. These enforced session boundaries align with industry best practices for [secure session management](https://www.pingidentity.com/en/resources/blog/post/session-management.html#rte-33a93a79-b461-4b8e-9b7b-5b10a2777b70) and represent standard authentication behavior.

With Clerk's model, forcing a user to re-authenticate every 60 seconds would not be a good user experience. In order to make this change work, Clerk introduces a new "token refresh" flow that runs in the background and is responsible for refreshing the token every minute.
...
- Seamlessly updates the session token before expiration
...
The token and cookie management is handled by Clerk's frontend SDKs, as many operations cannot be accomplished through API responses alone. For this reason, it's strongly recommended to use these SDKs rather than implementing custom authentication flows.

When your app makes a request from the frontend to your backend, if the backend is on the same origin, the `__session` cookie will automatically be sent along with the request. Your backend can then [cryptographically verify](/docs/backend-requests/handling/manual-jwt) the session token's signature and extract the user ID and other claims to authorize the request.

## The Handshake

The short-lived nature of session tokens introduces an edge case that requires special handling. Consider this scenario: A user signs in to your application and then closes their browser tab. When they return after five minutes by opening a new tab, their session token will have expired since the refresh mechanism could not run while the application was closed. At this point, Clerk needs to determine the user's authentication status and potentially issue a new session token.

For client-rendered applications, this process is straightforward. Clerk's frontend SDK makes a direct request to the Frontend API (FAPI), which validates the client token. If the token is valid, FAPI issues a new session token. If invalid, the user is redirected to the sign-in flow.

However, server-rendered applications present a unique challenge. Server-to-server requests cannot include browser cookies, as they are accessible to the browser and not the server, meaning the client token would not be available when the server attempts to communicate with FAPI. To solve this, Clerk implements a "handshake" flow:
