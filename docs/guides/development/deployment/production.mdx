...
1. You will need a production domain. For most apps, this is a custom domain that you own. For Vercel-hosted apps, Clerk also supports production instances on `*.vercel.app` domains with some [email feature limitations](#vercel-app-production-domains).
1. If you're using a custom domain, you will need to be able to add DNS records on your domain.
...
To see what DNS records you need, navigate to the [**Domains**](https://dashboard.clerk.com/~/domains) page in the Clerk Dashboard. If you're using a `*.vercel.app` domain, you can skip this section. See [Vercel `*.vercel.app` production domains](#vercel-app-production-domains) for setup details.
...
## Vercel `*.vercel.app` production domains {{ id: 'vercel-app-production-domains' }}
...
After you update your proxy's matcher, in the Clerk Dashboard, return to the [**Domains**](https://dashboard.clerk.com/~/domains) page and select **Verify proxy** to confirm that Clerk can reach the `__clerk` proxy path. If `__clerk` is missing from the matcher, requests to Clerk's proxied Frontend API can be skipped by middleware and authentication can fail.
...
### Authentication on the app domain {{ id: 'authentication-on-app-domain' }}
...
### Verifying session tokens {{ id: 'verifying-session-tokens' }}
...
### Custom OAuth provider credentials {{ id: 'custom-oauth-credentials' }}

When you replace Clerk's shared OAuth credentials with your own for a `*.vercel.app` production instance, set the provider's authorization callback URL to:

```
https://<your-app>.vercel.app/__clerk/v1/oauth_callback
```

Clerk's Frontend API processes the OAuth callback through the auto-proxy on your app domain. Using any other URL — such as a Clerk-hosted Frontend API host or the Account Portal subdomain — will fail with a token exchange error.
...
- **Next.js matcher:** If you're using a Next.js proxy, include `/__clerk/(.*)` as a separate matcher entry so Clerk proxy requests are handled.
- **Custom OAuth credentials:** If you're using your own OAuth credentials for a social provider, ensure the provider's authorization callback URL is set to `https://<your-app>.vercel.app/__clerk/v1/oauth_callback`. See [Custom OAuth provider credentials](#custom-oauth-credentials) for details.
