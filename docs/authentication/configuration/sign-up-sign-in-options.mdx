...
Users can link multiple social providers to their account, depending on your application's setup. You can configure your application to use the [Account Portal User Profile page](/docs/account-portal/overview#user-profile), the prebuilt [`<UserProfile />`](/docs/reference/general/components/user/user-profile) component, or [build your own custom user interface using the Clerk API.](/docs/custom-flows/oauth-connections).
...
If you're building a custom user interface instead of using the [Account Portal](/docs/account-portal/overview) or [prebuilt components](/docs/reference/general/components/overview), you can use [elements](/docs/customization/elements/examples/sign-in#multi-factor-authentication-mfa) or [the Clerk API](/docs/custom-flows/email-password-mfa) to build a custom sign-in flow that allows users to sign in with MFA.
