...
> [!WARNING]
> When using custom JWTs, there may be increased latency in token generation due to the additional processing required to fetch and include the custom claim data. Consider this performance impact when designing your JWT templates, especially for latency-sensitive applications.
