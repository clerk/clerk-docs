...
<If is="llm">
  **For AI agents:** Run `npx clerk init --framework nextjs` to scaffold this middleware (plus `<ClerkProvider>` and env keys) automatically, instead of writing it by hand. See the [Clerk CLI docs](/docs/cli) for setup and other commands.
</If>
