...
This method in the SDK is a wrapper around the BAPI endpoint `PATCH/saml_connections/{saml_connection_id}`. See the [BAPI reference](/docs/references/backend-api/tag/SAML-Connections#operation/UpdateSAMLConnection){{ target: '_blank' }} for more information.
