  ...
  <Tabs items={['Server-side', 'Client-side']}>
    <Tab>
      ...
      <If sdk="tanstack-react-start">
        ```tsx {{ filename: 'src/routes/index.tsx', collapsible: true }}
    ...
    <Tab>
      <If notSdk={["astro", "nuxt"]}>
        Use the [`useOrganization()`](/docs/reference/hooks/use-organization) hook to access information about the currently [Active Organization](!active-organization), including the current user's membership and Role.
      </If>
      ...
      <If sdk="astro">
        To access information about the currently [Active Organization](!active-organization) on the client-side, use the [`$organizationStore`](/docs/reference/astro/client-side-helpers/organization-store) store, which returns the [`Organization`](/docs/reference/objects/organization) object. This requires that you've [set up your Astro app to be integrated with React](/docs/reference/astro/react).
