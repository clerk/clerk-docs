...
<Steps>
  ...
  Follow the [Next.js quickstart](/docs/getting-started/quickstart/app-router) to integrate the Clerk Next.js SDK into your application.
  ...
  To handle form submissions for adding and deleting user messages, create two Server Actions in `app/actions.ts`. Use Clerk's [`auth()` helper](/docs/references/nextjs/app-router/auth) to obtain the user ID, which will be used to interact with the database.
  ...
  To retrieve the user's messages, use Clerk's [`auth()` helper](/docs/references/nextjs/app-router/auth) to obtain the user's ID. Then, use this ID to query the database for the user's messages.
