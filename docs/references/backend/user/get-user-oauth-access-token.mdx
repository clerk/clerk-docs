...
This method in the SDK is a wrapper around the BAPI endpoint `GET/users/{user_id}/oauth_access_tokens/{provider}`. See the [BAPI reference](/docs/references/backend-api/tag/Users#operation/GetOAuthAccessToken){{ target: '_blank' }} for more information.
