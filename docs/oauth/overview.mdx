...
- **Resource service:** The service that holds the data or resources the **client** wants to access. Following the same example, Twitter's API would be the resource service. This is sometimes referred to as a **resource server** or **Service Provider (SP)**.
