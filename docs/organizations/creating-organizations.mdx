...
The only required parameter when creating a new organization is the organization name, which can be any string. In React and Next.js applications, the [`useOrganizationList()`](/docs/references/react/use-organization-list) hook returns the `createOrganization` method. In ClerkJS, the same method is available directly on the [`Clerk` object](/docs/references/javascript/clerk/create-organization).
