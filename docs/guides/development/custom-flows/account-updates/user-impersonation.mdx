...
<If sdk="expo">
  <Steps>
    ...
    Create a `generateActorToken+api.tsx` file with the following code. This creates an API route that will call Clerk's Backend API [`/actor_tokens`](/docs/reference/backend-api/tag/actor-tokens/post/actor_tokens){{ target: '_blank' }} endpoint to create an actor token.
    ...
    1. In the `hooks/` directory, create a `useUsers.tsx` file with the following code. This creates a hook that will fetch the list of your application's users.
...
<If sdk="ios">
  ```swift {{ filename: 'UserImpersonationView.swift', collapsible: true }}
  import SwiftUI
  import ClerkKit
  import Foundation

  struct ActorTokenResponse: Decodable {
    let token: String
  }

  struct UserImpersonationView: View {
    ...
    var body: some View {
      VStack {
        TextField("Enter actor user ID", text: $actorId)
        TextField("Enter target user ID", text: $targetUserId)
        Button("Impersonate user") {
          Task { await impersonateUser(actorId: actorId, targetUserId: targetUserId) }
        }
      }
    }
  }

  extension UserImpersonationView {

    func impersonateUser(actorId: String, targetUserId: String) async {
      do {
        // Call your backend endpoint that creates actor tokens.
        var request = URLRequest(url: URL(string: "https://example.com/generate-actor-token")!)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.httpBody = try JSONSerialization.data(
          withJSONObject: ["actorId": actorId, "userId": targetUserId]
        )

        let (data, _) = try await URLSession.shared.data(for: request)
        let actorToken = try JSONDecoder().decode(ActorTokenResponse.self, from: data).token

        let signIn = try await clerk.auth.signInWithTicket(actorToken)
