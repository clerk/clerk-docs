...
This method in the SDK is a wrapper around the BAPI endpoint `PATCH/email_addresses/{email_address_id}`. See the [BAPI reference](/docs/references/backend-api/tag/Email-Addresses#operation/UpdateEmailAddress){{ target: '_blank' }} for more information.
