...
Verifies a Clerk-generated token signature. Networkless if the `jwtKey` is provided. Otherwise, performs a network call to retrieve the JWKS from [Clerk's Backend API](https://clerk.com/docs/reference/backend-api/tag/JWKS#operation/GetJWKS).
...
| `apiUrl?` | `string` | The Clerk [Backend API](https://clerk.com/docs/reference/backend-api) endpoint. Defaults to `'https://api.clerk.com'`. |
