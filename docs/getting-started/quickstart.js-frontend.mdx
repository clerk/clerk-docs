...
<Tabs items={["NPM module", "<script>"]}>
  ...
  <Tab>
    <Steps>
      ...
      > [!IMPORTANT]
      > Any Clerk option passed as a script tag attribute must be prefixed with `data-clerk-`. For example, `publishableKey` becomes `data-clerk-publishable-key`, `proxyUrl` becomes `data-clerk-proxy-url`, and `domain` becomes `data-clerk-domain`. See [Clerk options](/docs/reference/objects/clerk#options) for the full list of available options.
