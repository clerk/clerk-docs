...
<Steps>
  ...
  > [!WARNING]
  > This implementation does not take rate limiting into account. If you have a large user base, you may want to implement a retry mechanism. Clerk will return a `429` status code if you exceed the rate limit. You can find more information in [Clerk's system limits documentation](/docs/guides/how-clerk-works/system-limits). Keep [Firebase's rate limits](https://firebase.google.com/docs/functions/quotas) in mind as well when exporting your users.
