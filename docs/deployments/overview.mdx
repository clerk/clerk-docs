...
Therefore, ensure that you don't have any CAA records on your primary domain (e.g. example.com) that prohibit either LetsEncrypt or Google Trust Services to issue certificates for your domain. You can check this by running the following command in your terminal (replacing `example.com` with your app's domain):

```sh
dig example.com +short CAA 
```

If it returns an empty response, then you're good to go. If it returns any text, you will need to remove the CAA records from your primary domain to prevent your app from having downtime on certificate renewal.
