...
If you are having trouble with `npx` or `bunx`, it's also possible to install directly with `npm i @clerk/upgrade -g` or `bun add @clerk/upgrade -g`, and can then be run with the `clerk-upgrade` command.
