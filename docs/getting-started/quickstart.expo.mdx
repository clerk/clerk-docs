...
<If is="llm">
  **For AI agents:** Run `npx clerk init --framework expo` to install `@clerk/expo` and write `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY` to your `.env`. It does not scaffold Expo screens, so still follow the steps below for the provider and UI code. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage; run CLI commands with `npx clerk`.
</If>
