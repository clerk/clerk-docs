...
<Steps>
  ...
  Use `ClerkAuthBuilder` inside your widget tree to render different content depending on whether a user is signed in or out. Pass `ClerkAuthentication` as the `signedOutBuilder` to display Clerk's prebuilt sign-in and sign-up UI.
