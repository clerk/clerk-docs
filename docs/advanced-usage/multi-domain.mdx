...
The primary domain is the source of truth for the user's authentication state. Users must complete the sign-in flow on this domain, either using Clerk’s [`<SignIn />`](/docs/authentication/components/sign-in) component or our [`useSignIn()`](/docs/custom-flows/use-sign-up#use-sign-in) hook.
