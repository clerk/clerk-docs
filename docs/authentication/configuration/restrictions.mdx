...
In **Restricted** mode, user access is controlled by the application admin(s). Users can be added to the application through [invitations](/docs/users/invitations), [enterprise connections](/docs/configure/auth-strategies/enterprise-connections/overview) or [manual user creation](/docs/users/creating-users). This mode is ideal for applications that are in private beta or internal tools.
