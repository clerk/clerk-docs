...
Because the Nuxt SDK is built on top of the Clerk Vue SDK, you can use the composables that the Vue SDK provides. These composables include access to the [`Clerk`](/docs/reference/objects/clerk) object, [`User` object](/docs/reference/objects/user), [`Organization` object](/docs/reference/objects/organization), and a set of useful helper methods for signing in and signing up. Learn more in the [Clerk Vue SDK reference](/docs/reference/vue/overview).
...
<Properties>
  ...
  An object that can be used to configure the behavior of the `event.context.auth()` function. It accepts the following properties:
...
The `clerkMiddleware()` helper integrates Clerk authentication and authorization into your Nuxt application through middleware. Learn more in the [`clerkMiddleware()` reference](/docs/reference/nuxt/clerk-middleware).

## `clerkClient`

<Include src="_partials/clerk-client-overview-section" />

Learn more about [reading user data](/docs/guides/users/reading).

## Protect pages and API routes

To protect pages, use the `useAuth()` helper to protect a single page, or use Nuxt's built-in [route middleware](https://nuxt.com/docs/4.x/guide/directory-structure/middleware) to protect multiple pages. Learn more about [protecting content](/docs/guides/secure/protect-content).

To protect API routes (`/api/**`), use the `clerkMiddleware()` helper. Learn more in the [`clerkMiddleware()` reference](/docs/reference/nuxt/clerk-middleware).
