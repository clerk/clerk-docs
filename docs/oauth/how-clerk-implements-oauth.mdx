...
With Clerk's FAPI, you can easily retrieve this metadata by visiting `<fapi>/.well-known/openid-configuration`. This endpoint exposes all relevant details in a standardized JSON format. To find the direct metadata link for your OAuth app in the Clerk Dashboard:

1. Navigate to the [**OAuth Applications page**](https://dashboard.clerk.com/last-active?path=user-authentication/oauth-applications).
2. Select your OAuth app.
3. Scroll down to **Application configuration URLs**.
4. Look for the **Discovery URL**, which points directly to your authorization server metadata. 
...
```json
{
  ...
  "userinfo_endpoint": "https://well-hagfish-71.clerk.accounts.dev/oauth/userinfo",
