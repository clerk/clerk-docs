...
<Properties>
  ...
  The type of token to accept. You can pass in `'any'` to accept all available token types. Defaults to `'session'`.
</Properties>
...
> - A `401` error for unauthenticated requests with session token type
> - A `404` error for unauthenticated requests with machine token types
...
You can use `auth()` to verify machine requests by passing in the `acceptsToken` parameter.
