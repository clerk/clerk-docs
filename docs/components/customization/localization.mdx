...
1. Fork the [https://github.com/clerkinc/javascript/](https://github.com/clerkinc/javascript/)
...
5. If you are adding a new language copy the en-US.ts file and name it according to your language. The naming is language-region but abbreviated. For French in Canada it would ne fr-CA.ts.
6. Go through the file and edit the entries.
7. Add the language to to the `packages/localizations/src/index.ts` file.
8. Commit your changes to bit, push them to your fork. Create a Pull Request from your fork to Clerk's repo againat the `main` branch. We will review and either approve or ask for updates.
...
To find the key for your translation (the `socialButtonsBlockButton` from the code example), open up our [English localization file](https://github.com/clerkinc/javascript/blob/main/packages/localizations/src/en-US.ts). Search the file for the term that you want to customize. 
...
If you want to customize multiple from the SignUp, the procedure would resemble the following:: 

```tsx
const localization = {
  signUp: {
    start: {
      subtitle: 'to access to {{applicationName}}',
    },
    emailCode: {
      subtitle: 'to access to {{applicationName}}'
    }
