---
title: '`<SignInWithMetamaskButton>` component'
description: The <SignInWithMetamaskButton> component is used to complete a one-click, cryptographically-secure sign-in flow using MetaMask.
sdk: expo, nextjs, nuxt, react, react-router, tanstack-react-start, vue
---

The `<SignInWithMetamaskButton>` component is used to complete a one-click, cryptographically-secure sign-in flow using MetaMask.

## Usage

### Basic usage

<If sdk="nextjs">
  ```jsx {{ filename: 'app/page.tsx' }}
  import { SignInWithMetamaskButton } from '@clerk/nextjs'

  export default function Home() {
    return <SignInWithMetamaskButton />
  }
  ```
</If>

<If sdk="react">
  ```jsx {{ filename: 'src/App.tsx' }}
  import { SignInWithMetamaskButton } from '@clerk/react'

  function App() {
    return <SignInWithMetamaskButton />
  }

  export default App
  ```
</If>

<If sdk="expo">
  <Include src="_partials/expo/web-only-callout" />

  ```jsx {{ filename: 'src/components/SignInWithMetamaskButton.web.tsx' }}
  import { SignInWithMetamaskButton } from '@clerk/expo/web'

  export default function Page() {
    return <SignInWithMetamaskButton />
  }
  ```
</If>

<If sdk="tanstack-react-start">
  ```tsx {{ filename: 'app/routes/index.tsx' }}
  import { SignInWithMetamaskButton } from '@clerk/tanstack-react-start'
  import { createFileRoute } from '@tanstack/react-router'

  export const Route = createFileRoute('/')({
    component: Home,
  })

  function Home() {
    return <SignInWithMetamaskButton />
  }
  ```
</If>

<If sdk="react-router">
  ```tsx {{ filename: 'app/routes/home.tsx' }}
  import { SignInWithMetamaskButton } from '@clerk/react-router'

  export default function Home() {
    return <SignInWithMetamaskButton />
  }
  ```
</If>

<If sdk="nuxt">
  ```vue {{ filename: 'pages/sign-in.vue' }}
  <script setup>
  // Components are automatically imported
  </script>

  <template>
    <SignInWithMetamaskButton />
  </template>
  ```
</If>

<If sdk="vue">
  ```vue {{ filename: 'example.vue' }}
  <script setup>
  import { SignInWithMetamaskButton } from '@clerk/vue'
  </script>

  <template>
    <SignInWithMetamaskButton />
  </template>
  ```
</If>

### Custom usage

In some cases, you will want to use your own button, or button text. You can do that by wrapping your button in the `<SignInWithMetamaskButton>` component.

<If sdk="nextjs">
  ```jsx {{ filename: 'pages/index.js' }}
  import { SignInWithMetamaskButton } from '@clerk/nextjs'

  export default function Home() {
    return (
      <SignInWithMetamaskButton mode="modal">
        <button>Custom sign in button</button>
      </SignInWithMetamaskButton>
    )
  }
  ```
</If>

<If sdk="react">
  ```jsx {{ filename: 'src/App.tsx' }}
  import { SignInWithMetamaskButton } from '@clerk/react'

  function App() {
    return (
      <SignInWithMetamaskButton mode="modal">
        <button>Custom sign in button</button>
      </SignInWithMetamaskButton>
    )
  }

  export default App
  ```
</If>

<If sdk="expo">
  <Include src="_partials/expo/web-only-callout" />

  ```jsx {{ filename: 'src/components/SignInWithMetamaskButton.web.tsx' }}
  import { SignInWithMetamaskButton } from '@clerk/expo/web'

  export default function Page() {
    return (
      <SignInWithMetamaskButton mode="modal">
        <button>Custom sign in button</button>
      </SignInWithMetamaskButton>
    )
  }
  ```
</If>

<If sdk="tanstack-react-start">
  ```tsx {{ filename: 'app/routes/index.tsx' }}
  import { SignInWithMetamaskButton } from '@clerk/tanstack-react-start'
  import { createFileRoute } from '@tanstack/react-router'

  export const Route = createFileRoute('/')({
    component: Home,
  })

  function Home() {
    return (
      <SignInWithMetamaskButton mode="modal">
        <button>Custom sign in button</button>
      </SignInWithMetamaskButton>
    )
  }
  ```
</If>

<If sdk="react-router">
  ```tsx {{ filename: 'app/routes/home.tsx' }}
  import { SignInWithMetamaskButton } from '@clerk/react-router'

  export default function Home() {
    return (
      <SignInWithMetamaskButton mode="modal">
        <button>Custom sign in button</button>
      </SignInWithMetamaskButton>
    )
  }
  ```
</If>

<If sdk="nuxt">
  ```vue {{ filename: 'pages/index.vue' }}
  <script setup>
  // Components are automatically imported
  </script>

  <template>
    <SignInWithMetamaskButton mode="modal">
      <button>Custom sign in button</button>
    </SignInWithMetamaskButton>
  </template>
  ```
</If>

<If sdk="vue">
  ```vue {{ filename: 'example.vue' }}
  <script setup>
  import { SignInWithMetamaskButton } from '@clerk/vue'
  </script>

  <template>
    <SignInWithMetamaskButton mode="modal">
      <button>Custom sign in button</button>
    </SignInWithMetamaskButton>
  </template>
  ```
</If>
