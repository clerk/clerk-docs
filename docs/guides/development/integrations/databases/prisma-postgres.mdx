...
<Steps>
  ...
  Ensure your `.env` file is loaded so `DATABASE_URL` is available before the Prisma config is evaluated. In Node.js, `.env` files can be loaded natively using the `--env-file=.env` flag when starting your app, or programmatically with `process.loadEnvFile()` at the top of your entry file.
