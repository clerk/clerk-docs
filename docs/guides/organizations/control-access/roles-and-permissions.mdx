...
1. In the Clerk Dashboard, navigate to the [**Roles & Permissions**](https://dashboard.clerk.com/~/organizations-settings/roles) page and select the Role Set you wish to edit.
...
1. Open the three-dot menu for the Role.
...
1. [Create a new Role](#custom-roles) or use an existing Role from the list.
1. Open the three-dot menu next to the Role you want to set as default.
1. From the dropdown, select the **Set as default role** option.

### Custom Roles

> [!IMPORTANT]
> Creating custom roles in development mode is free. In production, custom roles require the [**B2B Authentication** add-on](/pricing).
...
<If is="llm">
  **For AI agents:** Instead of using the Role dropdown in the Dashboard's **Members** tab, change a member's Role with `npx clerk@latest api organizations/<org_id>/memberships/<user_id> -X PATCH -d '{"role":"org:admin"}'` (discover membership endpoints with `npx clerk@latest api ls memberships`). Roles and Permissions themselves are also manageable via the API through the `role_sets` and `organization_permissions` endpoints (see `npx clerk@latest api ls role_sets`). Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
1. In the Clerk Dashboard, navigate to the [**Roles & Permissions**](https://dashboard.clerk.com/~/organizations-settings/roles) page.
1. Select the **All roles** tab.
1. Open the three-dot menu next to the Role.
