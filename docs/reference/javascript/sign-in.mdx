...
1. If verification is successful, set the newly created session as the active session by passing the `SignIn.createdSessionId` to the [`setActive()`](/docs/reference/javascript/clerk#set-active) method on the `Clerk` object. You may need to check for [session tasks](!session-tasks) before the session can be set as active.
...
<Properties>
  ...
  **Optional** if the `strategy` is set to `'oauth_<provider>'` or `'enterprise_sso'`. **Required** otherwise. The authentication identifier value for the current sign-in.
  ...
  Array of the [first factors](!first-factor-verification) that are supported in the current sign-in. Each factor contains information about the verification strategy that can be used. See the [`SignInFirstFactor`](/docs/reference/javascript/types/sign-in-first-factor) type reference for more information.
...
<Properties>
  ...
  - [`OAuthStrategy`](/docs/reference/javascript/types/sso#o-auth-strategy): The user will be authenticated with their [social connection account](/docs/guides/configure/auth-strategies/social-connections/overview).
...
<Properties>
  ...
  The Web3 provider to generate the signature with.  See [`Web3Provider`](/docs/reference/javascript/types/web3-wallet#web3-provider) for more information.
...
<Properties>
  - `strategy?`
  - <code>'password' | 'email\_link' | 'email\_code' | 'phone\_code' | [OAuthStrategy](/docs/reference/javascript/types/sso) | 'enterprise\_sso' | 'passkey' | 'web3\_base\_signature' | 'web3\_metamask\_signature' | 'web3\_coinbase\_wallet\_signature' | 'web3\_okx\_wallet\_signature' | 'web3\_solana\_signature' | 'ticket' | 'google\_one\_tap'</code>
...
<Properties>
  - `strategy`
  - <code>'email\_link' | 'email\_code' | 'phone\_code' | 'web3\_base\_signature' | 'web3\_metamask\_signature' | 'web3\_coinbase\_wallet\_signature' | 'web3\_okx\_wallet\_signature' | 'web3\_solana\_signature' | 'passkey' | [OAuthStrategy](/docs/reference/javascript/types/sso) | 'enterprise\_sso' | 'reset\_password\_phone\_code' | 'reset\_password\_email\_code'</code>
