...
This method in the SDK is a wrapper around the BAPI endpoint `POST/sign_in_tokens/{sign_in_token_id}/revoke`. See the [BAPI reference](/docs/references/backend-api/tag/Sign-in-Tokens#operation/RevokeSignInToken){{ target: '_blank' }} for more information.
