...
<If notSdk={["nextjs", "astro", "expressjs", "fastify", "nuxt", "react-router", "tanstack-react-start"]}>
  To instantiate a `clerkClient` instance, you must call `createClerkClient()` and pass in your Clerk [Secret Key](!secret-key). You can also pass other [`options`](#create-clerk-client-options) to customize the behavior of the `clerkClient` instance.
...
<If sdk={["nextjs", "astro", "expressjs", "fastify", "nuxt", "react-router", "tanstack-react-start"]}>
  ...
  <If sdk="expressjs">
    For Express, the `Auth` object is accessed using the `getAuth()` function. Learn more about [`getAuth()` in Express](/docs/reference/express/overview#get-auth).
