...
| `getToken()` | <code>(options?: GetTokenOptions) => Promise\<null \| string\></code> | A function that retrieves the current user's session token or a custom JWT template. Returns a promise that resolves to the token. See the [reference doc](/docs/reference/javascript/session#get-token). > [!NOTE] > To access auth data server-side, see the [`Auth` object reference doc](/docs/reference/backend/types/auth-object). **Throws** When called in a non-browser environment (code: `clerk_runtime_not_browser`) |
| `has()` | <code>(params: CheckAuthorizationParamsWithCustomPermissions) => false</code> | A function that checks if the user has specific Permissions or Roles. See the [reference doc](/docs/reference/backend/types/auth-object#has). |
