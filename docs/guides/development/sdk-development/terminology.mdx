...
| Name | Description |
| - | - |
| Client | A [client](/docs/reference/objects/client) represents the current device or software accessing an application such as your web browser, native application, Chrome Extension, or Electron app. |
| Session | A [session](/docs/reference/objects/session) is a secure representation of the authentication state of the current user. Each client can hold multiple sessions on the same device. This is identical to how Gmail works in a browser. |
| User | A user represents the current user of the session. The [`User`](/docs/reference/objects/user) object holds all the basic user information (e.g., name, email addresses, phone numbers), including their public, private, and unsafe metadata. |
| Organization | An [Organization](/docs/reference/objects/organization) represents the current Organization of the session. Users can belong to many Organizations. One of them will be the [Active Organization](!active-organization) of the session. |
