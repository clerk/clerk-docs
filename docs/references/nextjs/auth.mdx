...
`auth()` returns the [`Authentication`](/docs/references/nextjs/auth-object) object with a few extra properties:
...
`auth()` returns the [`Authentication`](/docs/references/nextjs/auth-object#auth-object) object, which includes the `has()` helper. `auth()` also returns the `protect()` helper.
