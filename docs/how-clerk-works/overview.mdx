...
>
> A benefit of this is that it allows the token's value and session claims to be accessed on the client side, which is often quite valuable, and allows developers to send the session token as a custom header in requests. This makes it possible to use a different subdomain (like `api.example.com`) for your backend, a scenario in which the cookie would otherwise not be sent.
>
