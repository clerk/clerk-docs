...
```tsx
function getOrganizationMembershipList: (params: GetOrganizationMembershipListParams) => Promise<ClerkBackendApiResponse<OrganizationMembership[]>>;
```
...
### `getOrganizationMembershipList()` with no parameters
...
const { data, errors, totalCount } = await clerkClient.organizations.getOrganizationMembershipList({
  organizationId,
