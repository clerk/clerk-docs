...
Frontend API requests are rate limited per user, identified by IP address.
...
Backend API requests are rate limited per application instance. The application is identified by the secret key that is provided when making Backend API requests.
