...
- **Description**: GitHub PR’s are our source of truth, so they should include as much context as possible. PR descriptions should be as detailed as possible ([example](https://github.com/clerk/clerk-docs/pull/3003)). If the PR is a result of a new feature or update, there should be links to the relevant PR’s in the source code repo (e.g. `clerk/javascript`) and an explanation of the behavior before —> now. If they are `clerk/dashboard` changes, please get pictures/videos of the before —> now updates. If all of this is included in the source code PR (`clerk/javascript`, `clerk/dashboard`, `clerk/clerk`, etc.), then just copy it over to the `clerk-docs` PR description, or vice-versa (if it’s in `clerk-docs` PR description, add it to the source code PR description). Writing up context shouldn’t be too hard - AI is our friend!
