...
## Add the global ClerkAuthorization type for roles and permissions

Clerk allows for custom roles and permissions. By default types related to roles and permissions such `OrganizationCustomRoleKey` and `OrganizationCustomPermissionKey` will be assigned to `string`.
In order to enhance typesafety in your project you can provide your own roles and permissions that reflect your custom bussiness logic.

<Tabs type="clerk-authorization-types" items={[ "Default roles","Custom roles and permissions"]}>
