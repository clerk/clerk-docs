...
Clerk provides a set of [hooks and helpers](/docs/reference/tanstack-react-start/overview#client-side-helpers) that you can use to access the active session and user data in your TanStack React Start application. Here are examples of how to use these helpers in both the client and server-side to get you started.
...
The [`useAuth()`](/docs/reference/hooks/use-auth){{ target: '_blank' }} hook provides information about the current auth state, as well as helper methods to manage the current active session.
