...
1. You will need a production domain. For most apps, this is a custom domain that you own. For Vercel-hosted apps, Clerk also supports production instances on `*.vercel.app` domains with some email feature limitations.
...
## Vercel `*.vercel.app` production domains
...
- Use a Clerk SDK version that supports Vercel auto-proxy.
- If you're using Next.js middleware or proxy, include `/(api|trpc|__clerk)(.*)` in your matcher.
- After configuring your app, return to the **Domains** page in the Clerk Dashboard and select **Verify proxy**.
...
- **SDK support:** Use a Clerk SDK version that supports Vercel auto-proxy for production `*.vercel.app` domains.
- **Next.js matcher:** If you're using Next.js middleware or proxy, include `/(api|trpc|__clerk)(.*)` in the matcher so Clerk proxy requests are handled.
