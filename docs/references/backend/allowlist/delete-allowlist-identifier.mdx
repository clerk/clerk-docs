...
This method in the SDK is a wrapper around the BAPI endpoint `DELETE/allowlist-identifiers/{identifier_id}`. See the [BAPI reference](/docs/references/backend-api/tag/Allow-list-Block-list#operation/DeleteAllowlistIdentifier){{ target: '_blank' }} for more information.
