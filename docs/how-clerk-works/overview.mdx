...
>
> Stateless authentication offers an elegant solution to this synchronization challenge. By using [signed tokens](/docs/how-clerk-works/tokens-signatures) that contain all necessary authentication information, the server can verify a user's credentials without querying a centralized sessions table. These cryptographically signed tokens ensure data integrity and allow authentication to occur without direct database interaction, effectively bypassing the synchronization complexities inherent in traditional, stateful authentication methods.
...
> ---
>
> Imagine this scenario: you hit your frontend and you have an expired session token, you need a new valid session token. Normally, to refresh your session token, you need to send your _valid_ session token to clerk, which it will verify, then send you a new valid one. This is _secure by default_, because anyone who has a valid session token is authenticated as you, as we have learned in the rest of this guide.
>
...
>
> - You could send the expired session JWT, but then any attacker with an expired session JWT would be able to mint a new one and hijack your session, which is not acceptable.
...
>
> The best way is to check to see if there’s a valid client token set on FAPI with a session that’s still active. If there is, it would be safe to mint a new session token and send that back.
>
> However, if your app is rendered on the server, your hands are tied. If we make an API request to FAPI it will not be able to check `__client`, because **cookies live in the browser**, so the FAPI request must come from your browser in order for  it to be sent with the `__client` cookie. A _server to server_ request will not include any cookies, since cookies are stored by browsers.
>
> However, when the server responds with a _redirect_ to FAPI, we end up going back through the browser. The server returns a redirect status code, the browser reads this, and the browser issues the redirect. This changes it to _server to browser to server,_ which means that the request to FAPI is delivered with the `__client` cookie, since it comes from the browser.
>
> If FAPI sees that you have a legitimate session (a valid client JWT), it will create a new session token for you and return a redirect back to your application domain with the new session token in a `Set-Cookie` header, and you’re back up and running.
>
