...
| `hasImage` | `boolean` | A getter boolean to check if the user has uploaded an image or one was copied from OAuth. `false` if Clerk is displaying an avatar for the user. |
