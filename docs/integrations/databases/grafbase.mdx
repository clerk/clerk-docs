...
<Steps>
  ...
  To retrieve Clerk's session token, use the `getToken()` method returned by the [`useAuth()`](/docs/reference/general/hooks/use-auth) hook. You'll need to pass the template name you created in the previous step to the `getToken()` method. The following example uses the default template name, `grafbase`.
