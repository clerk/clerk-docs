...
> [!DEPRECATED]
> The following properties are deprecated as of 2025-07-15 and will be removed in the next major version of Clerk. Use the replacements noted below instead.
...
<If sdk="js-frontend">
  To apply `variables` to all Clerk components, pass the `appearance` prop to the [`clerk.load()`](/docs/reference/objects/clerk#load) method. The `appearance` prop accepts the property `variables`.
