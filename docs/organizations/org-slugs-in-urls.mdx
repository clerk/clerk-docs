...
<Steps>
  ...
  Your application URLs should be structured to indicate which sections of your app are scoped to organizations versus [personal accounts](/docs/organizations/overview#allow-personal-accounts).
  ...
  With [`clerkMiddleware()`](/docs/references/nextjs/clerk-middleware), you can use the [`organizationSyncOptions`](/docs/references/nextjs/clerk-middleware#organization-sync-options) property to declare URL patterns that determine whether a specific organization or a user's personal account should be activated.
  ...
  The same pattern is used with `personalWorkspacePatterns` to match the user's personal account.
