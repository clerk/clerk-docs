...
<Properties>
  ...
  **Deprecated. Move `afterSignOutUrl` to [`<ClerkProvider />`](/docs/reference/general/components/clerk-provider).** The full URL or path to navigate to after a successful sign-out.
  ...
  Optional object to style your components. Will only affect [Clerk components](/docs/reference/general/components/overview) and not [Account Portal](/docs/account-portal/overview) pages.
