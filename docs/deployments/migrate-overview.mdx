...
A trickle migration is great for upgrading active users and sessions to take advantage of Clerk but there will always be some users who won't create an active session within the migration window and therefore will need to be migrated by other means, typically via [Basic Import / Export](#basic-export-import). A trickle migration lowers coordination risk because the remaining (in-active) users are fewer, so data de-synchronisation risk drops, too.
