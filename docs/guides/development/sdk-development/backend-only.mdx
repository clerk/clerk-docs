...
The source of truth for all BAPI endpoints is the [BAPI reference docs](/docs/reference/backend-api){{ target: '_blank' }}. For Node.js backend frameworks, you can build on top of the [`clerkClient`](/docs/reference/backend/overview) methods.
...
<Steps>
  ...
  Use [`createClerkClient`](/docs/reference/backend/overview#usage) from `@clerk/backend` to create your default Clerk client which will be used for the middleware.
