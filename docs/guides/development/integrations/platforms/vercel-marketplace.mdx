...
Connecting to a project will automatically sync your Clerk development API keys to your Vercel project's development environment variables. The following environment variables will be automatically synced:
...
When the connected Vercel project has a production `*.vercel.app` domain, Clerk can configure the production instance to use that domain. Ensure your Vercel production environment uses your production Clerk API keys (`pk_live_` and `sk_live_`). Production apps on `*.vercel.app` use Clerk's Vercel auto-proxy support at `https://<your-app>.vercel.app/__clerk`, so Next.js proxy matchers must include Clerk's proxy path, such as `/__clerk/:path*`. Keep this as a separate matcher entry because Clerk also serves browser assets from `/__clerk/npm/...`.
