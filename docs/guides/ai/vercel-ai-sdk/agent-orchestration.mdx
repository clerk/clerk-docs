...
For this guide, wire up two agents: a `parent-agent` that delegates work, and a `research-agent` (subagent) that performs the work. Create both machines and scope `parent-agent` to invoke `research-agent`.
