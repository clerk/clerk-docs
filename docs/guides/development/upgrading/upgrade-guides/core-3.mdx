...
<Tabs items={['Next.js']}>
  <Tab>
    ...
    Running the CLI updates dependencies and applies codemods. If you prefer a manual upgrade, skip this step and use the install commands below. The CLI’s `transform-protect-to-show` codemod migrates client-side `<Protect>` usage to `<Show>` and updates imports automatically. Use the previous CLI major if you must target an older Clerk release.
