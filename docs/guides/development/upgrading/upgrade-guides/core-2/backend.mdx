...
>
  ...
  <AccordionPanel>
    The `createEmail` import has been removed. There is no replacement at this time while the behavior of `createEmail` is being reevaluated to align with the newer `sendSms` method. If this is an issue for your implementation, [contact support](https://clerk.com/contact/support){{ target: '_blank' }}.
  </AccordionPanel>
