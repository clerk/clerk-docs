...
Use the Android organization APIs to create and manage Organizations, switch the [Active Organization](!active-organization), and build custom organization flows. Organization methods return `ClerkResult`, so handle both `ClerkResult.Success` and `ClerkResult.Failure`.
