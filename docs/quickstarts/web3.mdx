...
You need to create a Clerk application in your [Clerk Dashboard](https://dashboard.clerk.com/) and configure the application to use **Sign in with Metamask**. For more information on how to set up a Clerk application, see [Getting started with Clerk](/docs/getting-started). To enable Metamask authentication, check out this [guide](/docs/authentication/configuration/sign-in-options#social-o-auth-web3).
