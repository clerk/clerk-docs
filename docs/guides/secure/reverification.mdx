...
The following table shows the sensitive actions that require reverification along with the required strategy and time window for each action. When using Clerk's prebuilt components, reverification is automatically handled for these actions.

| Action | Strategy | Default window |
| - | - | - |
...
By default, users can perform Clerk-protected sensitive actions for 10 minutes after a successful sign-in or reverification. You can customize the reverification window to be between 1 and 10 minutes. A shorter window requires a more recent sign-in or reverification and can prompt users to verify their credentials more often.

1. In the Clerk Dashboard, navigate to the [**Sessions**](https://dashboard.clerk.com/~/sessions) page.
1. Under **Session lifetime**, enter a value between 1 and 10 minutes in the **Reverification window** field.
1. Select **Save**.

This setting applies only to the Clerk-protected sensitive actions listed above. For sensitive actions unique to your application, define the required time window in your application. See [How to require reverification](#how-to-require-reverification).
