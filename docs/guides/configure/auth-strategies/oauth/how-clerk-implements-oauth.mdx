...
CIMD clients always use PKCE with the `S256` method and always show the OAuth consent screen. You can explicitly allow clients, control whether previously unknown clients may connect, restrict their scopes, and review their metadata from the **CIMD Clients** tab on the [**OAuth applications**](https://dashboard.clerk.com/~/oauth-applications) page. Enable **Advertise CIMD support** on that tab to publish CIMD support in your authorization server metadata.
...
- In the Clerk Dashboard, open [**OAuth applications**](https://dashboard.clerk.com/~/oauth-applications), configure **Default scopes**, and save your changes. The **Default scopes** setting is available when **Advertise CIMD support** or **Dynamic client registration** is enabled.
...
To configure the token format for an OAuth application, navigate to the [**OAuth applications**](https://dashboard.clerk.com/~/oauth-applications) page in the Clerk Dashboard, select the **Settings** tab, and toggle the **Generate access tokens as JWTs** switch on or off depending on your needs.
...
- **CIMD:** Clerk advertises `client_id_metadata_document_supported: true` only when you enable **Advertise CIMD support** on the **CIMD Clients** tab. This lets compatible clients select [CIMD](!client-id-metadata-document) instead of Dynamic Client Registration. Clerk omits the field otherwise. Applications using Clerk's legacy OAuth implementation don't support or advertise CIMD.
