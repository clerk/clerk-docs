...
3. Has a session token (`__session`), but no active client is detected (`__client_uat` = 0 or missing)
4. Has active client (`__client_uat` > 0) but no session token
5. Client UAT is more recent than current session token issued at (`__client_uat` > `sessionToken.iat`)
6. Session token exists but is expired
7. Session token exists but is not active yet
