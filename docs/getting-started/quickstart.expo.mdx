...
| **JS + Native Sign-In** | [Custom forms](!custom-flow) + native OAuth buttons | Native (no browser) | Yes | Custom UI with native Google/Apple sign-in |
...
<Tabs items={["JavaScript", "JS + Native Sign-In", "Native Components"]}>
  ...
  <Tab>
    ...
    <Steps>
      ...
      To use native Google Sign-In, you'll also need Google OAuth client IDs. See the [Google Sign-In guide](/docs/guides/configure/auth-strategies/sign-in-with-google) for instructions on creating these credentials.
      ...
      export function GoogleSignInButton({ onSignInComplete }: { onSignInComplete?: () => void }) {
...
<Cards>
  ...
  - [Native Google Sign-In](/docs/guides/configure/auth-strategies/sign-in-with-google)
  - Add native Google Sign-In without browser redirects.

  ---

  - [Native Apple Sign-In](/docs/guides/configure/auth-strategies/sign-in-with-apple)
