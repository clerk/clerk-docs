...
Clerk provides a set of system permissions that power [Clerk's Frontend API](/docs/reference/frontend-api){{ target: '_blank' }} and [organization-related Clerk components](/docs/components/overview#organization-components). These permissions serve as the baseline required for Clerk to operate effectively.
...
> [!WARNING]
> System permissions aren't included in [session claims](/docs/backend-requests/session-requests/session-tokens#default-session-claims). To check permissions on the server-side, you must [create custom permissions](/docs/organizations/create-roles-permissions).
