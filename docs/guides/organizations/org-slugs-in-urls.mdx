...
Alternatively, [Organization IDs](/docs/reference/objects/organization#properties) can be used to identify Organizations in URLs:
...
<Steps>
  ...
  > [!TIP]
  > If your app doesn't use `clerkMiddleware()`, or you prefer to manually set the [Active Organization](!active-organization), use the [`setActive()`](/docs/reference/objects/clerk) method to control the Active Organization on the client-side.

  With [`clerkMiddleware()`](/docs/reference/nextjs/clerk-middleware), you can use the [`organizationSyncOptions`](/docs/reference/nextjs/clerk-middleware#organizationsyncoptions) property to declare URL patterns that determine whether a specific Organization should be activated.
  ...
  <Tabs items={["Server-side","Client-side"]}>
    ...
    <Tab>
      To get Organization information on the client-side, use the [`useOrganization()`](/docs/reference/hooks/use-organization) hook to access the [`organization`](/docs/reference/objects/organization) object.
