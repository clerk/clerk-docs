...
- **With Clerk Organizations** — the connection is attached to the IT admin's [Active Organization](!active-organization).
...
- [Self-serve SSO is enabled in the Clerk Dashboard](#enable-self-serve-sso).
...
When the IT admin opens a page that renders `<ConfigureSSO />`, they are guided through a flow with the following steps:
...
To learn about available props and other information regarding `<ConfigureSSO />`, see the [component reference](/docs/reference/components/authentication/configure-sso).
