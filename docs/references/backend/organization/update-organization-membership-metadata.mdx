...
| `publicMetadata?` | `{[string]: any}` | Metadata that can be read from both the Frontend API and [Backend API](/docs/reference/backend-api), but can be set only from the Backend API . |
| `privateMetadata?` | `{[string]: any}` | Metadata that is only visible to your [Backend API](/docs/reference/backend-api). |
...
This method in the SDK is a wrapper around the BAPI endpoint `PATCH/organizations/{organization_id}/memberships/{user_id}/metadata`. See the [BAPI reference](/docs/reference/backend-api/tag/Organization-Memberships#operation/UpdateOrganizationMembershipMetadata) for more details.
