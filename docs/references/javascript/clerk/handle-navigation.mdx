...
| --- | --- | --- |
| `params` | [`HandleMagicLinkVerificationParams`](#handlemagiclinkverificationparams) | Allows you to define the URLs where the user should be redirected to on successful verification and:
- Completed sign in or sign up attempt. - Pending sign in or sign up attempt.<br />If the magic link is successfully verified on another device, there's a callback function parameter that allows custom code execution. |
