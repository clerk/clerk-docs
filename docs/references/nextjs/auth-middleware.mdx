...
Some developers will need to handle specific cases such as detecting if the user is inside an organization, or handle redirects differently. This cases can be handled with `afterAuth()`.
...
If you need to additional middleware handlers to execute before Clerk's authentication middleware, use `beforeAuth()`. An example where the middleware handler from `next-intl` is executed can be seen below.
  ...
  rows={[
    ...
    },
    {
      cells: [
        <code>apiRoutes?</code>,
        <code>string[]</code>,
        `A list of routes that should return 401. You can use glob patterns to match multiple routes or a function to match against the request object.`
      ],
    },
    {
      cells: [
    ...
    },
    {
      cells: [
        ...
        <code>string[]</code>,
       `A list of routes that should be accessible without authentication. You can use glob patterns to match multiple routes or a function to match against the request object. The sign in and sign up URLs are included by default, unless a function is provided.`
      ],
    },
    {
      cells: [
