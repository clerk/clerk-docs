...
A background service worker runs on demand. When a request comes in from a [content script](https://developer.chrome.com/docs/extensions/develop/concepts/content-scripts) that wants to interact with Clerk, the background service worker is triggered and calls `createClerkClient()`. This creates a Clerk instance and refreshes the session token.

## Usage

The following example will demonstrate how to configure your Chrome Extension to use `createClerkClient()` to ensure that the user's session is always fresh. Your application may need to interact with the background service worker from content injected into a webpage or from a new tab that your extension opens. For this example, you will create a web page that the extension will open as a new tab, and that page will have a button on it that, when clicked, retrieves the signed in user's token.
...
<Steps>
  ...
  1. In the `background-worker-demo.html` file, paste the following code to create a basic HTML file that will house the React component for the content script.
     ...
     ```
  1. In the `tabs/` directory, create the `background-work-demo.tsx` file.
  1. In the `background-work-demo.tsx` file, paste the following code to create a React component with a button that will trigger the background service worker to get the token. If the token is returned, it will be displayed on the page.
