...
<Tabs items={["Next.js", "Expo"]}>
  ...
  <Tab>
    ...
    Install `expo-linking` to handle linking and `@clerk/types` for type definitions.
    ...
    <Steps>
      ...
      ```tsx {{ filename: 'hooks/useImpersonation.tsx', collapsible: true }}
      ...
      ```tsx {{ filename: 'hooks/useImpersonatedUser.tsx', collapsible: true }}
      ...
      ```tsx {{ filename: 'app/getImpersonatedUser+api.tsx', collapsible: true }}
      export async function POST(request: Request) {
