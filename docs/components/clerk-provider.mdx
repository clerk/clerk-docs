...
| `navigate?` (deprecated)               | `(to: string) => Promise<unknown> \| void`                                            | Deprecated in favor of `routerPush` or `routerReplace`, depending on your needs. If you would like to perform the same behavior as you had with the `navigate` prop, you can pass the exact same function to both `routerPush` and `routerReplace` and the behavior will be identical. For example, `<ClerkProvider navigate={ x => x }` would now be performed like: `<ClerkProvider routerPush={ x => x } routerReplace={ x => x } />`                                                                                                                               |
