...
<Properties>
  ...
  For example, if you want to render Clerk Elements in a modal, use the `'virtual'` routing mode. Default: `'path'`
</Properties>

The following data attributes are also added to the underlying element:

- `data-global-error` - Refers to the [`<GlobalError>`](/docs/guides/customizing-clerk/elements/reference/common#globalerror) status
