...
> [!BETA]
> Client ID Metadata Documents are currently in beta. To have them enabled for your account, [contact support](https://clerk.com/contact/support).
...
Clerk advertises `client_id_metadata_document_supported: true` only when you enable **Advertise CIMD support** on the **CIMD Clients** tab, which lets compatible clients select [CIMD](!client-id-metadata-document) instead of Dynamic Client Registration. Clerk omits the field otherwise. Applications that still use Clerk's legacy OAuth implementation never advertise it and don't support CIMD clients.
