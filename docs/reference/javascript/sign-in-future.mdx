...
<Properties>
  ...
  - `'complete'`: The user is signed in. Call `signIn.finalize()` to set the newly created session as the active session.
  - `'needs_identifier'`: The user's identifier (e.g., email address, phone number, username) hasn't been provided.
  - `'needs_first_factor'`: One of the following [first factor verification](!first-factor-verification) strategies is missing: `'email_link'`, `'email_code'`, `'phone_code'`, `'password'`, `'passkey'`, `'reset_password_email_code'`, `'reset_password_phone_code'`, `'web3_base_signature'`, `'web3_metamask_signature'`, `'web3_coinbase_wallet_signature'`, `'web3_okx_wallet_signature'`, `'web3_solana_signature'`, `'oauth_<provider>'`, or `'enterprise_sso'`.
...
```ts
function reset(): Promise<{ error: null }>
```
