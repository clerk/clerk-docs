...
<Steps>
  ...
  To protect the `/admin` route, in the `app/admin/page.tsx` file, add the following code. The `checkRole()` function checks if the user has the `admin` Role. If they don't, it redirects them to the home page.
