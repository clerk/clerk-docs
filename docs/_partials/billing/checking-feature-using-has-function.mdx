<If sdk="astro">
  > [!WARNING]
  > Astro SDK hasn't been updated to support "feature" in the `has()` function yet.
  ...
  export function PremiumContent() {
    ...
    if (!hasPremiumAccess) return <h1>Only subscribers with the Premium Access feature can access this content.</h1>
...
<If sdk="chrome-extension">
  ```tsx {{ filename: 'src/screens/page.tsx' }}
...
<If sdk="react">
  ```tsx {{ filename: 'src/pages/page.tsx' }}
...
<If sdk="expressjs">
  ```ts {{ filename: 'src/routes/page.ts' }}
...
<If sdk="fastify">
  ...
  export const pageRoutes = (fastify: FastifyInstance) => {
    fastify.get('/page', async (request: FastifyRequest, reply: FastifyReply) => {
