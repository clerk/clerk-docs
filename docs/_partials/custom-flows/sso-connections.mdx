The following example **will both sign up _and_ sign in users**, eliminating the need for a separate sign-up page. However, if you want to have separate sign-up and sign-in pages, the sign-up and sign-in flows are equivalent, meaning that all you have to do is swap out the `SignIn` object for the `SignUp` object using the [`useSignUp()`](/docs/references/general/hooks/use-sign-up) hook.

The following example:

1. Accesses the [`SignIn`](/docs/references/javascript/sign-in) object using the [`useSignIn()`](/docs/references/general/hooks/use-sign-in) hook.
