...
To enforce strict authentication, replace your existing Node SDK strict middleware, `ClerkExpressRequireAuth`, with the new Express SDK middleware, [`requireAuth()`](/docs/reference/express/overview#require-auth-deprecated).
...
```ts {{ del: [1, [5, 9], 14], ins: [2, 3, 10, 12, [15, 17]] }}
