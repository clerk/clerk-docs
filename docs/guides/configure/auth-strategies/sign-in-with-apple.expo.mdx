...
> [!TIP]
> Using [native components](/docs/reference/expo/native-components-overview)? Apple Sign-In is built-in to [`<AuthView />`](/docs/reference/expo/auth-view) automatically when Apple is enabled as a social connection in your Clerk Dashboard. This guide is for apps using custom sign-in UI with the [`useSignInWithApple()`](/docs/reference/expo/use-sign-in-with-apple) hook.
...
<Steps>
  ...
  1. <Include src="_partials/expo/use-sign-in-with-apple-example" />
