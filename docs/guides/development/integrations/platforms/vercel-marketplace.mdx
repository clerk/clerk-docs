...
When the connected Vercel project has a production `*.vercel.app` domain, Clerk can automatically configure the production instance domain and sync updated production API keys back to Vercel. Production apps on `*.vercel.app` use Clerk's Vercel auto-proxy support at `https://<your-app>.vercel.app/__clerk`, so Next.js middleware or proxy matchers must include `/(api|trpc|__clerk)(.*)`.
