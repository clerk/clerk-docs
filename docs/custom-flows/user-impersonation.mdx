...
<Tabs items={["Next.js", "Expo"]}>
  ...
  <Tab>
    ...
    <Steps>
      ...
      export default function GuestLayout() {
        ...
        return (
          <Protect
            ...
            fallback={<Text>You do not have the permissions to access the dashboard.</Text>}
          >
            <Stack />
          </Protect>
        )
      ...
      In your root folder, create the `generateActorToken+api.tsx` file with the following code. This creates an API route that will call Clerk's Backend API [`/actor_tokens`](/docs/reference/backend-api/tag/Actor-Tokens#operation/CreateActorToken) endpoint to create an actor token.
      ...
      ```tsx {{ filename: 'hooks/useUsers.tsx', collapsible: true }}
      ...
      1. In the `dashboard/` directory, create the `index.tsx` file with the following code.
