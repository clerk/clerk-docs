...
<Properties>
  ...
  Determines how OAuth authentication is performed. `"redirect"` will redirect to the OAuth provider on the current page, `"popup"` will open a new window, and `"auto"` will choose the best method based on whether the current domain typically requires the `"popup"` flow to correctly perform authentication. Defaults to `"auto"`.
</Properties>
