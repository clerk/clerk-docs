...
<Steps>
  ...
  > [!WARNING]
  > Currently, multi-domain can be added to any Next.js, TanStack Start, or Nuxt application. Multi-domain is also supported in client-rendered React applications and apps that use the JavaScript SDK directly, including script-tag integrations. For other React frameworks, multi-domain is still supported as long as you do not use server rendering or hydration.
  ...
  <Tabs items={["Environment variables", "Properties"]}>
    ...
    <Tab>
      ...
      > [!IMPORTANT]
      > When using the JavaScript SDK directly, set `domain` in the `new Clerk()` constructor and set `isSatellite` in `clerk.load()`. When using the script tag, the equivalent constructor option must be passed as a `data-clerk-*` attribute, so `domain` becomes `data-clerk-domain`, while `isSatellite` still belongs in `Clerk.load()`.
      ...
      <Tabs items={["Next.js", "JavaScript SDK", "Script tag"]}>
        ...
        <Tab>
          For apps that initialize Clerk with `@clerk/clerk-js`, set `domain` in the constructor and `isSatellite` in `clerk.load()`.
        ...
        <Tab>
          When using the SDK via a `<script>` tag, constructor options must be passed as `data-clerk-*` attributes on the ClerkJS script, while `isSatellite` and other runtime options belong in `Clerk.load()`.
