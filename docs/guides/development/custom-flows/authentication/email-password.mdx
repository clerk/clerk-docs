...
<Steps>
  ...
  <If is="llm">
    **For AI agents:** Instead of toggling these settings in the Dashboard, verify and enable email and password authentication from the terminal: run `npx clerk@latest config schema --keys auth_email auth_password` to see the shape of the `auth_email` and `auth_password` keys, then apply changes with `npx clerk@latest config patch --json`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
  </If>
  ...
  <If notSdk={["ios", "android"]}>
    ...
    1. Initiate the sign-up process by collecting the user's email address and password with the [`signUp.password()`](/docs/reference/objects/sign-up-future#password) method.
    1. Send a one-time code to the provided email address with the [`signUp.verifications.sendEmailCode()`](/docs/reference/objects/sign-up-future#verifications-send-email-code) method.
    1. Collect the user's one-time code and verify it with the [`signUp.verifications.verifyEmailCode()`](/docs/reference/objects/sign-up-future#verifications-verify-email-code) method.
    1. If the email address verification is successful, the `signUp.status` will be `complete`, and you can finish the sign-up flow with the [`signUp.finalize()`](/docs/reference/objects/sign-up-future#finalize) method to set the newly created session as the active session.
  ...
  <If notSdk={["ios", "android"]}>
    ...
    1. Initiate the sign-in process by collecting the user's email address and password with the [`signIn.password()`](/docs/reference/objects/sign-in-future#password) method.
    1. If the `signIn.status` is `'needs_second_factor'`, the user has MFA enabled. See the [MFA custom flow guide](/docs/guides/development/custom-flows/authentication/multi-factor-authentication) for how to handle this status. If the status is `'needs_client_trust'`, see the [Device Trust custom flow guide](/docs/guides/development/custom-flows/authentication/device-trust).
    1. If the `signIn.status` is `'complete'`, finish the sign-in flow with the [`signIn.finalize()`](/docs/reference/objects/sign-in-future#finalize) method to set the newly created session as the active session.
  ...
  <If sdk="android">
    <CodeBlockTabs options={["EmailPasswordSignInViewModel.kt", "EmailPasswordSignInActivity.kt"]}>
      ...
      combine(Clerk.userFlow, Clerk.isInitialized) { user, isInitialized ->
          ...
          _uiState.value =
            when {
              ...
              _uiState.value == EmailPasswordSignInUiState.NeedsDeviceTrust ->
                EmailPasswordSignInUiState.NeedsDeviceTrust
              ...
              SignIn.Status.NEEDS_CLIENT_TRUST -> {
                ...
                if (emailFactor != null) {
                  signIn
                    ...
                    .onSuccess { _uiState.value = EmailPasswordSignInUiState.NeedsDeviceTrust }
                    .onFailure {
                      // See https://clerk.com/docs/guides/development/custom-flows/error-handling
                      // for more info on error handling
                    }
      ...
      fun verifyDeviceTrust(code: String) {
      val signIn = Clerk.auth.currentSignIn ?: return
      viewModelScope.launch {
        signIn
      ...
      data object NeedsDeviceTrust : EmailPasswordSignInUiState
      ...
      setContent {
        ...
        EmailPasswordSignInView(
          state = state,
          onSubmit = viewModel::submit,
          onVerifyDeviceTrust = viewModel::verifyDeviceTrust,
          onResendDeviceTrustCode = viewModel::resendDeviceTrustCode,
        )
      }
      }
      }
      ...
      onSubmit: (String, String) -> Unit,
      onVerifyDeviceTrust: (String) -> Unit,
      onResendDeviceTrustCode: () -> Unit,
      ) {
      var email by remember { mutableStateOf("") }
      var password by remember { mutableStateOf("") }
      var deviceTrustCode by remember { mutableStateOf("") }
      ...
      when (state) {
        ...
        EmailPasswordSignInViewModel.EmailPasswordSignInUiState.NeedsDeviceTrust -> {
          Column(
            verticalArrangement = Arrangement.spacedBy(16.dp, Alignment.CenterVertically),
            horizontalAlignment = Alignment.CenterHorizontally,
          ) {
            ...
            TextField(
              ...
              onValueChange = { deviceTrustCode = it },
              label = { Text("Code") },
            )
            Button(onClick = { onVerifyDeviceTrust(deviceTrustCode) }) { Text("Verify") }
            Button(onClick = onResendDeviceTrustCode) { Text("I need a new code") }
          }
        }
