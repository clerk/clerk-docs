...
<Tabs items={["Standard setup", "Keyless Mode (Beta)"]}>
  <Tab>
    <Steps>
      ## Install `@clerk/nextjs`
  ...
  <Tab>
    > [!IMPORTANT]
    > [Keyless Mode](/docs/keyless-mode) is currently in beta. To enable this feature, set this environment variable `NEXT_PUBLIC_CLERK_ENABLE_KEYLESS=true` in your `.env.local` file.
