...
> [!QUIZ]
> Why are session tokens not `HttpOnly`? Is this a security issue?
>
> ---
>
> It is normally recommended to make session cookies `HttpOnly`. This flag prevents client-side JavaScript from accessing the cookie, which helps mitigate the risk of cross-site scripting (XSS) attacks.
>
> However, the way Clerk is architected makes this such that it would not work. Remember that FAPI is hosted on a different origin than your app (FAPI at `clerk.example.com` in production, and your app at `example.com`), and FAPI is responsile for handing authentication flows.
>
> If FAPI were to set the session cookie as `HttpOnly`, its domain value would be set to `clerk.example.com`, which would prevent it from being accessible to your app's domain (`example.com`). This is because the domain value of a cookie is set to the domain of the server that set the cookie. And as seen above, we strictly scope the domain of the session token so that it cannot be exfiltrated by a compromised app on a subdomain shared with your primary app. So, in this case, if the session cookie were set by FAPI as `HttpOnly`, it would be set with its domain value as `clerk.example.com`, and therefore would it not be sent from your client side to your app's server on `example.com`, since cookies are only sent to the domain that matches their domain value.
>
> To get around this limitation, Clerk returns the value of the session token from FAPI after a user has signed in, and then the Clerk client-side SDK uses JavaScript to set the cookie on your app's domain. Since it's set by JavaScript, it cannot be set as `HttpOnly`.
>
> This approach has some drawbacks and some benefits. On the plus side, it allows the token's value and session claims to be accessed on the client side, which is often quite valuable, and allows developers to send the session token as a custom header in requests, which make it possible to use a different subdomain (like `api.example.com`) for your backend, even in scenarios where the cookie would otherwise not be sent.
>
