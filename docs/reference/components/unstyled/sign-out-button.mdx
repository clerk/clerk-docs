...
You can sign out of a specific session by passing in a `sessionId` to the `signOutOptions` prop. This is useful for signing a single account out of a [multi-session application](/docs/guides/secure/session-options#multi-session-applications).
...
<If sdk="astro">
  ```astro {{ filename: 'pages/index.astro' }}
  ---
  import { SignInButton, SignOutButton, useAuth } from '@clerk/astro/components'
  ---
...
<Properties>
  ...
  **For Astro only:** If `true`, the `<SignOutButton>` component will render its children as a child of the component.
  ...
  Options for signing out. Includes `sessionId` which if passed, signs the user out of a specific session. Useful for [multi-session applications](/docs/guides/secure/session-options#multi-session-applications).
