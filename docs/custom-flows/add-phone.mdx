...
<Tabs items={["Next.js", "iOS", "Android"]}>
  ...
  <Tab>
    ```kotlin {{ filename: 'AddPhoneView.kt', collapsible: true }}
    @Composable
    fun AddPhoneView(modifier: Modifier = Modifier, viewModel: AddPhoneViewModel = viewModel()) {
      val state by viewModel.uiState.collectAsState()
      when (state) {
        AddPhoneViewModel.AddPhoneUiState.NeedsVerification -> {
          InputContentView(buttonText = "Continue", placeholder = "Enter phone number") {
             viewModel.createPhoneNumber(it) 
          }
        }

        AddPhoneViewModel.AddPhoneUiState.Verified -> {
          Text("Verified!")
        }

        AddPhoneViewModel.AddPhoneUiState.Verifying -> {
          InputContentView(buttonText = "Verify", placeholder = "Enter code") {
            viewModel.verifyCode(it, state.phoneNumber)
          }
        }
      }
    }
    ...
    class AddPhoneViewModel: ViewModel() {
      private val _uiState = MutableStateFlow<AddPhoneUiState>(AddPhoneUiState.NeedsVerification)
      ...
      fun createPhoneNumber(phoneNumber: String) {
        ...
        viewModelScope.launch {
            ...
            .onSuccess { 
              ...
              _uiState.value = AddPhoneUiState.NeedsVerification(it) 
            }
            .onFailure { Log.e("AddPhoneViewModel", "Failed to create phoneNumber and prepare verification: $it.longErrorMessageOrNull") }
        }
      }
        ...
        viewModelScope.launch {
          newPhoneNumber
            ...
            .onSuccess { 
              ...
              _uiState.value = AddPhoneUiState.Verified 
            }
            .onFailure { Log.e("AddPhoneUiState", "Failed to verify code: $it.longErrorMessageOrNull") }
        }
      }

      sealed interface AddPhoneUiState {
        data object NeedsVerification: AddPhoneUiState
        data class Verifying(val phoneNumber: PhoneNumber) : AddPhoneUiState
        data object Verified : AddPhoneUiState
      }
    }
    ```
  </Tab>
