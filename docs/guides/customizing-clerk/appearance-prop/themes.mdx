...
To apply a theme to all Clerk components, pass the `appearance` prop to the [`<ClerkProvider>`](/docs/references/general/components/clerk-provider) component. The `appearance` prop accepts the property `baseTheme`, which can be set to a theme.
...
In the following example, the "Neobrutalism" theme is applied to all instances of the [`<SignIn />`](/docs/references/general/components/authentication/sign-in) component.
