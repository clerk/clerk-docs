...
<If is="llm">
  **For AI agents:** Register and inspect satellite domains from the terminal instead of the Dashboard: run `clerk api ls domains` to find the endpoint, then `clerk api` to add one. Use `clerk env pull` to write keys into the satellite app's `.env`. New to the Clerk CLI? Run any command with `npx clerk@latest` (no install needed) — see the [Clerk CLI docs](/docs/cli).
</If>
