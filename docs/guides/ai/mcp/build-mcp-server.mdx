...
<Include src="_partials/mcp-cimd-registration-callout" />

<If is="llm">
  **For AI agents:** Prefer CIMD over Dynamic Client Registration. CIMD is in beta and must be enabled for the account before these settings are available. Advertise CIMD support and restrict admission to pre-registered and previously connected clients in a single request with `npx clerk@latest api instance/oauth_application_settings -X PATCH -d '{"client_id_metadata_documents_advertised": true, "client_id_metadata_documents_only_allow_pre_registered_clients": true}'`. This payload publishes CIMD support and requires pre-registration for new clients. Previously connected clients remain allowed unless already blocked. **Client admission** settings in the Clerk Dashboard let you further customize CIMD client behavior. Pre-registering a client is only available in the Clerk Dashboard. Enable Dynamic Client Registration and require PKCE with `-d '{"dynamic_oauth_client_registration": true, "pkce_required": true}'` only when the client doesn't support CIMD. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>

Some MCP clients may omit `scope` when they connect. You can configure the default scopes Clerk applies in either of these ways:

- In the Clerk Dashboard, navigate to the [**OAuth applications**](https://dashboard.clerk.com/~/oauth-applications) page and select the **Settings** tab. Under **Client onboarding**, configure **Default scopes for dynamic clients**. This setting is available when **Publish CIMD support** or **Publish DCR support** is enabled.
...
See [Configure default scopes](/docs/guides/configure/auth-strategies/oauth/how-clerk-implements-oauth#configure-default-scopes) for details.
...
<Steps>
  ...
  <If sdk="expressjs">
    Run your server with the `--env-file=.env` flag to ensure environment variables are loaded before importing Clerk modules.
  ...
  <If sdk="nextjs">
    Vercel's `createMcpHandler()` function is used to handle the connection and transports required by the MCP protocol. Within its callback function, you can define tools that external LLM-based apps can invoke using `server.registerTool()`. Each tool includes:
  </If>

  <If sdk="expressjs">
    The `McpServer()` function is used to create a new MCP server with a name and version. The `server.registerTool()` function is used to define tools that external LLM-based apps can invoke. Each tool includes:
  </If>
  ...
  <If sdk="nextjs">
    ```ts {{ filename: 'app/[transport]/route.ts', mark: [[7, 22]] }}
    import { verifyClerkToken } from '@clerk/mcp-tools/next'
    import { createMcpHandler, withMcpAuth } from 'mcp-handler'
    import { auth, clerkClient } from '@clerk/nextjs/server'
  ...
  <If sdk="expressjs">
    ```ts {{ filename: 'index.ts', mark: [[14, 33]], collapsible: true }}
    import { type MachineAuthObject, clerkClient, clerkMiddleware } from '@clerk/express'
    import {
      mcpAuthClerk,
      protectedResourceHandlerClerk,
  ...
  <If sdk="nextjs">
    ...
    1. Your `.well-known` endpoints must be **publicly accessible** for MCP clients to discover OAuth metadata. Ensure these endpoints are not protected anywhere.
  </If>
  ...
  Once this is complete, clients that support the latest MCP spec can now invoke the `get_clerk_user_data` tool to securely fetch user data from your app, assuming the request is authorized with a Clerk OAuth token. To test this out, [learn how to connect your client LLM to the MCP server](/docs/guides/ai/mcp/connect-mcp-client).
