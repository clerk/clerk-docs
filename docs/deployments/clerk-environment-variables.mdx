...
Components, such as [`<ClerkProvider>`](/docs/reference/general/components/clerk-provider), [`<SignIn>`](/docs/components/authentication/sign-in), and more, provide props for you to specify where users will be redirected. For example, `<ClerkProvider>` has the `signInFallbackRedirectUrl` and `signUpFallbackRedirectUrl` props.
