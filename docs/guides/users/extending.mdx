...
To store information about a user that Clerk doesn't collect, you can use metadata, which will get stored on the user's [`User` object](/docs/guides/users/managing). There are three types of metadata: "public", "private", and "unsafe".
...
On the frontend, it's available on the [`User`](/docs/references/javascript/user) object which can be accessed using the [`useUser()`](/docs/references/general/hooks/use-user) hook.
...
On the frontend, it is available on the [`User`](/docs/references/javascript/user) object, which you can access by using the [`useUser()`](/docs/references/general/hooks/use-user) hook.
