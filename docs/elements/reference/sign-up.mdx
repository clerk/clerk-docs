...
| ---- | ------------------- | ----------------------------------------------------------------- |
| `path?` | `string` | The root path the sign-up flow is mounted at. Default: `/sign-up` |
...
Renders the Cloudflare Turnstile widget. It must be used within the `<Step name="start">` component. By default, renders a `<div>`, specifically `<div id="clerk-captcha"></div>`.
