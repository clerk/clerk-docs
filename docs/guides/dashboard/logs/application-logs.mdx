...
| `sign_up.phone_number.verification.code_attempt_rejected` | Verification phone code attempt for sign-up was rejected before the code was checked (for example, the code was never sent) |
...
| `sign_in.phone_number.verification.code_attempt_rejected` | Verification phone code attempt for sign-in was rejected before the code was checked (for example, the code was never sent) |
...
| Event type | Description |
| - | - |
| `session.reverification.attempt_first_factor.failed` | First factor attempt for session reverification failed |
| `session.reverification.attempt_first_factor.passed` | First factor attempt for session reverification passed |
| `session.reverification.attempt_second_factor.failed` | Second factor attempt for session reverification failed |
| `session.reverification.attempt_second_factor.passed` | Second factor attempt for session reverification passed |
| `session.reverification.completed` | User completed reverifying their session |
| `session.reverification.prepare_first_factor.failed` | First factor preparation for session reverification failed |
| `session.reverification.prepare_first_factor.passed` | First factor preparation for session reverification passed |
| `session.reverification.prepare_second_factor.failed` | Second factor preparation for session reverification failed |
| `session.reverification.prepare_second_factor.passed` | Second factor preparation for session reverification passed |
| `session.reverification.required` | Request to a protected endpoint was rejected because the session had not been reverified, or its reverification had expired |
| `session.reverification.started` | User started reverifying their session |
...
| Event type | Description |
| - | - |
| `sms.accepted` | SMS was accepted for delivery (delivery not yet confirmed) |
| `sms.delivered` | SMS was delivered |
| `sms.failed` | SMS failed to send |
| `sms.unconfirmed` | SMS delivery was not confirmed |
| `sms.undeliverable` | SMS could not be delivered |

To learn how to interpret these events, understand failure reasons, and troubleshoot delivery, see [SMS delivery events](/docs/guides/dashboard/logs/sms-delivery-events).
