...
| **JS + Native Sign-in** | [Custom forms](!custom-flow) + native OAuth buttons | Native (no browser) | Yes | Custom UI with native Sign in withGoogle/Apple |
...
<Tabs items={["JavaScript", "JS + Native Sign-In", "Native Components"]}>
  ...
  <Tab>
    ...
    This approach combines [custom forms](!custom-flow) with native Sign in with Google and Sign in with Apple buttons that authenticate without opening a browser. This requires a [development build](https://docs.expo.dev/develop/development-builds/introduction/).
