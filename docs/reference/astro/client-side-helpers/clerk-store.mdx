...
The `$clerkStore` store provides a convenient way to access the [`Clerk`](/docs/reference/javascript/clerk){{ target: '_blank' }} object. This provides access to some methods that are not available in other stores.
