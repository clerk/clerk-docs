...
Playwright executes tests in isolated environments called browser contexts. So, each test case runs in a new browser context, which means that the user session is not shared between test cases. However, tests can load existing authenticated state in Playwright.

Following the [Playwright docs on Authentication](https://playwright.dev/docs/auth), you can test authenticated flows without having to sign in for each test case.

The steps below showcase an example of how to save the session storage in a file and load it in your test cases. Of course, you can customize this to fit your needs. You can also have a look at our example repository [here](https://github.com/clerk/clerk-playwright-nextjs).

<Steps>
  ### Add the storage file in your `.gitignore`

  Create a `playwright/.clerk` directory and add it to your `.gitignore`.
  ...
  ### Save the session in your setup file

  Save the session in your [global setup file](https://playwright.dev/docs/test-global-setup-teardown). This file is executed before all projects.
  ...
  setup('authenticate and save state to storage', async ({ page }) => {
    // here you can either use a helper function to sign in or do it manually
    await page.goto('/')
    await clerk.signIn({
      page,
      signInParams: {
  ...
  ### Load the stored session in your tests

  You can either load the stored session in a Playwright project:
  ...
  Or you can load the stored session directly inside a test file:

  ```tsx {{ filename: 'authenticated.spec.ts' }}
  import { test } from '@playwright/test'
