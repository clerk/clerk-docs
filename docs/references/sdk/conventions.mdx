...
Ideally, when a user switches from one Clerk SDK to another the general concepts of how it works, feature set, names, and exports should be pretty similar if not the same. By following some conventions you can get an SDK close to that ideal state, making it easier for users to quickly spin up a new project with it.
...
Any environment variable used in the context of Clerk should be prefixed with `CLERK_`. You can find all commonly used environment variables in [Clerk docs](/docs/deployments/clerk-environment-variables).
...
Clerk recommends implementing the [sign-in and sign-up redirects](/docs/deployments/clerk-environment-variables#sign-in-and-sign-up-redirects) environment variables.

## README

The README is most often the first thing a user sees of your SDK. It’s paramount to have all the necessary information in there — use the template below to setup your own README.
...
This is by large the biggest benefit you can provide to your users as they'll get IntelliSense autocompletion in their IDE.
...
This setup ensures that e.g. functions that only work on the server are bundled into client-side code. It also makes it easier for the bundler to tree-shake any unused code.

### CommonJS/ES Module

Authoring [dual packages](https://nodejs.org/api/packages.html#dual-commonjses-module-packages) is challenging but not impossible. Before you decide to publish both CommonJS (CJS) and ES Module (ESM) formats from your package, research if you can only publish your package in ESM format. Most modern frameworks allow you to do just that.
