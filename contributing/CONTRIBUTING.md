...
- **Failures** stop the build. These include invalid frontmatter (for example, an `sdk` value outside the set defined in [`scripts/lib/schemas.ts`](../scripts/lib/schemas.ts)), a missing `title`, [non-descriptive link text](../styleguides/STYLEGUIDE.md#use-descriptive-anchor-text) such as `here` or `Learn more`, [incorrect link-target annotations](../styleguides/STYLEGUIDE.md#use-consistent-link-targets), a lowercase [Clerk feature proper noun](../styleguides/STYLEGUIDE.md#capitalize-clerk-feature-proper-nouns) in prose, headings, frontmatter, or manifest nav titles (for example, "organization domains" instead of "Organization domains"), or frontmatter that fails to parse. (For a real example, [#3176](https://github.com/clerk/clerk-docs/pull/3176) shipped an invalid `sdk` key that broke the production build.)
...
The `search` frontmatter field controls how a page is indexed for docs search. Records are generated and pushed by `scripts/update-algolia-records.ts` — see the **Search index (Algolia)** section in `AGENTS.md` for how the index works. The field has the following subfields:
...
| `keywords` | `Array<string>` | `[]`    | Additional [searchable](https://www.algolia.com/doc/guides/managing-results/must-do/searchable-attributes/) keywords to include when indexing the page. These are not visible to users. Keywords rank with the same strength as the page **title**: a keyword match beats matches in headings and body content on other pages, so the page you add a keyword to will typically sweep the top results for that query. That makes a keyword the right tool when a page _should_ be the definitive answer for a term that isn't in its title — and a query hijack when it shouldn't. Every word of a multi-word keyword matches at title strength on its own — `native api` also boosts the page for plain "api" searches — and search treats hyphens as word breaks, so `reset-password` boosts "reset password". Only add a keyword when the page is the intended #1 result for that query _and_ no individual word in it hijacks a broader query; test the full keyword and each of its words, and run the search regression suite (see AGENTS.md) before merging. For example, [`docs/guides/secure/restricting-access.mdx`](https://clerk.com/docs/guides/secure/restricting-access) sets the `allowlist` keyword so it surfaces first when searching for "allowlist". To boost a page's ranking for terms it already contains, use `rank` instead. |
...
| `ignoreSdkWarning?` (optional) | `boolean`            | Build-time directive that suppresses SDK scope validation for this `<If>` block. Use on direct (non-partial) pages when an `<If>` intentionally references SDKs outside the page's frontmatter/manifest scope — e.g., showing a callout to mobile SDK users on a generic page. **Do not use in partials** — partials are automatically handled by `fromPartial` marking. |
