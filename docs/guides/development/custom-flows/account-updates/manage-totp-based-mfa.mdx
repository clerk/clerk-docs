...
<Steps>
  ...
  <If sdk="expo">
    ...
    <Tabs items={["Layout", "Account page", "Add TOTP page"]}>
      ...
      <Tab>
        In the `(account)` group, create an `manage-mfa.tsx` file with the following code. This page adds the functionality for generating the QR code and backup codes.
