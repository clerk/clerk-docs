...
Clerk's API keys feature allows your application's users to create API keys that can delegate access to your application's API on their behalf. API keys are typically long-lived, [opaque tokens](!opaque-token) associated with a user or organization that provide access to a set of API endpoints for third-party services.
...
API keys are a usage-based paid feature. The pricing is as follows:

- `$0.001` per key creation. The first 1000 creations per month are free.
- `$0.00001` per key verification. The first 100,000 verifications per month are free.
...
Clerk provides UI components that you can drop into your application to support your users in generating and managing API keys. You can integrate these components into your application's interface, or create your own custom logic for API key management using [backend SDK methods](#using-api-keys-via-clerks-sdks).
...
If you would like to create, list, and revoke API keys programmatically on the server side, you can use Clerk's Backend SDKs. This guide uses the [`clerkClient`](/docs/reference/backend/overview) wrapper to demonstrate usage, but **please note that the API keys feature is available in all of Clerk's Backend SDKs**.
