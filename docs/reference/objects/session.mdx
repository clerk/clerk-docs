...
{/* DOCS NOTE: Some of these parameters sections are manually written because as you will see, they are complex types that are difficult to generate with Typedoc. For example, many of these are union types where each of the union members share a property like `code`. So if we were to generate a single parameters table for all of the values, `code` would be duplicated for each of the union members, and the table wouldn't really make sense. */}
...
type WithReverification<T> = T & {
  /**
   * The reverification configuration to check for. This feature is currently in public beta. **It is not recommended for production use.** See https://clerk.com/docs/reference/backend/types/auth-object#reverificationconfig for more information.
   */
