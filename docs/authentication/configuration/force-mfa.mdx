...
<Steps>
  ...
  1. Under **Customize session token**, enter the following JSON in the **Claims** editor and select **Save**. The key can be any string, but the value must be the `user.two_factor_enabled` property. If you have already customized your session token, you may need to merge this with what you currently have.
