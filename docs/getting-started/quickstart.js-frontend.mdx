...
<Tabs items={["NPM module", "<script>"]}>
  <Tab>
    <Steps>
      ...
      To initialize Clerk, add the following code to your `main.js` file. This code imports your [Publishable Key](!publishable-key) using Vite's `import.meta.env` object, creates a new [`Clerk`](/docs/reference/javascript/clerk) instance, and calls the [`load()`](/docs/reference/javascript/clerk#load) method to initialize Clerk.
...
<Cards>
  ...
  - [Create a custom sign-in flow with MFA](/docs/guides/development/custom-flows/authentication/email-password-mfa)
