...
<Steps>
  ...
  <Tabs items={["Next.js", "JavaScript", "Expo", "iOS", "Android (beta)"]}>
    ...
    <Tab>
      ...
      1. In the `(auth)` group, create a `_layout.tsx` file with the following code. The [`useAuth()`](/docs/references/hooks/use-auth) hook is used to access the user's authentication state. If the user's already signed in, they'll be redirected to the home page.
