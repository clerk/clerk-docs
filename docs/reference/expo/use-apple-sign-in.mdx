...
2. **Configured Apple Developer Console**: Enabled Sign in with Apple capability for your App ID
3. **Enabled Apple in Clerk Dashboard**: Added Apple as an SSO connection
4. **Created a native build**: Using EAS Build or local prebuild (does not work with Expo Go)
...
2. **Native UI**: iOS displays the native Apple Sign-In sheet
3. **User authentication**: User authenticates with Face ID, Touch ID, or password
4. **ID token exchange**: The hook receives an identity token from Apple and sends it to Clerk using the `oauth_token_apple` strategy
5. **Transfer flow**: Clerk checks if the Apple ID exists:
   - **Exists**: Completes sign-in and returns the session
   - **New**: Automatically creates a sign-up with the transfer flow
6. **Session creation**: Returns the created session ID and `setActive` function
