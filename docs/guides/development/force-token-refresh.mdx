...
When using metadata, it's recommended to [store it in the user's session token](/docs/guides/configure/session-token) to avoid making an API request to Clerk's Backend API when retrieving it, as API requests are slower and are subject to rate limits.
...
To do so, follow the [guide on customizing your session token](/docs/guides/configure/session-token).
