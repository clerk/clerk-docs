...
The following example configures the header for screens in the `(home)` route group. Ensure the route group contains an `index.tsx` route.
...
### Add custom profile pages

Pass `customPages` to `userProfileProps` to add custom rows to the profile that `<UserButton />` opens. The following example adds a row that opens an external support page:
...
To render React Native content and navigate within the native profile, see [Add custom pages to `<UserProfileView />`](/docs/reference/expo/native-components/user-profile-view#add-custom-pages) for the available options and a complete example.
...
<Properties>
  ...
  Configuration for the user profile that opens when the user taps `<UserButton />`.
</Properties>
...
<Properties>
  ...
  Custom pages displayed as rows in the user profile. See [Add custom pages to `<UserProfileView />`](/docs/reference/expo/native-components/user-profile-view#add-custom-pages) for configuration options and examples.
</Properties>
