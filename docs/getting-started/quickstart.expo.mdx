...
<Steps>
  ...
  Replace the contents of your root layout with the following code. This wraps your app with `<ClerkProvider>` and uses `<Slot />` to render the active route group.
  ...
  > [!TIP]
  > The `tokenCache` from `@clerk/expo/token-cache` uses `expo-secure-store` to securely persist the user's session token. Run `npm install expo-secure-store` if you don't already have it installed.
  ...
  1. In the `(home)` group, create a `_layout.tsx` file with the following code. The [`useAuth()`](/docs/reference/hooks/use-auth) hook is used to check authentication state. Signed-out users are redirected to the sign-in page.
