...
- Faster resolution of the `isLoaded` property and the [`<ClerkLoaded>`](/docs/references/general/components/control/clerk-loaded) control component with only a single network fetch attempt. If the fetch fails, it gracefully falls back to cached resources.
...
<Steps>
  ...
  On [`<ClerkProvider>`](/docs/references/general/components/clerk-provider), pass the `resourceCache` object to the `__experimental_resourceCache` property, as shown in the following example:
