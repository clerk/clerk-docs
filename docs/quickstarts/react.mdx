...
You can use the `<SignedIn />` and `<SignedOut />` control components to restrict access to content, or even whole pages. The `<SignedIn />` component requires the user be signed in to view the content inside the tags. The `<SignedOut />` components reuires the user be signed out. Add some basic content and wrap some of with the `<SignedOut />` component and other content with the `<SignedIn />` component. 
...
Add the following anywhere inside `<ClerkProvider />` in your app. First add `import { SignOutButton, SignInButton, SignedIn, SignedOut } from "@clerk/clerk-react";` to the top of your file. Then add the `<SignInButton />` inside of the `<SignedOut />` tags and the `<SignOutButton />` inside of the `<SignedIn />` tags. Restart your application
...
export default App
```
You can now visit your application and use the buttons to sign-in to your application and see the protected content and the protected `<SignOutButton />`
