...
Clerk supports JIT account provisioning for all [supported SAML providers](/docs/authentication/saml/overview) out of the box. This feature is available for any Clerk app that has the **[Sync user attributes during Sign in](#sync-user-attributes-during-sign-in)** setting enabled, which is enabled by default.
