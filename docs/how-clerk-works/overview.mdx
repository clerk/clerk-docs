...
> ---
>
> Imagine this scenario: you hit your frontend and you have an expired session token, you need a new valid session token. Normally, to refresh your session token, you need to send your *valid* session token to clerk, which it will verify, then send you a new valid one. This is *secure by default*, because anyone who has a valid session token is authenticated as you, as we have learned in the rest of this guide.
>
...
> 
> However, if your app is rendered on the server, your hands are tied. If we make an API request to FAPI it will not be able to check `__client`, because **cookies live in the browser**, so the FAPI request must come from your browser in order for  it to be sent with the `__client` cookie. A *server to server* request will not include any cookies, since cookies are stored by browsers.
> 
> However, when the server responds with a *redirect* to FAPI, we end up going back through the browser. The server returns a redirect status code, the browser reads this, and the browser issues the redirect. This changes it to *server to browser to server,* which means that the request to FAPI is delivered with the `__client` cookie, since it comes from the browser.
> 
