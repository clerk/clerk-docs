...
<Tabs items={["NPM module", "<script>"]}>
  <Tab>
    <Steps>
      ...
      To initialize Clerk, import the [`Clerk`](/docs/reference/javascript/clerk) class and instantiate it with your Clerk [Publishable Key](!publishable-key). Then, call the `load()` method, as shown in the following example:
