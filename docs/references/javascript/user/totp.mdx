...
To see a complete example of how to use these methods, see the [TOTP example](#totp-example) section.

<Callout type="warning">
  Multi-factor authentication must be enabled in your Clerk settings for these methods to work. See the [Multi-factor authentication](/docs/authentication/configuration/sign-in-sign-up-options#set-up-multi-factor-authentication) section to learn more.
</Callout>
...
| Type | Description |
| --- | --- |
| <code>Promise\<[TOTPResource](#totp-resource)\></code> | This method returns a `Promise` which resolves with the newly created TOTP object. |
...
| Type | Description |
| --- | --- |
| <code>Promise\<[TOTPResource](#totp-resource)\></code> | This method returns a `Promise` which resolves with the newly verified TOTP object. |
...
| Type | Description |
| --- | --- |
| <code>Promise\<[DeletedObject](/docs/references/javascript/types/deleted-object)\></code> | This method returns a `Promise` which resolves with the reference to the deleted TOTP object. |
...
Can only be created for the user if the user has another multi-factor authentication method enabled for their account.
...
| Type | Description |
| --- | --- |
| <code>Promise\<[BackupCodeResource](#backup-code-resource)\></code> | This method returns a `Promise` which resolves with the newly created backup code. |
...
The following example demonstrates how to use the TOTP methods to create and verify a TOTP secret, disable TOTP, and create backup codes for a user. Upon any error, the example will display the error message to the user.
...
<Tabs type="npm-script" items={['NPM module', '<script>']}>
  <Tab>
    ...
    if (clerk.user) {
      ...
      // Create backup codes
      document
        .getElementById("create-backup-codes")
        .addEventListener("click", async () => {
          clerk.user
            .createBackupCode()
            .then((res) => {
