...
In this example, you can see that the returned response is the created `User` object, with `firstName` as `"Test"`, `lastName` as `"User"`, and `emailAddress` as an array of email addresses, which would include `'testclerk123@gmail.com'`.
