...
<Steps>
  ...
  <CodeBlockTabs options={["clerkMiddleware()", "authMiddleware()"]}>
    ```tsx {{ prettier: false, filename: 'middleware.tsx' }}
    ...
    ```tsx {{ prettier: false, filename: 'middleware.tsx', mark: [4] }}
  ...
  <Tabs type="framework" items={["Next.js", "Node"]}>
    <Tab>
      <CodeBlockTabs type="router" options={["App Router", "Pages Router"]}>
        ```ts {{ prettier: false, filename: 'app/api/webhooks/route.ts' }}
        ...
        ```ts {{ prettier: false, filename: 'pages/api/webhooks.ts' }}
        import { Webhook } from 'svix'
        import { WebhookEvent } from '@clerk/nextjs/server'
    ...
    <Tab>
      ```js {{ prettier: false, filename: 'clerkWebhookHandler.js' }}
      import { Webhook } from "svix";
