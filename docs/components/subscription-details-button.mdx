...
`<SubscriptionDetailsButton />` will throw an error if the `for` property is set to `'org'` and no [active organization](/docs/guides/organizations/overview#active-organization) is set.
