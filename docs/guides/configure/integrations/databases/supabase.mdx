...
<Steps>
  ...
  <Tabs items={["Client-side rendering", "Server-side rendering"]}>
    <Tab>
      The following example uses the [Next.js SDK](/docs/references/nextjs/overview) to access the [`useUser()`](/docs/reference/general/hooks/use-user) and [`useSession()`](/docs/reference/general/hooks/use-session) hooks, but you can adapt this code to work with any React-based Clerk SDK.
