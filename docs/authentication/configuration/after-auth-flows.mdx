...
Once enabled in the Clerk Dashboard, after-auth flows are **embedded by default** within the `<SignUp />` and `<SignIn />` components. For more customization, you can opt out of using the `<SignUp />` and `<SignIn />` components and [create custom pages for your after-auth flows](/docs/custom-flows/overview#after-auth-flows).
...
<CodeBlockTabs options={["React", "JavaScript"]}>
  ...
  if (session?.currentTask) {
    ...
    // users resolve these tasks, see https://clerk.com/docs/custom-flows/overview#after-auth-flows
...
#### Using the `NavigateToTasks` control component
