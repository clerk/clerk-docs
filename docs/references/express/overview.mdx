...
The [`clerkMiddleware()`](https://clerk.com/docs/references/nextjs/clerk-middleware) helper integrates Clerk authentication into your Express application. It is required to be set in the middleware chain before using other Clerk utilities, such as `requireAuth()` and `getAuth()`.

### `requireAuth()`

The [`requireAuth()`](/docs/references/express/require-auth) helper protects routes in your Express application by checking if the user is authenticated or authorized.

### `getAuth()`

The [`getAuth()`](https://clerk.com/docs/references/nextjs/get-auth) helper retrieves the authentication state, allowing you to protect your API routes or gather relevant data.

### `clerkClient`

You can use [Clerk's JavaScript Backend SDK](/docs/references/backend/overview) to interact with the Clerk API. All resource operations are mounted as sub-APIs on the `clerkClient` object. See the [reference documentation](/docs/references/backend/overview#usage) for more information.
