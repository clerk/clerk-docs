...
- The `sessions.getToken()` return value has been update to return `{ data, errors }` instead of the `jwt` string.
    ...
    import { clerkClient } from '@clerk/clerk-sdk-node';
    import { clerkClient } from 'gatsby-plugin-clerk/ssr';

    import { createClerkClient } from '@clerk/backend';
    clerkClient = createClerkClient({ secretKey: '...' });

    - const jwtString = await clerkClient.sessions.getToken({ template: '...' });
    + const { data, errors } = await clerkClient.sessions.getToken({ template: '...' });
...
- Replace `organizations.getPendingOrganizationInvitationList` with `organizations.getOrganizationInvitationList` and pass a `status` parameter.
    ...
    ```diff
    - clerkClient.organizations.getPendingOrganizationInvitationList({ ...params })
    + clerkClient.organizations.getOrganizationInvitationList({ ...params, status: "pending" })
    ```
...
- The `clerkClient.__unstable_options` property was removed. You could use it to update the internal options. Instead, create a new clerkClient instance using `createClerkClient`. For example:
    ...
    ```
- The `signJwt`, `hasValidSignature`, `decodeJwt`, `verifyJwt`, and `verifyToken` signatures have changed. Instead of returning a value or throwing an error those functions will **always** return a `{ data, error }` object. You will need to throw the error yourself:
...
<Accordion titles={["Removal of unstable/experimental properties", "Removal of fallback data for paginated methods", "Appearance", "Use .imageUrl for image related responses/resources"]} heading="h4">
  <AccordionPanel>
...
- The `MembershipRole` type was replaced with `OrganizationCustomRoleKey` (related to [roles and permissions](/docs/organizations/roles-permissions)). This TypeScript type can be found in such use cases:
    ...
    ```
- The `NEXT_PUBLIC_CLERK_JS` environment variable was renamed to `NEXT_PUBLIC_CLERK_JS_URL`. Make sure to update this in all environments (e.g. dev, staging, production).
...
- The `routing="path"` has become the default routing strategy for the following components and the `path` property is required for the path-based routing.
  ...
  ```jsx
    ...
    // No prop example for SignIn / SignUp requires {framework_prefix}CLERK_SIGN_UP_URL / {framework_prefix}CLERK_SIGN_IN_URL env variable to be defined
    <SignIn />
    <SignUp />
    ...
    // Usage example #2
    <UserProfile routing="hash"/> // or routing="virtual"
    <CreateOrganization routing="hash"/> // or routing="virtual"
    <OrganizationProfile routing="hash"/> // or routing="virtual"
    <SignIn routing="hash"/> // or routing="virtual"
    <SignUp routing="hash"/> // or routing="virtual"
  ```
