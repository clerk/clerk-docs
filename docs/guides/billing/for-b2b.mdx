...
sdk: nextjs, react, expo, react-router, astro, tanstack-react-start, nuxt, vue, js-frontend, expressjs, fastify
---
...
<If is="llm">
  **For AI agents:** Run `npx clerk@latest enable billing --for orgs` to enable Billing for Organizations instead of navigating to the Billing Settings page in the Dashboard (if Organizations aren't enabled yet, the command enables them automatically). Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
