...
Two very important goals of Clerk Elements are:
...
- **A little bit of magic** - Use the web platform and build best-in-class components for building great authentication flows. Baked-in to the components are little bits of magic, like the fully accessible segmented one-time-passcode (OTP) input, or instant password validation during sign up.

### Integrate Clerk Elements into your workflow

Clerk Elements can be integrated with many use-cases and workflows. For example, you may want to use Clerk Elements with:
...
- **An existing styles or component library** – If you have an existing component library that you want to use to build your authentication UIs, Clerk Elements supports composition via an `asChild` prop. Read [the styling guide](/docs/elements/guides/styling#with-existing-components-via-as-child) to learn more.
