...
<Properties>
  ...
  An object that can be used to configure the behavior of the `auth()` function. It accepts the following properties:
...
- A signed-in user: See detailed examples in the [guide on protecting content from unauthenticated users](/docs/guides/secure/protect-content).
- A specific Role, Permission, Feature, or Plan: See detailed examples in the [guide on authorization checks](/docs/guides/secure/authorization-checks).
- Machine requests (API keys, OAuth tokens, machine tokens): See the [section on verifying machine requests](#verify-machine-requests).
...
The following example shows how to use `redirectToSignIn()` to redirect the user to the sign-in page if they are not authenticated. For more detailed examples, see the [guide on protecting content from unauthenticated users](/docs/guides/secure/protect-content).
...
You can use `auth()` to check if `isAuthenticated` is true. If it's false, then there is not an authenticated (signed in) user. See detailed examples in the [guide on protecting content from unauthenticated users](/docs/guides/secure/protect-content).
...
- [OAuth access tokens](!oauth-access-token): See detailed examples in the [guide on verifying OAuth access tokens](/docs/guides/development/verifying-oauth-access-tokens).
- [M2M (machine-to-machine) tokens](/docs/guides/development/machine-auth/m2m-tokens)
- API keys: See detailed examples in the [guide on verifying API keys](/docs/guides/development/verifying-api-keys).

### Data fetching with `getToken()`

If you need to send a JWT along to a server, such as when making authenticated requests to an external API, use the `getToken()` method. `getToken()` retrieves the current user's [session token](/docs/guides/sessions/session-tokens) or a [custom JWT template](/docs/guides/sessions/jwt-templates). See detailed examples in the [`Auth` object reference](/docs/reference/backend/types/auth-object#get-token){{ target: '_blank' }}.
