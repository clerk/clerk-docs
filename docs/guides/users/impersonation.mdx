...
<Tabs items={["Hook", "JavaScript"]}>
  <Tab>
    You can use the [`useAuth()`](/docs/references/hooks/use-auth) hook to get access to the authentication context, which includes the `actor` object.
...
<Tabs items={["Next.js", "Express"]}>
  <Tab>
    The Next.js SDK provides the [`auth()`](/docs/references/nextjs/app-router/auth) helper to App Router apps to access the `Auth` object.
