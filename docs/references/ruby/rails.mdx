...
To access Clerk's authentication functionality in your controllers, include the `Clerk::Authenticatable` concern. This gives your controller and views access to the `clerk` object. The `clerk` object provides access to the [Ruby SDK's available methods](/docs/references/ruby/overview#available-methods).

{/* TODO: I don't think "This gives your controller and views access to the `clerk` object." is correct. Because that's what the middleware does. So what does the `Clerk::Authenticatable` concern specifically do? */}
