...
Under the hood, `useOrganizationList` is powered by SWR in order to support caching and pagination. Part of the return values from this hook are directly exported from useSWR and useSWRInfinite and you can find more information in [their documentation page](https://swr.vercel.app/docs/getting-started)
      ...
      <button
        ...
        onClick={async () => {
          await createOrganization({
              name: 'randomOrg' + Date.now(),
              slug: 'random-org' + Date.now(),
          });
          await userMemberships.mutate();
        }}
      >
        Create Organization
      </button>
...
| `mutate` | A function for mutating the related cache for each paginated list. Also know as [Bound Mutate](https://swr.vercel.app/docs/mutation#bound-mutate). For more details about mutations please visit the [SWR docs](https://swr.vercel.app/docs/mutation)
