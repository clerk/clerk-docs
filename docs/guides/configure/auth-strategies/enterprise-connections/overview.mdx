...
<If is="llm">
  **For AI agents:** Instead of editing SSO connection settings in the Dashboard, use the CLI: run `npx clerk@latest api enterprise_connections` to list connections, then update one with `npx clerk@latest api enterprise_connections/<id> -X PATCH -d '{"saml":{"allow_subdomains":true}}'` (or `{"disable_additional_identifications":false}` to allow additional identifiers; discover the endpoints via `npx clerk@latest api ls enterprise`). Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>

## SAML

Clerk supports Enterprise SSO via the SAML protocol, enabling you to create authentication strategies for an IdP. The following IdPs are supported:

- [Microsoft Azure AD](/docs/guides/configure/auth-strategies/enterprise-connections/saml/azure)
- [Google Workspace](/docs/guides/configure/auth-strategies/enterprise-connections/saml/google)
- [Okta Workforce](/docs/guides/configure/auth-strategies/enterprise-connections/saml/okta).

However, you can also [integrate with any other IdP](/docs/guides/configure/auth-strategies/enterprise-connections/saml/custom-provider) that supports the SAML protocol.
...
### Configure a SAML login hint

During a SAML sign-in that uses the [Service provider-initiated flow (recommended)](/docs/guides/configure/auth-strategies/enterprise-connections/authentication-flows#service-provider-initiated-flow-recommended), Clerk can send your IdP a `login_hint` parameter. This can help the IdP identify the user before they sign in.

> [!NOTE]
> Named SAML providers send the user's email address by default. Custom SAML connections don't send a login hint until you configure one.

To configure a login hint for a SAML connection:

1. In the Clerk Dashboard, navigate to the [**SSO connections**](https://dashboard.clerk.com/~/user-authentication/sso-connections) page.
1. Select the SAML connection you want to configure.
1. Select the **Settings** tab.
1. In the **Login hint** section, select one of the following options:
   - **Email address** to send the user's email address.
   - **Custom attribute** to send a custom value instead. Select one of the custom attributes configured for the connection. Clerk sends the value from the user's `publicMetadata`.
   - **Off** to send no login hint.
1. Select **Save**.

Before you select **Custom attribute**, [configure the custom attribute](/docs/guides/configure/auth-strategies/enterprise-connections/custom-attribute-mapping#define-custom-attributes) for the connection. If the selected attribute isn't set for a user or doesn't contain a single text value, Clerk omits the login hint for that sign-in.
