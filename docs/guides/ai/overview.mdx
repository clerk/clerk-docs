...
Learn more, including manual per-client setup, in the [Clerk MCP server guide](/docs/guides/ai/mcp/clerk-mcp-server).
...
Clerk supports MCP through Client ID Metadata Documents ([CIMD](!client-id-metadata-document)), Dynamic Client Registration for clients that still require it, OAuth consent screens, and SDK support.
...
## Set up Clerk with an agent

Many of Clerk's quickstarts include current prompts for AI-powered development tools like Cursor, Claude Code, Codex, GitHub Copilot, and more. These prompts guide AI assistants through adding Clerk to your application.

To get started, choose your framework in the [Clerk quickstarts](/docs/getting-started/quickstart/overview).
