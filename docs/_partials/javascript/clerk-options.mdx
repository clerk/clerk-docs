...
<Properties>
  ...
  Indicates whether the current app should behave as a satellite application. Set this in `load()`. When using satellite domains, configure `domain` in the `Clerk` constructor (or as `data-clerk-domain` on the script tag) and `isSatellite` here.
