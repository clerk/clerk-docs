...
1. You will need social sign-in (OAuth) credentials for any providers that you would like to use in production. Each [OAuth provider](/docs/configure/auth-strategies/social-connections/oauth) has a dedicated guide on how to set up OAuth credentials for Clerk production apps.
...
In production, these are not secure and you will need to provide your own. Each [OAuth provider](/docs/configure/auth-strategies/social-connections/oauth) has a dedicated guide on how to set up OAuth credentials for Clerk production apps.

## Webhooks

Your webhook endpoints will need to be updated to use your production instance's URL and signing secret. See the [guide on syncing data](/docs/configure/webhooks/syncing#configure-your-production-instance) for more information.
