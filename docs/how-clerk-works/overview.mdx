...
It's important to note that the client token is _only used in production environments_. In order to set up a production environment with Clerk, you alias Clerk's FAPI into your app's domain using a CNAME record (at the subdomain `clerk.yourdomain.com`), so this cookie does not need to be sent cross-domain.

In development environments, FAPI _is_ cross-domain (`localhost` -> `<slug>.accounts.dev`), so a cookie would not work here since we do not allow cross-domain cookies for [security & privacy reasons](docs/backend-requests/resources/cookies#tracking-cookies-and-privacy-concerns). Instead, we use a different construct called the "dev browser" in order to maintain session state across the session lifetime. This method makes Clerk work smoothly in local development, but is not secure enough for production and you should never deploy a development environment to production for this reason. There's [more information on Clerk's environments here](/docs/deployments/environments) if you're curious. The dev browser object (`__clerk_db_jwt`) should not be relied on anywhere in your code, as it will break your app when you deploy to production.
