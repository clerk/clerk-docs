...
1. Every user has a [`User`](/docs/references/javascript/user) object that represents their account. The `User` object has a `phoneNumbers` property that contains all the phone numbers associated with the user. The [`useUser()`](/docs/references/general/hooks/use-user) hook is used to get the `User` object.
1. The [`User.createPhoneNumber()`](/docs/references/javascript/user#create-phone-number) method is passed to the [`useReverification()`](/docs/references/general/hooks/use-reverification) hook to require the user to reverify their credentials before being able to add a phone number to their account.
