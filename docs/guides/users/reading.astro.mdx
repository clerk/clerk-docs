...
description: Learn how to use Clerk's helpers to read user data in your Astro application.
metadata:
...
<CodeBlockTabs options={[".astro component", "API Route"]}>
  ...
  ```tsx {{ filename: 'src/pages/api/me.ts' }}
...
To retrieve a token from a [JWT template](/docs/guides/sessions/jwt-templates) and fetch data from an external source, use the [`getToken()`](/docs/reference/backend/types/auth-object#get-token) method from the `auth()` local.
