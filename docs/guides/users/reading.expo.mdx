...
description: Learn how to use Clerk's hooks to protect content and access user data in your Expo application.
sdk: expo
---

<Include src="_partials/hooks/hooks-explanation" />

This guide demonstrates how to use the `useAuth()` and `useUser()` hooks to protect content and access user data in your Expo application.

## Session data example

{/* TODO: Keep in sync with _partials/hooks/use-auth */}

The [`useAuth()`](/docs/references/general/hooks/use-auth){{ target: '_blank' }} hook provides information about the current auth state, as well as helper methods to manage the current active session.
...
The [`useUser()`](/docs/references/general/hooks/use-user){{ target: '_blank' }} hook enables you to access the [`User`](/docs/references/javascript/user) object, which contains the current user's data such as their full name.
