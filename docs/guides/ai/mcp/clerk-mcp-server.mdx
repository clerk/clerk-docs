...
The fastest way to connect is with the [Clerk CLI](/docs/cli). Run the following command to detect the AI clients installed on your machine and register Clerk's MCP server in each one you select:

```bash
clerk mcp install
```

The CLI supports Claude Code, Cursor, GitHub Copilot (VS Code), Windsurf, Gemini CLI, Codex, opencode, OpenClaw, Warp, and Hermes Agent. The server is registered in each client's user-global configuration, so it's available in every project. Reload or restart the client after installing to activate the connection.
