...
import { useSignUp, isClerkAPIResponseError } from "@clerk/clerk-react";
...
function Example() {
  ...
  const attemptSignUp = async (e: SyntheticEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      if (result.status == 'complete') {
        ...
        // other possible status are 'abandoned' & 'missing_requirements'
      }
    } catch(err: unknown) {
...
```jsx
import { useState, SyntheticEvent } from "react";
import { useSignIn, isClerkAPIResponseError } from "@clerk/clerk-react";
import { ClerkAPIError } from '@clerk/types';
...
function Example() {
  ...
  const attemptSignIn = async (e: SyntheticEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      if (result.status == 'complete') {
        ...
        // other possible status are 'needs_identifier', 'needs_first_factor', 'needs_second_factor' & 'needs_new_password'
      }
    } catch(err: unknown) {
      if (isClerkAPIResponseError(err)) setErrors(err.errors);
      return;
