...
<Properties>
  ...
  The full URL or path to navigate to after selecting the [personal workspace](/docs/organizations/overview#allow-personal-workspaces). Defaults to `undefined`.
  ...
  A boolean that controls whether `<OrganizationList />` will include the user's [personal workspace](/docs/organizations/overview#allow-personal-workspaces) in the organization list. Setting this to `true` will hide the personal workspace option, and users will only be able to switch between organizations. Defaults to `false`.
...
## Personal workspaces disabled (organization‑first)

Personal workspaces are disabled by default. When they're disabled, every user **must** belong to an organization.
Set `hidePersonal={true}` to remove the personal‑workspace option entirely and funnel the user straight into organization creation or selection.
For the full UX (and how to customise it), see [Steps after sign‑up / sign‑in](/docs/authentication/configuration/after-auth-flows).
