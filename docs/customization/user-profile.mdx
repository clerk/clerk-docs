...
The following example adds a custom link that navigates to the homepage to the sidenav of the `<UserProfile />` component.
...
<Tabs items={["Next.js", "Astro", "Vue"]}>
  ...
  <Tab>
    <CodeBlockTabs options={["<UserButton />", "Dedicated page"]}>
      ```vue {{ filename: 'App.vue' }}
      <script setup lang="ts">
      import { UserButton, UserProfile } from '@clerk/vue'
      </script>
