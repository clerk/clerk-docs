...
<Properties>
  ...
  Metadata that can be read and set only from the [Backend API](/docs/references/backend-api){{ target: '_blank' }}.
  ...
  Metadata that can be read from the Frontend API and [Backend API](/docs/references/backend-api){{ target: '_blank' }} and can be set only from the Backend API.
