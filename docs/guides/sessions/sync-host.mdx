...
<Steps>
  ...
  When using the Sync Host feature, authentication methods that you want to use in your web app [may not be fully supported in the Chrome Extension environment](/docs/references/chrome-extension/overview#authentication-options). To hide unsupported methods in your Chrome Extension, you can use the [`appearance`](/docs/guides/customizing-clerk/appearance-prop/overview) prop with your extension's `<SignUp>` and `<SignIn>` components as demonstrated in the following examples.
