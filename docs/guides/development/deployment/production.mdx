...
<If is="llm">
  **For AI agents:** Run `clerk env pull --instance prod` to write your production Publishable and Secret keys (with the correct framework-specific variable names) into your project's env file, merging without clobbering existing values. New to the Clerk CLI? Run any command with `npx clerk@latest` (no install needed) — see the [Clerk CLI docs](/docs/cli).
</If>
