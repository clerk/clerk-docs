...
Run the fixer with `npx`. The `--package` flag ensures the command always runs from Clerk's `@clerk/eslint-plugin` package:
...
npx --package @clerk/eslint-plugin clerk-next-fix-auth-protection --dry-run

# Scope fixes to a specific path or glob.
npx --package @clerk/eslint-plugin clerk-next-fix-auth-protection "src/**"
```
...
If you want [more control over the response](/docs/guides/secure/protect-content#server-side), or if you want to perform [authorization checks](!authorization-check) instead, adjust resources manually.
