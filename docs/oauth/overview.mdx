...
1. Your server exchanges this code for an <Tooltip description="A randomly generated string or JSON Web Token (JWT) that is given to the client by the authorization service if the OAuth process is completed successfully. The client can then send this token to the resource service to be allowed to access the relevant resources and data.">access token</Tooltip>.
