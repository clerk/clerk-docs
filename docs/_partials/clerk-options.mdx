<Properties>
  ...
  Your Clerk [Publishable Key](!publishable-key). Optional when using the Backend API resources. Required when using [`authenticateRequest()`](/docs/reference/backend/authenticate-request), unless `acceptsToken` is set to `'api_key'` or `'m2m_token'`. If required and not provided, Clerk throws an error.
