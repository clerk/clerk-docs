...
Use `Clerk.auth.setActive(sessionId = sessionId, organizationId = organizationId)` to switch the [Active Organization](!active-organization) for a session. Omit `organizationId`, or pass `null`, to select the user's [Personal Account](!personal-account). The Personal Account option isn't allowed when `Clerk.organizationSelectionIsForced` is `true`.
