...
- The [`<OrganizationSwitcher />` component](/docs/components/organization/organization-switcher){{ target: '_blank' }} provides a way for your users to select which organization is active. The [`useOrganizationList()` hook](/docs/organizations/custom-organization-switcher){{ target: '_blank' }} can be used for more control.
- The [`useOrganization()` hook](/docs/references/react/use-organization){{ target: '_blank' }} can be used to fetch the current, active organization.
- The [`<Protect />` component](/docs/components/protect){{ target: '_blank' }} enables you to limit who can view certain pages based on their role. Additionally, Clerk exposes a number of helper functions, such as [`auth()`](/docs/references/nextjs/auth){{ target: '_blank' }}, and hooks, such as [`useAuth()`](/docs/references/react/use-auth#how-to-use-the-use-auth-hook){{ target: '_blank' }}, to check the user's authorization throughout your app and API endpoints.
