...
The Progressive Sign Up flow can be toggled on via an instance setting, using the [Instance Settings](/docs/references/backend-api/tag/Beta-Features#operation/UpdateInstanceAuthConfig){{ target: '_blank' }} endpoint in Backend API. In the future, this setting will be exposed as a Dashboard toggle as well.

If you're using the [prebuilt UI components](/docs/references/general/components/overview), there are no changes required on your behalf.
