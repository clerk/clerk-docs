...
Your Clerk application will use the [Account Portal](/docs/account-portal/overview) pages out of the box. However, if you would like more control over your authentication flow, Clerk provides a set of [components](/docs/components/overview) that you can use. If the prebuilt components don't meet your specific needs, you can build fully [custom sign-up and sign-in flows](/docs/custom-flows/overview).

After signing in with Metamask on your sign-in page, you'll be presented with the Next.js default start screen. Let's modify the start screen to display a user's primary Web3 address on the page.
