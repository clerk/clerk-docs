...
The `skipCache` property is only available on the client-side. To access `getToken()` from the client-side, use the [`useSession()`](/docs/references/hooks/use-session) or [`useAuth()`](/docs/references/hooks/use-auth) hooks.
...
<Tabs items={["API route", "Client-side page"]}>
  <Tab>
    The `/update-user-metadata` API route uses Clerk's [`updateUserMetadata()`](/docs/references/backend/user/update-user-metadata) method from the [JavaScript Backend SDK](/docs/js-backend/getting-started/quickstart) to update the user's birthday metadata.
    ...
    export async function POST(req: NextRequest) {
      ...
      // Use `auth()` to access `isAuthenticated` and the current user's ID
...
The `user.reload()` method refreshes the current user's `User` object and their session token. It can be accessed from the [`useUser()`](/docs/references/hooks/use-user) hook.
