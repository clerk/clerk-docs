...
To protect routes based on user authentication status, use [`auth().isAuthenticated`](/docs/references/backend/types/auth-object){{ target: '_blank' }} to check if `isAuthenticated` is true. If it's not, the user is not authenticated, and you can redirect them to the sign-in page.
