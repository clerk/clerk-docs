...
<Steps>
  ...
  `clerkPlugin` provides session and user context to Clerk's components and composables. It's required to pass your Clerk [Publishable Key](!publishable-key) as an option. You can add an `if` statement to check that the key is imported properly. This prevents the app from running without the Publishable Key and catches TypeScript errors.
