...
1. If you have not already, [create a new user](https://clerk.com/docs/references/backend-api/tag/Users#operation/CreateUser).
1. [Create a new session](https://clerk.com/docs/references/backend-api/tag/Sessions#operation/createSession) for the user.
1. [Create a session token](https://clerk.com/docs/references/backend-api/tag/Sessions#operation/CreateSessionToken) using the session ID returned in the previous step.
...
Obtained via the [Backend API](/docs/references/backend-api/tag/Testing-Tokens){{ target: '_blank' }}, Testing Tokens are short-lived and valid only for the specific instance for which they are issued. Testing Tokens are **only available in development instances.**
