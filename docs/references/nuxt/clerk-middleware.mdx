...
- [Authorization-based protection](#authorization-based-protection): Verify if the user has the required privileges, such as a role, permission, feature, or plan. Learn more about [authorization checks](/docs/guides/authorization-checks).

Also, see how to [protect multiple routes](#protect-multiple-routes) using the `createRouteMatcher()` helper.
...
To protect routes based on user authorization status, you can use the `has()` helper to check if the user has the required [privileges, such as a role, permission, feature, or plan](/docs/guides/authorization-checks). The `has()` helper is available on the [`auth`](/docs/references/nuxt/overview#auth-object) object.
