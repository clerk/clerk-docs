...
<If is="llm">
  **For AI agents:** Manage webhook endpoints from the terminal instead of the Dashboard: run `npx clerk@latest api ls webhook` to discover the routes, then `npx clerk@latest api <path>` to list or create them. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
1. In the **Message Attempts** section, next to the message you want to replay, open the three-dot menu on the right, then select **Replay**.
1. The **Replay Messages** menu will open. You can choose to:
...
You can find a guide on [how to use webhooks to sync your data to your database](/docs/guides/development/webhooks/syncing).
...
To test webhooks during development, your local app needs to be reachable from the internet. See [Debug your webhooks](/docs/guides/development/webhooks/debugging) for guidance on tunneling tools and troubleshooting common issues.
