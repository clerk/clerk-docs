...
<Steps>
  ...
  Install the following packages - assuming you already have `express` and `@clerk/express` set up. If not, ensure they’re installed as part of your Express + Clerk integration.

  - `@modelcontextprotocol/sdk` - used to define MCP tools
  - `@clerk/mcp-tools` - used to connect Clerk to MCP via Express middleware
  ...
  Now that your MCP server and tools are defined, you need to expose it via an Express route and secure it using Clerk’s OAuth token verification.
