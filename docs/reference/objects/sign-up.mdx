...
> [!WARNING]
> This page describes our legacy API, which will be removed in a future release. We recommend upgrading to [Core 3](/docs/development/upgrading/upgrade-guides/core-3) and using the [`SignUpFuture`](/docs/reference/objects/sign-up-future) object instead.
...
Attempts to verify an email address by validating the [OTP](!otp) provided by the user against the code sent during the prepare verification step. This is a convenience method that wraps [`SignUp.attemptVerification()`](/docs/reference/objects/sign-up#attempt-verification) with the `'email_code'` strategy.
...
Attempts to verify a phone number by validating the [OTP](!otp) provided by the user against the OTP sent during the prepare verification step. This is a convenience method that wraps [`SignUp.attemptVerification()`](#attempt-verification) with the `'phone_code'` strategy.
...
<Properties>
  ...
  Optional metadata to attach to the sign-in attempt. See [`SignUpUnsafeMetadata`](/docs/reference/types/metadata#sign-up-unsafe-metadata) for more information.
</Properties>
...
<Properties>
  ...
  The full URL or path that the OAuth provider should redirect to after successful authorization on their part. Typically, this will be a simple `/sso-callback` route that either calls [`Clerk.handleRedirectCallback`](/docs/reference/objects/clerk#handle-redirect-callback) or mounts the [`<AuthenticateWithRedirectCallback />`](/docs/reference/components/control/authenticate-with-redirect-callback) component. See the [custom flow](/docs/guides/development/custom-flows/authentication/oauth-connections) for implementation details.
...
> [!WARNING]
> Once the sign-up process is complete, pass the `createdSessionId` to the [`setActive()`](/docs/reference/objects/clerk#set-active) method on the `Clerk` object. This will set the newly created session as the active session.
...
<Properties>
  ...
  **Required** if [Web3 authentication](/docs/guides/configure/auth-strategies/sign-up-sign-in-options#web3-authentication) is enabled. The Web3 wallet address, made up of 0x + 40 hexadecimal characters or a `base58` encoded `ed25519` public key (for Solana wallets).
  ...
  If `strategy` is set to `'oauth_<provider>'` or `'enterprise_sso'`, this specifies full URL or path that the OAuth provider should redirect to after successful authorization on their part. Typically, this will be a simple `/sso-callback` route that either calls [`Clerk.handleRedirectCallback()`](/docs/reference/objects/clerk#handle-redirect-callback) or mounts the [`<AuthenticateWithRedirectCallback />`](/docs/reference/components/control/authenticate-with-redirect-callback) component. See the [custom flow](/docs/guides/development/custom-flows/authentication/oauth-connections) for implementation details.
...
Initiates an email verification process by sending a [OTP](!otp) to the email address associated with the current sign-up attempt. This is a convenience method that wraps [`SignUp.prepareVerification()`](#prepare-verification) with the `'email_code'` strategy.
...
Initiates a phone number verification process by sending an [OTP](!otp) via SMS to the phone number associated with the current sign-up attempt. This is a convenience method that wraps [`SignUp.prepareVerification()`](#prepare-verification) with the `'phone_code'` strategy.
...
<Properties>
  ...
  If `strategy` is set to `'oauth_<provider>'` or `'enterprise_sso'`, this specifies the full URL or path that the OAuth provider should redirect to after successful authorization. Typically, this will be a simple `/sso-callback` route that either calls [`Clerk.handleRedirectCallback()`](/docs/reference/objects/clerk#handle-redirect-callback) or mounts the [`<AuthenticateWithRedirectCallback />`](/docs/reference/components/control/authenticate-with-redirect-callback) component. See the [custom flow](/docs/guides/development/custom-flows/authentication/oauth-connections) for implementation details.
...
```typescript
function validatePassword(password: string, callback?: (errors: string[]) => void): boolean | undefined
```
...
**Experimental.** Fetches the list of enterprise connections available for this sign-up. Returns an array of enterprise connection resources (e.g. for enterprise SSO).
