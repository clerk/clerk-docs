...
The `auth()` helper returns the [`Auth`](/docs/references/nextjs/auth-object) object of the currently active user. It must be used server-side, such as in Server Components, Route Handlers, and Server Actions.

The `auth()` helper does requires [Middleware](/docs/references/nextjs/clerk-middleware).
...
<Properties>
  ...
  A function that returns a boolean based on the permission or role provided as parameter. Can be used for authorization. See the dedicated [`has()`](/docs/references/nextjs/auth-object#has) section for more information.
