...
`requireAuth` is a middleware function that you can use to protect routes in your Express application. This function checks if the user is authenticated, and sends an error to the error middleware if they are not.
...
// Handle unauthorized requests through an error middleware
app.use((err, req, res, next) => {
  if (err instanceof UnauthorizedError) {
    return res.status(401).send('Unauthorized')
  }
