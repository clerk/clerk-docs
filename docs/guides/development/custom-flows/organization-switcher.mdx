...
<Tabs items={["Next.js", "JavaScript"]}>
  <Tab>
    ...
    1. Map over the `data` array to display the user's organization memberships in a table, providing a button that calls `setActive()` to set the selected organization as the active organization.
       - If there are no organizations, the [`<CreateOrganization />` component (custom-flow version, not the Clerk component)](/docs/guides/development/custom-flows/create-organizations) is rendered to allow the user to create an organization.
