...
Whether you're using the App Router, Pages Router, or Middleware, there are several methods you can use to protect routes within your app that require a logged in user to access. This guide will go through a variety of examples of how to do this.
...
### Protect a route
You can protect an entire route tree or an individual page by using Middleware and the `clerkMiddleware` helper. There are two methods that you can use depending on your use case:
...
#### Use `auth().protect()`
The following example require a user to be signed in to view the content. If the user is not signed in, they will be redirected to the sign-in route.

<CodeBlockTabs type="router" options={["app/dashboard/page.tsx", "app/subscribe/page.tsx"]}>
```tsx filename="app/middlware.ts"
...
export default clerkMiddleware((auth, request) => {
    if (!auth().userId && isProtectedRoute(request)) {
        // run custom logic before redirecting
        logger('unauthenticated access');
...
You can protect an individual page by using Clerk helpers. There are two methods that you can use depending on your use case:
...
<CodeBlockTabs type="router" options={["app/subscribe/page.tsx"]}>
```tsx filename="app/subscribe/page.tsx"
