...
You can use the `isAuthenticated` property from the [`auth()`](/docs/references/nextjs/app-router/auth) helper to protect your Server Actions. If `isAuthenticated` is `false`, then the user is not signed in.
...
For more examples on how to use the `auth()` helper, see the [`auth()`](/docs/references/nextjs/app-router/auth) reference.

### Read user data

To retrieve information about the current user in your Server Actions, you can use the [`currentUser()`](/docs/references/nextjs/app-router/current-user) helper, which returns the [`Backend User`](/docs/references/backend/types/backend-user) object of the currently active user. **It does count towards the [Backend API request rate limit](/docs/guides/how-clerk-works/system-limits)** so it's recommended to use the [`useUser()`](/docs/references/hooks/use-user) hook on the client-side when possible and only use `currentUser()` when you specifically need user data in a server context. For more information on this helper, see the [`currentUser()`](/docs/references/nextjs/app-router/current-user) reference.
