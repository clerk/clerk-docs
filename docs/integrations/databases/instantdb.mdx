...
<Steps>
  ...
  InstantDB uses Clerk's [session token](/docs/backend-requests/session-requests/session-tokens) to authenticate users. To use InstantDB with Clerk, you need to include the `email` claim in your session token.
