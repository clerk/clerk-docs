...
<Steps>
  ...
  `@clerk/backend` is a wrapper around the [Backend API](/docs/reference/backend-api){{ target: '_blank' }} that makes it easier to interact with the API **for JavaScript environments**. It provides utilities to verify session tokens and manage users.
  ...
  1. In your `clerk.ts` file, configure the Clerk client using the Clerk [Secret Key](!secret-key) you set, using [`createClerkClient`](/docs/reference/backend/overview) from `@clerk/backend`:
