...
| `sessions` | <code>SessionResource[]</code> | A list of sessions that have been registered on the client device. |
| `setActive()` | <code>(setActiveParams: SetActiveParams) => Promise\<void\></code> | A function that sets the active session and/or organization. See the [reference doc](/docs/reference/javascript/clerk#set-active). |
</Tab>
</Tabs>
