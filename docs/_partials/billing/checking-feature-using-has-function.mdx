  ...
  export async function GET(request: Request) {
    ...
    if (!hasPremiumAccess) {
      return Response.json(
        { error: 'Only subscribers with the Premium Access feature can access this content.' },
        { status: 403 },
      )
    }

    return Response.json({ message: 'Our Exclusive Content' })
  }
  ```
</If>
...
<If sdk="expo">
  ```tsx {{ filename: 'src/app/(home)/premium-content.tsx' }}
...
<If sdk="expressjs">
  ...
  router.get('/premium-content', async (req, res) => {
