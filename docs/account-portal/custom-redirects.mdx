...
If a user accesses an Account Portal page directly without being redirected the `redirect_url` query param will not be there, to prevent this we recommend you always specify the redirect in the link when sharing it. 
...
In case a user does visit directly without the query param, don't worry - Clerk offers a fallback configuration via the dashboard as well.
