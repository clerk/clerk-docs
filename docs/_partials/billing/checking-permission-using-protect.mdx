<If sdk="astro">
  > [!WARNING]
  > Astro SDK hasn't been updated to support "permission" in the `Protect` component yet.
...
<If sdk="chrome-extension">
  ```tsx {{ filename: 'src/screens/protected-manage-content.tsx' }}
  import { Protect } from '@clerk/chrome-extension'

  export default function ProtectedManageContentScreen() {
    return (
      <Protect
        permission="premium_access:manage"
        fallback={
...
<If sdk="nuxt">
  ...
  <template>
    <main>
      <Protect permission="premium_access:manage" fallback="Only subscribers with the Premium Access Manage permission can access this content.">
        <h1>Exclusive Management Content</h1>
        <p>This content is only visible to users with Premium Access Manage permission.</p>
      </Protect>
    </main>
  ...
  export default function ProtectedManageContentPage() {
    return (
      <Protect
        permission="premium_access:manage"
        fallback={
          <Text>Only subscribers with the Premium Access Manage permission can access this content.</Text>
        }
      >
        <View>
