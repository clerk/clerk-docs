...
### Using `satelliteAutoSync: false` with sign-in links

When `satelliteAutoSync` is set to `false`, you must ensure your sign-in and sign-up links on the satellite domain use [`Clerk.buildSignInUrl()`](/docs/reference/javascript/clerk#build-sign-in-url) and [`Clerk.buildSignUpUrl()`](/docs/reference/javascript/clerk#build-sign-up-url) instead of hardcoded URLs. These methods automatically append the sync trigger parameter (`__clerk_synced=false`) to the redirect URL, which tells the satellite to sync the session when the user returns from the primary domain.
...
<CodeBlockTabs options={["Vanilla JS", "React"]}>
  ...
  function SignInButton() {
    ...
    return (
      <a href={buildSignInUrl()}>Sign in</a>
    )
  }
  ```
</CodeBlockTabs>
...
> [!IMPORTANT]
> If you hardcode sign-in URLs (e.g., `<a href="https://primary.dev/sign-in">`) instead of using `buildSignInUrl()`, the sync trigger parameter will not be appended. When the user returns to the satellite after signing in, the satellite will not recognize the session.
