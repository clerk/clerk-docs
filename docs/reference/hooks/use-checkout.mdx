...
`useCheckout()` accepts a single object that is required if the hook is used without a `<CheckoutProvider />` wrapping the component tree. It contains the following properties:

<Typedoc src="shared/use-checkout-params" />
