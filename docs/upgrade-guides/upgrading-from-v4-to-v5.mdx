...
In v4 of our SDKs, if you decode the session token that Clerk returns from the server, you'll currently find an `orgs` claim on it. It lists all the orgs associated with the given user. In v5 Clerk returns the `org_id`, `org_slug`, and `org_role` of the **active** organization.
...
- The response payload of **every** backend API request was changed. Instead of returning the data on success and throwing an error on failure, you'll receive this object:
    ...
    <Callout type="warning" > 
      You'll need to throw errors yourself now as the `@clerk/backend` functions are not handling this for you anymore. See the examples below on how to do that. 
    </Callout>
...
- The `Clerk.isReady()` has been removed. You can use the `Clerk.loaded` property instead.
...
<Accordion titles={["Removal of unstable/experimental properties", "Appearance"]} heading="h4">
  <AccordionPanel>
...
- [Replace `apiKey` with `secretKey`](#replace-api-key-with-secret-key)
- [Replace `frontendApi` with `publishableKey`](#replace-frontend-api-with-publishable-key)
- A new `@clerk/nextjs/internal` subpath import has been added. It will be used to export internal or experimental APIs. It should be treated as **unstable** and can break at any time.
