The following example uses the [`useAuth()`](/docs/references/general/hooks/use-auth) hook to access the current auth state, as well as helper methods to manage the current active session.
