...
The [Active Organization](!active-organization) is stored on `Session.lastActiveOrganizationId` for the current session. Use `clerk.organization` to read the Active Organization and `clerk.organizationMembership` to read the signed-in user's Active Organization membership. See the [Clerk reference docs](/docs/reference/native-mobile/clerk#access-the-active-organization-state) to find out more about available methods and properties.
