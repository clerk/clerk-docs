...
If you are not using React or Next.js, you can access the `getToken` method from the `session` property of the [`Clerk`](/docs/references/javascript/clerk) object. This assumes you have already followed the instructions on [setting up ClerkJS](/docs/references/javascript/overview) and provided it with your Frontend API URL.
