...
The `auth()` helper returns the [`Auth`](/docs/reference/backend/types/auth-object) object of the currently active user, as well as the [`redirectToSignIn()`](#redirecttosignin) method. It includes a single method, `protect()`, which you can use to check if a user is authenticated or authorized to access certain parts of your application or even entire routes.
...
- The [`redirectToSignIn()`](#redirecttosignin) method.
