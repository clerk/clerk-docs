...
| Type | Description |
| --- | --- |
| <code>Promise\<[EmailAddress][email-ref]\></code> | This method returns a `Promise` which resolves to the email address created for the user. |
...
- If the connection failed, then the `externalAccount.verification.status` will be `failed` and the `externalAccount.verification.error` will contain the error encountered, which you can present to the user.
