...
> [!TIP]
> A user who has saved their backup codes can sign in after losing their phone without an admin resetting their MFA, so make saving them a required step in your enrollment UI. For users who are already locked out, see [MFA account recovery](/docs/guides/secure/mfa-recovery).
