...
Your Clerk-generated [session tokens](/docs/guides/how-clerk-works/session-tokens) are essentially JWTs which are signed using your instance's private key and can be verified using your instance's public key. Depending on your architecture, these tokens will be in your backend requests either via a cookie named `__session` or via the Authorization header.
...
<Steps>
  ...
  1. Use the Backend API in JSON Web Key Set (JWKS) format at the following endpoint [https://api.clerk.com/v1/jwks](https://clerk.com/docs/references/backend-api/tag/JWKS#operation/GetJWKS).
