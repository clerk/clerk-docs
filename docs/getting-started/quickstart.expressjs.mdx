...
<Steps>
  ...
  Node can load `.env` files natively. When running your app, use the `--env-file` flag so environment variables can be accessed. Alternatively, you can load the file programmatically using `process.loadEnvFile()` before your app starts.
  ...
  // If you choose to load your .env file programmatically, call `process.loadEnvFile()` before exporting the config
  process.loadEnvFile()
