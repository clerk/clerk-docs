...
- A [**free trial**](/docs/guides/billing/free-trials): a time-boxed period that can convert to paid automatically when it ends, as long as a default payment method is on file and the customer doesn't cancel
...
Use a free trial when someone is evaluating the product and, with a default payment method on file, should automatically move to a paid Subscription afterward. Trials are configured on a Plan, shown in checkout flows like [`<PricingTable />`](/docs/reference/components/billing/pricing-table), and only available to users with a verified primary email address who have never held a paid Subscription or used a free trial in the application.

Use a trial when you want:

- A fixed number of free days, then an automatic charge (if a default payment method is on file and the customer doesn't cancel)
...
| Who's eligible? | Verified primary email, never paid or trialed | Anyone on a paid Plan | Anyone |
