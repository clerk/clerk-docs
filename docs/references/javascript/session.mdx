...
Retrieves the current user's [session token](/docs/backend-requests/session-requests/session-tokens) or a [custom JWT template](/docs/backend-requests/session-requests/making/jwt-templates).
