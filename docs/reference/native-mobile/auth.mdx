...
Sign in using a trusted-device credential that was enrolled on the current app installation:

```kotlin
val signIn = Clerk.auth.signInWithTrustedDevice()
```

See the [biometric sign-in guide](/docs/guides/development/custom-flows/authentication/biometric-sign-in) to learn how to enroll, use, and revoke trusted-device credentials.
...
If the Backend API created the sign-in token with `org_id`, Clerk activates that Organization for the resulting session. After sign-in succeeds, the Organization ID is available from `Clerk.session?.lastActiveOrganizationId`.

### Sign in with email link

Send an email link to the supplied email address. After the user opens the link, handle the callback with [`Clerk.auth.handle(intent.data)`](#deep-linking).

```kotlin
Clerk.auth
    .startEmailLinkSignIn("user@email.com")
    .onSuccess {
...
Send the user an email link to verify their sign-in. After the user opens the link, handle the callback with [`Clerk.auth.handle(intent.data)`](#deep-linking).
...
Sign up using an OAuth provider (e.g., Google, GitHub, see [all providers](/docs/guides/configure/auth-strategies/social-connections/overview#supported-social-providers)):
...
Send the user an email link to verify their account. After the user opens the link, handle the callback with [`Clerk.auth.handle(intent.data)`](#deep-linking).
...
Hosted authentication opens Clerk's [Account Portal](/docs/guides/account-portal/overview) in a browser authentication session. Account Portal supports the sign-in and sign-up methods enabled for your Clerk application. When authentication completes, the SDK closes the browser session and activates the new session in your app.

Call `startHostedAuth()` from a coroutine to open the Account Portal sign-in page:

```kotlin
Clerk.auth.startHostedAuth()
```

Pass `HostedAuthMode.SIGN_UP` to open the sign-up page first:
...
- `mode` - The Account Portal page to open first, as a `com.clerk.api.auth.HostedAuthMode`. Defaults to sign-in.
- `redirectUrl` - A custom native callback. Defaults to `clerk://<package-name>.callback`, which the SDK registers for you. Custom values require a matching intent filter in your app manifest, and your app must forward the received URI to `Clerk.auth.handle()`.

`startHostedAuth()` returns a `ClerkResult<Session, ClerkErrorResponse>`. On success, the SDK has already activated the returned session. When the user dismisses the browser or another flow supersedes the attempt, the failure's `throwable` is a `HostedAuthCancellationException`, so you can branch on cancellation without matching message strings:
...
when (val result = Clerk.auth.startHostedAuth()) {
    ...
    is ClerkResult.Failure -> {
        if (result.throwable is HostedAuthCancellationException) {
...
See the [hosted authentication guide](/docs/guides/account-portal/hosted-auth) for setup instructions and tradeoffs.
...
Handle OAuth, SSO, and email link deep link callbacks in your Activity:
