...
<If sdk="ios">
    ...
    var body: some View {
      VStack {
        ...
        Button("Update organization") {
          Task { await updateActiveOrganization(name: name) }
        }
      }
    }
  }
  ...
  extension UpdateOrganizationView {
    ...
    func updateActiveOrganization(name: String) async {
      do {
        guard let organization = try await activeOrganization() else { return }
        let updatedOrganization = try await organization.update(name: name)
        print(updatedOrganization.name)
      } catch {
