...
To use these commands, you must import them in your [Cypress E2E Support file](https://docs.cypress.io/guides/core-concepts/writing-and-organizing-tests#Support-file), which by default is the `cypress/support/e2e.{js,jsx,ts,tsx}` file.
...
- `password`: The user's password. This is required only if the `strategy` is set to `'password'`.
