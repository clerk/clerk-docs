...
When Client Trust is enabled, Clerk treats every new device as untrusted until the user has verified their identity with a [second factor](!second-factor-verification).

## How Client Trust works

Client Trust automatically requires a [second factor](!second-factor-verification) when **all of the following conditions are met**:
