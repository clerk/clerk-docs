...
<Steps>
  ...
  > [!IMPORTANT]
  > Load environment variables before importing any Clerk modules. With Node, prefer running your app with `node --env-file=.env` (or your runtime's equivalent) so env vars are available during module initialization. For more information, refer to the [Fastify docs](https://fastify.dev/docs/latest/Guides/Getting-Started/#loading-order-of-your-plugins).

  ```ts {{ filename: 'index.ts', mark: [2, 6] }}
