...
The [`<OrganizationSwitcher />`][orgswitcher-ref] component allows a user to switch between their accounts - their personal account and their organization account(s). This component is useful for applications that have a multi-tenant architecture, where users can be part of multiple organizations.
