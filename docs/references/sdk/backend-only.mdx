...
The source of truth for all BAPI endpoints is the [BAPI reference docs](/docs/reference/backend-api){{ target: '_blank' }}. For Node.js backend frameworks, you can build on top of the [JavaScript Backend SDK](/docs/references/backend/quickstart).
...
<Steps>
  ...
  Use [`createClerkClient`](/docs/references/backend/quickstart#usage) from `@clerk/backend` to create your default Clerk client which will be used for the middleware.
