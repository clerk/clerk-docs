...
This method in the SDK is a wrapper around the BAPI endpoint `POST/sessions/{session_id}/tokens/{template_name}`. See the [BAPI reference](/docs/references/backend-api/tag/Sessions#operation/CreateSessionTokenFromTemplate){{ target: '_blank' }} for more information.
