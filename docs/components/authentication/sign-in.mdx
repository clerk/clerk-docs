...
| `redirectUrl` | `string` | Full URL or path to navigate after successful sign in or sign up.<br /> The same as setting `afterSignInUrl` and `afterSignUpUrl` to the same value. Use [the environment variables](/docs/deployments/clerk-environment-variables#sign-in-and-sign-up-redirects) instead. |
