...
<If notSdk="expo">
  ...
  <Steps>
    ...
    1. Enable **Sign-in with email**. Because this guide focuses on email links, disable **Email verification code** and enable **Email verification link**. By default, [**Require the same device and browser**](!require-same-device-browser) setting is enabled. For this guide, leave this setting enabled.
    ...
    **This example is written for Next.js App Router but it can be adapted for any React-based framework, such as React Router or Tanstack React Start.**
    ...
    1. On the page that called `signIn.emailLink.sendLink()`, call `signIn.emailLink.waitForVerification()` to await the result of the verification.
