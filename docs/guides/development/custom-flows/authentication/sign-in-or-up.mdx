  ...
  extension SignInOrSignUpView {
    ...
    func submit(emailAddress: String, password: String) async {
      do {
        ...
        // If the identifier is not found, the user is not signed up yet.
        // So this includes the flow for signing up a new user.
        if clerkApiError?.code == "form_identifier_not_found" {
          do {
            // Start the sign-up process using the email and password provided.
            let signUp = try await clerk.auth.signUp(emailAddress: emailAddress, password: password)
    ...
    func verify(code: String) async {
      do {
        ...
        if let signUp = clerk.auth.currentSignUp {
          ...
          case .complete:
            // If verification was completed, the created session is available on `clerk.session`.
            dump(clerk.session)
          default:
            // If the status is not complete, check why. User may need to
            // complete further steps.
