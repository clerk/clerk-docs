...
<If sdk={["js-frontend", "ios", "android"]}>
  <If sdk={["ios", "android"]}>
    > [!WARNING]
  ...
  1. Calls [`clerk.apiKeys.getAll()`](/docs/reference/objects/api-keys#get-all) to retrieve the list of API keys for the active user or organization.
  1. Displays the API keys in a table.
  1. Provides a form to create new API keys using [`clerk.apiKeys.create()`](/docs/reference/objects/api-keys#create).
  1. Allows revoking API keys using [`clerk.apiKeys.revoke()`](/docs/reference/objects/api-keys#revoke).
