...
<Tabs type="framework" items={["General", "Next.js"]}>
  ...
  <Tab>
    | Variable | Description |
    | - | - |
    | `NEXT_CLERK_AFTER_SIGN_UP_URL` | Full URL or path to navigate to after successful sign up. Defaults to `/`. `NEXT_PUBLIC_CLERK_SIGN_UP_FALLBACK_REDIRECT_URL` and `NEXT_CLERK_SIGN_UP_FORCE_REDIRECT_URL` have priority and should be used instead. |
