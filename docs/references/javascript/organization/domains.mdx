...
| Type | Description |
| --- | --- |
| <code>Promise\<[PaginatedResponse][pag-res-ref]\<[OrganizationDomain][org-domain-ref]\>></code> | This method returns a `Promise` which resolves with a list of [`OrganizationDomain`][org-domain-ref] objects or rejects with `ClerkAPIResponseError` or Network error. |
