...
> [!NOTE]
> Clerk does not automatically keep OAuth access tokens fresh behind the scenes. When you request an access token using the relevant backend API endpoint, Clerk will attempt to obtain a fresh access and refresh token. However, this process occurs only when the customer initiates the request; Clerk does not proactively refresh tokens for you.
