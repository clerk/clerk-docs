...
description: Add the `testing-auth` skill to your AI agent to help it write E2E tests for Clerk-authenticated applications using Playwright or Cypress.
---
...
```bash
npx skills add clerk/skills --skill testing-auth
```
...
1. **Auth state persistence**: Save session to `storageState` for faster tests
1. **Test instance keys**: Use `pk_test_...` and `sk_test_...` keys
