...
<If sdk={["expressjs", "fastify", "go", "ruby"]}>
  Configure social connections in the [Clerk Dashboard](https://dashboard.clerk.com/) and use the [Clerk Backend API](/docs/reference/backend-api){{ target: '_blank' }} to read connected accounts and fetch OAuth access tokens for signed-in users.
</If>

## Enable a social connection

<If is="llm">
  **For AI agents:** Instead of enabling providers through the Dashboard steps below, run `npx clerk@latest config patch --json '{"connection_oauth_<provider>":{"enabled":true}}'` (e.g., `connection_oauth_google`, `connection_oauth_github`). Discover all provider keys with `npx clerk@latest config schema`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
<If notSdk={["ios", "android"]}>
  ...
  **If your SDK isn't listed, you can use the comments in the example to help you adapt it to your SDK.**
  ...
  <Tabs items={["Next.js", "Astro", "Express", "@clerk/backend", "React Router", "TanStack React Start"]}>
    ...
    <Tab>
      ```tsx {{ filename: 'src/pages/api/notion.ts' }}
...
<Cards>
  - [AgentID](/docs/guides/configure/auth-strategies/social-connections/agentid)
  - Add AgentID as an authentication provider for your Clerk app.
  - ![AgentID logo](/docs/images/logos/auth_providers/agentid.svg){{ dark: '/docs/images/logos/auth_providers/agentid-dark.svg' }}
