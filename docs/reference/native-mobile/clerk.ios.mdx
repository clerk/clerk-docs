...
<Properties>
  ...
  The active organization for the current session. Returns `nil` when there is no active session, no selected organization, or the active user's memberships don't include the selected organization.
...
Use `clerk.auth.setActive(sessionId:organizationId:)` to switch the active organization for a session. Omit `organizationId`, or pass `nil`, to select the user's personal account. Personal account selection isn't allowed when `clerk.environment?.organizationSettings.forceOrganizationSelection` is `true`.
