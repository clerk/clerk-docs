...
To authenticate requests when using a Clerk SDK, you must pass Clerk's short-lived [session token](/docs/backend-requests/resources/session-tokens) to your server. The session token contains cryptographically signed claims about the user's identity and authentication state. [Read more about making requests](/docs/guides/development/making-requests).
