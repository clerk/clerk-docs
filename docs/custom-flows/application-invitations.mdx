...
When a user visits an [invitation](/docs/users/invitations) link, Clerk first checks whether a custom redirect URL was provided.

**If you specified [a redirect URL when creating the invitation](/docs/users/invitations#redirect-url)**, the user is redirected there, and you must handle the sign-up flow in your code for that page. You can either embed the [`<SignUp />`](/docs/components/authentication/sign-up) component on that page, or if the prebuilt component doesn't meet your specific needs or if you require more control over the logic, you can rebuild the existing Clerk flows using the Clerk API.

**If no redirect URL was specified**, Clerk redirects the user to the default sign-up or sign-in page for your app. This depends on your configuration:
- If Account Portal is enabled, the user will be redirected to the [Account Portal sign-up page](/docs/account-portal/overview#sign-up) and **their email address will be automatically verified.**
- If Account Portal is disabled, the user will be redirected to the sign-up or sign-in path you configured in the [**Paths**](https://dashboard.clerk.com/last-active?path=paths) page of the Clerk Dashboard.
