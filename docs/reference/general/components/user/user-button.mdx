...
The `<UserButton />` component renders the familiar user button UI popularized by Google. When selected, it opens a dropdown menu with options to manage account settings and sign out. The "Manage account" option launches the [`<UserProfile />`](/docs/reference/general/components/user/user-profile) component, providing access to profile and security settings.
...
<Properties>
  ...
  **Deprecated. Move `afterMultiSessionSingleSignOutUrl` to [`<ClerkProvider />`](/docs/reference/general/components/clerk-provider).** The full URL or path to navigate to after signing out from a currently active account in a multi-session app.
  ...
  **Deprecated. Move `afterSignOutUrl` to [`<ClerkProvider />`](/docs/reference/general/components/clerk-provider).** The full URL or path to navigate to after a successful sign-out.
  ...
  Optional object to style your components. Will only affect [Clerk components](/docs/reference/general/components/overview) and not [Account Portal](/docs/guides/customizing-clerk/account-portal) pages.
  ...
  Controls whether selecting the **Manage your account** button will cause the [`<UserProfile />`](/docs/reference/general/components/user/user-profile) component to open as a modal, or if the browser will navigate to the `userProfileUrl` where `<UserProfile />` is mounted as a page. Defaults to: `'modal'`.
  ...
  Specify options for the underlying [`<UserProfile />`](/docs/reference/general/components/user/user-profile) component. For example: `{additionalOAuthScopes: {google: ['foo', 'bar'], github: ['qux']}}`.
