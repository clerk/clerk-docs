...
<Callout type="info">
  `auth()` on the server-side can only access redirect URLs defined via [environment variables](/docs/deployments/clerk-environment-variables#sign-in-and-sign-up-redirects) or [middleware options](/docs/references/nextjs/clerk-middleware#clerk-middleware-options).
</Callout>
