...
To retrieve information about the current user in your Route Handler, you can use the [`currentUser()`](/docs/references/nextjs/current-user) helper, which returns the [`Backend User`](/docs/references/backend/types/backend-user) object of the currently active user. **It does count towards the [Backend API request rate limit](/docs/guides/how-clerk-works/system-limits)** so it's recommended to use the [`useUser()`](/docs/hooks/use-user) hook on the client side when possible and only use `currentUser()` when you specifically need user data in a server context. For more information on this helper, see the [`currentUser()`](/docs/references/nextjs/current-user) reference.
