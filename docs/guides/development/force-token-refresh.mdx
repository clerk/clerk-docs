...
- **Client-side:** Access `getToken()` from the [`useSession()`](/docs/reference/general/hooks/use-session) or [`useAuth()`](/docs/reference/general/hooks/use-auth) hooks.
...
The `user.reload()` method refreshes the current user's `User` object and their session token. It can be accessed from the [`useUser()`](/docs/reference/general/hooks/use-user) hook.
