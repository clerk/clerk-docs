...
<Steps>
  ...
  First, you need to tell your app where to redirect users when they have pending session tasks. You can do this by setting the [`taskUrls` option on your Clerk integration](/docs/guides/configure/session-tasks#using-the-task-urls-option).
  ...
  What if your user exits the session task flow before completing their tasks? What if your user tries navigating to a different page before completing their tasks?
