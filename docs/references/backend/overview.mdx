...
<Tabs items={["Backend SDK", "With other SDKs"]}>
  <Tab>
    INPUT COPY AND EXAMPLE FOR HOW TO USE REQUEST.AUTH
  </Tab>
  ...
  <Tab>
    <Tabs items={["Next.js", "Astro",  "Express",  "React Router", "Remix", "Tanstack React Start"]}>
        ...
        <Tabs items={["ESM", "CJS"]}>
          <Tab>
            ...
            app.get('/user', async (req, res) => {
              // Use `req.auth` to get the user's ID
      ...
      <Tab>
        Use the following tabs to see examples of how to use the the Backend SDK to get the [`getAuth()`](/docs/references/nextjs/auth) helper to get the user's ID, and then use the Backend SDK's [`getUser()`](/docs/references/backend/user/get-user) method to get the [Backend User object](/docs/references/backend/types/backend-user) in Remix Loader and Action functions.
