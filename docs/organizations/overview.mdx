...
> [!IMPORTANT]
> Personal accounts being disabled by default was released on \[RELEASE DATE]. Applications created before this date will not be able to see the **Allow personal accounts** setting, because personal account were enabled by default.
...
When personal accounts are enabled, users initially sign in to their personal account with **no** active organization set.\
When personal accounts are **disabled** (the default), users must select or create an organization to continue.\
