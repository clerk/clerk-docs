...
Personal workspaces are disabled by default. When they're disabled, every user **must** belong to an organization. Set `hidePersonal={true}` to remove the personal workspace option entirely from the UI and drive users into the organization creation / selection flow.
