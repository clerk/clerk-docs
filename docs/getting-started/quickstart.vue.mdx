...
<Steps>
  ...
  You can control which content signed-in and signed-out users can see with Clerk's [prebuilt control components](/docs/references/general/components/overview#control-components). The following example creates a header using the following components:
  ...
  - [`<SignInButton />`](/docs/references/general/components/unstyled/sign-in-button): An unstyled component that links to the sign-in page or displays the sign-in modal.
...
<Cards>
  ...
  - [Client-side helpers (composables)](/docs/references/general/composables/use-user)
