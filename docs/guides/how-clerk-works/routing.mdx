...
For example, say a user uses their email address to fill out the [`<SignUp />`](/docs/reference/general/components/authentication/sign-up) form. Once they submit the form, they are redirected from `/sign-up` to `/sign-up/verify-email-address`, which renders Clerk's UI for verifying a user's email address. This redirect is handled by Clerk's internal routing.
