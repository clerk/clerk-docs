...
- [**`signUpIfMissing` flow**](#sign-in-or-up-with-sign-up-if-missing): The sign-in proceeds to verification regardless of whether an account exists. Only after verification does the backend reveal whether the account exists or needs to be created. This prevents user enumeration attacks.
  - The same behavior [`<SignIn />`](/docs/reference/components/authentication/sign-in) uses when [**strict** user enumeration protection](/docs/guides/secure/user-enumeration-protection#sign-in-or-up-under-strict-protection) is enabled. Under **strict**, an Account Portal sign-in also proceeds to verification without revealing whether the account exists, but it can't transfer to sign-up.

> [!TIP]
> If you render `<SignIn />` in your own app, you don't need a custom flow for either approach — both are built in, selected by your [user enumeration protection](/docs/guides/secure/user-enumeration-protection) setting. Follow this guide when you're building your own user interface, or when you want the `signUpIfMissing` behavior without strict protection enabled. The [Account Portal](/docs/guides/account-portal/overview) hosts sign-in and sign-up on separate pages, so a sign-in-or-up experience means rendering `<SignIn />` in your own app or building the flow below.
  ...
  extension SignInOrSignUpView {
    ...
    func handle(signIn: SignIn) async throws {
      ...
      case .needsClientTrust:
        // This is required when Device Trust is enabled and the user
        // is signing in from a new device.
        // See https://clerk.com/docs/guides/secure/device-trust
...
> [!TIP]
> This flow is the same behavior [`<SignIn />`](/docs/reference/components/authentication/sign-in) uses when [**strict** user enumeration protection](/docs/guides/secure/user-enumeration-protection#sign-in-or-up-under-strict-protection) is enabled. Build it yourself only if you aren't rendering `<SignIn />` in your own app, or if you want this behavior without strict protection enabled.
...
<If notSdk={["ios", "android"]}>
  ...
  - **Not available in Invite-only or Waitlist access modes.** The instance must be in [**Open** access mode](/docs/guides/secure/restricting-access#open).
