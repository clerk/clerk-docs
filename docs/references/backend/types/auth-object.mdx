...
<Properties>
  ...
  A function that gets the current user's [session token](/docs/backend-requests/resources/session-tokens) or a [custom JWT template](/docs/guides/development/jwt-templates).
...
`getToken()` retrieves the current user's [session token](/docs/backend-requests/resources/session-tokens) or a [custom JWT template](/docs/guides/development/jwt-templates).
