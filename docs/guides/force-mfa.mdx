...
<Steps>
  ...
  <Tabs items={['Next.js', 'Astro', 'Nuxt']}>
    ...
    <Tab>
      By default, the Nuxt SDK automatically adds [the `clerkMiddleware()` helper](/docs/sdk/nuxt/clerk-middleware) to your Nuxt application. To manually configure the middleware, in your `nuxt.config.ts` file, under the `clerk` property, set `skipServerMiddleware: true`.
