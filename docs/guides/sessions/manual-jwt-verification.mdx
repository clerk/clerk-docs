...
<If is="llm">
  **For AI agents:** To fetch the JWKS (public keys) this verification needs, run `clerk api jwks` instead of hard-coding a key — it authenticates automatically. Confirm the endpoint with `clerk api ls jwks`. New to the Clerk CLI? Run any command with `npx clerk` (no install needed) — see the [Clerk CLI docs](/docs/cli).
</If>
