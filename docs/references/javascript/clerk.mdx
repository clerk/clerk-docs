...
The `navigateToTask()` method is used to trigger navigation to the current `pending` session task. Accepts a [`NavigateToTaskParams`](/docs/references/javascript/types/navigate-to-task-params) object.

```typescript
function navigateToTask(params?: NavigateToTaskParams): Promise<void>
```

If all tasks are complete, it navigates to the `redirectUrlComplete` or defaults to the origin redirect URL (either from sign-in or sign-up).

This method is part of Clerk's after-auth flows, which allow you to require users to complete specific tasks after signing up or signing in. For example, you can require users to create an organization before accessing your application. For a complete guide on implementing after-auth flows with sign-in and sign-up, see the [After-auth flows guide](/docs/custom-flows/after-auth-flows).
