...
> [!NOTE]
> For Next.js applications, it's recommended to use the [`auth()`](/docs/sdk/nextjs/auth) helper instead of `useAuth()`. Since `auth()` must be used in Server Components, you'll need to pass auth data to Client Components as needed. If you prefer `useAuth()`, you must pass the `dynamic` prop to `<ClerkProvider>`, but be aware this switches the app to dynamic rendering. Learn more [here](/docs/sdk/nextjs/rendering-modes).
