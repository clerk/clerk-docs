...
If your client and server are on different origins (e.g. making an API call to a server on `api.foo.com` from JavaScript running on a client at `foo.com`), the [session token](/docs/backend-requests/session-requests/session-tokens) needs to be passed in a network request header. There are a few different ways this can be done on the front-end.
