...
Select the tab for the approach you want to use:
...
<Tabs items={["JavaScript (Native sign-in optional)", "Native components"]}>
  ...
  <Tab>
    ...
    <Steps>
      ...
      With native components, you can build a complete app in a single file. The [`<AuthView />`](/docs/reference/expo/native-components/auth-view) component handles all sign-in and sign-up flows, and [`<UserButton />`](/docs/reference/expo/native-components/user-button) provides a profile avatar that opens the [`<UserProfileView />`](/docs/reference/expo/native-components/user-profile-view).
