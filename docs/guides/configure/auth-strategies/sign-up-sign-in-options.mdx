...
- **Email verification code**: Users receive a [one-time password (OTP)](!otp) to their email address to sign in.
...
Users can authenticate with their phone number and verify it with a [one-time password (OTP)](!otp).
