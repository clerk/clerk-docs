...
sdk: astro, chrome-extension, expo, nextjs, nuxt, react, react-router, tanstack-react-start, vue, fastify, js-frontend, expressjs, js-backend, go, ruby
---
...
<If sdk="js-frontend">
  It's recommended to add this to the [`clerk.load()`](/docs/reference/objects/clerk#load) method so that it's applied to all Clerk components, as shown in the following example. The example names the layer `clerk` but you can name it anything you want.
