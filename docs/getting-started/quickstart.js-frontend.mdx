...
<Tabs items={["NPM module", "<script>"]}>
  <Tab>
    <Steps>
      ...
      VITE_CLERK_FRONTEND_API_URL={{fapi_url}}
      ```
      ...
      - Imports your [Publishable Key](!publishable-key) and [Frontend API URL](!frontend-api-url).
      - Dynamically loads the Clerk UI bundle.
      - Creates a new [`Clerk`](/docs/reference/javascript/clerk) instance.
      - Calls the [`load()`](/docs/reference/javascript/clerk#load) method to initialize Clerk.

      ```js {{ filename: 'src/main.js' }}
      import { Clerk } from '@clerk/clerk-js'

      const publishableKey = import.meta.env.VITE_CLERK_PUBLISHABLE_KEY
      const frontendApiUrl = import.meta.env.VITE_CLERK_FRONTEND_API_URL

      await new Promise((resolve, reject) => {
        const script = document.createElement('script')
        script.src = `https://${frontendApiUrl}/npm/@clerk/ui@1/dist/ui.browser.js`
        script.async = true
  ...
  <Tab>
    <Steps>
      ...
      Below the `<script>` tags that initialize the SDK, create another `<script>` tag that listens for the `load` event and calls Clerk's [`load()`](/docs/reference/javascript/clerk#load) method to load the SDK, as shown in the following example:
