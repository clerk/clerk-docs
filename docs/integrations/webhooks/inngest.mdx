...
Webhooks allow you to [synchronize data](/docs/users/sync-data) from Clerk to your application backend. You can either handle them directly in your backend with an endpoint or use a tool like [Inngest](https://inngest.com/) which receives the webhook events for you and reliably executes functions in your codebase. When handling webhooks, Inngest receives the webhook events for you and uses a built-in queue to reliably execute longer running functions with additional functionality including:
...
* Trigger multiple functions from a single event ([fan-out jobs](https://www.inngest.com/docs/guides/fan-out-jobs)).
* [Delay code](https://www.inngest.com/docs/functions/multi-step) to run after a period of time.
* [Debounce events](https://www.inngest.com/docs/reference/functions/debounce) to reduce duplicate processing.
...
To follow this guide, you need an Inngest account and have Inngest [set up](https://www.inngest.com/docs/quick-start) in your codebase.

## Setting up the Inngest webhook

To create an Inngest webhook endpoint and add it to your Clerk account, open the [Webhooks](https://dashboard.clerk.com/last-active?path=webhooks) page at the

Clerk dashboard. Next, select the **Add Endpoint** button.
...
After setup, as webhook events are sent from Clerk to Inngest, new `clerk `events will appear  in your Inngest dashboard. Event names will be the prefixed `clerk/` followed by the event name. See [webhook events](/docs/integrations/webhooks#supported-webhook-events) for a full list.
...
The code below features the `inngest.createFunction` method, which inserts a new user into the database. It will be triggered whenever `clerk/user.created` event occurs.
...
The `event` object contains all of the relevant data for the event. The `event.data` will match the `data` object from the standard Clerk webhook [payload structure](/docs/integrations/webhooks#payload-structure). With this `clerk/user.created` event, the `event.data` will be a Clerk User json object.
...
While it is possible to add this logic at the end of your sync function as seen in the previous section, it's better to decouple logic into different functions so issues with one task do not affect the other ones. For example, if your email fails to send, it should not affect adding the user to an email list.
...
The code below uses `step.sleep `to send a welcome email, then wait for three days, and follow up with a message offering a free trial:
...
To quickly get events to test within Dev Server, you can select any individual event from the Inngest Events tab then select the **Send to Dev Server**.
