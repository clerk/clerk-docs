...
| `redirectUrl` (deprecated) | `string` | The redirect URL after the sign-up flow has completed. **`signInFallbackRedirectUrl` has priority over the legacy `redirectUrl`. Use `fallbackRedirectUrl` or `forceRedirectUrl` instead of `redirectUrl`.** |
