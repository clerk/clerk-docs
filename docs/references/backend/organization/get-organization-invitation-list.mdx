...
```tsx
function getOrganizationInvitationList: (params: GetOrganizationInvitationListParams) => Promise<ClerkBackendApiResponse<OrganizationInvitation[]>>;
```
...
### `getOrganizationInvitationList()` With No Parameters
...
/* In this example, you can see that data is an array of OrganizationInvitation objects, and is populated with two OrganizationInvitations.
{
  data: [
...
const { data, errors, totalCount } = await clerkClient.organizations.getOrganizationInvitationList({
  organizationId,
