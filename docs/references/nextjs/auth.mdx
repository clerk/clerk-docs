...
<Properties>
  ...
  The type of token that the auth object should accept. You can pass in `'any'` to accept all available token types. Defaults to `'session'`.
</Properties>
...
| No | No | Redirect the user to the sign-in page\*\* for session requests, or a `401` error for machine requests. |
