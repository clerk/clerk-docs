...
<Properties>
  ...
  **Optional** if the `strategy` is set to `OAuthStrategy` or `EnterpriseSSOStrategy`. **Required** otherwise.<br />The authentication identifier value for the current sign-in.
