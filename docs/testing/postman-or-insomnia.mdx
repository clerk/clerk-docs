...
Open Postman/Insomnia and create a new request. In the URL field for both Postman and Insomnia enter the URL and route for the API Route you want to test. To the immediate left of the URL select the method for your API Route, or if your API Route handles multiple methods select you want to test. In the Authorization tab, select Bearer Token. Paste the token you copied from the console as the Bearer Token. Your request will now authenticate as the user you created the token with.
