<If
  notSdk={["nextjs", "astro", "expo", "expressjs", "fastify", "js-frontend", "nuxt", "react", "react-router", "tanstack-react-start", "vue"]}
>
  ...
  export async function GET(request: Request) {
    ...
    if (!hasBronzePlan) {
      return Response.json({ error: 'For Bronze subscribers only' }, { status: 403 })
    }

    return Response.json({
      message: 'Only subscribers to the Bronze plan can access this content.',
    })
  }
  ```
</If>
...
<If sdk="expo">
  ```tsx {{ filename: 'src/app/(home)/bronze-content.tsx' }}
...
<If sdk="expressjs">
  <Include src="_partials/billing/express-route-module-note" />
  ...
  router.get('/bronze-content', async (req, res) => {
    const { has, isAuthenticated } = getAuth(req)
...
<If sdk="fastify">
  <Include src="_partials/billing/fastify-route-module-note" />
