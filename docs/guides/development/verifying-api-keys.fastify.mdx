...
By default, `acceptsToken` is set to `session_token`, which means API keys will **not** be accepted unless explicitly configured. You can pass either a **single token type** or an **array of token types** to `acceptsToken`. To learn more about the supported token types, see the [`getAuth()` options documentation](/docs/reference/fastify/get-auth#getauth-options).

Below are two examples of verifying API keys in a Fastify route. Both assume that [`clerkPlugin()`](/docs/reference/fastify/clerk-plugin) has been registered before the routes that call `getAuth()`.
