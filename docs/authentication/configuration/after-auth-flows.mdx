...
| - | - | - |
| [Personal account disabled (default)](/docs/organizations/overview#allow-personal-accounts) | `select-organization` | Requires users to select or create an organization after authenticating. |
...
### Check for the current session task

Once enabled in the Clerk Dashboard, after-auth flows are **embedded by default** within the `<SignUp />` and `<SignIn />` components. For more customization, you can opt out of using the `<SignUp />` and `<SignIn />` components and create custom pages for your after-auth flows.
...
| Name | Component |
| - | - |
| [Personal account disabled (default)](/docs/organizations/overview#allow-personal-accounts) | [`<TaskSelectOrganization />`](/docs/components/authentication/task-select-organization) |

If the prebuilt components don't meet your specific needs or if you require more control over the logic, you can also build your own UI using the `Session.currentTask` property. After calling [`setActive()`](/docs/references/javascript/clerk#set-active), use the `session.currentTask` property to check if the user has pending after-auth tasks that need to be completed before their session becomes fully `active`. Refer to the [custom flows documentation](/docs/custom-flows/overview) for specific implementation guides.
...
<CodeBlockTabs options={["React", "JavaScript"]}>
  ...
  if (clerk.session?.currentTask) {
    ...
    // to complete after-auth flow, see https://clerk.com/docs/custom-flows/overview#after-auth-flow
    console.log(clerk.session.currentTask)
  }
  ```
