...
[`<SignInButton>`](/docs/components/unstyled/sign-in-button) and [`<SignUpButton>`](/docs/components/unstyled/sign-up-button) also take these properties.

<Callout type="info">
It is recommended to define both `signInFallbackRedirectUrl` and `signUpFallbackRedirectUrl` redirects in each button as some users may choose to sign up instead after attempting to sign in.
</Callout>
