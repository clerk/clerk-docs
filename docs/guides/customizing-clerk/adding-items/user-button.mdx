...
<If sdk="astro">
  In Astro, the `<UserButton.Action />` component is used to add a "Help" menu item to the `<UserButton />` component. The `open` prop is set to `"help"` to open the `/help` page when the menu item is selected.
