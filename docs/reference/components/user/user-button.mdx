...
For users that have [multi-session](/docs/guides/secure/session-options#multi-session-applications) enabled, the `<UserButton />` also allows users to sign into multiple accounts at once and instantly switch between them without the need for a full page reload. Learn more about [multi-session applications](/docs/guides/secure/session-options#multi-session-applications).
...
<If sdk="js-frontend">
  ...
  - [`unmountUserButton()`](#unmountuserbutton)
...
<Properties>
  ...
  **Deprecated. Move `afterSignOutUrl` to [`<ClerkProvider />`](/docs/reference/components/clerk-provider).** The full URL or path to navigate to after a successful sign-out.
  ...
  See [Pass props to `<UserProfile />`](#pass-props-to-userprofile) for an example.
