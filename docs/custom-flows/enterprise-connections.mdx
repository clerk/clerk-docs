...
<Tabs items={["Next.js", "Expo"]}>
  ...
  <Tab>
    ...
    1. Calls the `startSSOFlow()` method with the `strategy` param set to `enterprise_sso`, the `identifier` param set to the user's email address that they provided, and a `redirect_url` param to redirect the user once they finish the authentication flow.
