...
> [!IMPORTANT]
> Clerk Billing is **not** the same as Stripe Billing. Plans and pricing need to be managed directly through the Clerk Dashboard and won't sync with your existing Stripe products or plans. Clerk uses Stripe **only** for payment processing, so you don't need to set up Stripe Billing. 
