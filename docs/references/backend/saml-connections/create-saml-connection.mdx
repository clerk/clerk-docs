...
Creates a new [`SamlConnection`](/docs/references/backend/types/backend-saml-connection).
...
This method in the SDK is a wrapper around the BAPI endpoint `POST/saml_connections`. See the [BAPI reference](/docs/references/backend-api/tag/SAML-Connections#operation/CreateSAMLConnection){{ target: '_blank' }} for more information.
