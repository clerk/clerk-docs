...
Account linking is the process of connecting an IdP identity from an enterprise connection (SAML or OIDC) to an existing Clerk user account. It enables seamless sign-in using Enterprise SSO alongside other authentication methods like username/password. Clerk automatically attempts to link accounts that share the same email address, assuming a single owner for each email.
