...
<Properties>
  ...
  - `setData`
  - `CacheSetter<undefined | ClerkPaginatedResponse<CommercePaymentResource>[]>` when `infinite` is `true`, or `CacheSetter<undefined | ClerkPaginatedResponse<CommercePaymentResource>>` when `infinite` is `false`
