...
description: The Show component protects content or even entire routes based on authentication, and optionally, authorization.
sdk: astro, chrome-extension, expo, nextjs, nuxt, react, react-router, tanstack-react-start, vue
---
...
<If sdk="expo">
  ```tsx {{ filename: 'src/app/(billing)/bronze.tsx' }}
  import { Show } from '@clerk/expo'
  import { Text } from 'react-native'
...
<If sdk="expo">
  ```tsx {{ filename: 'src/app/(billing)/premium-access.tsx' }}
  import { Show } from '@clerk/expo'
  import { Text } from 'react-native'
...
<If sdk="nextjs">
  ```tsx {{ filename: 'src/app/dashboard/settings/page.tsx' }}
...
<Properties>
  ...
  An element to be rendered when a user doesn't have the correct type of access control to access the protected content.
  ...
  A boolean that indicates whether to treat [pending sessions](/docs/reference/types/session-status#properties) as signed out. Defaults to `true`.
