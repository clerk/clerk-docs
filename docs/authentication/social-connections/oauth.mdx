...
The easiest way to add social connections to your Clerk app is by using [prebuilt components](/docs/components/overview). If you prefer a more custom solution, check out how to [build a social connection flow using Clerk's API](/docs/custom-flows/oauth-connections).
...
Currently, the prebuilt components are not supported in native applications, but you can use Clerk's API to [build a custom flow for authenticating with social connections.](/docs/custom-flows/oauth-connections)
