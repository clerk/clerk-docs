...
> [!NOTE]
> This quickstart is for apps that use Pages Router. Using App Router? Follow the [App Router quickstart](/docs/getting-started/quickstart) instead.
...
<Steps>
  ...
  1. In your `proxy.ts` file, export the `clerkMiddleware()` helper:
     ...
     ```
  1. By default, `clerkMiddleware()` will not protect any routes. All routes are public and you must opt-in to protection. To require a signed-in user, protect resources close to where they're used, as shown in the [guide on reading user data](/docs/guides/users/reading#pages-router).
  ...
  ```tsx {{ filename: 'pages/_app.tsx', mark: [2, [7, 27]] }}
