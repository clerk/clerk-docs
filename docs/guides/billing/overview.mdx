...
This limitation is particularly relevant in the UK and EU, where banks commonly require 3D Secure (3DS) challenges. It also affects [free trials](/docs/guides/billing/free-trials): even if you disable the payment method requirement so users can start a trial without entering payment details, renewals at the end of the trial will still fail if the user's payment method requires a 3DS challenge — because the charge happens in the background with no user present to complete it.
