...
<Properties>
  ...
  - `passwordHasher?`
  - `'argon2i' | 'argon2id' | 'awscognito' | 'bcrypt' | 'bcrypt_sha256_django' | 'md5' | 'md5_salted' | 'pbkdf2_sha256' | 'pbkdf2_sha256_django' | 'pbkdf2_sha1' | 'pbkdf2_sha512' | 'pbkdf2_sha512_hex' | 'phpass' | 'scrypt_firebase' | 'scrypt_werkzeug' | 'sha256' | 'sha256_salted' | 'sha512_symfony'`
  ...
  - [`pbkdf2_sha512_hex`](https://en.wikipedia.org/wiki/PBKDF2)
    ...
    - Both the salt and the hash are expected to be hex-encoded. Use this if your source system stores the salt bytes as hex rather than as a raw string.
