The following example demonstrates how to use the [`$sessionStore`](/docs/reference/astro/client-side-helpers/session-store) store to access the [`Session`](/docs/reference/objects/session) object. The `Session` object is used to access the `lastActiveAt` property, which is a `Date` object used to show the time the session was last active.
