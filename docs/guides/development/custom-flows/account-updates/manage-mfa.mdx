...
<Steps>
  ...
  <If sdk="ios">
    ```swift {{ filename: 'ManageTOTPBasedMFAView.swift', collapsible: true }}
    import SwiftUI
    import ClerkKit

    struct ManageTOTPBasedMFAView: View {
      @Environment(Clerk.self) private var clerk
      ...
      var body: some View {
        VStack {
          if isVerifying {
            ...
            Text(totpURI)
            TextField("Enter TOTP verification code", text: $verificationCode)
            Button("Verify") {
              Task { await verifyTotpMfa(code: verificationCode) }
            }
          } else {
            Button("Set up TOTP MFA") {
              Task { await setupTotpMfa() }
            }
          }

          Button("Disable TOTP MFA") {
            Task { await disableTotpMfa() }
          }
        }
      }
    }

    extension ManageTOTPBasedMFAView {

      func setupTotpMfa() async {
        do {
          guard let user = clerk.user else { return }
          let totp = try await user.createTOTP()

          // Display and share the TOTP URI with the user for authenticator setup.
          totpURI = totp.uri ?? ""
          isVerifying = true
        } catch {
          // See https://clerk.com/docs/guides/development/custom-flows/error-handling
      ...
      func verifyTotpMfa(code: String) async {
        do {
          guard let user = clerk.user else { return }
          try await user.verifyTOTP(code: code)
        } catch {
          // See https://clerk.com/docs/guides/development/custom-flows/error-handling
          // for more info on error handling
          dump(error)
      ...
      func disableTotpMfa() async {
        do {
          try await clerk.user?.disableTOTP()
        } catch {
          // See https://clerk.com/docs/guides/development/custom-flows/error-handling
          // for more info on error handling
          dump(error)
