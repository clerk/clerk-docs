...
<Tabs items={["NPM module", "<script>"]}>
  ...
  <Tab>
    <Steps>
      ...
      Clerk's [prebuilt components](/docs/reference/general/components/overview) are the easiest way to add authentication and user management to your app. They come styled out-of-the-box and are customizable to fit your app's design.

      To get started, add the following components to your app:

      - [`<SignIn />`](/docs/reference/general/components/authentication/sign-in): Renders a user interface for signing in.
