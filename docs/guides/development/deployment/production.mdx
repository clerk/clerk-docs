...
To see what DNS records you need, navigate to the [**Domains**](https://dashboard.clerk.com/~/domains) page in the Clerk Dashboard. If you're using a `*.vercel.app` domain, you can skip this step; see the [Vercel production domains](#vercel-app-production-domains) section for more information.
...
After you configure your app, in the Clerk Dashboard, return to the [**Domains**](https://dashboard.clerk.com/~/domains) page and select **Verify proxy** to confirm that Clerk can reach the `__clerk` proxy path. If `__clerk` is missing from the matcher, requests to Clerk's proxied Frontend API can be skipped by middleware and authentication can fail.
...
- Custom domains are still recommended when you need full branded production behavior.

Core auth emails are delivered through Clerk's shared `accounts.dev` email infrastructure:

| Works on `*.vercel.app` | Requires a custom domain |
| - | - |
| Sign-in and sign-up verification | Custom email templates |
| Verification codes | Email link authentication |
| Password reset codes | Application invitations |
...
- **Production keys:** Your Vercel production environment must use live Clerk keys (`pk_live_` and `sk_live_`). Preview and development environments should continue to use development keys (`pk_test_` and `sk_test_`).
