...
Clerk's SDKs expose an instance of the Clerk Backend SDK for use in server environments.
...
You will need your `CLERK_SECRET_KEY` to initialize the SDK, which you can find in the [API Keys](https://dashboard.clerk.com/last-active?path=) section in the Clerk Dashboard.

This example shows how to access the `users` sub-api from the Backend SDK. The [`getUser`](/docs/references/backend/get-user) method is then used to retrieve a user by their user ID.
...
SMS message operations are exposed under `clerkClient.smsMessages`. The following methods are available:

### Create

- [`createSMSMessage()`](/docs/references/backend/sms-message/create-sms-message)
