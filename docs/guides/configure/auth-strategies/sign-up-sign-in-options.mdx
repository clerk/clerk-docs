...
> [!TIP]
> Enable **Backup codes** alongside whichever other strategies you offer. If a user loses access to every other factor they've enrolled, an unused backup code can let them sign in without an admin resetting their MFA. See [MFA account recovery](/docs/guides/secure/mfa-recovery).
...
1. In the **Two step verifications** section, open the three-dot menu next to **Authenticator app** or **SMS code** and select **Remove method**.
