...
<Tabs items={["Next.js", "iOS", "Android (beta)"]}>
  ...
  <Tab>
    ...
    @Composable
    fun AddEmailViewModel(modifier: Modifier = Modifier, viewModel: AddEmailViewModel = viewModel()) {
      val state by viewModel.uiState.collectAsState()
      when (state) {
        AddEmailViewModel.AddEmailUiState.NeedsVerification -> {
          InputContentView(buttonText = "Continue", placeholder = "Enter email address") {
             viewModel.createEmailAddress(it) 
          }
        }

        AddEmailViewModel.AddEmailUiState.Verified -> {
          Text("Verified!")
        }

        AddEmailViewModel.AddEmailUiState.Verifying -> {
    ...
    @Composable
    fun InputContentView(buttonText: String, placeholder: String, modifier: Modifier = Modifier, onClick: (String) -> Unit) {
    ...
    class AddEmailViewModel: ViewModel() {
      private val _uiState = MutableStateFlow<AddEmailUiState>(AddEmailUiState.NeedsVerification)
      ...
      fun createEmailAddress(emailAddress: String) {
        ...
        viewModelScope.launch {
            ...
            .onSuccess { 
              ...
              _uiState.value = AddEmailUiState.NeedsVerification(it) 
            }
            .onFailure { Log.e("AddEmailViewModel", "Failed to create email address and prepare verification: $it.longErrorMessageOrNull") }
        }
      }
        ...
        viewModelScope.launch {
          newEmailAddress
            ...
            .onSuccess { 
              ...
              _uiState.value = AddEmailUiState.Verified 
            }
            .onFailure { Log.e("AddEmailViewModel", "Failed to verify email address: $it.longErrorMessageOrNull") }
        }
      }

      sealed interface AddEmailUiState {
        data object NeedsVerification: AddEmailUiState
        data class Verifying(val emailAddress: EmailAddress) : AddEmailUiState
        data object Verified : AddEmailUiState
      }
    }
    ```
  </Tab>
