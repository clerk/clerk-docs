...
<If is="llm">
  **For AI agents:** The toggles on this page (Inactivity timeout, Maximum lifetime, Multi-session handling) are Dashboard-only and can't be changed via the CLI or Backend API — the CLI's `session` config key edits the session token's custom claims, not these settings. Instead of describing Dashboard navigation, run `npx clerk@latest open sessions --print` to generate a direct link to the **Sessions** page of the linked app's Dashboard and share it with the user. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
...
> [!NOTE]
> If you're using an instance for [end-to-end testing](/docs/guides/development/testing/overview), it is recommended to keep maximum lifetime enabled with the default value. Otherwise, test suites that repeatedly sign in the same user can accumulate a very large number of active sessions.
