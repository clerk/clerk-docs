...
1. **Authentication fails**: On the [**OAuth applications**](https://dashboard.clerk.com/~/oauth-applications) page in the Clerk Dashboard, check the client's status and metadata on the **CIMD Clients** tab. Refresh metadata if the fetch failed. If the client doesn't support [CIMD](!client-id-metadata-document), ensure **Dynamic client registration** is enabled.
