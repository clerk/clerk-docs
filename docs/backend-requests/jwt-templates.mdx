...
If you need to generate a token that includes both session-bound data (like `sid`) and any additional claims, the recommended approach is to use a custom session token instead of a JWT template. You can do this by adding custom claims to the session token using this [guide](/docs/backend-requests/custom-session-token).
