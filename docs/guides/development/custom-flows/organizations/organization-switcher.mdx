...
<If sdk="js-frontend">
  ...
  1. Calls the [`getOrganizationMemberships()`](/docs/reference/objects/user#getorganizationmemberships) method to retrieve the list of Organizations the current user is a part of. This method returns `data`, which is an array of [`OrganizationMembership`](/docs/reference/types/organization-membership) objects.
  1. Maps over the `data` array to display the user's Organization memberships in a list, providing a button that calls [`setActive()`](/docs/reference/objects/clerk#setactive) to set the selected Organization as the [Active Organization](!active-organization).
