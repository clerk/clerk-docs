...
Personal Accounts are disabled by default for applications using organizations. This means users must create or join an organization after signing up, rather than having access to a personal workspace.

Most multi-tenant applications want all users to be part of an organization rather than operating in individual personal accounts. Personal Accounts are available as an opt-in feature for applications that need them.

To enable Personal Accounts for your application, toggle "Enable Personal Account" in the [**Organizations Settings**](https://dashboard.clerk.com/last-active?path=organizations-settings) page.

When Personal Accounts are disabled, you'll need to implement logic to handle users who don't have an active organization. For detailed implementation guidance including middleware setup, custom flows, and handling pending sessions, see the [force organizations guide](/docs/organizations/force-organizations).

For general information on how application protection works using SDK components and server utilities, see the [after-auth flow guides](/docs/authentication/configuration/after-auth-flows).

> [!NOTE]
> This setting is available for applications using organizations. If you need assistance configuring Personal Accounts for your application, please [contact our support team](https://clerk.com/support)
