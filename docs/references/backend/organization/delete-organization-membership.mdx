...
This method in the SDK is a wrapper around the BAPI endpoint `DELETE/organizations/{organization_id}/memberships/{user_id}`. See the [BAPI reference](/docs/references/backend-api/tag/Organization-Memberships#operation/DeleteOrganizationMembership){{ target: '_blank' }} for more information.
