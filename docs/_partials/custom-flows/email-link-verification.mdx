...
1. Every user has a [`User`](/docs/reference/objects/user) object that represents their account. The `User` object has a `emailAddresses` property that contains all the email addresses associated with the user. Use the [`useUser()`](/docs/reference/hooks/use-user) hook to access the `User` object.
1. Pass the [`User.createEmailAddress()`](/docs/reference/objects/user#create-email-address) method to the [`useReverification()`](/docs/reference/hooks/use-reverification) hook to require the user to [reverify their credentials](/docs/guides/secure/reverification) before being able to add an email address to their account.
1. If the `createEmailAddress()` function is successful, a new [`EmailAddress`](/docs/reference/types/email-address) object is created and stored in `User.emailAddresses`. Use the newly created `EmailAddress` object to access the [`createEmailLinkFlow()`](/docs/reference/types/email-address#create-email-link-flow) method.
1. Use the `createEmailLinkFlow()` method to access the [`startEmailLinkFlow()`](/docs/reference/types/email-address#create-email-link-flow) method. Call the `startEmailLinkFlow()` method to send an email with a verification link to the user. This method requires specifying a `redirectUrl`, which is the URL that the user will be redirected to after visiting the link in their email.
1. On your verification page (the URL you set as the `redirectUrl` parameter), call the [`useClerk()`](/docs/reference/hooks/use-clerk) hook to access the [`handleEmailLinkVerification()`](/docs/reference/objects/clerk#handle-email-link-verification) method.
