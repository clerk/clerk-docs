...
<SignedOut>
  ...
  1. If you are using [the Sync Host feature](/docs/guides/development/sync-host), you will also need to set the `PLASMO_PUBLIC_CLERK_SYNC_HOST` environment variable. The value should be the domain your web app's production server runs on. For example, `https://clerk.com`.
...
> [!WARNING]
> If you are using the Sync Host feature and followed [the configuration guide](/docs/guides/development/sync-host), then you have already completed this step (the `http://$PLASMO_PUBLIC_CLERK_SYNC_HOST/*` value points to your production domain.)
