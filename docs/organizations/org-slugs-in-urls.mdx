...
<Steps>
  ...
  > [!TIP]
  > If your app doesn't use `clerkMiddleware()`, or you prefer to manually set the active organization, use the [`setActive()`](/docs/references/javascript/clerk/session-methods) method to control the active organization on the client-side. See [this guide](/docs/organizations/force-organizations#set-an-active-organization-based-on-the-url) to learn how to manually activate a specific organization based on the URL.
  ...
  <Tabs items={["Server-side","Client-side"]}>
    <Tab>
      ...
      To access additional organization information like the organization name, you'll need to [customize the Clerk session token](/docs/backend-requests/session-requests/making/custom-session-token) to include these details:
