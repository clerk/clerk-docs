    ...
    <Tab>
      ...
      <Tabs items={["Primary app", "Satellite app"]}>
        <Tab>
          ...
          You will also need to add the `allowedRedirectOrigins` property to `<ClerkProvider>` to ensure that the redirect back from primary to satellite domain works correctly. For example:
