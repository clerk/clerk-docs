...
<Steps>
  ...
  <Tabs items={["Next.js", "JavaScript", "Expo", "iOS", "Android"]}>
    ...
    <Tab>
      ```kotlin {{ filename: 'MFASignInView.kt', collapsible: true }}
      @Composable
      fun MFASignInView(viewModel: MFASignInViewModel = viewModel()) {
        ...
        Column(
          ...
          when (state) {
            MFASignInViewModel.UiState.NeedsSecondFactor -> {
              ...
              Button(onClick = { viewModel.verify(code) }) { Text("Submit") }
            }
      ...
      class MFASignInViewModel : ViewModel() {
        ...
        fun verify(code: String) {
          ...
          viewModelScope.launch {
            inProgressSignIn.attemptSecondFactor(SignIn.AttemptSecondFactorParams.TOTP(code)).onSuccess {
