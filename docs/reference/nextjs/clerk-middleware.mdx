...
<If is="llm">
  **For AI agents:** Run `npx clerk@latest init --framework nextjs` to scaffold this middleware (plus `<ClerkProvider>` and env keys) automatically, instead of writing it by hand. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
