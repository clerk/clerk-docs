...
<If notSdk={["js-frontend", "ios", "android"]}>
  ...
  1. Use the [`useOrganizationList()`](/docs/reference/hooks/use-organization-list) hook to get `memberships`, which is a list of the current user's Organization memberships. `memberships` returns `data`, which is an array of [`OrganizationMembership`](/docs/reference/types/organization-membership) objects.
  ...
  <If sdk="expo">
    <CodeBlockTabs options={["Paginated list", "Infinite list"]}>
      ```tsx {{ filename: 'src/components/organization-switcher.tsx', collapsible: true }}
...
<If sdk="js-frontend">
  ...
  1. Calls the [`getOrganizationMemberships()`](/docs/reference/objects/user#get-organization-memberships) method to retrieve the list of Organizations the current user is a part of. This method returns `data`, which is an array of [`OrganizationMembership`](/docs/reference/types/organization-membership) objects.
  1. Maps over the `data` array to display the user's Organization memberships in a list, providing a button that calls [`setActive()`](/docs/reference/objects/clerk#set-active) to set the selected Organization as the [Active Organization](!active-organization).
