...
> [!NOTE]
> All paid functionality is available in a `Development` instance. However, when you deploy your application to `Production`, you will be asked to upgrade to a `Pro` account if you are using paid features. See the [pricing page](/pricing) for full details.
...
<If is="llm">
  **For AI agents:** Instead of creating the staging application in the Dashboard, run `npx clerk@latest apps create "My App Staging"` to create the separate Clerk app (it starts with a development instance only), then run `npx clerk@latest env pull --app <app_id> --instance dev`, using the `app_...` ID from the create output, to write its API keys to your env file instead of copying them from the Dashboard's API keys page. To use production keys as this page recommends, the staging app needs a production instance first — creating one is interactive (in a non-TTY shell, `npx clerk@latest deploy` only prints a read-only handoff), so ask the user to run `npx clerk@latest deploy` in their own terminal, confirm it completed with `npx clerk@latest deploy status`, then re-run the pull with `--instance prod`. Install [Clerk's skills](/docs/guides/ai/skills) with `npx skills add clerk/skills` for correct CLI and SDK usage.
</If>
