...
> [!TIP]
> Protect access as close to the resource as possible. See [Protect pages and routes](/docs/guides/secure/protect-pages).
...
> [!TIP]
> For API and machine routes, check the token type in the route handler itself with [`auth({ acceptsToken })`](/docs/reference/nextjs/app-router/auth). See [Verifying OAuth access tokens](/docs/guides/development/verifying-oauth-access-tokens).
